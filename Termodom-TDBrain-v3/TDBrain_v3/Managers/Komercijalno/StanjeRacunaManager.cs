...
namespace TDBrain_v3.Managers.Komercijalno
{
    ...
    public static class StanjeRacunaManager
    {
        ...
        {
            using (FbCommand cmd = new FbCommand("SELECT * FROM STANJE_RACUNA WHERE RACUN = @RACUN", con))
            {
                ...
                using (FbDataReader dr = cmd.ExecuteReader())
                {
                    if (dr.Read())
                    {
                        return new StanjeRacuna()
                        {
                            ...
                            KasaID = dr["KASAID"] is DBNull ? null : (int?)Convert.ToInt32(dr["KASAID"]),
                            DozvoljeniMinus = dr["DOZVOLJENI_MINUS"] is DBNull ? null : (double?)Convert.ToDouble(dr["DOZVOLJENI_MINUS"]),
                            DozvoljeniMinusVaziDo = dr["DM_VAZI_DO"] is DBNull ? null : (DateTime?)Convert.ToDateTime(dr["DM_VAZI_DO"])
                        };
                    }
                }
            }
        ...
        public static StanjeRacunaDictionary Dictionary(FbConnection con)
        {
            ...
            using(FbCommand cmd = new FbCommand("SELECT * FROM STANJE_RACUNA", con))
            {
                using(FbDataReader dr = cmd.ExecuteReader())
                {
                    while(dr.Read())
                    {
                        dict.Add(dr["RACUN"].ToString(), new StanjeRacuna()
                        {
                            Racun = dr["RACUN"].ToString(),
                            PPID = Convert.ToInt32(dr["PPID"]),
                            PocDuguje = Convert.ToDouble(dr["POC_DUGUJE"]),
