            ...
            using (FbConnection con = new FbConnection(DB.Settings.ConnectionStringTDOffice_v2.ConnectionString()))
            {
                ...
                using (FbCommand cmd = new FbCommand("SELECT * FROM USERS", con))
                {
                    using (FbDataReader dr = cmd.ExecuteReader())
                    {
                        while (dr.Read())
                        {
                            dict.Add(Convert.ToInt32(dr["ID"]), new User()
                            {
                                ID = Convert.ToInt32(dr["ID"]),
                                Username = dr["USERNAME"].ToString(),
                                PW = dr["PW"].ToString(),
                                MagacinID = Convert.ToInt32(dr["MAGACINID"]),
                                KomercijalnoUserID = dr["KOMERCIJALNO_USER_ID"] is DBNull ? null : Convert.ToInt32(dr["KOMERCIJALNO_USER_ID"]),
                                OpomeniZaNeizvrseniZadatak = Convert.ToInt32(dr["OPOMENI_ZA_NEIZVRSENI_ZADATAK"]),
