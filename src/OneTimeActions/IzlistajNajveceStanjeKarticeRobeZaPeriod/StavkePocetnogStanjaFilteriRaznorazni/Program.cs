...
void StartStavkePocetnogStanjaSaKolicinomIAbsolutnomRazlikomProdajneINabavneCene()
{
	...
	Console.WriteLine();
	Console.WriteLine(
		"Ova akcija ce selektovati i ispisati sve stavke pocetnog stanja kojima je razlika izmedju prodajne i nabavne cene veca ili manja od X%"
	);
	UnosBrojaDokumenta();
	UnosZnaka();
	UnosRazlikeProcenti();
	...
	var stavke = ctx
		.Stavke.Where(x => x.VrDok == vrDok && x.BrDok == brDok && x.Kolicina != 0)
		.ToList();
