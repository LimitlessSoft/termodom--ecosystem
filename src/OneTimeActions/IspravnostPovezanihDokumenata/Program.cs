...
var startniVrDok = new HashSet<int>() { 1, 19, 25, 35 };
var bazaConnString = "C:\\Poslovanje\\Baze\\2025\\FRANSIZA2025TCMD.FDB";
Console.WriteLine("===");
Console.WriteLine("Baza: " + bazaConnString);
Console.WriteLine("===");
Console.WriteLine("Izlistavam neslaganja linije dokumenata i stavki u njima:");
Console.WriteLine("1. Osnovna neslaganja (ima izlazni dokument, broj stavki, roba, kolicine)");
...
)
{
	...
	{
		Console.WriteLine(
			$"Dokument [{dokument.VrDok} {dokument.BrDok}] ima upisan OUT [{dokument.VrdokOut} {dokument.BrdokOut}] ali ja ne mogu da ga pronadjem u bazi!"
		);
		return;
	}
	...
	{
		var transportovanoBrojStavki = izlazniDokument.Stavke.Count(x => IsStandardRoba(x.RobaId));
		if (originalnoBrojStavki != transportovanoBrojStavki)
		{
			if (!printedHeader)
			{
				PrintHeader(dokument, izlazniDokument);
				printedHeader = true;
			}
	...
	{
		var distinctRobaIdsUTransportovanomDokumentu = izlazniDokument
	...
	if (whatToCheck.Kolicine)
	{
		if (sveStavkeIste)
		{
			...
			{
				var stavkaDestinacionogDokumenta = izlazniDokument
					.Stavke.Where(x => IsStandardRoba(x.RobaId))
	...
	// 				.First(x => x.RobaId == stavka.RobaId);
	// 			if (
	// 				Math.Abs(stavkaDestinacionogDokumenta.ProdajnaCena - stavka.ProdajnaCena) > 0.01
	// 			)
	// 			{
	// 				if (!printedHeader)
	// 				{
	...
	// 				.Stavke.Where(x => IsStandardRoba(x.RobaId))
	// 				.First(x => x.RobaId == stavka.RobaId);
	// 			if (Math.Abs(stavkaDestinacionogDokumenta.NabavnaCena - stavka.NabavnaCena) > 0.01)
	// 			{
	// 				if (!printedHeader)
	// 				{
	// 					PrintHeader(dokument, izlazniDokument);
...
// 		{
// 			Console.WriteLine(
// 				$"Nasao sam neki invalidan interni transport [IzVrDok: {d.Item1}, IzBrDok: {d.Item2}]"
// 			);
// 			continue;
// 		}
// 		IzlistajStavkeSaKolicinomIAbsolutnomRazlikomProdajneINabavneCeneDokumenta(
...
// 			continue;
// 		}
// 		IzlistajStavkeSaKolicinomIAbsolutnimRabatomOdDokumenta(dd, znak, procenti);
// 	}
// }
