...
public class MCPartnerCenovnikItemManager (ILogger<MCPartnerCenovnikItemManager> logger, TDOfficeDbContext dbContext)
    : LSCoreManagerBase<MCPartnerCenovnikItemManager, MCPartnerCenovnikItemEntity>(logger, dbContext),
        IMCPartnerCenovnikItemManager
{
    public void Delete(LSCoreIdRequest request) =>
        HardDelete(request.Id);

    public List<MCpartnerCenovnikItemEntityGetDto> GetMultiple(MCPartnerCenovnikItemGetRequest request) =>
        Queryable()
            .Where(x =>
