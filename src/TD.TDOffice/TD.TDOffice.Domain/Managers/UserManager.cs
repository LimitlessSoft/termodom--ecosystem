...
public class UserManager(ILogger<UserManager> logger, TDOfficeDbContext dbContext)
	: LSCoreManagerBase<UserManager, User>(logger, dbContext),
		IUserManager
{
	public UserDto Get(LSCoreIdRequest request)
	{
		var user = Queryable().FirstOrDefault(x => x.Id == request.Id);
	...
	public List<UserDto> GetMultiple() => Queryable().ToList().ToUserDtoList();
}
