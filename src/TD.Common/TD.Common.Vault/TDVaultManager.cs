...
public class TDVaultManager : ITDVaultManager
{
	...
	{
		_authMethodInfo = new UserPassAuthMethodInfo(
			tdVaultConfiguration.Username,
			tdVaultConfiguration.Password
		);
	...
	public async Task<TDto> GetSecretsAsync<TDto>(string path) =>
		(
			await _vaultClient.V1.Secrets.KeyValue.V2.ReadSecretAsync<TDto>(
				path,
				mountPoint: _tdVaultConfiguration.Engine
			)
		)
			.Data
			.Data;
...
public class TDVaultManager<TDefaultDto>(TDVaultConfiguration tdVaultConfiguration)
	: TDVaultManager(tdVaultConfiguration),
		ITDVaultManager<TDefaultDto>
{
	/// <summary>
