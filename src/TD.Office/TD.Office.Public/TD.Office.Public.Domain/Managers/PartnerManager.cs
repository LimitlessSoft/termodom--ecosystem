...
) : IPartnerManager
{
	public PartnerYearsDto GetPartnersReportByYearsKomercijalnoFinansijsko()
	{
		...
		response.DefaultTolerancija = settingRepository.GetValue<int>(
			SettingKey.PARTNERI_PO_GODINAMA_DEFAULT_TOLERANCIJA
		);

		response.Status = komercijalnoIFinansijskoPoGodinamaStatusRepository
			.GetAllStatuses()
			.ToMappedList<KomercijalnoIFinansijskoPoGodinamaStatusEntity, IdNamePairDto>();
	...
	)
	{
		...
		foreach (var year in request.Years)
		{
			...
			var istorijaUplataDataTask = yearApi.GetMultipleIstorijaUplataAsync(
				new IstorijaUplataGetMultipleRequest() { PPID = ppids }
			);

			var promeneDobavljacDataTask = yearApi.GetMultiplePromeneAsync(
				new PromenaGetMultipleRequest() { PPID = ppids, KontoStartsWith = "43" }
			);

			var promeneKupacDataTask = yearApi.GetMultiplePromeneAsync(
				new PromenaGetMultipleRequest() { PPID = ppids, KontoStartsWith = "204" }
			);

			var izvodiDataTask = yearApi.GetMultipleIzvodAsync(
				new IzvodGetMultipleRequest() { PPID = ppids }
			);
			...
			var mappedIstorijaUplataByPPID = istorijaUplataData
				.GroupBy(x => x.PPID)
				.ToDictionary(x => x.Key, x => x.ToList());
			var mappedPromeneDobavljacByPPID = promeneDobavljacData
				.GroupBy(x => x.PPID)
				.ToDictionary(x => x.Key, x => x.ToList());
			var mappedPromeneKupacByPPID = promeneKupacData
				.GroupBy(x => x.PPID)
				.ToDictionary(x => x.Key, x => x.ToList());
			var mappedIzvodiByPPID = izvodiData
				.GroupBy(x => x.PPID)
				.ToDictionary(x => x.Key, x => x.ToList());
			var mappedDokumentiByPPID = dokumentiData
				.GroupBy(x => x.PPID)
				.ToDictionary(x => x.Key, x => x.ToList());
			...
			{
				var psKupac = !mappedIstorijaUplataByPPID.ContainsKey(ppid)
					? 0
					: mappedIstorijaUplataByPPID[ppid]
						.Where(x => x.Datum.Day == 1 && x.Datum.Month == 1 && x.VrDok == -61)
						.Sum(x => x.Iznos);

				var psDobavljac = !mappedIstorijaUplataByPPID.ContainsKey(ppid)
					? 0
					: mappedIstorijaUplataByPPID[ppid]
						.Where(x => x.Datum.Day == 1 && x.Datum.Month == 1 && x.VrDok == -59)
						.Sum(x => x.Iznos);

				var pocetnoFinansijskoKupac = !mappedPromeneKupacByPPID.ContainsKey(ppid)
					? 0
					: mappedPromeneKupacByPPID[ppid]
						.Where(x => (x.VrDok == -61 || x.VrDok == 0))
						.Sum(x => x.Potrazuje - x.Duguje) ?? 0;

				var pocetnoFinansijskoDobavljac = !mappedPromeneDobavljacByPPID.ContainsKey(ppid)
					? 0
					: mappedPromeneDobavljacByPPID[ppid]
						.Where(x => (x.VrDok == -59 || x.VrDok == 0))
						.Sum(x => x.Potrazuje - x.Duguje) ?? 0;
				...
				foreach (
					var dd in mappedDokumentiByPPID.TryGetValue(ppid, out var ddList) ? ddList : []
				)
				{
					...
					if (
						(dd.VrDok == 13 || dd.VrDok == 14 || dd.VrDok == 15 || dd.VrDok == 40)
						&& new[] { NacinUplate.Gotovina, NacinUplate.Kartica }.Contains(
							(NacinUplate)dd.NuId
						)
					)
						komercijalnoKraj[year][ppid] += (double)dd.Potrazuje;
					...
					if (
						dd.VrDok == 22
						&& dd.NuId != (short)NacinUplate.Kartica
						&& dd.NuId != (short)NacinUplate.Gotovina
					)
						komercijalnoKraj[year][ppid] += (double)dd.Potrazuje;
				...
				foreach (
					var id in mappedIzvodiByPPID.TryGetValue(ppid, out var idList) ? idList : []
				)
				{
				...
				foreach (
					var pk in mappedPromeneKupacByPPID.TryGetValue(ppid, out var pkList)
						? pkList
						: []
				)
				{
				...
				foreach (
					var pd in mappedPromeneDobavljacByPPID.TryGetValue(ppid, out var pdList)
						? pdList
						: []
				)
				{
		...
		foreach (var ppid in ppids)
		{
			...
			foreach (var year in request.Years)
			{
				...
				var komercijalnoDto = new YearStartEndDto
				{
					Year = year,
					Pocetak =
						komercijalnoPocetak.TryGetValue(year, out var startData)
						&& startData.TryGetValue(ppid, out var startValue)
							? (double)startValue
							: 0,
					Kraj =
						komercijalnoKraj.TryGetValue(year, out var endData)
						&& endData.TryGetValue(ppid, out var endValue)
							? (double)endValue
							: 0
				};
				...
				var finansijskoKupacDto = new YearStartEndDto
				{
					Year = year,
					Pocetak =
						finansijskoKupacPocetak.TryGetValue(year, out var kupacStartData)
						&& kupacStartData.TryGetValue(ppid, out var kupacStartValue)
							? (double)kupacStartValue
							: 0,
					Kraj =
						finansijskoKupacKraj.TryGetValue(year, out var kupacEndData)
						&& kupacEndData.TryGetValue(ppid, out var kupacEndValue)
				...
				var finansijskoDobavljacDto = new YearStartEndDto
				{
					Year = year,
					Pocetak =
						finansijskoDobavljacPocetak.TryGetValue(year, out var dobavljacStartData)
						&& dobavljacStartData.TryGetValue(ppid, out var dobavljacStartValue)
							? (double)dobavljacStartValue
							: 0,
					Kraj =
						finansijskoDobavljacKraj.TryGetValue(year, out var dobavljacEndData)
						&& dobavljacEndData.TryGetValue(ppid, out var dobavljacEndValue)
			...
			for (var i = 0; i < KomercijalnoDto.Count; i++)
			{
				if (
					Math.Abs(
						KomercijalnoDto[i].Pocetak
							- (FinansijskoKupacDto[i].Pocetak + FinansijskoDobavljacDto[i].Pocetak)
					) > request.Tolerancija
					|| (
						(KomercijalnoDto[i].Year != DateTime.Now.Year)
						&& Math.Abs(
							KomercijalnoDto[i].Kraj
								- (FinansijskoKupacDto[i].Kraj + FinansijskoDobavljacDto[i].Kraj)
						) > request.Tolerancija
					)
				)
				{
				...
				if (
					Math.Abs(KomercijalnoDto[i].Pocetak - KomercijalnoDto[i - 1].Kraj)
						> request.Tolerancija
					|| Math.Abs(FinansijskoKupacDto[i].Pocetak - FinansijskoKupacDto[i - 1].Kraj)
						> request.Tolerancija
					|| Math.Abs(
						FinansijskoDobavljacDto[i].Pocetak - FinansijskoDobavljacDto[i - 1].Kraj
					) > request.Tolerancija
				)
				{
					isOk = false;
			...
			{
				entity = komercijalnoIFinansijskoPoGodinamaRepository.Create(ppid, statusDefaultId);
			}

			finalData.Add(
				new GetPartnersReportByYearsKomercijalnoFinansijskoDto()
				{
					Status = entity.StatusId,
					Komentar = entity.Comment,
					PPID = entity.PPID,
					Naziv =
						partners.Payload.FirstOrDefault(x => x.Ppid == ppid)?.Naziv ?? "Nema naziv",
	...
	{
		var recentPartnersCreationLogs = logRepository
			.GetMultiple()
			.Where(x =>
				x.Key == LogKey.NoviKomercijalnoPartner && x.CreatedAt >= DateTime.Now.AddDays(-7)
			)
	...
	public bool SaveKomercijalnoFinansijskoKomentar(
		SaveKomercijalnoFinansijskoCommentRequest request
	)
	{
		request.Validate();
	...
	public async Task<Dictionary<string, Dictionary<int, object>>> GetPartnerAnalysisAsync(
	...
	private Task<Dictionary<int, object>> GetPartnerAnalysisPrometAsync(long ppid)
	{
		...
		Parallel.For(
			0,
			partnerAnalysisHistoryYearsBackSetting,
			i =>
			{
				var year = DateTime.Now.Year - i;
				var yearApi = komercijalnoApiManagerFactory.Create(year);
				var dokumentiPartnera = yearApi
				...
				promet.Add(
					year,
					dokumentiPartnera.Sum(x => x.VrDok == 13 ? x.Duguje : x.Potrazuje)
				);
			}
		);
	...
	private Task<Dictionary<int, object>> GetPartnerAnalysisOdobrenoRabataAsync(long ppid)
	{
		...
		Parallel.For(
			...
			i =>
			{
				...
				odobrenoRabata.Add(
					year,
					dokumentiPartnera.Sum(x => x.VrDok == 13 ? 0 : x.Duguje - x.Potrazuje)
				);
			}
		);
	...
	public Dictionary<string, string> GetCompanyTypes() =>
		Enum.GetValues<CompanyType>().ToDictionary(x => x.ToString(), x => x.GetDescription()!);
}
