...
public class SaveKomercijalnoFinansijskoStatusRequestValidator
	: LSCoreValidatorBase<SaveKomercijalnoFinansijskoStatusRequest>
{
	public SaveKomercijalnoFinansijskoStatusRequestValidator(OfficeDbContext dbContext)
	{
		RuleFor(x => x.StatusId)
			.Custom(
				(statusId, context) =>
				{
					var status = dbContext
						.KomercijalnoIFinansijskoPoGodinamaStatus.Where(x =>
							x.IsActive && x.Id.Equals(statusId)
						)
						.FirstOrDefault();
					if (status == null)
					{
