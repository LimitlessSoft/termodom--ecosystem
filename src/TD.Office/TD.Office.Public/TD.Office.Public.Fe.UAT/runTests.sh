...
remove_existing_containers() {
    docker stop office-public-be office-public-fe selenium-driver >/dev/null 2>&1 || true
    docker rm office-public-be office-public-fe selenium-driver >/dev/null 2>&1 || true
...
docker build -t limitlesssoft/termodom-office-public-api:temp -f src/TD.Office/TD.Office.Public/TD.Office.Public.Api/Dockerfile . >/dev/null 2>&1
docker run -d --name office-public-be --network test-network -p 8081:8080 \
    -e AllowedHosts="*" -e AllowedHosts="*" -e VAULT_URI="$VAULT_ADDR" -e VAULT_USERNAME="$VAULT_USERNAME" -e VAULT_PASSWORD="$VAULT_PASSWORD" -e VAULT_ENGINE="develop" -e VAULT_PATH="office/public/api" limitlesssoft/termodom-office-public-api:temp >/dev/null 2>&1

docker build --file src/TD.Office/TD.Office.Public/TD.Office.Public.Fe/Dockerfile --build-arg "DEPLOY_ENV=develop" --build-arg "OVERRIDE_DEPLOY_ENV=http://office-public-be:8080" -t limitlesssoft/termodom-office-public-fe:temp . >/dev/null 2>&1
docker run -d --name office-public-fe --network test-network -p 3001:3000 limitlesssoft/termodom-office-public-fe:temp >/dev/null 2>&1
