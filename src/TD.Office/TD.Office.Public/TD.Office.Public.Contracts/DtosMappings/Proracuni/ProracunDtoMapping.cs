using LSCore.Common.Extensions;
using LSCore.Mapper.Contracts;
using TD.Office.Common.Contracts.Entities;
using TD.Office.Public.Contracts.Dtos.Proracuni;

namespace TD.Office.Public.Contracts.DtosMappings.Proracuni;

public class ProracunDtoMapping : ILSCoreMapper<ProracunEntity, ProracunDto>
{
	public ProracunDto ToMapped(ProracunEntity sender) =>
		new()
		{
			Id = sender.Id,
			CreatedAt = sender.CreatedAt,
			MagacinId = sender.MagacinId,
			State = sender.State,
			Referent = sender.User.Nickname,
			KomercijalnoDokument =
				sender.KomercijalnoVrDok == null
					? ""
					: $"{sender.KomercijalnoVrDok} - {sender.KomercijalnoBrDok}",
			PPID = sender.PPID,
			NUID = sender.NUID,
			Type = sender.Type.GetDescription()!,
			Email = sender.Email,
			RecommendedValue = sender.RecommendedValue,
			Items = sender
				.Items.Where(x => x.IsActive)
				.Select(x => new ProracunItemDto
				{
					Id = x.Id,
					RobaId = x.RobaId,
					Kolicina = x.Kolicina,
					CenaBezPdv = x.CenaBezPdv,
					Pdv = x.Pdv,
					Rabat = x.Rabat,
				})
				.ToList()
		};
}
