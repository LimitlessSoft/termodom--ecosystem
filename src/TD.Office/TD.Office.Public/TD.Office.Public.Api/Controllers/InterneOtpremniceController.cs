    ...
    : ControllerBase
{
    ...
    public async Task<IActionResult> SaveItem([FromBody] InterneOtpremniceItemCreateRequest request) =>
        Ok(await interneOtpremniceManager.SaveItemAsync(request));

    [HttpDelete]
    [Route("/interne-otpremnice/{Id}/items/{ItemId}")]
    public async Task<IActionResult> DeleteItem([FromRoute] InterneOtpremniceItemDeleteRequest request)
    {
