...
public class InternaOtpremnicaRepository(InterneOtpremniceDbContext dbContext)
	: LSCoreRepositoryBase<InternaOtpremnicaEntity>(dbContext),
		IInternaOtpremnicaRepository
{
	...
	public InternaOtpremnicaEntity Create(
		int polazniMagacinId,
		int dolazniMagacinId,
		long createdBy
	)
	{
	...
	public InternaOtpremnicaItemEntity SaveItem(
		long? requestId,
		long internaOtpremnicaId,
		int robaId,
		decimal kolicina
	)
	{
		...
		var item = requestId.HasValue
			? dbContext.InterneOtpremniceItems.FirstOrDefault(x =>
				x.Id == requestId.Value && x.IsActive
			)
	...
	{
		var item = dbContext.InterneOtpremniceItems.FirstOrDefault(x =>
			x.Id == requestId && x.IsActive
		);
		if (item is null)
			throw new LSCoreNotFoundException();
