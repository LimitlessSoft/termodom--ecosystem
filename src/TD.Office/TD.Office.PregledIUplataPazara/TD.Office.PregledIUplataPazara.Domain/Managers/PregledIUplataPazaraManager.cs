...
public class PregledIUplataPazaraManager(ITDKomercijalnoClientFactory komercijalnoClientFactory, IConfigurationRoot configurationRoot, TDOfficeClient officeClient) : IPregledIUplataPazaraManager
{
    public async Task<PregledIUplataPazaraResponse> GetAsync(GetPregledIUplataPazaraRequest request)
    {
        ...
        foreach (var magacinId in request.Magacin)
        {
            ...
            {
                var client = komercijalnoClientFactory.Create(DateTime.UtcNow.Year,
                ...
                async Task FetchDokumentiAsync()
                {
                    if (!dokumenti.ContainsKey(year))
                        dokumenti.Add(year, []);
                    var resp = await client.Dokumenti.GetMultiple(new DokumentGetMultipleRequest()
                    {
                        VrDok = [15, 22],
                ...
                async Task FetchIzvodiAsync()
                {
                    if (!izvodi.ContainsKey(year))
                        izvodi.Add(year, []);
                    izvodi[year].AddRange(await client.Izvodi.GetMultipleAsync(new IzvodGetMultipleRequest()));
                }
            }
            ...
            {
                var izvodiNaDan_N = izvodi[datumObrade.Year].Where(x =>
                ...
                var mpRacuni_N = !dokumenti[datumObrade.Year].ContainsKey(15)
                    ? 0
                    : dokumenti[datumObrade.Year][15].Where(x =>
                ...
                var povratnice_N = !dokumenti[datumObrade.Year].ContainsKey(22)
                    ? 0
                    : dokumenti[datumObrade.Year][22].Where(x =>
                            x.Datum.Date == datumObrade.Date
                            && x.MagacinId == magacinId
                            && x.NuId != 1
                        )
                ...
                {
                    var item = new PregledIUplataPazaraResponse.Item
                    {
                        Konto = konto_N,
                    ...
                    if (item.Razlika > request.Tolerancija)
                    {
                        dto.Items.Add(item);
