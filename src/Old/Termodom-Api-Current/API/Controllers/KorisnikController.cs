	...
	public class KorisnikController : ControllerBase
	{
			...
			return Task.Run<IActionResult>(() =>
			{
				try
				{
					...
					if (korisnik.Ime.Length < MIN_CHARACTERS["IME"])
						return StatusCode(
							400,
							$"Ime korisnika mora biti duze od {MIN_CHARACTERS["IME"]} karaktera!"
						);

					if (korisnik.Ime.Length > MAX_CHARACTERS["IME"])
						return StatusCode(
							400,
							$"Ime korisnika mora biti krace od {MAX_CHARACTERS["IME"]} karaktera!"
						);
					...
					if (korisnik.Nadimak.Length < MIN_CHARACTERS["NADIMAK"])
						return StatusCode(
							400,
							$"Nadimak korisnika mora biti duze od {MIN_CHARACTERS["NADIMAK"]} karaktera!"
						);

					if (korisnik.Nadimak.Length > MAX_CHARACTERS["NADIMAK"])
						return StatusCode(
							400,
							$"Nadimak korisnika mora biti krace od {MAX_CHARACTERS["NADIMAK"]} karaktera!"
						);
					...
					if (korisnik.AdresaStanovanja.Length < MIN_CHARACTERS["ADRESA_STANOVANJA"])
						return StatusCode(
							400,
							$"Adresa stanovanja korisnika mora biti duza od {MIN_CHARACTERS["ADRESA_STANOVANJA"]} karaktera!"
						);

					if (korisnik.AdresaStanovanja.Length > MAX_CHARACTERS["ADRESA_STANOVANJA"])
						return StatusCode(
							400,
							$"Adresa stanovanja korisnika mora biti kraca od {MAX_CHARACTERS["ADRESA_STANOVANJA"]} karaktera!"
						);
					...
					if (korisnik.Opstina.Length < MIN_CHARACTERS["OPSTINA"])
						return StatusCode(
							400,
							$"Opstina korisnika mora biti duza od {MIN_CHARACTERS["OPSTINA"]} karaktera!"
						);

					if (korisnik.Opstina.Length > MAX_CHARACTERS["OPSTINA"])
						return StatusCode(
							400,
							$"Opstina korisnika mora biti kraca od {MAX_CHARACTERS["OPSTINA"]} karaktera!"
						);

					if (
						!string.IsNullOrWhiteSpace(korisnik.Komentar)
						&& korisnik.Komentar.Length > MAX_CHARACTERS["KOMENTAR"]
					)
						return StatusCode(
							400,
							$"Komentar korisnika mora biti kraci od {MAX_CHARACTERS["KOMENTAR"]} karaktera!"
						);
					...
					using (MySqlConnection con = new MySqlConnection(Program.ConnectionString))
					{
						...
						using (
							MySqlCommand cmd = new MySqlCommand("SELECT MAX(ID) FROM KORISNIK", con)
						)
								...
								if (dr.Read())
									return StatusCode(
										...
										new Models.Korisnik()
										{
											...
											DatumKreiranja = Convert.ToDateTime(
												dr["DATUM_KREIRANJA"]
											),
						...
						)
						{
							...
							using (MySqlDataReader dr = cmd.ExecuteReader())
								if (dr.Read())
									...
									{
										string bearer = SimpleHash(
											Random.Next(Int32.MaxValue).ToString()
										);
		...
		private IActionResult SetujParametar(int korisnikID, string parametar, object vrednost)
		{
			...
			if (vrednost is string)
			{
				...
				if (
					MIN_CHARACTERS.ContainsKey(parametar)
					&& vrednost.ToString().Length < MIN_CHARACTERS[parametar]
				)
					return StatusCode(
						400,
						$"Vrednost za parametar {parametar} mora biti duza od {MIN_CHARACTERS[parametar]} karaktera!"
					);

				if (
					MAX_CHARACTERS.ContainsKey(parametar)
					&& vrednost.ToString().Length < MAX_CHARACTERS[parametar]
				)
					return StatusCode(
						400,
						$"Vrednost za parametar {parametar} ne sme biti duza od {MAX_CHARACTERS[parametar]} karaktera!"
					);
			}
		...
		[ProducesResponseType(StatusCodes.Status404NotFound)]
		[SwaggerOperation(Tags = new[] { "api/Korisnik" })]
		public Task<IActionResult> AdresaStanovanjaSet(
		...
		[ProducesResponseType(StatusCodes.Status404NotFound)]
		[SwaggerOperation(Tags = new[] { "api/Korisnik" })]
		public Task<IActionResult> DatumRodjenjaSet(
			[Required] int korisnikID,
			[Required] DateTime vrednost
		)
		{
			return Task.Run<IActionResult>(() =>
			{
		...
		[ProducesResponseType(StatusCodes.Status404NotFound)]
		[SwaggerOperation(Tags = new[] { "api/Korisnik" })]
		public Task<IActionResult> PoslatRodjendanskiSMSSet(
			[Required] int korisnikID,
			[Required] bool vrednost
		)
		{
			return Task.Run<IActionResult>(() =>
			{
