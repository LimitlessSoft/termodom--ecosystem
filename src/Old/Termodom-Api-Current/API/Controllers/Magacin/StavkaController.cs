...
namespace API.Controllers.Magacin
{
	...
	public class StavkaController : ControllerBase
	{
		...
		public Task<IActionResult> Update([Required] DTO.Magacin.StavkaUpdateDTO stavka)
		{
			...
			{
				if (stavka == null || stavka.ID <= 0 || stavka.Kolicina <= 0)
					return StatusCode(400, "ID i kolicina moraju biti veci od 0!");
				try
				{
					using (
						MySqlConnection con = new MySqlConnection(Program.ConnectionStringMagacin)
					)
					{
						con.Open();
						using (
							MySqlCommand cmd = new MySqlCommand(
								@"UPDATE STAVKA SET
                        KOLICINA = @KOLICINA WHERE STAVKAID = @ID",
								con
							)
						)
						{
		...
		public Task<IActionResult> Insert([Required] DTO.Magacin.StavkaInsertDTO stavka)
		{
			return Task.Run<IActionResult>(() =>
			{
				if (
					...
					return StatusCode(
						400,
						"Neispravan parametar. VRDOK, BRDOK, ROBAID i KOLICINA moraju biti prosledjeni i veci od 0!"
					);
				try
				{
					using (
					...
					)
					{
						...
						using (
							MySqlCommand cmd = new MySqlCommand(
								"INSERT INTO STAVKA (ROBAID, VRDOK, BRDOK, KOLICINA) VALUES (@ROBAID, @VRDOK, @BRDOK, @KOLICINA)",
								con
							)
						)
						{
			...
			return Task.Run<IActionResult>(() =>
			{
				...
				try
				{
					...
					)
					{
						...
						using (
							MySqlCommand cmd = new MySqlCommand(
								"DELETE FROM STAVKA WHERE STAVKAID = @ID",
								con
							)
						)
						{
			...
			return Task.Run<IActionResult>(() =>
			{
				...
				try
				{
					...
					)
					{
						...
						using (
							MySqlCommand cmd = new MySqlCommand(
								"SELECT STAVKAID, ROBAID, VRDOK, BRDOK, KOLICINA FROM STAVKA WHERE STAVKAID = @SID",
								con
							)
						)
						{
		...
		public Task<IActionResult> List(int? vrDok, int? brDok)
		{
			return Task.Run<IActionResult>(() =>
			{
				...
				try
				{
					if (vrDok != null && brDok == null || brDok == null && brDok != null)
						return StatusCode(
							400,
							"Ukoliko prosledjujete vrdok ili brdok, morate proslediti i drugi parametar!"
						);
					...
					)
					{
						con.Open();
						string cmdText =
							"SELECT STAVKAID, ROBAID, VRDOK, BRDOK, KOLICINA FROM STAVKA WHERE 1";
