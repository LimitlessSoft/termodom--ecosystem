	...
	public class DokumentController : ControllerBase
	{
					...
					)
					{
						...
						using (
							MySqlCommand cmd = new MySqlCommand(
								"SELECT VRDOK, BRDOK, DATUM, KORISNIKID FROM DOKUMENT WHERE VRDOK = @V AND BRDOK = @B",
								con
							)
						)
						{
			...
			return Task.Run<IActionResult>(() =>
			{
				try
				{
					...
					)
					{
						...
						using (
							MySqlCommand cmd = new MySqlCommand(
								"INSERT INTO DOKUMENT (VRDOK, BRDOK, DATUM, KORISNIKID) VALUES (@VRDOK, @BRDOK, @DATUM, @KORISNIKID)",
								con
							)
						)
						{
		...
		private int _maxBrDok(int vrDok)
		{
			using (MySqlConnection con = new MySqlConnection(Program.ConnectionStringMagacin))
			{
				...
				using (
					MySqlCommand cmd = new MySqlCommand(
						"SELECT MAX(BRDOK) DOKUMENT WHERE VRDOK = @V",
						con
					)
				)
				{
