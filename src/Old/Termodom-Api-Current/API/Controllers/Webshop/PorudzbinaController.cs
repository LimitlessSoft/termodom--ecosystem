...
namespace API.Controllers.Webshop
{
	...
	public class PorudzbinaController : ControllerBase
	{
					...
					)
					{
						...
						using (
							MySqlCommand cmd = new MySqlCommand(
								"SELECT COUNT(ID) FROM PORUDZBINA WHERE ID = @ID",
								con
							)
						)
						{
                        ...
                        IME_I_PREZIME = @IME_I_PREZIME WHERE ID = @I",
								con
							)
						)
						{
							...
							cmd.Parameters.AddWithValue(
								"@INTERNI_KOMENTAR",
								porudzbina.InterniKomentar
							);
							cmd.Parameters.AddWithValue(
								"@NACIN_PLACANJA",
								porudzbina.NacinPlacanja
							);
							cmd.Parameters.AddWithValue("@HASH", porudzbina.Hash);
							cmd.Parameters.AddWithValue("@K", porudzbina.K);
							cmd.Parameters.AddWithValue(
								"@USTEDA_KORISNIKA",
								porudzbina.UstedaKorisnik
							);
							cmd.Parameters.AddWithValue(
								"@USTEDA_KLIJENTA",
								porudzbina.UstedaKlijent
							);
							cmd.Parameters.AddWithValue("@DOSTAVA", porudzbina.Dostava);
							cmd.Parameters.AddWithValue(
								"@KOMERCIJALNO_INTERNI_KOMENTAR",
								porudzbina.KomercijalnoInterniKomentar
							);
							cmd.Parameters.AddWithValue(
								"@KOMERCIJALNO_KOMENTAR",
								porudzbina.KomercijalnoKomentar
							);
							cmd.Parameters.AddWithValue(
								"@KOMERCIJALNO_NAPOMENA",
								porudzbina.KomercijalnoNapomena
							);
							cmd.Parameters.AddWithValue("@NAPOMENA", porudzbina.Napomena);
							cmd.Parameters.AddWithValue(
								"@ADRESA_ISPORUKE",
								porudzbina.AdresaIsporuke
							);
							cmd.Parameters.AddWithValue(
								"@KONTAKT_MOBILNI",
								porudzbina.KontaktMobilni
							);
							cmd.Parameters.AddWithValue("@IME_I_PREZIME", porudzbina.ImeIPrezime);
							cmd.Parameters.AddWithValue(
								"@REFERENT_OBRADE",
								porudzbina.ReferentObrade
							);
							cmd.Parameters.AddWithValue(
								"@KOMERICJLNO_KOMENTAR",
								porudzbina.KomercijalnoKomentar
							);
							cmd.ExecuteNonQuery();
						}
				...
				try
				{
					lock (_insertLock)
					{
						...
						)
						{
							...
							using (
								MySqlCommand cmd = new MySqlCommand(
									"SELECT COALESCE(MAX(ID), 0) FROM PORUDZBINA",
									con
								)
							)
							{
							...
							using (
								MySqlCommand cmd = new MySqlCommand(
									@"INSERT INTO PORUDZBINA(ID, KORISNIK_ID, BR_DOK_KOMERCIJALNO, DATUM, STATUS, 
                            ...
                            @NAPOMENA, @ADRESA_ISPORUKE, @KONTAKT_MOBILNI, @IME_I_PREZIME)",
									con
								)
							)
							{
			...
			return Task.Run<IActionResult>(() =>
			{
				try
				{
					...
					)
					{
						...
						using (
							MySqlCommand cmd = new MySqlCommand(
								@"SELECT ID, KORISNIK_ID, BR_DOK_KOMERCIJALNO, DATUM, STATUS, 
                            MAGACIN_ID, PPID, INTERNI_KOMENTAR, REFERENT_OBRADE, NACIN_PLACANJA, HASH, K, USTEDA_KORISNIK, USTEDA_KLIJENT,
                            DOSTAVA, KOMERCIJALNO_INTERNI_KOMENTAR, KOMERCIJALNO_KOMENTAR, KOMERCIJALNO_NAPOMENA, NAPOMENA,
                            ADRESA_ISPORUKE, KONTAKT_MOBILNI, IME_I_PREZIME FROM PORUDZBINA WHERE ID = @I",
								con
							)
						)
						{
							...
							using (MySqlDataReader dt = cmd.ExecuteReader())
							{
								if (dt.Read())
								{
									return StatusCode(
										200,
										new Models.Webshop.Porudzbina()
										{
											ID = Convert.ToInt32(dt["ID"]),
											KorisnikID = Convert.ToInt32(dt["KORISNIK_ID"]),
											BrDokKomercijalno =
												dt["BR_DOK_KOMERCIJALNO"] is DBNull
													? null
													: Convert.ToInt32(dt["BR_DOK_KOMERCIJALNO"]),
											Datum = DateTime.SpecifyKind(
												Convert.ToDateTime(dt["DATUM"]),
												DateTimeKind.Utc
											),
											...
											PPID =
												dt["PPID"] is DBNull
													? null
													: Convert.ToInt32(dt["PPID"]),
											InterniKomentar = dt["INTERNI_KOMENTAR"].ToString(),
											ReferentObrade =
												dt["REFERENT_OBRADE"] is DBNull
													? null
													: Convert.ToInt32(dt["REFERENT_OBRADE"]),
											NacinPlacanja = Convert.ToInt32(dt["NACIN_PLACANJA"]),
											Hash = dt["HASH"].ToString(),
											K = Convert.ToDouble(dt["K"]),
											UstedaKorisnik = Convert.ToDouble(
												dt["USTEDA_KORISNIK"]
											),
											UstedaKlijent = Convert.ToDouble(dt["USTEDA_KLIJENT"]),
											Dostava = Convert.ToInt32(dt["DOSTAVA"]),
											KomercijalnoInterniKomentar = dt[
												"KOMERCIJALNO_INTERNI_KOMENTAR"
											]
												.ToString(),
											KomercijalnoKomentar = dt["KOMERCIJALNO_KOMENTAR"]
												.ToString(),
											KomercijalnoNapomena = dt["KOMERCIJALNO_NAPOMENA"]
												.ToString(),
		...
		public Task<IActionResult> List()
		{
			return Task.Run<IActionResult>(() =>
			{
				...
				try
				{
                    ...
                    MAGACIN_ID, PPID, INTERNI_KOMENTAR, REFERENT_OBRADE, NACIN_PLACANJA, HASH, K, USTEDA_KORISNIK, USTEDA_KLIJENT,
                    DOSTAVA, KOMERCIJALNO_INTERNI_KOMENTAR, KOMERCIJALNO_KOMENTAR, KOMERCIJALNO_NAPOMENA, NAPOMENA,
                    ADRESA_ISPORUKE, KONTAKT_MOBILNI, IME_I_PREZIME FROM PORUDZBINA ORDER BY DATUM DESC LIMIT 1000",
								con
							)
						)
						{
		...
		public Task<IActionResult> Remove(string hash, string mobilni)
		{
			return Task.Run<IActionResult>(() =>
			{
				...
				try
				{
					...
					string query = string.IsNullOrEmpty(hash)
						? " KONTAKT_MOBILNI = @MOBILNI"
						: " HASH = @HASH";
					using (
						MySqlConnection con = new MySqlConnection(Program.ConnectionStringWebshop)
					)
					{
						...
						using (
							MySqlCommand cmd = new MySqlCommand(
								@"SELECT ID FROM PORUDZBINA WHERE" + query,
								con
							)
						)
						{
						...
						using (
							MySqlCommand cmd = new MySqlCommand(
								"DELETE FROM STAVKA WHERE PORUDZBINA_ID = @ID",
								con
							)
						)
						{
						...
						using (
							MySqlCommand cmd = new MySqlCommand(
								"DELETE FROM PORUDZBINA WHERE ID = @ID",
								con
							)
						)
						{
		...
		[ProducesResponseType(StatusCodes.Status200OK)]
		[ProducesResponseType(StatusCodes.Status400BadRequest)]
		public Task<IActionResult> BrDokKomercijalnoSet(
			[Required] int porudzbinaid,
			[Required] int brDokKomercijalno
		)
		{
			return Task.Run<IActionResult>(() =>
			{
