					...
					)
					{
						...
						using (
							MySqlCommand cmd = new MySqlCommand(
								"SELECT COUNT(ID) FROM STAVKA WHERE ID = @ID",
								con
							)
						)
						{
			...
			return Task.Run<IActionResult>(() =>
			{
				...
				try
				{
					...
					)
					{
						...
						using (
							MySqlCommand cmd = new MySqlCommand(
								"SELECT COALESCE(MAX(ID), 0) FROM STAVKA",
								con
							)
						)
						...
						using (
                            ...
                            VALUES
                            (@ID, @PID, @RID, @KOLICINA, @VPCENA, @RABAT)",
								con
							)
						)
						{
			...
			return Task.Run<IActionResult>(() =>
			{
				try
				{
					...
					)
					{
						...
						using (
							MySqlCommand cmd = new MySqlCommand(
								"SELECT ID, PORUDZBINA_ID, ROBA_ID, KOLICINA, VP_CENA, RABAT FROM STAVKA WHERE ID = @ID",
								con
							)
						)
						{
			...
			return Task.Run<IActionResult>(() =>
			{
				...
				try
				{
					...
					)
					{
						con.Open();
						string cmd_text =
							"SELECT ID, PORUDZBINA_ID, ROBA_ID, KOLICINA, VP_CENA, RABAT FROM STAVKA WHERE 1";
			...
			return Task.Run<IActionResult>(() =>
			{
				try
				{
					...
					)
					{
						...
						using (
							MySqlCommand cmd = new MySqlCommand(
								"DELETE FROM STAVKA WHERE ID = @ID",
								con
							)
						)
						{
