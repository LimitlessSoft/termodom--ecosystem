		...
		public Task<IActionResult> Get(int id, string naziv)
		{
			return Task.Run<IActionResult>(() =>
			{
				if (id == 0 && string.IsNullOrWhiteSpace(naziv))
					return StatusCode(
						400,
						"Barem jedan parametar mora biti prosledjen (id ili naziv)"
					);

				if (id > 0 && !string.IsNullOrWhiteSpace(naziv))
					return StatusCode(
						400,
						"Prosledili ste oba parametra sto nije validno! Prosledite ili id ili naziv!"
					);
					...
					)
					{
						...
						using (
							MySqlCommand cmd = new MySqlCommand(
								"INSERT INTO GRUPA (NAZIV, DISPLAY_INDEX) VALUES (@NAZIV, @DISPLAY_INDEX)",
								con
							)
						)
						{
			...
			return Task.Run<IActionResult>(() =>
			{
				try
				{
					...
					)
					{
						...
						using (
							MySqlCommand cmd = new MySqlCommand(
								"SELECT ID, NAZIV, DISPLAY_INDEX FROM GRUPA",
								con
							)
						)
						{
