			...
			return Task.Run<IActionResult>(() =>
			{
				try
				{
					...
					using (MySqlConnection con = new MySqlConnection(Program.ConnectionString))
					{
						...
						using (
							MySqlCommand cmd = new MySqlCommand(
								"SELECT NIVO, CENOVNIK_GRUPAID FROM USER_CENOVNIK WHERE USERID = @UID",
								con
							)
						)
						{
							...
							using (MySqlDataReader dr = cmd.ExecuteReader())
								while (dr.Read())
									uslovi.Add(
										new Tuple<int, int>(
											Convert.ToInt32(dr["CENOVNIK_GRUPAID"]),
											Convert.ToInt32(dr["NIVO"])
										)
									);
						}
					}
					...
					foreach (Models.Webshop.Proizvod proizvod in proizvodi)
					{
						...
						double razlika =
							(maxCena - minCena) * (1 - OD_UKUPNE_RAZLIKE_NAMA_OSTAJE_SIGURNIH);
						double namaOstaje = razlika;

						Tuple<int, int> korisnikovCenovniUslovZaOvajProizvod = null;
						try
						{
							korisnikovCenovniUslovZaOvajProizvod = uslovi.FirstOrDefault(x =>
								x.Item1 == proizvod.CenovnaGrupaID
							);
						}
						catch { }

						int nivo =
							korisnikovCenovniUslovZaOvajProizvod == null
								? 0
								: korisnikovCenovniUslovZaOvajProizvod.Item2;

						double K = razlika / (nProfiCenovnikNivoa - 1) * nivo;

						if (minCena <= 0 || maxCena <= 0)
							c.Add(
								new Models.Cenovnik.Artikal()
								{
									ID = proizvod.RobaID,
									Cena = new Models.Cena() { VPCena = -999999 }
								}
							);
						else
							c.Add(
								...
								{
									ID = proizvod.RobaID,
									Cena = new Models.Cena()
									{
										VPCena = maxCena - K,
										PDV = proizvod.PDV / 100
									}
								}
							);
					}
			...
			return Task.Run<IActionResult>(() =>
			{
				try
				{
					...
					)
					{
						...
						using (
							MySqlCommand cmd = new MySqlCommand(
								"INSERT INTO KORISNIK (ID, STATUS) VALUES (@ID, @STATUS)",
								con
							)
						)
						{
			...
			return Task.Run<IActionResult>(() =>
			{
				try
				{
					...
					)
					{
						...
						using (
							MySqlCommand cmd = new MySqlCommand(
								"SELECT TIP FROM KORISNIK_TIP WHERE KORISNIK_ID = @KID",
								con
							)
						)
						{
							...
							return dr.Read()
								? StatusCode(200, Convert.ToInt32(dr[0]))
								: StatusCode(204);
						}
					}
				}
			...
			return Task.Run<IActionResult>(() =>
			{
				try
				{
					...
					)
					{
						...
						using (
							MySqlCommand cmd = new MySqlCommand(
								"INSERT INTO KORISNIK_TIP (KORISNIK_ID, TIP) VALUES (@KID, @TIP) ON DUPLICATE KEY UPDATE TIP = @TIP",
								con
							)
						)
						{
			...
			return Task.Run<IActionResult>(() =>
			{
				try
				{
					...
					using (MySqlConnection con = new MySqlConnection(Program.ConnectionString))
					{
						...
						using (
							MySqlCommand cmd = new MySqlCommand(
								"SELECT ID FROM KORISNIK WHERE ID = @ID",
								con
							)
						)
						{
						...
						using (
							MySqlCommand cmd = new MySqlCommand(
								@"INSERT INTO POSLEDNJI_PUT_VIDJEN
                            (KORISNIK_ID, DATUM)
                            VALUES
                            (@KORISNIK_ID, @DATUM)
			...
			return Task.Run<IActionResult>(() =>
			{
				try
				{
					using (MySqlConnection con = new MySqlConnection(Program.ConnectionString))
					{
						...
						using (
							MySqlCommand cmd = new MySqlCommand(
								"SELECT DATUM FROM POSLEDNJI_PUT_VIDJEN WHERE KORISNIK_ID = @ID",
								con
							)
						)
						{
			...
			return Task.Run<IActionResult>(() =>
			{
				try
				{
					...
					)
					{
						...
						using (
							MySqlCommand cmd = new MySqlCommand(
								"SELECT ZANIMANJE FROM KORISNIK WHERE ID = @ID",
								con
							)
						)
						{
				...
				try
				{
					...
					)
					{
						...
						)
						{
							...
							cmd.Parameters.AddWithValue(
								"@PRIMA_OBAVESTENJA",
								korisnik.PrimaObavestenja
							);
							cmd.Parameters.AddWithValue("@REFERENT", korisnik.Referent);
							cmd.Parameters.AddWithValue(
								"@DATUM_ODOBRENJA",
								korisnik.DatumOdobrenja
							);
							cmd.Parameters.AddWithValue(
								"@POSLEDNJI_PUT_VIDJEN",
								korisnik.PoslednjiPutVidjen
							);
			...
			return Task.Run<IActionResult>(() =>
			{
				...
				try
				{
							...
							using (MySqlDataReader dr = cmd.ExecuteReader())
								if (dr.Read())
									k = new Korisnik()
									{
										...
										AdresaStanovanja = dr["ADRESA_STANOVANJA"]
											.ToStringOrDefault(),
										...
										PPID =
											dr["PPID"] is DBNull
												? null
												: (Int16?)Convert.ToInt32(dr["PPID"]),
										DatumKreiranja = Convert.ToDateTime(dr["DATUM_KREIRANJA"]),
										PoslatRodjendanskiSMS =
											Convert.ToInt16(dr["POSLAT_RODJENDANSKI_SMS"]) == 1,
					...
					)
					{
						...
						using (
							MySqlCommand cmd = new MySqlCommand(
								"SELECT STATUS, ZANIMANJE, REFERENT, POSETA, MAGACINID, PRIMA_OBAVESTENJA, DATUM_ODOBRENJA, POSLEDNJI_PUT_VIDJEN, KOMENTAR, NADIMAK, ZANIMANJE FROM KORISNIK WHERE ID = @ID",
								con
							)
						)
						{
							...
							using (MySqlDataReader dt = cmd.ExecuteReader())
							{
								...
								{
									k.Status =
										k.Status == 1 ? Convert.ToInt32(dt["STATUS"]) : k.Status;
									k.Referent =
										dt["REFERENT"] is DBNull
											? null
											: Convert.ToInt16(dt["REFERENT"]);
									k.Poseta =
										dt["POSETA"] is DBNull ? 0 : Convert.ToInt32(dt["POSETA"]);
									k.MagacinID = Convert.ToInt16(dt["MAGACINID"]);
									k.PrimaObavestenja = Convert.ToBoolean(dt["PRIMA_OBAVESTENJA"]);
									k.DatumOdobrenja =
										dt["DATUM_ODOBRENJA"] is DBNull
											? null
											: Convert.ToDateTime(dt["DATUM_ODOBRENJA"]);
									k.PoslednjiPutVidjen =
										dt["POSLEDNJI_PUT_VIDJEN"] is DBNull
											? null
											: Convert.ToDateTime(dt["POSLEDNJI_PUT_VIDJEN"]);
									k.Komentar =
										dt["KOMENTAR"] is DBNull ? null : dt["KOMENTAR"].ToString();
			...
			return Task.Run<IActionResult>(() =>
			{
				try
				{
					...
					)
					{
						...
						using (
							MySqlCommand cmd = new MySqlCommand(
								"SELECT ID, STATUS, ZANIMANJE, REFERENT, POSETA, MAGACINID, PRIMA_OBAVESTENJA, DATUM_ODOBRENJA, POSLEDNJI_PUT_VIDJEN, KOMENTAR, NADIMAK FROM KORISNIK",
								con
							)
						)
						{
					...
					{
						DataRow webShop = webShopKorisnik
							.Select("ID = " + api["ID"])
							.FirstOrDefault();
						if (webShop == null)
							continue;
						list.Add(
							new Korisnik()
							{
								...
								Status =
									Convert.ToInt32(api["STATUS"]) == 1
										? Convert.ToInt32(webShop["STATUS"])
										: Convert.ToInt32(api["STATUS"]),
								...
								PoslatRodjendanskiSMS = Convert.ToBoolean(
									api["POSLAT_RODJENDANSKI_SMS"]
								),
								PIB = api["PIB"] is DBNull ? null : api["PIB"].ToString(),
								Zanimanje = Convert.ToInt16(webShop["ZANIMANJE"]),
								Poseta =
									webShop["POSETA"] is DBNull
										? 0
										: Convert.ToInt32(webShop["POSETA"]),
								MagacinID = Convert.ToInt16(webShop["MAGACINID"]),
								PrimaObavestenja = Convert.ToBoolean(webShop["PRIMA_OBAVESTENJA"]),
								Referent =
									webShop["REFERENT"] is DBNull
										? null
										: Convert.ToInt16(webShop["REFERENT"]),
								DatumOdobrenja =
									webShop["DATUM_ODOBRENJA"] is DBNull
										? null
										: Convert.ToDateTime(webShop["DATUM_ODOBRENJA"]),
								PoslednjiPutVidjen =
									webShop["POSLEDNJI_PUT_VIDJEN"] is DBNull
										? null
										: Convert.ToDateTime(webShop["POSLEDNJI_PUT_VIDJEN"]),
								Komentar =
									webShop["KOMENTAR"] is DBNull
										? null
										: webShop["KOMENTAR"].ToString(),
...
@IME, @PW, @TIP, @STATUS, @MOBILNI, @MAIL, @ADRESA_STANOVANJA, @OPSTINA, @DATUM_RODJENJA, @PPID, @DATUM_KREIRANJA, @POSLAT_RODJENDANSKI_SMS, @PIB
						...
						)
						{
							...
							using (
								MySqlCommand cmd = new MySqlCommand(
									"INSERT INTO KORISNIK(ID ,ZANIMANJE, POSETA, STATUS, MAGACINID, PRIMA_OBAVESTENJA, NADIMAK) VALUES(@ID, @ZANIMANJE, 0, 0, @MAGACINID, 1, @NADIMAK)",
									con
								)
							)
							{
		...
		{
			return SimpleHash(
				SimpleHash(SimpleHash(SimpleHash(SimpleHash(SimpleHash(RawPassword)))))
			);
		}
	}
}
