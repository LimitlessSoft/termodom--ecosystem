...
namespace API.Controllers
{
	...
	public class RobaController : ControllerBase
	{
		...
		public Task<IActionResult> Update([Required] Models.Roba roba)
		{
			...
			{
				if (
					string.IsNullOrWhiteSpace(roba.KatBr)
					...
					using (MySqlConnection con = new MySqlConnection(Program.ConnectionString))
					{
						...
						using (
							MySqlCommand cmd = new MySqlCommand(
								"UPDATE ROBA SET KATBR = @KATBR, NAZIV = @NAZIV, JM = @JM WHERE ROBAID = @ID",
								con
							)
						)
						{
				...
				try
				{
					lock (___insertLock)
					{
						using (MySqlConnection con = new MySqlConnection(Program.ConnectionString))
						{
							...
							using (
								MySqlCommand cmd = new MySqlCommand(
									"INSERT INTO ROBA(ROBAID, KATBR, NAZIV, JM) VALUES (@ROBAID, @KATBR, @NAZIV, @JM)",
									con
								)
							)
							{
			...
			return Task.Run<IActionResult>(() =>
			{
				...
				try
				{
					using (MySqlConnection con = new MySqlConnection(Program.ConnectionString))
					{
						...
						using (
							MySqlCommand cmd = new MySqlCommand(
								"SELECT ROBAID, KATBR, NAZIV, JM FROM ROBA WHERE ROBAID = @ID",
								con
							)
						)
						{
			...
			return Task.Run<IActionResult>(() =>
			{
				try
				{
					...
					using (MySqlConnection con = new MySqlConnection(Program.ConnectionString))
					{
						...
						using (
							MySqlCommand cmd = new MySqlCommand(
								"SELECT ROBAID, KATBR, NAZIV, JM FROM ROBA",
								con
							)
						)
						{
