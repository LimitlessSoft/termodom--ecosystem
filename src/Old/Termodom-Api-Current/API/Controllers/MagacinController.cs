...
namespace API.Controllers
{
	...
	public class MagacinController : Controller
	{
		...
		private string[] VALID_PARAMETERS = new string[7]
		{
			"ID",
			"ADRESA",
			"GRAD",
			...
			return Task.Run<IActionResult>(() =>
			{
				try
				{
					...
					if (magacin.Naziv.Length < MIN_CHARACTERS["NAZIV"])
						return StatusCode(
							400,
							$"Naziv magacina mora biti duzi od {MIN_CHARACTERS["NAZIV"]} karaktera"
						);

					if (magacin.Naziv.Length > MAX_CHARACTERS["NAZIV"])
						return StatusCode(
							400,
							$"Naziv magacina ne moze imati vise od {MAX_CHARACTERS["NAZIV"]} karaktera"
						);
					...
					if (magacin.Adresa.Length < MIN_CHARACTERS["ADRESA"])
						return StatusCode(
							400,
							$"Adresa mora biti duza od {MIN_CHARACTERS["ADRESA"]} karaktera"
						);

					if (magacin.Adresa.Length > MAX_CHARACTERS["ADRESA"])
						return StatusCode(
							400,
							$"Adresa ne moze da sadrzi vise od {MAX_CHARACTERS["ADRESA"]} karaktera"
						);
					...
					if (magacin.Telefon[0] != '+' && magacin.Telefon[0] != '0')
						return StatusCode(
							400,
							"Broj telefona nije validan, mora poceti sa 0 ili +"
						);
					...
					if (magacin.Grad.Length < MIN_CHARACTERS["GRAD"])
						return StatusCode(
							400,
							$"Grad ne moze imati manje od {MIN_CHARACTERS["GRAD"]} karaktera"
						);

					if (magacin.Grad.Length > MAX_CHARACTERS["GRAD"])
						return StatusCode(
							400,
							$"Grad ne moze imati vise od {MAX_CHARACTERS["GRAD"]} karaktera"
						);
						...
						using (MySqlConnection con = new MySqlConnection(Program.ConnectionString))
						{
							...
							using (
								MySqlCommand cmd = new MySqlCommand(
									"SELECT COALESCE(MAX(ID),0) FROM MAGACIN",
									con
								)
							)
							{
							...
							using (
								MySqlCommand cmd = new MySqlCommand(
									"INSERT INTO MAGACIN(ID, ADRESA, GRAD, EMAIL, KOORDINATE, TELEFON, NAZIV) VALUES(@ID, @ADRESA, @GRAD, @EMAIL, @KOORDINATE, @TELEFON, @NAZIV)",
									con
								)
							)
							{
			...
			return Task.Run<IActionResult>(() =>
			{
				try
				{
					using (MySqlConnection con = new MySqlConnection(Program.ConnectionString))
					{
						...
						using (
							MySqlCommand cmd = new MySqlCommand(
								"SELECT ID, ADRESA, GRAD, EMAIL, KOORDINATE, TELEFON, NAZIV FROM MAGACIN WHERE ID = @ID",
								con
							)
						)
						{
			...
			return Task.Run<IActionResult>(() =>
			{
				try
				{
					...
					using (MySqlConnection con = new MySqlConnection(Program.ConnectionString))
					{
						...
						using (
							MySqlCommand cmd = new MySqlCommand(
								"UPDATE MAGACIN SET ADRESA = @ADRESA, GRAD = @GRAD, EMAIL = @EMAIL, KOORDINATE = @KOORDINATE, TELEFON = @TELEFON WHERE ID = @ID",
								con
							)
						)
						{
			...
			return Task.Run<IActionResult>(() =>
			{
				try
				{
					...
					using (MySqlConnection con = new MySqlConnection(Program.ConnectionString))
					{
						...
						using (
							MySqlCommand cmd = new MySqlCommand(
								"SELECT ID, ADRESA, GRAD, EMAIL, KOORDINATE, TELEFON, NAZIV FROM MAGACIN WHERE 1 = 1",
								con
							)
						)
						{
		...
		private IActionResult SetujParametar(int magacinID, string parametar, object vrednost)
		{
			...
			if (vrednost is string)
			{
				...
				if (
					...
					return StatusCode(
						400,
						$"Vrednost za parametar ADRESA ne sme biti duza od {MAX_CHARACTERS[parametar]} karaktera!"
					);
			}
			...
			using (MySqlConnection con = new MySqlConnection(Program.ConnectionString))
			{
				...
				using (
					MySqlCommand cmd = new MySqlCommand(
						"SELECT ID FROM MAGACIN WHERE ID = @ID",
						con
					)
				)
				{
		...
		[ProducesResponseType(StatusCodes.Status404NotFound)]
		[SwaggerOperation(Tags = new[] { "api/Magacin" })]
		public Task<IActionResult> KoordinateSet(
			[Required] int magacinID,
