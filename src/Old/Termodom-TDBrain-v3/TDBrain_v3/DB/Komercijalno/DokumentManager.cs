...
WHERE VRDOK = @VRDOK AND BRDOK = @BRDOK",
		...
		)
		{
			...
			)
			{
				...
				cmd.Parameters.AddWithValue(
					"@LINKED",
					ProcedureManager.NextLinked(DateTime.Now, magacinID)
				);
				cmd.Parameters.AddWithValue("@MAGACINID", magacinID);
		...
		public static DokumentManager? Get(int? magacinID, int godina, int vrDok, int brDok)
		{
			...
			using (
				FbConnection con = new FbConnection(
					DB.Settings.ConnectionStringKomercijalno[(int)magacinID, godina]
				)
			)
			{
				con.Open();
		...
		public static DokumentManager? Get(FbConnection con, int vrDok, int brDok)
		{
			using (
				FbCommand cmd = new FbCommand(
					@"SELECT * FROM DOKUMENT WHERE VRDOK = @VRDOK AND BRDOK = @BRDOK",
					con
				)
			)
			{
		...
		/// <returns></returns>
		public static List<DokumentManager> List(
			int? godina = null,
			int[]? magaciniID = null,
		...
		)
		{
			...
			foreach (int magacin in magaciniID)
			{
				using (
					FbConnection con = new FbConnection(
						DB.Settings.ConnectionStringKomercijalno[magacin, (int)godina]
					)
				)
				{
					con.Open();
		...
		/// <param name="queryParameters"></param>
		/// <returns></returns>
		public static Termodom.Data.Entities.Komercijalno.DokumentDictionary Dictionary(
			FbConnection con,
			List<string>? queryParameters = null
		)
		{
			...
			Dictionary<int, Dictionary<int, Termodom.Data.Entities.Komercijalno.Dokument>> dict =
				new Dictionary<
					int,
				...
				using (FbDataReader dr = cmd.ExecuteReader())
					while (dr.Read())
					{
						if (!dict.ContainsKey(Convert.ToInt32(dr["VRDOK"])))
							dict.Add(
								Convert.ToInt32(dr["VRDOK"]),
								new Dictionary<int, Termodom.Data.Entities.Komercijalno.Dokument>()
							);

						dict[Convert.ToInt32(dr["VRDOK"])]
							.Add(
								Convert.ToInt32(dr["BRDOK"]),
								new Termodom.Data.Entities.Komercijalno.Dokument()
								{
									VrDok = Convert.ToInt32(dr["VRDOK"]),
									BrDok = Convert.ToInt32(dr["BRDOK"]),
									Datum = Convert.ToDateTime(dr["DATUM"]),
									...
									PPID =
										dr["PPID"] is DBNull
											? null
											: (int?)Convert.ToInt32(dr["PPID"]),
									Troskovi =
										dr["TROSKOVI"] is DBNull
											? 0
											: Convert.ToDouble(dr["TROSKOVI"]),
									VrDokOut =
										dr["VRDOKOUT"] is DBNull
											? 0
											: Convert.ToInt32(dr["VRDOKOUT"]),
									VrDokIn =
										dr["VRDOKIN"] is DBNull
											? 0
											: Convert.ToInt32(dr["VRDOKIN"]),
									BrDokOut =
										dr["BRDOKOUT"] is DBNull
											? 0
											: Convert.ToInt32(dr["BRDOKOUT"]),
									BrDokIn =
										dr["BRDOKIN"] is DBNull
											? 0
											: Convert.ToInt32(dr["BRDOKIN"]),
									RefID = Convert.ToInt32(dr["REFID"]),
									ZapID = Convert.ToInt32(dr["ZAPID"]),
									Placen = Convert.ToInt32(dr["PLACEN"]),
									MagID =
									...
									AliasU =
										dr["ALIASU"] is DBNull
											...
											: (int?)Convert.ToInt32(dr["ALIASU"]),
									OpisUpl =
										dr["OPISUPL"] is DBNull ? null : dr["OPISUPL"].ToString(),
									Popust1Dana =
										dr["POPUST1DANA"] is DBNull
											? null
											: (int?)Convert.ToInt32(dr["POPUST1DANA"]),
