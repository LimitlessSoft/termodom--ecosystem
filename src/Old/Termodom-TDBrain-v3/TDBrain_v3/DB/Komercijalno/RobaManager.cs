...
ROBAID = @ROBAID",
		...
		)
		{
			using (
				FbCommand cmd = new FbCommand(
					@"INSERT INTO ROBA (ROBAID, KATBR, KATBRPRO, NAZIV,
...
@VRSTA, @AKTIVNA, @GRUPAID, @PODGRUPA, @PROID, @JM, @TARIFAID, @TRPAK, @TRKOL) RETURNING ROBAID",
					con
				)
			)
			{
				cmd.Parameters.Add(
					new FbParameter("ROBAID", FbDbType.Integer)
					{
		...
		/// <returns></returns>
		public static Termodom.Data.Entities.Komercijalno.Roba? Get(
			int robaID,
			int godinaBaze,
			int magacinID
		)
		{
			using (
				FbConnection con = new FbConnection(
					DB.Settings.ConnectionStringKomercijalno[magacinID, godinaBaze]
				)
			)
			{
				con.Open();
		...
		{
			Dictionary<int, Termodom.Data.Entities.Komercijalno.Roba> dict =
				new Dictionary<int, Termodom.Data.Entities.Komercijalno.Roba>();
			using (
				FbConnection con = new FbConnection(
					Settings.ConnectionStringKomercijalno[Settings.MainMagacinKomercijalno, godina]
				)
			)
			{
				con.Open();
					...
					using (FbDataReader dr = cmd.ExecuteReader())
						while (dr.Read())
							dict.Add(
								...
								new Termodom.Data.Entities.Komercijalno.Roba()
								{
									ID = Convert.ToInt32(dr["ROBAID"]),
									Naziv = dr["NAZIV"].ToString(),
									KatBr = dr["KATBR"].ToString(),
