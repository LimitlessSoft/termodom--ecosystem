...
namespace TDBrain_v3.DB.Komercijalno
{
	...
	public class PartnerManager
	{
		...
		)
		{
			...
			foreach (
				string connString in DB.Settings.ConnectionStringKomercijalno.GetConnectionStringsDistinct(
					DateTime.Now.Year - 1
				)
			)
			{
				using (FbConnection con = new FbConnection(connString))
				{
					...
					using (
						FbCommand cmd = new FbCommand(
							"SELECT MAX(PPID) FROM PARTNER WHERE PPID < 100000",
							con
						)
					)
					using (FbDataReader dr = cmd.ExecuteReader())
						if (dr.Read())
							if (maxID != -1 && Convert.ToInt32(dr[0]) != maxID)
								throw new Exceptions.PartnerInsertKonfliktIDevaException(
									$"Maksimalni ID partnera (prvi manji od 100000) nije jedank u svim bazama! {connString}"
								);
							else
			...
			)
			{
				using (FbConnection con = new FbConnection(connString))
				{
					...
					if (noviID != -1 && n != noviID)
						throw new Exception(
							"Insertovan je novi partner u nekoliko baza ali je doslo do konflikta ID-eva! Ispratite rucno sta se desilo!"
						);
		...
		)
		{
            ...
            @IMAUGOVOR, @VRSTACENOVNIKA, @DRZAVLJANSTVOID, @ZANIMANJEID, @WEB_STATUS, @GPPID, @CENE_OD_GRUPE, @VPCID, @PROCPC, @ZAPID) RETURNING PPID",
					con
				)
			)
			{
				cmd.Parameters.Add(
					new FbParameter("PPID", FbDbType.Integer)
					{
		...
		public static PartnerManager? Get(int godina, int ppid)
		{
			using (
				FbConnection con = new FbConnection(
					DB.Settings.ConnectionStringKomercijalno[50, godina]
				)
			)
			{
				con.Open();
		...
		public static PartnerManager? Get(FbConnection con, int ppid)
		{
			using (
				FbCommand cmd = new FbCommand(
					@"SELECT PPID, NAZIV, ADRESA, POSTA, MESTO, TELEFON, FAX,
                EMAIL, KONTAKT, PIB, KATEGORIJA, MBROJ, MOBILNI, MESTOID, AKTIVAN, PDVO,
                DRZAVAID, DUGUJE, POTRAZUJE FROM PARTNER WHERE PPID = @PPID",
					con
				)
			)
			{
...
#pragma warning disable CS8601 // Possible null reference assignment.
						return new PartnerManager()
						{
							...
							MestoID =
								dr["MESTOID"] is DBNull ? null : Convert.ToString(dr["MESTOID"]),
							Telefon =
								dr["TELEFON"] is DBNull ? null : Convert.ToString(dr["TELEFON"]),
							Fax = dr["FAX"] is DBNull ? null : Convert.ToString(dr["FAX"]),
							Email = dr["EMAIL"] is DBNull ? null : Convert.ToString(dr["EMAIL"]),
							Kontakt =
								dr["KONTAKT"] is DBNull ? null : Convert.ToString(dr["KONTAKT"]),
							...
							Mobilni =
								dr["MOBILNI"] is DBNull ? null : Convert.ToString(dr["MOBILNI"]),
		...
		public static PartnerManager? Get(FbConnection con, string pib)
		{
			using (
				FbCommand cmd = new FbCommand(
					@"SELECT PPID, NAZIV, ADRESA, POSTA, MESTO, TELEFON, FAX,
                EMAIL, KONTAKT, PIB, KATEGORIJA, MBROJ, MOBILNI, MESTOID, AKTIVAN, PDVO,
                DRZAVAID, DUGUJE, POTRAZUJE FROM PARTNER WHERE PIB = @PIB",
					con
				)
			)
			{
		...
		public static Dictionary<int, PartnerManager> Dict(FbConnection con)
		{
			...
			using (
				FbCommand cmd = new FbCommand(
					@"SELECT PPID, NAZIV, ADRESA, POSTA, MESTO, TELEFON, FAX,
                EMAIL, KONTAKT, PIB, KATEGORIJA, MBROJ, MOBILNI, MESTOID, AKTIVAN, PDVO,
                DRZAVAID, DUGUJE, POTRAZUJE FROM PARTNER",
					con
				)
			)
			{
