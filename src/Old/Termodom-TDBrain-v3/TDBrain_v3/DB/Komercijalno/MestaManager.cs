...
namespace TDBrain_v3.DB.Komercijalno
{
	public class MestaManager
	{
		...
		/// </summary>
		/// <returns></returns>
		public static Termodom.Data.Entities.Komercijalno.MestoDictionary Dictionary(
			int? godina = null
		)
		{
			Dictionary<string, Termodom.Data.Entities.Komercijalno.Mesto> dict =
				new Dictionary<string, Termodom.Data.Entities.Komercijalno.Mesto>();
				...
				using (FbCommand cmd = new FbCommand("SELECT * FROM MESTA", con))
				{
					using (FbDataReader dr = cmd.ExecuteReader())
					{
						while (dr.Read())
						{
							dict.Add(
								dr["MESTOID"].ToString(),
								new Termodom.Data.Entities.Komercijalno.Mesto()
								{
