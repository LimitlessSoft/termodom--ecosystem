...
namespace TDBrain_v3.DB.Komercijalno
{
	public class VrstaDokManager
	{
		...
		{
			using (
				FbCommand cmd = new FbCommand("SELECT * FROM VRSTADOK WHERE VRDOK = @VRDOK", con)
			)
			{
				...
				using (FbDataReader dr = cmd.ExecuteReader())
				{
					if (dr.Read())
					{
						return new VrstaDok()
						{
							...
							DatumPosled =
								dr["DATUMPOSLED"] is DBNull
									? null
									: (DateTime?)Convert.ToDateTime(dr["DATUMPOSLED"]),
							KljucMenjaDatum = Convert.ToInt32(dr["KLJUCMENJADATUM"]),
							IzmenaMenjaDatum = Convert.ToInt32(dr["IZMENAMENJADATUM"]),
							Poslednji =
								dr["POSLEDNJI"] is DBNull
									? null
									: (int?)Convert.ToInt32(dr["POSLEDNJI"])
						};
					}
				}
			}
