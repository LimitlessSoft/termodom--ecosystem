...
namespace TDBrain_v3.DB.Komercijalno
{
	public class StavkaManager
	{
		...
		public static int Insert(
			int godina,
			int magacinId,
			DokumentManager dokument,
			Termodom.Data.Entities.Komercijalno.Roba roba,
			RobaUMagacinuManager robaUMagacinu,
			double? kolicina,
			double rabat,
			...
			)
			{
				...
				cmd.Parameters.AddWithValue(
					...
					prodajnaCenaBezPDV == null
						? ProcedureManager.ProdajnaCenaNaDan(
							dokument.MagacinID,
							DateTime.Now.Year,
							roba.ID,
							dokument.Datum
		...
		public static List<Termodom.Data.Entities.Komercijalno.Stavka> List(
			FbConnection con,
		...
		/// <returns></returns>
		public static Termodom.Data.Entities.Komercijalno.StavkaDictionary Dictionary(
			FbConnection con,
			List<string>? queryParameters = null
		)
		{
			...
			Dictionary<int, Termodom.Data.Entities.Komercijalno.Stavka> dict =
				new Dictionary<int, Termodom.Data.Entities.Komercijalno.Stavka>();
			...
			)
			{
				using (FbDataReader dr = cmd.ExecuteReader())
				{
					while (dr.Read())
						dict.Add(
							Convert.ToInt32(dr["STAVKAID"]),
							new Termodom.Data.Entities.Komercijalno.Stavka()
							{
								StavkaID = Convert.ToInt32(dr["STAVKAID"]),
								VrDok = Convert.ToInt32(dr["VRDOK"]),
								BrDok = Convert.ToInt32(dr["BRDOK"]),
