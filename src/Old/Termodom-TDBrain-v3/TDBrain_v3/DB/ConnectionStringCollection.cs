...
namespace TDBrain_v3.DB
{
	...
	public class ConnectionStringCollection
	{
		...
		/// </summary>
		private Dictionary<int, Dictionary<int, string>> _putanjeDoBaza =
		...
		public string this[int magacinId, int godina]
		{
			...
			{
				if (
					!_putanjeDoBaza.ContainsKey(godina)
					|| !_putanjeDoBaza[godina].ContainsKey(magacinId)
				)
		...
		public string[] GetConnectionStringsDistinct(int godina)
		{
			...
			for (int i = 0; i < output.Length; i++)
				output[i] =
					$"data source={Settings.ServerName}; initial catalog = {paths[i]}; user={Settings.User}; password={Settings.Password}";
		...
		{
			TDBrain_v3.Settings.DTO.DBSettingsDTO.PutanjeDoBazaDTO dto =
				new TDBrain_v3.Settings.DTO.DBSettingsDTO.PutanjeDoBazaDTO();
			...
			foreach (int godina in _putanjeDoBaza.Keys)
			{
				...
				{
					dto.Items.Add(
						new TDBrain_v3.Settings.DTO.DBSettingsDTO.PutanjeDoBazaDTO.Item()
						{
							MagacinID = magacinid,
							Godina = godina,
