	...
	public static partial class BigBrain
	{
		...
		private static async Task SmsCheckupLoop_UpdateAsync()
		{
			await Task.Run(() =>
			{
				GSMModem.QueueSMS(
					null,
					$"SMS Server Check. Next check sceduled for:"
					...
					try
					{
						...
						switch (parts[0])
						{
							case "SENDSMS":
								...
								{
									Debug.Log(
										" WEB: Pokrenuta je akcija slanja sms-a sa tekstom '"
											+ parts[2]
											+ "' na broj "
											+ parts[1]
											+ "!"
		...
		private static async Task PretvoriUProracun(int brPorudzbine)
		{
			try
			{
				...
				PorudzbinaDTO? porudzbina = JsonConvert.DeserializeObject<PorudzbinaDTO>(
					await (
						await TDWebAPI.GetAsync($"/webshop/porudzbina/get?id={brPorudzbine}")
					).Content.ReadAsStringAsync()
				);
				var stavkePorudzbineResp = await TDWebAPI.GetAsync(
					$"/webshop/stavka/list?porudzbinaID={brPorudzbine}"
				);
				List<StavkaDTO>? stavke = JsonConvert.DeserializeObject<List<StavkaDTO>>(
					await stavkePorudzbineResp.Content.ReadAsStringAsync()
				);
				...
				)
				{
					...
					int brDokKom = DB.Komercijalno.DokumentManager.Insert(
						con,
						32,
						"WEB " + brPorudzbine,
						null,
					...
					DB.Komercijalno.DokumentManager? dokument = DB.Komercijalno.DokumentManager.Get(
						con,
						32,
						brDokKom
					);
					...
					List<Termodom.Data.Entities.Komercijalno.Roba> roba = DB
						.Komercijalno.RobaManager.Collection(con)
						.Values.ToList();
					List<DB.Komercijalno.RobaUMagacinuManager> rums =
						DB.Komercijalno.RobaUMagacinuManager.List(con);
					var tarife = Managers.Komercijalno.TarifeManager.Dictionary(con);
					foreach (var stavka in stavke)
					{
						Termodom.Data.Entities.Komercijalno.Roba? rob = roba.FirstOrDefault(x =>
							x.ID == stavka.RobaID
						);
						DB.Komercijalno.RobaUMagacinuManager? rum = rums.FirstOrDefault(x =>
						...
						{
							Debug.Log(
								$"Roba u magacinu sa datim ID-om ({stavka.RobaID}) nije pronadjen!"
							);
							break;
						}
						...
						double prodajnaCenaNaDan =
							DB.Komercijalno.ProcedureManager.ProdajnaCenaNaDan(
								con,
								porudzbina.MagacinID + 100,
								stavka.RobaID,
								DateTime.Now
						...
						DB.Komercijalno.StavkaManager.Insert(
							con,
							dokument,
							rob,
							rum,
					...
					DB.Komercijalno.KomentariManager.Insert(
						con,
						dokument.VrDok,
						dokument.BrDok,
						komentar.ToString(),
					...
					var res = await TDWebAPI.PostAsync(
						$"/webshop/porudzbina/brdokkomercijalno/set?porudzbinaid={brPorudzbine}&brDokKomercijalno={brDokKom}"
					);

					if ((int)res.StatusCode != 200)
						Debug.Log(
							"Greska prilikom azuriranja broja dokumenta komercijalnog poslovanja u porudzbini na web-u!"
						);

					res = await TDWebAPI.PostAsync(
						$"/webshop/porudzbina/status/set?porudzbinaid={brPorudzbine}&status=1"
					);
