	...
	public class TDWebAPI
	{
		...
		private static async Task LoginAsync()
		{
			var response = await PostAsync(
				"/api/Korisnik/GetToken?username=" + _APIUsername + "&password=" + _APIPassword
			);
		...
		public static async Task<HttpResponseMessage> GetAsync(string endpoint)
		{
			...
			while ((int)resp.StatusCode == 403)
			{
				...
				if (currTry > _requestTries)
					throw new Exception(
						"Premasio sam maksimalni broj pokusaja autorizacije na termdom API!"
					);
		...
		public static Task<HttpResponseMessage> PostAsync(string endpoint)
		{
			return Task.Run(async () =>
			{
				...
				while ((int)resp.StatusCode == 403)
				{
					...
					if (currTry > _requestTries)
						throw new Exception(
							"Premasio maksimalni broj pokusaja autorizacije na termdom API!"
						);
