...
namespace TDBrain_v3.Managers.Komercijalno
{
    ...
    {
        public static int Insert(FbConnection con, int vrDok, int brDok, int robaId, double kolicina, double rabat, double? prodajnaCenaBezPDV = null)
        {
                ...
                @MTID, 'P', 0, 0, 0, @POREZ) RETURNING STAVKAID", con))
            {
                ...
                cmd.Parameters.AddWithValue("@CENA_SA_PDV", prodajnaCenaBezPDV == null ?
                    ProceduraManager.ProdajnaCenaNaDan(con, dokument.MagacinID, roba.ID, dokument.Datum) :
                    prodajnaCenaBezPDV * (1 + (tarife[roba.TarifaID].Stopa / 100)));
                cmd.Parameters.AddWithValue("@CENA_BEZ_PDV", prodajnaCenaBezPDV == null ?
                (double)cmd.Parameters["@CENA_SA_PDV"].Value * (1 - (tarife[roba.TarifaID].Stopa / (100 + tarife[roba.TarifaID].Stopa))) :
                    ProceduraManager.ProdajnaCenaNaDan(con, dokument.MagacinID, roba.ID, dokument.Datum));
