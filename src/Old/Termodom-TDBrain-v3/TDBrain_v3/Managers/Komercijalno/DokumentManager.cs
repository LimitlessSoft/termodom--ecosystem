...
namespace TDBrain_v3.Managers.Komercijalno
{
	...
	public class DokumentManager
	{
				...
				using (FbDataReader dr = cmd.ExecuteReader())
				{
					if (dr.Read())
					{
						return new Dokument()
						{
							...
							VrDokOut =
								dr["VRDOKOUT"] is DBNull ? null : Convert.ToInt32(dr["VRDOKOUT"]),
							VrDokIn =
								dr["VRDOKIN"] is DBNull ? null : Convert.ToInt32(dr["VRDOKIN"]),
							BrDokOut =
								dr["BRDOKOUT"] is DBNull ? null : Convert.ToInt32(dr["BRDOKOUT"]),
							BrDokIn =
								dr["BRDOKIN"] is DBNull ? null : Convert.ToInt32(dr["BRDOKIN"]),
		...
		public static int Insert(
			...
			bool dozvoliDaljeIzmeneUKomercijalnom = true
		)
		{
			...
			)
			{
				...
				cmd.Parameters.AddWithValue(
					"@LINKED",
					ProceduraManager.NextLinked(con, DateTime.Now, magacinId)
				);
				cmd.Parameters.AddWithValue("@MAGACINID", magacinId);
