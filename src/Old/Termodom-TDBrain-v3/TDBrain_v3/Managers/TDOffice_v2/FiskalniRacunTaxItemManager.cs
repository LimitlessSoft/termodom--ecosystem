...
namespace TDBrain_v3.Managers.TDOffice_v2
{
	public static class FiskalniRacunTaxItemManager
	{
		...
		public static void Insert(FbConnection con, FiskalniRacunTaxItem taxItem)
		{
			using (
				FbCommand cmd = new FbCommand(
					"INSERT INTO FISKALNI_RACUN_TAX_ITEM (ID, INVOICE_NUMBER, LABEL, AMOUNT, RATE, CATEGORY_NAME) VALUES (((SELECT COALESCE(MAX(ID), 0) FROM FISKALNI_RACUN_TAX_ITEM) + 1), @IN, @L, @A, @R, @C)",
					con
				)
			)
			{
		...
		public static FiskalniRacunTaxItemDictionary Dictionary(
			FbConnection con,
		...
		)
		{
			...
			Dictionary<string, List<FiskalniRacunTaxItem>> dict =
				new Dictionary<string, List<FiskalniRacunTaxItem>>();
			using (
				FbCommand cmd = new FbCommand(
					"SELECT * FROM FISKALNI_RACUN_TAX_ITEM " + whereQuery,
					con
				)
			)
			{
				...
				using (FbDataReader dr = cmd.ExecuteReader())
					while (dr.Read())
					{
						if (!dict.ContainsKey(dr["INVOICE_NUMBER"].ToString()))
							dict.Add(
								dr["INVOICE_NUMBER"].ToString(),
								new List<FiskalniRacunTaxItem>()
							);

						dict[dr["INVOICE_NUMBER"].ToString()]
							.Add(
								new FiskalniRacunTaxItem()
								{
