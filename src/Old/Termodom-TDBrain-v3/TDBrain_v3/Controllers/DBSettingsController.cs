...
#pragma warning restore CS8604 // Possible null reference argument.
		...
		public Task<IActionResult> BazaKomercijalnoAdd(
			[FromForm] [Required] int magacinId,
			[FromForm] [Required] int godina,
			[FromForm] [Required] string putanja
		)
		{
			return Task.Run<IActionResult>(() =>
			{
				try
				{
					DB.Settings.ConnectionStringKomercijalno.AddOrUpdate(
						magacinId,
						godina,
						putanja
					);
		...
		public Task<IActionResult> BazaKomercijalnoDelete(
			[FromForm] int magacinId,
			[FromForm] int godina
		)
		{
			return Task.Run<IActionResult>(() =>
