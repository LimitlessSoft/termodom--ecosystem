...
namespace TDBrain_v3.Controllers.Komercijalno
{
	...
	public class IzvodController : Controller
	{
		...
		public Task<IActionResult> Collection(
			[FromQuery] [Required] int bazaId,
			[FromQuery] [Required] int godinaBaze,
			[FromQuery] string? pozNaBroj
		)
		{
		...
		public Task<IActionResult> DugujeSum(
			[FromQuery] [Required] int bazaId,
			[FromQuery] [Required] int godinaBaze,
			[FromQuery] [Required] string tekuciRacun
		)
		{
			return Task.Run<IActionResult>(() =>
			{
		...
		public Task<IActionResult> PotrazujeSum(
			[FromQuery] [Required] int bazaId,
			[FromQuery] [Required] int godinaBaze,
			[FromQuery] [Required] string tekuciRacun
		)
