			...
			return Task.Run<IActionResult>(() =>
			{
				try
				{
					...
					if (naziv.Length > 50)
						return StatusCode(
							400,
							"Parametar 'naziv' ne sme imati vise od 50 karaktera!"
						);

					int noviID = DB.Komercijalno.PartnerManager.Insert(
						naziv,
						adresa,
						posta,
						telefon,
			...
			return Task.Run<IActionResult>(() =>
			{
				try
				{
					if (ppid == null && pib == null)
						return StatusCode(
							400,
							"Morate proslediti barem 1 uslov pretrage (pib ili ppid)"
						);

					if (ppid != null && pib != null)
						return StatusCode(
							400,
							"Morate proslediti samo 1 uslov pretrage (pib ili ppid)"
						);
			...
			return Task.Run<IActionResult>(() =>
			{
				try
				{
					...
					)
					{
						con.Open();
						return Json(
							TDBrain_v3.Managers.Komercijalno.PartnerManager.Dictionary(con)
						);
					}
				}
				catch (Exception ex)
