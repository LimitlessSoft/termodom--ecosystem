			...
			return Task.Run<IActionResult>(() =>
			{
				try
				{
					...
					)
					{
						con.Open();
						return Json(
							new Termodom.Data.Entities.Komercijalno.DokumentDictionary(
								DB.Komercijalno.DokumentManager.Dictionary(con, whereParameters)
							)
						);
					}
				}
				...
				try
				{
					...
					if (datumOd != null)
					{
						if (datumOd.Length != 10)
							return StatusCode(
								400,
								"Pogresno formatirana vrednost 'datumOd'! Potreban format 'dd-MM-yyyy'."
							);
					...
					if (datumDo != null)
					{
						if (datumDo.Length != 10)
							return StatusCode(
								400,
								"Pogresno formatirana vrednost 'datumDo'! Potreban format 'dd-MM-yyyy'."
							);
					...
					List<DB.Komercijalno.DokumentManager> list =
						new List<DB.Komercijalno.DokumentManager>();

					foreach (int godina in godine)
						list.AddRange(
							DB.Komercijalno.DokumentManager.List(godina, magacinID, queryParameters)
						);
					...
					)
					{
						...
						return StatusCode(
							...
							DokumentManager.Insert(
								...
								request.PPID,
								request.NuId,
								request.KomercijalnoKorisnikId,
								request.DozvoliDaljeIzmeneUKomercijalnom
							)
						);
					}
				}
