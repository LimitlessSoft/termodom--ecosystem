	...
	public class StavkaController : Controller
	{
					...
					using (
						FbConnection con = new FbConnection(
							DB.Settings.ConnectionStringKomercijalno[
								request.BazaId,
								request.GodinaBaze ?? DateTime.Now.Year
							]
						)
					)
					{
			...
			return Task.Run<IActionResult>(() =>
			{
				try
				{
					...
					)
					{
						...
						return StatusCode(
							201,
							ProceduraManager.NapraviUslugu(
								con,
								request.VrDok,
								request.BrDok,
								request.RobaId,
		...
		public Task<IActionResult> Kopiraj([FromBody] KopirajDTO dto)
		{
			return Task.Run<IActionResult>(() =>
			{
				try
				{
					string izvornaBazaConnString = DB.Settings.ConnectionStringKomercijalno[
						dto.izvornaBaza.magacinID,
						dto.izvornaBaza.godina
					];
					string destinacionaBazaConnString = DB.Settings.ConnectionStringKomercijalno[
						dto.destinacionaBaza.magacinID,
						dto.destinacionaBaza.godina
					];
					...
					DB.Komercijalno.DokumentManager? dokIzvor = DB.Komercijalno.DokumentManager.Get(
						conIzvor,
						dto.izvorniDokument.VrDok,
						dto.izvorniDokument.BrDok
					);
					DB.Komercijalno.DokumentManager? dokDestinacija =
						DB.Komercijalno.DokumentManager.Get(
							conDestinacija,
							dto.destinacioniDokument.VrDok,
							dto.destinacioniDokument.BrDok
						);

					if (dokIzvor == null)
						return StatusCode(
							400,
							$"Izvorni dokument [{dto.izvorniDokument.VrDok}, {dto.izvorniDokument.BrDok}] ne postoji u izvornoj bazi {dto.izvornaBaza}!"
						);

					if (dokDestinacija == null)
						return StatusCode(
							400,
							$"Destinacioni dokument [{dto.destinacioniDokument.VrDok}, {dto.destinacioniDokument.BrDok}] ne postoji u destinacionoj bazi {dto.destinacionaBaza}!"
						);
					...
					Termodom.Data.Entities.Komercijalno.StavkaDictionary izvorneStavke =
						DB.Komercijalno.StavkaManager.Dictionary(
							conIzvor,
							new List<string>()
							{
					...
					if (dto.destinacioniDokumentMoraBitiPrazan)
						...
						else if (dto.destinacioniDokumentOnDuplikatStavkeOpcije == null)
							return StatusCode(
								400,
								$"U slucaju da destinacioni dokument ne mora biti prazan morate proslediti parametar 'destinacioniDokumentOnDuplikatStavkeOpcije'!"
							);

					Termodom.Data.Entities.Komercijalno.RobaDictionary robaCollection =
						DB.Komercijalno.RobaManager.Collection(conDestinacija);
					DB.Komercijalno.RobaUMagacinuManager.RobaUMagacinuCollection robaUMagacinuCollection =
						DB.Komercijalno.RobaUMagacinuManager.Collection(conDestinacija);
					...
					)
					{
						if (!dto.destinacioniDokumentMoraBitiPrazan)
							if (!destinacioneStavke.ContainsKey(izvornaStavka.RobaID))
								if (
									dto.destinacioniDokumentOnDuplikatStavkeOpcije.OnDuplikatStavke
									== DTO.Komercijalno
										.Stavka
										.KopirajDestinacioniDokumentMoraBitiPrazanOptionsOnDuplikatStavke
										.ZaobidjiStavku
								)
									continue;

						Termodom.Data.Entities.Komercijalno.Roba roba = robaCollection[
							izvornaStavka.RobaID
						];
						DB.Komercijalno.RobaUMagacinuManager robaUMagacinu =
							robaUMagacinuCollection[izvornaStavka.MagacinID][izvornaStavka.RobaID];

						int novaDestinacionaStavkaID = DB.Komercijalno.StavkaManager.Insert(
							conDestinacija,
							dokDestinacija,
							roba,
							robaUMagacinu,
							izvornaStavka.Kolicina,
							izvornaStavka.Rabat
						);

						if (dto.nabavneCeneKaoUIzvornomDokumentu)
						{
							DB.Komercijalno.StavkaManager? destinacionaStavka =
								DB.Komercijalno.StavkaManager.Get(
									conDestinacija,
									novaDestinacionaStavkaID
								);

							if (destinacionaStavka == null)
							{
								_logger.LogError(
									"Greska prilikom ucitavanja novokreirane destinacione stavke!"
								);
						...
						if (dto.prodajneCeneKaoUIzvornomDokumentu)
						{
							...
							double prodajnaCenaBP = VPDokumenti.Contains(dto.izvorniDokument.VrDok)
								? izvornaStavka.ProdajnaCena
								: izvornaStavka.ProdCenaBP;
							destinacionaStavka.ProdajnaCena = VPDokumenti.Contains(
								dto.destinacioniDokument.VrDok
							)
								? prodajnaCenaBP
								: izvornaStavka.ProdajnaCena;
			...
			return Task.Run<IActionResult>(() =>
			{
				...
				)
				{
					...
					return Json(
						DB.Komercijalno.StavkaManager.Dictionary(
							con,
							new List<string>() { $"VRDOK = {vrDok} AND BRDOK = {brDok}" }
						)
					);
				}
			});
