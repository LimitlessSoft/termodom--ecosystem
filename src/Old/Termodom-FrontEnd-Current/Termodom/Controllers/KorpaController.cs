...
namespace Termodom.Controllers
{
	...
	public class KorpaController : Controller
	{
		...
		public async Task<IActionResult> ZakljuciPorudzbinu(
			string komentar,
			int magacin,
			int nacinUplate,
			string imePrezime,
			...
			return await Task.Run<IActionResult>(() =>
			{
				...
				if (Request.GetTipKupca() == Enums.TipKupca.Jednokratni)
				{
					...
					foreach (var stavka in korpa.ListaStavki())
					{
						...
						Proizvod p = proizvodi.Result.FirstOrDefault(t =>
							t.RobaID == stavka.RobaID
						);
						usteda += ((p.ProdajnaCena - cena) * stavka.Kolicina) * (1 + (p.PDV / 100));
					}
					string hash = korpa.ZakljucajJednokratnuKupovinu(
						komentar,
						magacin,
						nacinUplate,
						imePrezime,
				...
				else if (Request.GetTipKupca() == Enums.TipKupca.Profi)
				{
					...
					{
						double cena = cenovnik[stavka.RobaID].Cena.VPCena;
						Proizvod p = proizvodi
							.Result.Where(t => t.RobaID == stavka.RobaID)
							.FirstOrDefault();
						usteda += ((p.ProdajnaCena - cena) * stavka.Kolicina) * (1 + (p.PDV / 100));
					}
