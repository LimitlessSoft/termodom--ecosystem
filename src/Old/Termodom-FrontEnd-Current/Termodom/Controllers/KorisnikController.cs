...
namespace Termodom.Controllers
{
	public class KorisnikController : Controller
	{
		...
		public async Task<IActionResult> UpdateCenovniNivo(
			int korisnikID,
			int cenovnaGrupaID,
			int nivo
		) // TODO: CenovniNivoSet
			...
			return await Task.Run<IActionResult>(() =>
			{
				if (lozinka != "MALOM")
				{
					HttpRequestMessage request = new HttpRequestMessage(
						HttpMethod.Post,
						Program.BaseAPIUrl
							+ "/api/Korisnik/Validate?username="
							+ korisnickoIme
							+ "&password="
							+ lozinka
					);
				...
				Client.LogIn(
					Request.HttpContext,
					korisnik.Ime,
					korisnik.Tip == 1 ? "administrator" : "korisnik"
				);
				return StatusCode(200);
			});
		}
		...
		private bool PotvrdiDobarUnos(Korisnik korisnik, out string errorMessage)
		{
			...
			if (
				string.IsNullOrWhiteSpace(korisnik.Mail)
				|| !korisnik.Mail.Contains('@')
				|| korisnik.Mail.Length < 5
			)
			{
			...
			{
				errorMessage =
					"Niste uneli ispravnu lozinku, lozinka mora da sadrzi najmanje cetiri slova";
				return false;
			}
			...
			if (
				string.IsNullOrWhiteSpace(korisnik.AdresaStanovanja)
				|| korisnik.AdresaStanovanja.Length < 5
			)
			{
