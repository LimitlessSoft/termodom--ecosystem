    ...
    public class KontrolnaTablaController : Controller
    {
        ...
        public async Task<IActionResult> GetSkorasnje()
        {
            return await Task.Run(() =>
            {
                ...
                List<Porudzbina> porudzbine = porudzbineTask.Result.Where(x => Math.Abs((x.Datum.ToUniversalTime() - DateTime.UtcNow).TotalDays) <= 3).OrderByDescending(x => x.Datum).ToList();
                ...
                return View("/Views/Porudzbina/_pPrikaz_Mali.cshtml", new Tuple<List<Porudzbina>, List<Korisnik>>(porudzbine, korisnici));
            });
        }
        ...
        public async Task<IActionResult> Filter(int? tip, int strana)
        {
            return await Task.Run(() =>
            {
                ...
                return View("/Views/KontrolnaTabla/_pPrikaz_Mali.cshtml", list.OrderByDescending(x => x.ID)
                                                        .Skip(strana * 15)
                                                        .Take(15)
                                                        .ToList());
            });
        ...
        public async Task<IActionResult> GetSlavljenici(int narednihXDana)
        {
            return await Task.Run(() =>
            {
                ...
                DateTime compareTo = new DateTime(4, DateTime.Now.Month, DateTime.Now.Day).AddDays(narednihXDana);
                ...
                return View("/Views/KontrolnaTabla/_pSlavljenici.cshtml", new Tuple<int, List<Korisnik>>(narednihXDana, slavljenici));
            });
        }
        #endregion
        ...
        public async Task<IActionResult> PosaljiRodjendanskeSMSove(int narednihXDana)
        {
            return await Task.Run(() =>
            {
                ...
                List<Korisnik> slavljenici = new List<Korisnik>();
                Parallel.ForEach(Korisnik.List(), korinsik =>
                {
                    DateTime rodjenje = new DateTime(4, korinsik.DatumRodjenja.Month, korinsik.DatumRodjenja.Day);
                ...
                foreach(var slavljenik in slavljenici)
                {
                    ...
                    try
                    {
                        ...
                        {
                            SMS.SendSMS(slavljenik.Mobilni, "Po TERMODOM podacima uskoro je tvoj rodjendan. SRECNO!!!" +
                                "U to ime poklanjamo ti rodjendanski bonus 50% na alat. www.termodom.rs", this.HttpContext.GetKorisnik().ID);
                ...
                Parallel.ForEach(slavljenici, slavljenik => {
                    try
                    {
                        if (!slavljenik.PoslatRodjendanskiSMS)
                        {
                            SMS.SendSMS(slavljenik.Mobilni, "Po TERMODOM podacima uskoro je tvoj rodjendan. SRECNO!!! U to ime poklanjamo ti rodjendanski bonus 50% na alat. www.termodom.rs", this.HttpContext.GetKorisnik().ID);
                            slavljenik.PoslatRodjendanskiSMS = true;
                            slavljenik.Update();
                        }
                    }
            ...
            return await Task.Run(() =>
            {
                ...
                switch(kome)
                {
                    case 1:
                        ...
                        break;
                    case 2:
                        brojevi.AddRange(Korisnik.List().Where(x => x.PrimaObavestenja).Select(x => x.Mobilni));
                        break;
                    case 8:
                        porudzbine = Porudzbina.List();
                        brojevi.AddRange(Korisnik.List().Where(x => porudzbine.Count(y => y.UserID == x.ID) == 0).Select(x => x.Mobilni));
                        break;
                    case 7:
                        porudzbine = Porudzbina.List();
                        brojevi.AddRange(Korisnik.List().Where(x => porudzbine.Where(y => y.Datum < DateTime.Now.AddDays(-30)).Count(y => y.UserID == x.ID) == 0).Select(x => x.Mobilni));
                        break;
                    case 4:
                        porudzbine = Porudzbina.List();
                        brojevi.AddRange(Korisnik.List().Where(x => porudzbine.Where(y => y.Status == PorudzbinaStatus.Preuzeto).Count(y => y.UserID == x.ID) == 0).Select(x => x.Mobilni));
                        break;
                    case 3:
                        brojevi.AddRange(Korisnik.List().Where(x => x.PoslednjiPutVidjen < DateTime.Now.AddDays(-30)).Select(x => x.Mobilni));
                        break;
                    case 5:
                        porudzbine = Porudzbina.List();
                        brojevi.AddRange(Korisnik.List().Where(x => (DateTime)x.PoslednjiPutVidjen < DateTime.Now.AddDays(-30) || porudzbine.Count(y => y.Datum < DateTime.Now.AddDays(-30)) == 0).Select(x => x.Mobilni));
                        break;
                    case 6:
                        brojevi.AddRange(Korisnik.List().Where(x => x.PoslednjiPutVidjen == null || x.DatumOdobrenja != null && ((DateTime)x.PoslednjiPutVidjen).AddDays(-1) < x.DatumOdobrenja).Select(x => x.Mobilni));
                        break;
                    default:
