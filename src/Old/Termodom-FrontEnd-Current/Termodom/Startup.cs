					...
					if (ep != null)
					{
						foreach (object o in ep.Metadata)
						{
							if (o is Attributes.DefinisaniKorisnikAttribute)
							{
								...
								if (
									context.GetTipKupca() == Enums.TipKupca.NULL
									|| context.GetTipKupca() == Enums.TipKupca.Profi
										&& Client.Get(context) == null
										&& ep.DisplayName
											!= "Termodom.Controllers.KorisnikController.LogovanjeValidacija (Termodom)"
								)
								{
				...
				async (context, next) =>
				{
					if (string.IsNullOrWhiteSpace(context.Request.Cookies["flag-bio-danas"]))
					{
						context.Response.Cookies.Append(
							"flag-bio-danas",
							"1",
							new CookieOptions() { Expires = DateTime.Now.AddHours(6) }
						);
