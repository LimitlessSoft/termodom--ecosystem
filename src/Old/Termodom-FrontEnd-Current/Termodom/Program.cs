...
namespace Termodom
{
    public class Program
    {
        ...
        public static string BelgradeDateTime { get; set; } = TimeZoneInfo.FindSystemTimeZoneById("Central European Standard Time").Id;
        /// <summary>
        ...
        /// </summary>
        public static string ConnectionString { get; set; } = "Server=174.138.184.42;Database=termodom_db_main;Uid=masdos_mdoas;Pwd=j1cnH38$;Pooling=false;";
        /// <summary>
        ...
        /// </summary>
        public static List<Session> AktivneSesijeLogovanihKorisnika { get; set; } = new List<Session>();
        /// <summary>
        ...
        private static void StartPosetilacaCounterResetLoopAsync()
        {
            Thread t1 = new Thread(() =>
            {
                ...
                {
                    if (DateTime.Now.Day != _posetilacaCounterReset.Day ||
        ...
        private static void StartOldSessionCollectionLoopAsync()
        {
            Thread t1 = new Thread(() =>
            {
                while (Program.IsRunning)
                {
                    Program.AktivneSesije.RemoveAll(x => Math.Abs((x.PoslednjiPutAktivna - DateTime.Now).TotalSeconds) > 10);
                    Program.AktivneSesijeLogovanihKorisnika.RemoveAll(x => Math.Abs((x.PoslednjiPutAktivna - DateTime.Now).TotalSeconds) > 10);
                    for (int i = 0; i < 5; i++)
                        Thread.Sleep(1000);
                }
            });
        ...
        public static Task RefreshTokenAsync()
        {
            return Task.Run(() =>
            {
                ...
                {
                    HttpRequestMessage request = new HttpRequestMessage(HttpMethod.Post, Program.BaseAPIUrl + "/api/Korisnik/GetToken?username=" + APIUsername + "&password=" + APIPassword);
