	...
	public class LSSms
	{
		...
		public void Log()
		{
			lock (____LogLock)
			{
				...
				List<LSSms> smss = JsonConvert.DeserializeObject<List<LSSms>>(
					File.ReadAllText(Path.Combine("Ls", "sentSms.ar"))
				);
				...
				File.WriteAllText(
					Path.Combine("Ls", "sentSms.ar"),
					JsonConvert.SerializeObject(smss)
				);
			}
		}
		...
		public static Task<List<LSSms>> GetSentAsync()
		{
			Task<List<LSSms>> t = new Task<List<LSSms>>(() =>
			{
				...
				return JsonConvert.DeserializeObject<List<LSSms>>(
					File.ReadAllText(Path.Combine("Ls", "sentSms.ar"))
				);
			});
