...
namespace Termodom.Models
{
	public static class JednokratnaKupovina
	{
		...
		{
			var korpa = new Korpa() { Identifikator = "-1" };
			korpa.DodajStavku(
				Proizvod.ListAktivnih().OrderByDescending(x => x.ProdajnaCena).First().RobaID,
				Int16.MaxValue
			);
		...
		public static Cenovnik GetCenovnik(Korpa korpa)
		{
			...
			foreach (Proizvod p in sviProizvodi)
			{
				...
				c.Add(
					new Cenovnik.Artikal()
					{
						ID = p.RobaID,
						Cena = new Cena() { VPCena = prodajnaCenaZaOvuKorpu, PDV = p.PDV / 100 }
					}
				);
			}
