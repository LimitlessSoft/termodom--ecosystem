...
namespace Termodom.Models
{
    ...
    public class PoslovniPartner
    {
        ...
        public static string GetName(int PPID)
        {
            ...
            {
                con.Open();
                using (MySqlCommand cmd = new MySqlCommand("SELECT NAZIV FROM POSLOVNI_PARTNER WHERE PPID = @P", con))
                {
        ...
        public static PoslovniPartner GetPartner(int PPID)
        {
            ...
            {
                con.Open();
                using (MySqlCommand cmd = new MySqlCommand("SELECT PPID, NAZIV, ADRESA, PIB FROM POSLOVNI_PARTNER WHERE PPID = @P", con))
                {
                    cmd.Parameters.AddWithValue("@P", PPID);
