...
namespace Termodom.Models
{
    public class Zanimanje // TODO: ZanimanjeModel????
    {
        ...
        {
            HttpRequestMessage request = new HttpRequestMessage(HttpMethod.Get, Program.BaseAPIUrl + "/Webshop/Zanimanje/List");

            APIRequestFailedLog failedLog = null;
            HttpResponseMessage response = APIRequest.Send(request, out failedLog);

            if (response.StatusCode == System.Net.HttpStatusCode.OK)
                return JsonConvert.DeserializeObject<List<Zanimanje>>(response.Content.ReadAsStringAsync().Result);
        ...
        {
            HttpRequestMessage request = new HttpRequestMessage(HttpMethod.Post, Program.BaseAPIUrl + "/Webshop/Zanimanje/Insert?naziv=" + naziv);
        ...
        {
            HttpRequestMessage request = new HttpRequestMessage(HttpMethod.Post, Program.BaseAPIUrl + "/Webshop/Porudzbina/Remove?id=" + id);
            APIRequestFailedLog failedLog = null;
            HttpResponseMessage response = APIRequest.Send(request, out failedLog);
