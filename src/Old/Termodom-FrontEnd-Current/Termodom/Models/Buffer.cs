...
namespace Termodom.Models
{
	...
	public class Buffer<T>
	{
		...
		public T Get(bool updateIfNotFound = true)
		{
			...
			if (Value != null)
				if (
					(LastUpdate - DateTime.Now).TotalMilliseconds
					< _maximumObselence.TotalMilliseconds
				)
					return Value;
		...
		public T Get(TimeSpan maximumObselence, bool updateIfNotFound = true)
		{
			if (Value != null)
				if (
					(LastUpdate - DateTime.Now).TotalMilliseconds
					< maximumObselence.TotalMilliseconds
				)
					return Value;
		...
		{
			if (
				Math.Abs((DateTime.Now - LastUpdate).TotalMilliseconds) < interval.TotalMilliseconds
			)
				return true;
