		...
		public class Cenovnik : Models.Cenovnik
		{
			...
			public static Cenovnik Get(int korisnikID)
			{
				...
				foreach (Proizvod proizvod in sviProizvodi)
				{
					...
					try
					{
						korisnikovCenovniUslovZaOvajProizvod = uslovi
							.Where(x => x.Item1 == proizvod.CenovnaGrupaID)
							.FirstOrDefault();
					}
					catch (Exception) { }
					...
					if (minCena <= 0 || maxCena <= 0)
						c.Add(
							...
							{
								ID = proizvod.RobaID,
								Cena = new Cena() { VPCena = -999999, PDV = proizvod.PDV / 100 }
							}
						);
					else
					{
						...
						c.Add(
							...
							{
								ID = proizvod.RobaID,
								Cena = new Cena() { VPCena = cenaZaKupca, PDV = proizvod.PDV / 100 }
							}
						);
					}
				}
