...
namespace Termodom.Models
{
	public partial class Korisnik
	{
		...
		public DateTime? PoslednjiPutVidjen
		{
			get
			{
				HttpRequestMessage request = new HttpRequestMessage(
					HttpMethod.Get,
					Program.BaseAPIUrl + "/Webshop/Korisnik/Poseta/Get"
				);
		...
		public void Update()
		{
			HttpRequestMessage request = new HttpRequestMessage(
				HttpMethod.Post,
				Program.BaseAPIUrl + "/Webshop/Korisnik/Update"
			);
		...
		public void LogujPosetu()
		{
			HttpRequestMessage request = new HttpRequestMessage(
				HttpMethod.Post,
				Program.BaseAPIUrl + "/Webshop/Korisnik/Poseta/Log?ID=" + this.ID
			);
			APIRequestFailedLog failedLog = null;
			HttpResponseMessage response = APIRequest.Send(request, out failedLog);
		...
		public bool ImaPlatinumCenu(MySqlConnection con = null)
		{
			...
			using (
				MySqlCommand cmd = new MySqlCommand(
					"SELECT COUNT(USERID) FROM USER_CENOVNIK WHERE USERID = @UID AND NIVO = 3 AND CENOVNIK_GRUPAID <> 8",
					con
				)
			)
			{
		...
		public void SetCenovniUslov(MySqlConnection con, int cenovnaGrupaID, int nivo)
		{
			using (
				MySqlCommand cmd = new MySqlCommand(
					"SELECT COUNT(NIVO) FROM USER_CENOVNIK WHERE CENOVNIK_GRUPAID = @CGID AND USERID = @UID",
					con
				)
			)
			{
				...
				if (insert)
				{
					using (
						MySqlCommand cmd1 = new MySqlCommand(
							"INSERT INTO USER_CENOVNIK (NIVO, CENOVNIK_GRUPAID, USERID) VALUES (@N, @CGID, @UID)",
							con
						)
					)
					{
				...
				else
				{
					using (
						MySqlCommand cmd1 = new MySqlCommand(
							"UPDATE USER_CENOVNIK SET NIVO = @N WHERE CENOVNIK_GRUPAID = @CGID AND USERID = @UID",
							con
						)
					)
					{
		...
		public static void UpdateZanimanje(int id, int zanimanje)
		{
			HttpRequestMessage request = new HttpRequestMessage(
				HttpMethod.Get,
				Program.BaseAPIUrl
					+ "/Webshop/Korisnik/Zanimanje/Update?id="
					+ id
					+ "&zanimanje="
					+ zanimanje
			);
		...
		)
		{
			HttpRequestMessage request = new HttpRequestMessage(
				HttpMethod.Post,
				Program.BaseAPIUrl + "/Webshop/Korisnik/Insert"
			);
			request.Content = System.Net.Http.Json.JsonContent.Create<KorisnikInsertDTO>(
				new KorisnikInsertDTO()
				{
					Ime = ime,
					PW = rawPW,
					Tip = tip,
		...
		public static Korisnik Join(
			DTO.Webshop.KorisnikDTO webShopKorisnik,
			DTO.API.KorisnikDTO apiKorisnik
		)
		{
			return new Korisnik
			{
		...
		public static void SetSifra(int id, string password)
		{
			HttpRequestMessage request = new HttpRequestMessage(
				HttpMethod.Post,
				Program.BaseAPIUrl
					+ "/api/Korisnik/PW/Set?korisnikID="
					+ id
					+ "&vrednost="
					+ password
			);
