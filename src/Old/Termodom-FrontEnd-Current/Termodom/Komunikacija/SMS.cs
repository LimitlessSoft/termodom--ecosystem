...
namespace Termodom
{
    public class SMS
    {
                ...
                while (true)
                {
                    try
                    {
                        ...
                        for (int i = _smsQueue.Length - 1; i >= 0; i--)
                        {
                            if (_smsQueue[i] != null)
                            {
                                AKC.Insert($"SENDSMS|{_smsQueue[i].Item1.Mobile}|{_smsQueue[i].Item1.Message}|0", _smsQueue[i].Item1.SenderID);
        ...
        public static void AdministratorSendSMS(string text, int senderID,bool sendImmediatelly = false)
        {
            string[] mobile = File.ReadAllLines(Path.Combine(Program.WebRootPath, "Admin", "telefoni.txt"));
            for (int i = 0; i < mobile.Length; i++)
            {
                SendSMS(mobile[i], text, senderID, sendImmediatelly);
            }
        }
        public static Task SendSMSAsync(string Mobilni, string text, int SenderID, bool sendImmediatelly = false)
        {
            return Task.Run(() =>
            {
        ...
        public static void SendSMS(string Mobilni, string text, int SenderID, bool sendImmediatelly = false)
        {
