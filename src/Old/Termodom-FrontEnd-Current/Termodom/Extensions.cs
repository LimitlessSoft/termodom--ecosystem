...
namespace Termodom
{
    public static class Extenstions
    {
        ...
        public static void PrijaviDaSamIDaljeNaSajtu(this HttpContext context)
        {
            ...
            Program.AktivneSesije.Add(new Session()
            {
                ID = sessionID,
            ...
            {
                Program.AktivneSesijeLogovanihKorisnika.RemoveAll(x => x.ID == logovaniKorisnik.ID.ToString());

                Program.AktivneSesijeLogovanihKorisnika.Add(new Session()
                {
        ...
        /// <returns></returns>
        public static string DivideOnCapital(this object sender, CharacterState FirstCharacterState = CharacterState.Initial, CharacterState SeparatedCharacterState = CharacterState.Lowercase)
        {
        ...
        /// <param name="CapitalOnSeparate"></param>
        /// <returns></returns>
        public static string DivideOnCapital(this string sender, CharacterState FirstCharacterState = CharacterState.Initial, CharacterState SeparatedCharacterState = CharacterState.Lowercase)
        {
            string returned = "";

            sender = sender.Trim();

            returned += FirstCharacterState == CharacterState.Initial ? sender[0] : FirstCharacterState == CharacterState.Capital ? char.ToUpper(sender[0]) : char.ToLower(sender[0]);
            ...
            foreach (char c in sender)
            {
                ...
                {
                    returned += ' ';
                    returned += SeparatedCharacterState == CharacterState.Initial ? c : SeparatedCharacterState == CharacterState.Capital ? char.ToUpper(c) : char.ToLower(c); ;
                    continue;
                }
                returned += c;
            }
