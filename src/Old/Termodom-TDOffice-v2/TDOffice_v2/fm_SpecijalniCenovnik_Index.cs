...
namespace TDOffice_v2
{
	public partial class fm_SpecijalniCenovnik_Index : Form
	{
		...
		private Task<List<Komercijalno.Dokument>> _dokumentiNabavke { get; set; } =
			Task.Run(() =>
			{
				return Komercijalno.Dokument.List($"MAGACINID = 50 AND VRDOK IN (0, 1, 2, 36)");
			});
		private Task<List<Komercijalno.Stavka>> _stavkeNabavke { get; set; } =
			Task.Run(() =>
			{
				return Komercijalno.Stavka.List(
					DateTime.Now.Year,
					$"MAGACINID = 50 AND VRDOK IN (0, 1, 2, 36)"
				);
			});
		private Task<
			List<TDOffice.PartnerKomercijalno>
		> _partnerKomercijalnoKojiSadrzeOvajCenovnik { get; set; }
		private Task<List<Komercijalno.Partner>> _partneriKomercijalno { get; set; } =
			Komercijalno.Partner.ListAsync();
		public static readonly List<Tuple<int, string>> _uslovTipovi = new List<
			Tuple<int, string>
		>()
		{
			...
			new Tuple<int, string>(
				3,
				"Prodajna Cena - % (modifikator) od razlike izmedju Pocetne Cene i Prodajne Cene"
			)
		};
		...
		{
			return Komercijalno.Tarife.List();
		});
		private Task<List<DTO.TDBrain_v3.NabavnaCenaDTO>> _realneNabavneCene = Task.Run(async () =>
		{
			...
			HttpResponseMessage response = await TDBrain_v3.GetAsync(
				$"/komercijalno/roba/getnabavnacena?datum={DateTime.Now.ToString("dd-MM-yyyy")}"
			);
			if ((int)response.StatusCode == 200)
			{
				list = JsonConvert.DeserializeObject<List<DTO.TDBrain_v3.NabavnaCenaDTO>>(
		...
		private void UcitajSpecijalniCenovnik(int cenovnikID)
		{
			...
			_partnerKomercijalnoKojiSadrzeOvajCenovnik = Task.Run(() =>
			{
				...
				foreach (TDOffice.PartnerKomercijalno p in list)
					if (
						p.SpecijalniCenovnikPars != null
						&& p.SpecijalniCenovnikPars.SpecijalniCenovnikList != null
						&& p.SpecijalniCenovnikPars.SpecijalniCenovnikList.Contains(sc.ID)
					)
						newList.Add(p);
			...
			)
			{
				...
				foreach (TDOffice.SpecijalniCenovnik.Item item in _items)
				{
					...
					var rnc = _realneNabavneCene.Result.FirstOrDefault(x =>
						x.RobaID == item.RobaID
					);
					if (rnc == null)
					{
						MessageBox.Show(
							$"Roba [({r.ID}) {r.Naziv}] nema pronadjen ni jedan ulaz (nabavnu cenu)! Bice zobidjena i nece biti prikazana u cenovniku!"
						);
						continue;
					}
					Komercijalno.Tarife tarifa = _tarife.Result.FirstOrDefault(x =>
						x.TarifaID == r.TarifaID
					);
					Tuple<double, double> cenaNaDan = Komercijalno.Procedure.CenaNaDan(
						con,
						12,
						item.RobaID,
						DateTime.Now
					...
					dr["TrenutnaProdajnaCena"] =
						(1 - ((double)tarifa.Stopa / (100 + (double)tarifa.Stopa)))
						* cenaNaDan.Item2;
					dr["ProdajnaCenaSaUslovima"] = IzracunajSpecijalnuCenu(
						realnaNabavnaCena,
						cenaNaDan.Item2,
						item.UslovTip,
						item.UslovModifikator,
						(double)tarifa.Stopa,
						item.NabavnaCenaMargina
					);
					dt.Rows.Add(dr);
				}
			}
			...
			dataGridView1.Columns.Add(
				new DataGridViewComboBoxColumn()
				{
					...
					Width = 400,
					ReadOnly = false,
					HeaderCell = new DataGridViewColumnHeaderCell() { Value = "Uslov Tip" },
		...
		public static double IzracunajSpecijalnuCenu(
			FbConnection con,
			TDOffice.SpecijalniCenovnik.Item specijalniCenovnikItem,
			int magacinID,
			List<Komercijalno.Dokument> dokumentiNabavke = null,
			List<Komercijalno.Stavka> stavkeNabavke = null
		)
		{
			if (dokumentiNabavke == null)
				dokumentiNabavke = Komercijalno.Dokument.List(
					$"MAGACINID = 50 AND VRDOK IN (0, 1, 2, 36)"
				);

			if (stavkeNabavke == null)
				stavkeNabavke = Komercijalno.Stavka.List(
					DateTime.Now.Year,
					$"MAGACINID = 50 AND VRDOK IN (0, 1, 2, 36)"
				);
			...
			return IzracunajSpecijalnuCenu(
				...
				cenaNaDan.Item2,
				specijalniCenovnikItem.UslovTip,
				specijalniCenovnikItem.UslovModifikator,
				(double)tarifa.Stopa,
				specijalniCenovnikItem.NabavnaCenaMargina
			);
		}
		...
		/// <returns></returns>
		private static double IzracunajSpecijalnuCenu(
			double nabavnaCena,
			double prodajnaCena,
			int uslovTip,
			double uslovModifikator,
			double stopa,
			double nabavnaCenaMargina
		)
		{
			double prodajnaBezPDV,
				pocetnaCena,
				razlika;
			switch (uslovTip)
			{
				case 1:
					return nabavnaCena
						* (1 + (nabavnaCenaMargina / 100))
						* (1 + (uslovModifikator / 100));
				case 2:
			...
			using (IzborRobe ir = new IzborRobe(12))
			{
				ir.OnRobaClickHandler += (Komercijalno.RobaUMagacinu[] rums) =>
				{
					...
					)
					{
						...
						{
							Komercijalno.Roba r = _roba.Result.FirstOrDefault(x =>
								x.ID == rum.RobaID
							);
							if (_items.Any(x => x.RobaID == rum.RobaID))
							{
								MessageBox.Show(
									$"Proizvod {r.Naziv} vec postoji u ovom cenovniku!"
								);
								continue;
							}
							int itemID = TDOffice.SpecijalniCenovnik.Item.Insert(
								sc.ID,
								rum.RobaID,
								2,
								0,
							...
							_items.Add(
								new TDOffice.SpecijalniCenovnik.Item()
								{
									ID = itemID,
									CenovnikID = sc.ID,
									MaxRabat = 10,
							...
							double realnaNabavnaCena = _realneNabavneCene
								.Result.First(x => x.RobaID == rum.RobaID)
								.NabavnaCenaBezPDV;
							DataRow dr = dt.NewRow();
							dr["ID"] = itemID;
							dr["CenovnikID"] = sc.ID;
		...
		)
		{
			...
			double realnaNabavnaCena = _realneNabavneCene
				.Result.First(x => x.RobaID == item.RobaID)
				.NabavnaCenaBezPDV;
			...
			{
				case "UslovTip":
					item.UslovTip = _uslovTipovi
						.FirstOrDefault(x => x.Item2 == e.FormattedValue.ToString())
						.Item1;
					item.Update();
					dataGridView1.Rows[e.RowIndex].Cells["ProdajnaCenaSaUslovima"].Value =
						IzracunajSpecijalnuCenu(
							realnaNabavnaCena,
							cenaNaDan.Item2,
							item.UslovTip,
							item.UslovModifikator,
				...
				case "NabavnaCenaMargina":
					...
					item.Update();
					dataGridView1.Rows[e.RowIndex].Cells["PocetnaCena"].Value =
						realnaNabavnaCena * (1 + (item.NabavnaCenaMargina / 100));
					dataGridView1.Rows[e.RowIndex].Cells["ProdajnaCenaSaUslovima"].Value =
						IzracunajSpecijalnuCenu(
							realnaNabavnaCena,
							cenaNaDan.Item2,
		...
		private void button3_Click(object sender, EventArgs e)
		{
			using (
				InputBox ib = new InputBox(
					"Kloniraj",
					"Kloniraj stavke u cenovnik ID (stavke koje ne postoje ce biti dodate)"
				)
			)
			{
				ib.ShowDialog();
		...
		private void button4_Click(object sender, EventArgs e)
		{
			...
			{
				Komercijalno.Partner kp = _partneriKomercijalno.Result.FirstOrDefault(x =>
					x.PPID == p.PPID
				);
				DataRow dr = dt.NewRow();
				dr["PPID"] = p.PPID;
