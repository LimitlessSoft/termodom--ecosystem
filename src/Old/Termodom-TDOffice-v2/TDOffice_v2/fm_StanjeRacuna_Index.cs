    ...
    public partial class fm_StanjeRacuna_Index : Form
    {
        ...
        private async Task UcitajPodatkeAsync()
        {
            try
            {
                ...
                foreach (Firma f in firme.Values)
                {
                    TekuciRacunList tekuciRacuni = await Komercijalno.TekuciRacun.ListAsync(f.GlavniMagacin, godina);
                    DokumentDictionary dokumenti = await Komercijalno.DokumentManager.DictionaryAsync(f.GlavniMagacin, godina, new int[] { 90 });
                    foreach (Termodom.Data.Entities.Komercijalno.TekuciRacun tr in tekuciRacuni.Where(x => x.PPID == f.PPID))
                    {
                        StanjeRacuna stanjeRacuna = await StanjeRacunaManager.GetAsync(f.GlavniMagacin, godina, tr.Racun);
                        double duguje = await IzvodManager.DugujeSumAsync(f.GlavniMagacin, godina, tr.Racun);
                        double potrazuje = await IzvodManager.PotrazujeSumAsync(f.GlavniMagacin, godina, tr.Racun);
                        ...
                        dr["Racun"] = tr.Racun;
                        dr["Valuta"] = tr.Valuta;
                        dr["Poc. stanje Duguje"] = stanjeRacuna == null ? 0 : stanjeRacuna.PocDuguje;
                        dr["Poc. stanje Potrazuje"] = stanjeRacuna == null ? 0 : stanjeRacuna.PocPotrazuje;
                        dr["Duguje"] = duguje;
                        dr["Potrazuje"] = potrazuje;
                        dr["Stanje (iz baze)"] = tr.Stanje;
                        dr["Stanje (izracunato)"] = ((stanjeRacuna == null ? 0 : stanjeRacuna.PocPotrazuje) + potrazuje) - ((stanjeRacuna == null ? 0 : stanjeRacuna.PocDuguje) + duguje);
                        dr["Nezakljucanih izvoda"] = dokumenti.ContainsKey(90) ? dokumenti[90].Values.Count(x => x.OpisUpl == tr.Racun && x.Flag == 0) : 0;
                        dt.Rows.Add(dr);
                    }
                }
                ...
                this.Invoke((MethodInvoker)delegate
                {
                    ...
                    dataGridView1.Columns["Stanje (iz baze)"].DefaultCellStyle.Format = "#,##0.00";
                    dataGridView1.Columns["Stanje (iz baze)"].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;

                    dataGridView1.Columns["Stanje (izracunato)"].DefaultCellStyle.Format = "#,##0.00";
                    dataGridView1.Columns["Stanje (izracunato)"].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
