...
namespace TDOffice_v2
{
	public partial class fm_LagerListaStavki_Index : Form
	{
		...
		private void UcitajRobu_Begin()
		{
			...
			{
				return Komercijalno
					.Roba.List(DateTime.Now.Year)
					.Where(x => _robaIDs.Contains(x.ID))
					.ToList();
			});
		}
		...
		private Task UcitajStavkeAsync()
		{
			return Task.Run(() =>
			{
				List<int> godine = Komercijalno
					.Komercijalno.CONNECTION_STRING.Keys.OrderByDescending(x => x)
					.Take(5)
					.ToList();
				...
				{
					MessageBox.Show(
						$"Greska prilikom ucitavanja pocetnog stanja za dati magacin {_magacinID}"
					);
					return;
				}
				...
				Dictionary<int, List<Komercijalno.Stavka>> stavkePocetnihStanja =
					new Dictionary<int, List<Komercijalno.Stavka>>();
				Dictionary<int, List<Komercijalno.RobaUMagacinu>> krajnjeStanjeStavki =
					new Dictionary<int, List<Komercijalno.RobaUMagacinu>>();
				...
				Parallel.ForEach(
					...
					god =>
					{
						...
						{
							con.Open();
							krajnjeStanjeStavki.Add(
								god,
								Komercijalno.RobaUMagacinu.ListByMagacinID(con, _magacinID)
							);
							stavkePocetnihStanja.Add(
								god,
				...
				foreach (int robaid in _robaIDs)
				{
					...
					{
						Komercijalno.Stavka stavka = stavkePocetnihStanja[god]
							.FirstOrDefault(x => x.RobaID == robaid);
						Komercijalno.RobaUMagacinu rum = krajnjeStanjeStavki[god]
							.FirstOrDefault(x => x.RobaID == robaid);
						dr[$"Pocetak {god} (kol)"] = stavka == null ? 0 : stavka.Kolicina;
						dr[$"Kraj {god} (kol)"] = rum == null ? 0 : rum.Stanje;
						dr[$"Pocetak {god} (vr)"] =
							stavka == null ? 0 : stavka.Kolicina * stavka.ProdajnaCena;
					...
					(MethodInvoker)
						delegate
						{
							...
							foreach (int god in godine)
							{
								...
								dataGridView1.Columns[$"Kraj {god} (kol)"].DefaultCellStyle.Format =
									"#,##0";
								dataGridView1
									.Columns[$"Kraj {god} (kol)"]
									.DefaultCellStyle
									.Alignment = DataGridViewContentAlignment.MiddleRight;

								dataGridView1
									.Columns[$"Pocetak {god} (vr)"]
									.DefaultCellStyle
									.Format = "#,##0";
								dataGridView1
									.Columns[$"Pocetak {god} (vr)"]
								...
								dataGridView1.Columns[$"Kraj {god} (vr)"].DefaultCellStyle.Format =
									"#,##0";
								dataGridView1
									.Columns[$"Kraj {god} (vr)"]
									.DefaultCellStyle
		...
		private void ObojiRazlike()
		{
			foreach (DataGridViewRow row in dataGridView1.Rows)
			{
				...
				for (int i = 2; i < row.Cells.Count; i++)
				{
					...
					if (
						Convert.ToDouble(row.Cells[i].Value)
						!= Convert.ToDouble(row.Cells[i + 1].Value)
					)
