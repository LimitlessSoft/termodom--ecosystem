...
namespace TDOffice_v2
{
	public partial class fm_CheckLista_Index : Form
	{
		...
		private void OsveziListu()
		{
			...
			List<TDOffice.CheckListItem> items = TDOffice.CheckListItem.ListByKorisnikID(
				korisnikID
			);
			...
			foreach (TDOffice.CheckListItem it in items)
			{
				...
				dr["ProbioRok"] =
					it.DatumIzvrsenja == null
					|| (DateTime.Now - (DateTime)it.DatumIzvrsenja).TotalDays > it.IntervalDana
						? true
						: false;
				...
				dr["Izvrsen"] =
					it.DatumIzvrsenja == null
						? "Nikada"
						: ((DateTime)it.DatumIzvrsenja).ToString("dd.MM.yyyy [HH:mm:ss]");
			...
			dataGridView1.Columns["IntervalDana"].AutoSizeMode =
		...
		private void dodajZadatak_btn_Click(object sender, EventArgs e)
		{
			...
			int noviID = TDOffice.CheckListItem.Insert(
				(TDOffice.CheckList.Jobs)jobID,
				korisnikID,
				7
			);
			...
			try
			{
				using (
					InputBox ib = new InputBox(
						"Novi interval",
						"Unesite novi interval dana za ovaj zadatak."
					)
				)
				{
					ib.ShowDialog();
		...
		private void ukloniToolStripMenuItem_Click(object sender, EventArgs e)
		{
			...
			if (
				MessageBox.Show(
					"Da li sigurno zelite ukloniti ovaj zadatak korisniku?",
					"Potvrdi!",
		...
		private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
		{
			...
			{
				TDOffice.CheckList.Jobs job = (TDOffice.CheckList.Jobs)
					Convert.ToInt32(dataGridView1.Rows[e.RowIndex].Cells["ZadatakID"].Value);

				switch (job)
				{
					case TDOffice.CheckList.Jobs.IzvestajProdajeRobe:
						using (
							fm_Izvestaj_Prodaja_Roba_Setup set =
								new fm_Izvestaj_Prodaja_Roba_Setup()
						)
							if (!set.IsDisposed)
								set.ShowDialog();
						break;
					case TDOffice.CheckList.Jobs.DetaljnaAnalizaPartnera:
						using (
							fm_Partner_Analiza_General_Index gen =
								new fm_Partner_Analiza_General_Index()
						)
							if (!gen.IsDisposed)
								gen.ShowDialog();
						break;
					case TDOffice.CheckList.Jobs.NeaktivniPartneri:
						using (
							fm_Partner_Analiza_Neaktivni_Index np =
								new fm_Partner_Analiza_Neaktivni_Index()
						)
							if (!np.IsDisposed)
								np.ShowDialog();
						break;
					case TDOffice.CheckList.Jobs.PrometMagacina:
						if (
							Application
								.OpenForms.OfType<fm_Izvestaj_Promet_Magacina>()
								.FirstOrDefault() != null
						)
							return;
						Task.Run(() =>
						{
							using (
								fm_Izvestaj_Promet_Magacina pm = new fm_Izvestaj_Promet_Magacina()
							)
								if (!pm.IsDisposed)
									pm.ShowDialog();
						});
					...
					case TDOffice.CheckList.Jobs.OdobreniRabatiUProdaji:
						if (
							Application.OpenForms.OfType<fm_OdobreniRabati>().FirstOrDefault()
							!= null
						)
							return;
						Task.Run(() =>
