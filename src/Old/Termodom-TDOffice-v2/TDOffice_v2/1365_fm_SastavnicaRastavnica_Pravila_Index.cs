...
namespace TDOffice_v2
{
    public partial class _1365_fm_SastavnicaRastavnica_Pravila_Index : Form
    {
        ...
        {
            List<TDOffice.StavkaSastavnica> stavkeSastavnice = TDOffice.StavkaSastavnica.ListBySastavnicaID(_sastavnica.ID);
            ...
            this.Text = "Pravilo Sastavnice ["+_sastavnica.ID.ToString()+"] - "+_sastavnica.Tag;
        }

        private void NapuniDGV1(int rid)
        {
            List<TDOffice.StavkaSastavnica> ss = TDOffice.StavkaSastavnica.List().Where(x => x.SastavnicaID == _sastavnica.ID  && x.RobaID == rid).ToList();
        ...
        {
            List<TDOffice.StavkaSastavnica> ssp = TDOffice.StavkaSastavnica.List().Where(x => x.SastavnicaID == _sastavnica.ID  && x.RobaID == rid).ToList();
            if (ssp.Count > 0)
            {
                return;
            }
        ...
        private void _1365_fm_SastavnicaRastavnica_Pravila_Index_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (_izmena)
                if (MessageBox.Show("Imate nesnimljenih izmena, da li ih zelite snimiti?", "Izmene", MessageBoxButtons.YesNo) == DialogResult.Yes)
