...
namespace TDOffice_v2
{
	public partial class IzborRobe : Form
	{
		...
		{
			get { return _dozvoliMenjanjeMagacina; }
			set
			{
		...
		private void InitializeBaseData()
		{
			...
			List<Komercijalno.RobaUMagacinu> robaUMagacinuZaDatiMgacinID =
				MagacinID == null ? null : _robaUMagacinu.Result;
			List<int> robaIDeviDatogMagacina =
				MagacinID == null
					? null
					: robaUMagacinuZaDatiMgacinID.Select(z => z.RobaID).ToList();
			List<Komercijalno.Roba> robaKojaPripadaDatomMagacinuID =
				MagacinID == null
					? _roba.Result
					: _roba.Result.Where(x => robaIDeviDatogMagacina.Any(y => y == x.ID)).ToList();

			Parallel.ForEach(
				robaKojaPripadaDatomMagacinuID,
				...
				r =>
				{
					Komercijalno.RobaUMagacinu rum =
						MagacinID == null
							? null
							: robaUMagacinuZaDatiMgacinID
								.Where(x => x.RobaID == r.ID)
								.FirstOrDefault();
		...
		private void dataGridView1_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
		{
			...
			if (OnRobaClickHandler != null)
				OnRobaClickHandler(
					_robaUMagacinu
						.Result.Where(x => selectedRobaID.Contains(x.RobaID))
						.DistinctBy(z => z.RobaID)
						.ToArray()
				);
		...
		private void OnDodatniFilterFilterChanged(IzborRobe_DodatniFilteri.OnFilterChangedArgs args)
		{
			...
			List<Komercijalno.RobaUMagacinu> robaUMagacinu = Komercijalno
				.RobaUMagacinu.BufferedList(TimeSpan.FromMinutes(60))
				.Where(x =>
					x.MagacinID == (magacini.Count(y => y.ID == MagacinID) == 0 ? 50 : MagacinID)
				)
				.ToList();
			...
			{
				if (
					args.IzvorRobe == IzborRobe_DodatniFilteri.DodatniFilterIzvorRobe.NePopisanaRoba
				)
				{
					...
					args.IzvorRobe == IzborRobe_DodatniFilteri.DodatniFilterIzvorRobe.RobaSaPrometom
				)
				{
					...
					List<Komercijalno.Dokument> dokumenti = Komercijalno
						.Dokument.List()
						.Where(x =>
					...
					robaInFilter.AddRange(
						stavke
							.Where(y =>
								dokumenti.Any(z => z.VrDok == y.VrDok && z.BrDok == y.BrDok)
							)
							.Select(y => y.RobaID)
					);
				}
				...
				{
					List<int> robaIDDokumenta = TDOffice
						.StavkaPopis.List()
						.Where(x => x.BrDok == (int)args.IzvorRobeTag)
						.Select(x => x.RobaID)
						.ToList();
			...
			if (args.StanjeRobe != IzborRobe_DodatniFilteri.DodatniFilterTrenutnoStanjeRobe.NULL)
			{
				robaInFilter.AddRange(
					robaUMagacinu
						.Where(x =>
							args.StanjeRobe
							== IzborRobe_DodatniFilteri.DodatniFilterTrenutnoStanjeRobe.ImaNaStanju
								? x.Stanje > 0
								: x.Stanje == 0
						)
						.Select(x => x.RobaID)
			...
			if (
				args.OptimalneKriticneZalihe
				!= IzborRobe_DodatniFilteri.DodatniFilterKriticneOptimalneZalihe.NULL
			)
			{
				robaInFilter.AddRange(
					robaUMagacinu
						.Where(x =>
							...
							== IzborRobe_DodatniFilteri
								.DodatniFilterKriticneOptimalneZalihe
								.IspodKriticnihZaliha
			...
			MessageBox.Show(
				this,
				"Dodatni filter primenjen u izboru robe. Izlistano "
					+ rows.Count().ToString()
					+ " stavki."
			);
		}
		...
		private void jednostavnoToolStripMenuItem_Click(object sender, EventArgs e)
		{
			using (
				InputBox ib = new InputBox(
					"Filtriraj jednostavno",
					"Unesite broj stavki koji zelite nasumicno da prikazem."
				)
			)
			{
				ib.ShowDialog();
		...
		private void grupisiPoPopisnimGrupamaToolStripMenuItem_Click(object sender, EventArgs e)
		{
			...
			DataRow[] rows = dataGridViewDataTable
				.Copy()
				.Select("KatBrPro LIKE '%?" + popisnaGrupa + "%'")
				.Take(vrednost)
				.ToArray();
			if (rows.Count() > 0)
				dataGridView1.DataSource = rows.CopyToDataTable();
		...
		private void poslednje4GodineToolStripMenuItem_Click(object sender, EventArgs e)
		{
			...
			if (
				MessageBox.Show(
					"Izdvojicete robu koja ima neslaganja za magacin "
						+ Komercijalno.Magacin.Get(DateTime.Now.Year, (int)MagacinID).Naziv
						+ ". Nastaviti?",
					"Potvrdi",
			...
			Thread t1 = new Thread(() =>
			{
				...
				Dictionary<string, List<Komercijalno.Stavka>> stavke =
					new Dictionary<string, List<Komercijalno.Stavka>>();
				...
				for (int i = 1; i < 4; i++)
				{
					using (
						FbConnection con = new FbConnection(
							Komercijalno.Komercijalno.CONNECTION_STRING[year - i]
						)
					)
					{
						con.Open();
						stavke[(year - i).ToString()] = Komercijalno.Stavka.ListByMagacinID(
							con,
							(int)MagacinID
						);
					}
					...
					baseRow =>
					{
						f.Invoke(
							(MethodInvoker)
								...
								{
									slogova_lbl.Text =
										"Racunam stanja (1/2): " + index + " / " + baseRows.Length;
								}
						);
						...
						for (int i = 0; i < 4; i++)
						{
							dr["PS_" + (year - i)] = stavke[(year - i).ToString()]
								.Where(x => x.RobaID == robaID && x.VrDok == 0)
								.Sum(x => x.Kolicina);
							double ulazi = stavke[(year - i).ToString()]
								.Where(x =>
									x.RobaID == robaID
									&& vrDokKojiPovecavajuStanje.Contains(x.VrDok)
								)
								.Sum(x => x.Kolicina);
							double izlazi = stavke[(year - i).ToString()]
								.Where(x =>
									x.RobaID == robaID && vrDokKojiSmanjujuStanje.Contains(x.VrDok)
								)
								.Sum(x => x.Kolicina);
				...
				foreach (Dictionary<string, object> o in finalData)
				{
					f.Invoke(
						(MethodInvoker)
							...
							{
								slogova_lbl.Text =
									"Racunam stanja (2/2): " + index + " / " + baseRows.Length;
							}
					);
				...
				for (int r = dt.Rows.Count - 1; r >= 0; r--)
				{
					this.Invoke(
						(MethodInvoker)
							...
							{
								slogova_lbl.Text =
									"Trazim nepravilnosti... "
									+ index
									+ " / "
									+ baseDataTable.Rows.Count;
							}
					);
				...
				this.Invoke(
					(MethodInvoker)
						delegate
						{
							...
							for (int i = 0; i < 4; i++)
							{
								dataGridView1.Columns["PS_" + (year - i)].DefaultCellStyle.Format =
									"#,##0.00";
								dataGridView1.Columns["KS_" + (year - i)].DefaultCellStyle.Format =
									"#,##0.00";
							}
		...
		private void karticaRobeToolStripMenuItem_Click(object sender, EventArgs e)
		{
			...
			using (
				_7_fm_Komercijalno_Roba_Kartica ist = new _7_fm_Komercijalno_Roba_Kartica(
					robaID,
					(int)MagacinID
				)
			)
