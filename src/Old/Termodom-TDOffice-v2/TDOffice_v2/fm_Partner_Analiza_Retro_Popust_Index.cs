...
namespace TDOffice_v2
{
	public partial class fm_Partner_Analiza_Retro_Popust_Index : Form
	{
		...
		private List<Tuple<int, string>> _meseci { get; set; } =
		...
		private List<Tuple<int, string>> _naciniPlacanja { get; set; } =
			new List<Tuple<int, string>>()
			{
...
",
				...
				)
				{
					...
					cmd.Parameters.AddWithValue(
						"@OD",
						new DateTime(DateTime.Now.Year, mesec, 1, 0, 0, 1)
					);
					cmd.Parameters.AddWithValue(
						"@DO",
						new DateTime(
							DateTime.Now.Year,
							mesec,
							DateTime.DaysInMonth(DateTime.Now.Year, mesec),
							23,
							59,
							59
						)
...
GROUP BY DOKUMENT.VRDOK, DOKUMENT.BRDOK, DOKUMENT.DATUM",
						con
					)
				)
				{
					...
					using (FbDataAdapter da = new FbDataAdapter(cmd))
					{
						...
						dr5["Vrednost"] =
							dt.Rows.Count == 0
								? "0"
								: Convert
									.ToDouble(dt.Compute("SUM(PRODAJNA_VREDNOST)", "VRDOK = 15"))
									.ToString("#,##0.00 RSD");
						dt2.Rows.Add(dr5);
						...
						dr2["Vrednost"] =
							dt.Rows.Count == 0
								? "0"
								: Convert
									.ToDouble(dt.Compute("SUM(NABAVNA_VREDNOST)", String.Empty))
									.ToString("#,##0.00 RSD");
						dt2.Rows.Add(dr2);
						...
						dr3["Vrednost"] =
							dt.Rows.Count == 0
								? "0"
								: Convert
									.ToDouble(dt.Compute("SUM(RAZLIKA)", String.Empty))
									.ToString("#,##0.00 RSD");
						dt2.Rows.Add(dr3);
						...
						dr4["Vrednost"] =
							dt.Rows.Count == 0
								? "0"
								: Convert
									.ToDouble(dt.Compute("SUM(RABAT_VREDNOST)", String.Empty))
									.ToString("#,##0.00 RSD");
						dt2.Rows.Add(dr4);
						...
						dataGridView2.Columns["Opis"].DefaultCellStyle.Alignment =
						...
						dataGridView1.Columns["NABAVNA_VREDNOST"].DefaultCellStyle.Format =
							"#,##0.00";
						dataGridView1.Columns["NABAVNA_VREDNOST"].DefaultCellStyle.Alignment =
							DataGridViewContentAlignment.MiddleRight;

						dataGridView1.Columns["PRODAJNA_VREDNOST"].HeaderText = "Prodajna Vrednost";
						dataGridView1.Columns["PRODAJNA_VREDNOST"].DefaultCellStyle.Format =
							"#,##0.00";
						dataGridView1.Columns["PRODAJNA_VREDNOST"].DefaultCellStyle.Alignment =
							DataGridViewContentAlignment.MiddleRight;
						...
						dataGridView1.Columns["RABAT_VREDNOST"].DefaultCellStyle.Format =
							"#,##0.00";
						dataGridView1.Columns["RABAT_VREDNOST"].DefaultCellStyle.Alignment =
							DataGridViewContentAlignment.MiddleRight;
					}
				}
			}
