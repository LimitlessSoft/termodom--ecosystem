...
namespace TDOffice_v2
{
	public partial class _1332_fm_Proracun_Index : Form
	{
		...
		private Task<_7_fm_Komercijalno_Roba_Kartica> _karticaRobe { get; set; } =
			Task.Run(() =>
			{
		...
		private Task<Termodom.Data.Entities.Komercijalno.MagacinDictionary> _magacini { get; set; } =
			Komercijalno.MagacinManager.DictionaryAsync(DateTime.Now.Year);
		private Task<Termodom.Data.Entities.Komercijalno.PartnerDictionary> _partneri { get; set; } =
			Komercijalno.PartnerManager.DictionaryAsync(DateTime.Now.Year);
		private DataTable stavkeDT = null;
		private TDOffice.Config<int> _maksimalnaZastarelostProracunaPrilikomPretvaranjaUMPRacun { get; set; } =
			TDOffice.Config<int>.Get(5283);
		...
		private async void _1332_fm_PredlogProracuna_Index_LoadAsync(object sender, EventArgs e)
		{
			...
			_proracunStatusLoop = Task.Run(() =>
			{
				while (!this.IsDisposed)
				{
					...
					{
						Komercijalno.Dokument proracunDok = Komercijalno.Dokument.Get(
							_dokument.Datum.Year,
							32,
							(int)_dokument.KomercijalnoProracunBroj
						);
						if (proracunDok != null && proracunDok.Flag == 1)
						{
							this.Invoke(
								(MethodInvoker)
									delegate
									{
										brDokKomProracun_txt.BackColor = Color.FromArgb(
											255,
											192,
											192
										);
		...
		private async Task SetupUIAsync()
		{
			...
			foreach (
				Komercijalno.NacinUplate nu in (Komercijalno.NacinUplate[])
					Enum.GetValues(typeof(Komercijalno.NacinUplate))
			)
			{
				if (((int)nu == 1) || ((int)nu == 5))
					listaNacinaUplate.Add(
						new Tuple<int, string>((int)nu, nu.ToString().DivideOnCapital())
					);
			}

			listaNacinaUplate.Add(
				new Tuple<int, string>((int)-1, (string)"<Izaberi nacin placanja>")
			);
			...
			_ = Task.Run(async () =>
			{
				...
				List<Termodom.Data.Entities.Komercijalno.Partner> partneri = (
					await _partneri
				).Values.ToList();
				//ppid_cmb.Text = "Ucitavanje...";
				partneri.Add(
					new Termodom.Data.Entities.Komercijalno.Partner()
					{
						PPID = -1,
						Naziv = "< partner nije bitan >"
					}
				);
			...
			referent_txt.Text = _korisnici
				.Result.Where(x => x.ID == _dokument.UserID)
				.FirstOrDefault()
				.Username;
			...
			dataGridView1.Columns["Kolicina"].ReadOnly =
				_dokument.Status == DokumentStatus.Otkljucan ? false : true;
			dataGridView1.Columns["Kolicina"].DefaultCellStyle.BackColor =
				_dokument.Status == DokumentStatus.Otkljucan ? Color.LightYellow : Color.White;

			magacin_cmb.Enabled = Program.TrenutniKorisnik.ImaPravo(133204)
				? (_dokument.Status == DokumentStatus.Otkljucan ? true : false)
				: false;
			//ppid_cmb.Enabled = _dokument.Status == DokumentStatus.Otkljucan;
			dokumentStatus_btn.Enabled =
				_dokument.Status == DokumentStatus.Otkljucan
					? Program.TrenutniKorisnik.ImaPravo(133202)
					: Program.TrenutniKorisnik.ImaPravo(133203);
			...
			if (_dokument.KomercijalnoProracunBroj != null)
			{
				Komercijalno.Dokument proracunKomercijalno = Komercijalno.Dokument.Get(
					_dokument.Datum.Year,
					32,
					(int)_dokument.KomercijalnoProracunBroj
				);
				...
				klonirajUKomercijalno_btn.Visible =
					_dokument.Status == DokumentStatus.Zakljucan
					&& proracunKomercijalno.BrDokOut == null;
			}

			pretvoriUMPRacun_btn.Visible = _dokument.KomercijalnoProracunBroj != null;

			this.dokumentStatus_btn.Text =
				_dokument.Status == DokumentStatus.Otkljucan ? "Zakljucaj" : "Otkljucaj";
			this.BackColor =
				_dokument.Status == DokumentStatus.Otkljucan
					? Color.Green
					: _dokument.KomercijalnoProracunBroj == null
						? Color.Red
						: Color.Purple;
		...
		private void SetDGV()
		{
			...
			dataGridView1.Columns["JM"].DefaultCellStyle.Alignment =
		...
		private void OnIzborRobeTrigger(Komercijalno.RobaUMagacinu[] args)
		{
			...
			using (InputBox ib = new InputBox("Kolicina", "Unesite kolicinu"))
			{
				...
				try
				{
					...
					DokumentProracun.Stavka.Insert(
						_dokument,
						r.RobaID,
						kol,
		...
		private void dataGridView1_CellValidated(object sender, DataGridViewCellEventArgs e)
		{
			if (dataGridView1.Columns[e.ColumnIndex].Name == "Kolicina")
			{
				...
				double staraKolicina = _stavke
					.Result.Where(x => x.ID == idStavke)
					.FirstOrDefault()
					.Kolicina;
				double novaKolicina = Convert.ToDouble(
					dataGridView1.Rows[e.RowIndex].Cells["Kolicina"].Value
				);
		...
		private void KlonirajUProracun()
		{
			...
			)
			{
				...
				MagacinClan mvl = MagacinClan
					.ListByMagacinID(_dokument.MagacinID)
				...
				{
					MessageBox.Show(
						"Magacin " + _dokument.MagacinID + " nema dodeljenog vlasnika magacina!"
					);
					return;
				}
				...
				{
					MessageBox.Show(
						$"TDOffice korisniku [{mv.ID}]{mv.Username} nije povezan nalog sa komercijalnim poslovanjem!"
					);
					return;
				}
				...
				if (_dokument.KomercijalnoProracunBroj == null)
				{
					proracunKomercijalno = Komercijalno.Dokument.Insert(
						con,
						32,
						"TD " + _dokument.ID,
				...
				Komercijalno.Dokument dokumentKomercijalno = Komercijalno.Dokument.Get(
					con,
					32,
					proracunKomercijalno
				);
				dokumentKomercijalno.Flag = 0;
				dokumentKomercijalno.Update();

				// Brisem stavke iz proracuna za slucaj da je vec kreiran
				List<Komercijalno.Stavka> stavke = Komercijalno.Stavka.ListByDokument(
					DateTime.Now.Year,
					32,
					proracunKomercijalno
				);
				...
				Komercijalno.Dokument.SetKomentar(
					dokumentKomercijalno.Datum.Year,
					32,
					proracunKomercijalno,
					_dokument.Komentar
				);
				...
				List<Komercijalno.RobaUMagacinu> rumKomercijalno = Komercijalno.RobaUMagacinu.List(
					con
				);

				foreach (DokumentProracun.Stavka s in _stavke.Result)
				{
					Komercijalno.Roba r = robaKomercijalno
						.Where(x => x.ID == s.RobaID)
						.FirstOrDefault();
					Komercijalno.RobaUMagacinu rum = rumKomercijalno
						.Where(x => x.RobaID == s.RobaID && x.MagacinID == _dokument.MagacinID)
						.FirstOrDefault();
					Komercijalno.Stavka.Insert(
						DateTime.Now.Year,
						dokumentKomercijalno,
						r,
						rum,
						s.Kolicina,
						0,
		...
		private void PretvoriUMPRacun()
		{
			...
			int maxd = Convert.ToInt32(
				_maksimalnaZastarelostProracunaPrilikomPretvaranjaUMPRacun.Tag
			);
			...
			)
			{
				...
				List<Komercijalno.Roba> robaKomercijalno = Komercijalno.Roba.List(con);
				List<Komercijalno.RobaUMagacinu> rumKomercijalno = Komercijalno
					.RobaUMagacinu.List(con)
					.Where(x => x.MagacinID == _dokument.MagacinID)
					.ToList();
				...
				{
					mpRacunBroj = Komercijalno.Dokument.Insert(
						con,
						15,
						"TD " + _dokument.ID,
						...
						User.Get(_dokument.UserID).KomercijalnoUserID,
						null
					);
				...
				{
					MessageBox.Show(
						"MP Racun mora biti otkljucan kako bi proracun bio ponovo prebacen u njega!"
					);
					return;
				}
				...
				foreach (DokumentProracun.Stavka s in _stavke.Result)
				{
					...
					int novaStavkaID = Komercijalno.Stavka.Insert(
						con,
						mpRacunKomercijalno,
						r,
						rum,
						...
						0,
						null
					); //Ovo je ID stavke za koju treba proracunati rabat
					stavkaKomerc = Komercijalno.Stavka.Get(DateTime.Now.Year, novaStavkaID);
					stavkaKomercProracun = Komercijalno
						.Stavka.ListByDokument(
							DateTime.Now.Year,
							32,
							(int)_dokument.KomercijalnoProracunBroj
						)
						.FirstOrDefault(x => x.RobaID == s.RobaID);
				...
				SetupUIAsync()
					.ContinueWith(
						(prev) =>
						{
							...
							{
								var stRobaKojeNemaNaStanju = string.Join(
									"," + Environment.NewLine,
									robaKojeNemaNaStanju
								);
		...
		private void cmb_NacinPlacanja_SelectedIndexChanged(object sender, EventArgs e)
		{
			...
			if (
				(Komercijalno.NacinUplate)Convert.ToInt32(cmb_NacinPlacanja.SelectedValue)
					== Komercijalno.NacinUplate.Virman
				& (int)ppid_cmb.SelectedValue <= 0
			)
			{
				MessageBox.Show("Za ovaj nacin uplate morate izabrati partnera!");
			...
			_dokument.NUID = (Komercijalno.NacinUplate)
				Convert.ToInt32(cmb_NacinPlacanja.SelectedValue);
			_dokument.Update();
		}

		private void podesiMaksimalnuZastarelostProracunaPrilikomPretvaranjaUMPRacunToolStripMenuItem_Click(
			object sender,
			EventArgs e
		)
		{
			if (
				MessageBox.Show(
					$"Trenutna maksimalna zastarelost proracuna je {_maksimalnaZastarelostProracunaPrilikomPretvaranjaUMPRacun.Tag} dana, da li zelite da je promenite?",
					"Promena",
			...
			using (
				InputBox ib = new InputBox(
					"Nova vrednost zastarelosti proracuna...",
			...
			)
			{
				...
				int novaVrednost;
				if (
					string.IsNullOrWhiteSpace(ib.returnData)
					|| !int.TryParse(ib.returnData, out novaVrednost)
				)
				{
