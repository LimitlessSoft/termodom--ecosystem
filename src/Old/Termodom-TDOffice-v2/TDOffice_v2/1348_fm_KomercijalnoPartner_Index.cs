...
namespace TDOffice_v2
{
	public partial class _1348_fm_Partner_Index : Form
	{
		...
		private Task<List<Komercijalno.Zemlja>> _zemlja = Task.Run(() =>
		{
			...
			zemlja.Add(new Komercijalno.Zemlja() { DrzavaID = -1, Naziv = "<Sve zemlje>" });
		...
		private void PopulateData()
		{
			try
			{
				...
				specijalniCenovnikOtherUslov_cmb.SelectedValue =
					_tdofficePartnerKomercijalno == null
						? 2
						: _tdofficePartnerKomercijalno.SpecijalniCenovnikPars.OtherTip;
				specijalniCenovnikOtherModifikator_txt.Text =
					_tdofficePartnerKomercijalno == null
						? "0"
						: _tdofficePartnerKomercijalno.SpecijalniCenovnikPars.OtherModifikator.ToString();
			}
			catch (Exception ex)
			{
		...
		private async Task LoadAsync()
		{
			...
			opstine.Add(new Opstina() { ID = -1, Naziv = "Sve opstine" });
			...
			try
			{
				...
				specijalniCenovnikOtherUslov_cmb.DataSource =
					fm_SpecijalniCenovnik_Index._uslovTipovi;
			...
			this.Location = new System.Drawing.Point(
				10 + (System.DateTime.Now.Millisecond / 5),
				10 + (System.DateTime.Now.Millisecond / 5)
			);
		...
		private void ToggleUI(bool state)
		{
			...
			clbSpecijalniCenovniciNacinUplate.Enabled =
				state == true ? !_partnerId.HasValue : false;
			specijalniCenovnikOtherUslov_cmb.Enabled = state == true ? !_partnerId.HasValue : false;
			specijalniCenovnikOtherModifikator_txt.Enabled =
				state == true ? !_partnerId.HasValue : false;
		}
		...
		private async void btnSacuvaj_ClickAsync(object sender, EventArgs e)
		{
			try
			{
				...
				{
					MessageBox.Show(
						"Neispravan tip uslova nedefinisane robe u specijalnom cenovniku!"
					);
					return;
				}
				...
				{
					MessageBox.Show(
						"Neispravna vrednost modifikatora nedefinisane robe u specijalnom cenovniku!"
					);
					return;
				}
				...
				try
				{
					specijalniCenovnikOtherModifikator = Convert.ToInt32(
						specijalniCenovnikOtherModifikator_txt.Text
					);
				}
				catch
				...
				if (_partner.PPID == 0)
				{
					...
					if (_tekuciRacun.Racun == null)
					{
						...
						if (
							MessageBox.Show(
								"Niste uneli Racun. Da li zelite da unesete novog partnera bez Racuna?",
								"Novi partner",
					...
					int NewPPID;
					NewPPID = await Komercijalno.Partner.Insert(
						_partner.Naziv,
						_partner.Adresa,
						_partner.Posta,
						_partner.Telefon,
						...
						_partner.RefID,
						_partner.PDVO,
						_partner.NazivZaStampu,
						_partner.Valuta,
						_partner.DozvoljeniMinus,
						_partner.ImaUgovor,
						_partner.VrstaCenovnika,
						_partner.DrzavljanstvoID,
						_partner.ZanimanjeID,
						_partner.WEB_Status,
						_partner.GPPID,
						_partner.Cene_od_grupe,
						_partner.VPCID,
						_partner.PROCPC
					);
					...
					if (
						!(
							await TekuciRacunManager.InsertAsync(
								NewPPID,
								_tekuciRacun.Racun,
								_tekuciRacun.BankaID,
								_partner.Valuta,
								0,
								null
							)
				...
				else
				{
					...
					{
						TDOffice.PartnerKomercijalno.Insert(
							_partner.PPID,
							new TDOffice.PartnerKomercijalno.SpecijalniCenovnikDTO()
						);
						_tdofficePartnerKomercijalno = TDOffice.PartnerKomercijalno.Get(
							_partner.PPID
						);
					}

					_tdofficePartnerKomercijalno.SpecijalniCenovnikPars.OtherModifikator =
						specijalniCenovnikOtherModifikator;
					_tdofficePartnerKomercijalno.SpecijalniCenovnikPars.OtherTip = Convert.ToInt32(
						specijalniCenovnikOtherUslov_cmb.SelectedValue
					);
		...
		private void cmbMesto_Validated(object sender, EventArgs e)
		{
			...
			List<Termodom.Data.Entities.Komercijalno.Mesto> list =
				cmbMesto.DataSource as List<Termodom.Data.Entities.Komercijalno.Mesto>;
		...
		private void clbSpecijalniCenovnici_SelectedIndexChanged(object sender, EventArgs e)
		{
			...
			{
				_tdofficePartnerKomercijalno.SpecijalniCenovnikPars =
					new TDOffice.PartnerKomercijalno.SpecijalniCenovnikDTO()
					{
		...
		private void clbSpecijalniCenovniciNacinUplate_SelectedIndexChanged(
			object sender,
			EventArgs e
		)
		{
			...
			_tdofficePartnerKomercijalno
				.SpecijalniCenovnikPars
				.NaciniUplateZaKojiVazeSpecijalniCenovnici = scs;
		}
	}
}
