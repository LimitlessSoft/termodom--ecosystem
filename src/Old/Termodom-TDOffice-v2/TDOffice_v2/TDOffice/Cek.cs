...
WHERE ID = @ID",
		...
		public static Cek Get(FbConnection con, int id)
		{
			using (
				FbCommand cmd = new FbCommand(
					"SELECT ID, STATUS, MAGACINID, PODNOSILAC_BANKA, TR_GRADJANA, BROJ_CEKA, DATUM_VALUTE, VREDNOST, ZADUZIO, DATUM FROM CEK WHERE ID = @ID",
					con
				)
			)
			{
				...
				using (FbDataReader dr = cmd.ExecuteReader())
				{
					if (dr.Read())
					{
						return new Cek()
						{
							...
							Zaduzio =
								dr["ZADUZIO"] is DBNull
									? null
									: (int?)Convert.ToInt32(dr["ZADUZIO"]),
		...
		public static List<Cek> List(FbConnection con)
		{
			...
			using (
				FbCommand cmd = new FbCommand(
					"SELECT ID, STATUS, MAGACINID, PODNOSILAC_BANKA, TR_GRADJANA, BROJ_CEKA, DATUM_VALUTE, VREDNOST, ZADUZIO, DATUM FROM CEK",
					con
				)
			)
			{
		...
		public static List<Cek> ListByMagacinID(FbConnection con, int magacinID)
		{
			...
			using (
				FbCommand cmd = new FbCommand(
					"SELECT ID, STATUS, MAGACINID, PODNOSILAC_BANKA, TR_GRADJANA, BROJ_CEKA, DATUM_VALUTE, VREDNOST, ZADUZIO, DATUM FROM CEK WHERE MAGACINID = @MID",
					con
				)
			)
			{
		...
		public static List<Cek> ListByDatum(FbConnection con, DateTime datum)
		{
			...
			using (
				FbCommand cmd = new FbCommand(
					"SELECT ID, STATUS, MAGACINID, PODNOSILAC_BANKA, TR_GRADJANA, BROJ_CEKA, DATUM_VALUTE, VREDNOST, ZADUZIO, DATUM FROM CEK WHERE DATUM = @D",
					con
				)
			)
			{
