...
namespace TDOffice_v2.TDOffice
{
    public class StavkaSastavnica
    {
        ...
        {
            using (FbCommand cmd = new FbCommand("Update STAVKA_SASTAVNICA SET SASTAVNICA_ID = @SID, ROBAID = @RID, KOLICINA = @K WHERE ID = @ID", con))
            {
        ...
        {
            using(FbCommand cmd = new FbCommand(@"INSERT INTO STAVKA_SASTAVNICA (ID, SASTAVNICA_ID, ROBAID, KOLICINA) 
                VALUES 
        ...
        {
            using (FbCommand cmd = new FbCommand ("SELECT ID, SASTAVNICA_ID, ROBAID, KOLICINA FROM STAVKA_SASTAVNICA WHERE ID = @ID", con))
            {
                cmd.Parameters.AddWithValue("@ID", id);
        ...
        public static List<StavkaSastavnica> List(FbConnection con)
        {
            ...
            using (FbCommand cmd = new FbCommand("SELECT ID, SASTAVNICA_ID, ROBAID, KOLICINA FROM STAVKA_SASTAVNICA", con))
            {
                using (FbDataReader dr = cmd.ExecuteReader())
                    while (dr.Read())
                        list.Add(new StavkaSastavnica()
                        {
                            ID = Convert.ToInt32(dr["ID"]),
                            SastavnicaID = Convert.ToInt32(dr["SASTAVNICA_ID"]),
                            RobaID = Convert.ToInt32(dr["ROBAID"]),
        ...
        {
            List<StavkaSastavnica> list = new List<StavkaSastavnica>();
            using (FbCommand cmd = new FbCommand("SELECT ID, SASTAVNICA_ID, ROBAID, KOLICINA FROM STAVKA_SASTAVNICA WHERE SASTAVNICA_ID = @SID", con))
            {
                cmd.Parameters.AddWithValue("@SID", sastavnicaID);
                using (FbDataReader dr = cmd.ExecuteReader())
                    while (dr.Read())
        ...
        {
            using(FbCommand cmd = new FbCommand("DELETE FROM STAVKA_SASTAVNICA WHERE ID = @ID", con))
            {
                cmd.Parameters.AddWithValue("@ID", id);
