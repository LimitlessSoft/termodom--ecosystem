	...
	public partial class SpecifikacijaNovca
	{
		...
		public void Update(FbConnection con)
		{
			...
			using (
				FbCommand cmd = new FbCommand(
					"UPDATE SPECIFIKACIJA_NOVCA SET TAG = @T, MAGACINID = @M, DATUM =@D WHERE ID = @ID",
					con
				)
			)
			{
		...
		public static SpecifikacijaNovca Get(int specifikacijaID)
		{
			using (FbConnection con = new FbConnection(TDOffice.connectionString))
			{
				...
				using (
					FbCommand cmd = new FbCommand(
						"SELECT ID, DATUM, MAGACINID, TAG FROM SPECIFIKACIJA_NOVCA WHERE ID = @ID",
						con
					)
				)
				{
		...
		public static SpecifikacijaNovca Get(FbConnection con, int magacinID, DateTime datum)
		{
			using (
				FbCommand cmd = new FbCommand(
					"SELECT ID, DATUM, MAGACINID, TAG FROM SPECIFIKACIJA_NOVCA WHERE DATUM = @D AND MAGACINID = @M",
					con
				)
			)
			{
		...
		public static int Insert(FbConnection con, int magacinID, Detalji detalji)
		{
			using (
				FbCommand cmd = new FbCommand(
					"INSERT INTO SPECIFIKACIJA_NOVCA (ID, DATUM, MAGACINID, TAG) VALUES (((SELECT COALESCE(MAX(ID), 0) FROM SPECIFIKACIJA_NOVCA) + 1), @D, @M, @T) RETURNING ID",
					con
				)
			)
			{
