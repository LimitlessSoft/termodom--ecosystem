...
namespace TDOffice_v2.TDOffice
{
	public static class FiskalniRacunManager
	{
		...
		public async static Task<List<string>> InsertAsync(
			List<Termodom.Data.Entities.TDOffice_v2.FiskalniRacun> fiskalniRacuni
		)
		{
			var response = await TDBrain_v3.PostAsync(
				$"/tdoffice/fiskalniracun/insert",
				fiskalniRacuni
			);
		...
		/// <exception cref="Exception"></exception>
		/// <exception cref="Termodom.Data.Exceptions.APIUnhandledStatusException"></exception>
		public async static Task<FiskalniRacunDictionary> DictionaryAsync(
			string odDatuma,
			string doDatuma
		)
		{
			var response = await TDBrain_v3.GetAsync(
				$"/tdoffice/fiskalniracun/dictionary?odDatuma={odDatuma}&doDatuma={doDatuma}"
			);

			switch ((int)response.StatusCode)
			{
				case 200:
					return JsonConvert.DeserializeObject<FiskalniRacunDictionary>(
						await response.Content.ReadAsStringAsync()
					);
				case 500:
					throw new Termodom.Data.Exceptions.APIServerException();
