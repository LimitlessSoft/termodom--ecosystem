...
namespace TDOffice_v2.TDOffice
{
    public class DokumentSastavnica
    {
        ...
        {
            using (FbCommand cmd = new FbCommand(@"UPDATE DOKUMENT_SASTAVNICA SET DATUM = @D,
        ...
        public static int Insert(FbConnection con, int magacinID, int korisnik, DokumentSastavnicaTip tip, string tag)
        {
            ...
            using(FbCommand cmd = new FbCommand(@"INSERT INTO DOKUMENT_SASTAVNICA (ID, MAGACINID, DATUM, KORISNIK, TIP, TAG, STATUS)
                                            VALUES
        ...
        {
            using(FbCommand cmd = new FbCommand("SELECT ID, DATUM, KORISNIK, BRDOK_KOM, TIP, TAG, MAGACINID, STATUS FROM DOKUMENT_SASTAVNICA WHERE ID = @ID", con))
            {
                cmd.Parameters.AddWithValue("@ID", id);
                ...
                using (FbDataReader dr = cmd.ExecuteReader())
                    if (dr.Read())
                        return new DokumentSastavnica()
                        {
                            ...
                            Datum = dr["DATUM"] is DBNull ? null : (DateTime?)Convert.ToDateTime(dr["DATUM"]),
                            ...
                            BrDokKom = dr["BRDOK_KOM"] is DBNull ? null : (int?)Convert.ToInt32(dr["BRDOK_KOM"]),
        ...
        public static List<DokumentSastavnica> List(FbConnection con)
        {
            ...
            using (FbCommand cmd = new FbCommand("SELECT ID, DATUM, KORISNIK, BRDOK_KOM, TIP, MAGACINID, STATUS, TAG FROM DOKUMENT_SASTAVNICA", con))
            {
                using (FbDataReader dr = cmd.ExecuteReader())
                    while (dr.Read())
                        list.Add(new DokumentSastavnica()
                        {
                            ID = Convert.ToInt32(dr["ID"]),
                            MagacinID = Convert.ToInt32(dr["MAGACINID"]),
                            Datum = dr["DATUM"] is DBNull ? null : (DateTime?)Convert.ToDateTime(dr["DATUM"]),
        ...
        {
            List<DokumentSastavnica> list = new List<DokumentSastavnica>();
            using (FbCommand cmd = new FbCommand("SELECT ID, DATUM, KORISNIK, BRDOK_KOM, TIP, MAGACINID, STATUS, TAG FROM DOKUMENT_SASTAVNICA WHERE MAGACINID = @MID", con))
            {
                cmd.Parameters.AddWithValue("@MID", magacinID);
                using (FbDataReader dr = cmd.ExecuteReader())
                    while (dr.Read())
        ...
        public static List<DokumentSastavnica> ListByTip(FbConnection con, DokumentSastavnicaTip tip)
        {
            List<DokumentSastavnica> list = new List<DokumentSastavnica>();
            using (FbCommand cmd = new FbCommand("SELECT ID, DATUM, KORISNIK, BRDOK_KOM, TIP, MAGACINID, TAG, STATUS FROM DOKUMENT_SASTAVNICA WHERE TIP = @TIP", con))
            {
