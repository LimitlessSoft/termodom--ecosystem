	...
	public class PartnerKomercijalno
	{
		...
		public SpecijalniCenovnikDTO SpecijalniCenovnikPars
		{
			get
			{
				...
				{
					_specijalniCenovnikParsBuffer =
						JsonConvert.DeserializeObject<SpecijalniCenovnikDTO>(
							_specijalniCenovnikPars
						);
				}
				return _specijalniCenovnikParsBuffer;
			}
			set { _specijalniCenovnikParsBuffer = value; }
		}
		...
		public void Update(FbConnection con)
		{
			using (
				FbCommand cmd = new FbCommand(
					"UPDATE PARTNER_KOMERCIJALNO SET SPECIJALNI_CENOVNIK_PARS = @SCP WHERE PPID = @PPID",
					con
				)
			)
			{
				cmd.Parameters.AddWithValue(
					"@SCP",
					JsonConvert.SerializeObject(SpecijalniCenovnikPars)
				);
		...
		public static void Insert(
			FbConnection con,
			int ppid,
			SpecijalniCenovnikDTO specijalniCenovnikParameters
		)
		{
			using (
				FbCommand cmd = new FbCommand(
					"INSERT INTO PARTNER_KOMERCIJALNO (PPID, SPECIJALNI_CENOVNIK_PARS) VALUES (@PPID, @SCP)",
					con
				)
			)
			{
				cmd.Parameters.AddWithValue(
					"@SCP",
					JsonConvert.SerializeObject(specijalniCenovnikParameters)
				);
				cmd.Parameters.AddWithValue("@PPID", ppid);
		...
		public static PartnerKomercijalno Get(FbConnection con, int ppid)
		{
			using (
				FbCommand cmd = new FbCommand(
					"SELECT PPID, SPECIJALNI_CENOVNIK_PARS FROM PARTNER_KOMERCIJALNO WHERE PPID = @PPID",
					con
				)
			)
			{
				...
				using (FbDataReader dr = cmd.ExecuteReader())
					if (dr.Read())
						return new PartnerKomercijalno()
						{
							...
							_specijalniCenovnikPars = dr["SPECIJALNI_CENOVNIK_PARS"]
								.ToStringOrDefault()
						};
			}
			return null;
		...
		public static List<PartnerKomercijalno> List(FbConnection con)
		{
			...
			using (
				FbCommand cmd = new FbCommand(
					"SELECT PPID, SPECIJALNI_CENOVNIK_PARS FROM PARTNER_KOMERCIJALNO",
					con
				)
			)
			{
