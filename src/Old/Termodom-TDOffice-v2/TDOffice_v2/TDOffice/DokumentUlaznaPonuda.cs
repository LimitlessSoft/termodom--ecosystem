...
FROM DOKUMENT_ULAZNA_PONUDA WHERE ID = @ID", con))
            {
...
WHERE 1 = 1 " + whereQuery, con))
            {
                using (FbDataReader dr = cmd.ExecuteReader())
                    while (dr.Read())
                        list.Add(new DokumentUlaznaPonuda()
                        {
                            ID = Convert.ToInt32(dr["ID"]),
                            Datum = Convert.ToDateTime(dr["DATUM"]),
                            Korisnik = Convert.ToInt32(dr["KORISNIK"]),
        ...
        public static int Insert(int korisnikID, DateTime datumPocetkaVazenja, DateTime datumKrajaVazenja)
        {
            using(FbConnection con = new FbConnection(TDOffice.connectionString))
            {
                con.Open();
        ...
        public static int Insert(FbConnection con, int korisnikID, DateTime datumPocetkaVazenja, DateTime datumKrajaVazenja)
        {
            using(FbCommand cmd = new FbCommand(@"
