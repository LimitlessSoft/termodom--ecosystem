	...
	public class StavkaPopis
	{
		...
		public void Update(FbConnection con)
		{
			using (
				FbCommand cmd = new FbCommand(
					@"UPDATE STAVKA_POPIS SET
                    KOLICINA = @KOL,
                    PORUCENO = @POR,
                    SKART = @SKAR,
                    PRODAJNA_CENA = @PC
                    WHERE STAVKA_POPIS_ID = @ID",
					con
				)
			)
			{
		...
		public static List<StavkaPopis> List(FbConnection con)
		{
			...
			using (
				FbCommand cmd = new FbCommand(
					@"
                SELECT STAVKA_POPIS_ID, KOLICINA, BRDOK, ROBAID, PORUCENO, SKART, PRODAJNA_CENA FROM STAVKA_POPIS",
					con
				)
			)
			{
					...
					while (dr.Read())
						list.Add(
							new StavkaPopis()
							{
								...
								Kolicina =
									dr["KOLICINA"] is DBNull
										? null
										: (double?)Convert.ToDouble(dr["KOLICINA"]),
		...
		public static List<StavkaPopis> ListByDokument(FbConnection con, int brojPopisa)
		{
			...
			using (
				FbCommand cmd = new FbCommand(
					@"
                SELECT STAVKA_POPIS_ID, KOLICINA, BRDOK, ROBAID, PORUCENO, SKART, PRODAJNA_CENA FROM STAVKA_POPIS WHERE BRDOK = @BRDOK",
					con
				)
			)
			{
		...
		)
		{
			lock (_insertLock)
			{
				using (
                    ...
                    @QUANTITY, @ROBAID, @PORUCENO, @SKART, @BRDOK, @PC) RETURNING STAVKA_POPIS_ID",
						con
					)
				)
				{
		...
		public static void Remove(FbConnection con, int stavkaID)
		{
			using (
				FbCommand cmd = new FbCommand(
					"DELETE FROM STAVKA_POPIS WHERE STAVKA_POPIS_ID = @ID",
					con
				)
			)
			{
		...
		{
			using (
				FbCommand cmd = new FbCommand("DELETE FROM STAVKA_POPIS WHERE BRDOK = @BRDOK", con)
			)
			{
				cmd.Parameters.AddWithValue("@BRDOK", brDok);
				cmd.ExecuteNonQuery();
