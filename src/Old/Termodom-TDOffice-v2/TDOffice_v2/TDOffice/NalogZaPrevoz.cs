...
namespace TDOffice_v2.TDOffice
{
	public class NalogZaPrevoz
	{
		...
		public class Destinacija
		{
			...
			public Enums.NalogZaPrevozOdredisteNacinPlacanja NacinPlacanja { get; set; } =
				Enums.NalogZaPrevozOdredisteNacinPlacanja.NaRuke;
		}
		...
		public static int Insert(int magacinID, int userID, Detalji detalji)
		{
			using (FbConnection con = new FbConnection(TDOffice.connectionString))
			{
				...
				using (
					FbCommand cmd = new FbCommand(
						@"INSERT INTO NALOG_ZA_PREVOZ
                    (ID, DATUM, KORISNIKID, MAGACINID, STATUS, NACIN_PLACANJA, CENA_MARSUTE_BEZ_PDV, PREVOZNIK, TAG)
                    VALUES
                    (((SELECT COALESCE(MAX(ID), 0) FROM NALOG_ZA_PREVOZ) + 1), @D, @U, @M, @S, @NP, @CMBP, 0, @T) RETURNING ID",
						con
					)
				)
				{
					...
					cmd.Parameters.AddWithValue(
						"@T",
						Encoding.UTF8.GetBytes(JsonConvert.SerializeObject(detalji))
					);
					cmd.Parameters.Add(
						new FbParameter
						{
		...
		public static NalogZaPrevoz Get(int id)
		{
			using (FbConnection con = new FbConnection(TDOffice.connectionString))
			{
				...
				using (
					FbCommand cmd = new FbCommand(
						@"SELECT ID, DATUM, STATUS, MAGACINID, KORISNIKID, NACIN_PLACANJA, CENA_MARSUTE_BEZ_PDV,
                    PREVOZNIK, TAG FROM NALOG_ZA_PREVOZ WHERE ID = @ID",
						con
					)
				)
				{
					...
					using (FbDataReader dr = cmd.ExecuteReader())
					{
						if (dr.Read())
							return new NalogZaPrevoz()
							{
								...
								NacinPlacanja = (NacinPlacanjaPrevozniku)
									Convert.ToInt32(dr["NACIN_PLACANJA"]),
								NamaNaplacenPrevozBezPDV =
									dr["CENA_MARSUTE_BEZ_PDV"] is DBNull
										? 0
										: Convert.ToDouble(dr["CENA_MARSUTE_BEZ_PDV"]),
								Prevoznik = Convert.ToInt32(dr["PREVOZNIK"]),
								Tag = JsonConvert.DeserializeObject<Detalji>(
		...
		public static List<NalogZaPrevoz> List(FbConnection con)
		{
			...
			using (
				FbCommand cmd = new FbCommand(
					"SELECT ID, DATUM, MAGACINID, KORISNIKID, NACIN_PLACANJA, CENA_MARSUTE_BEZ_PDV, PREVOZNIK, TAG, STATUS FROM NALOG_ZA_PREVOZ",
					con
				)
			)
			{
		...
		public static List<NalogZaPrevoz> List(FbConnection con, string whereQuery)
		{
			...
			using (
				FbCommand cmd = new FbCommand(
					"SELECT ID, DATUM, MAGACINID, KORISNIKID, NACIN_PLACANJA, CENA_MARSUTE_BEZ_PDV, PREVOZNIK, TAG, STATUS FROM NALOG_ZA_PREVOZ WHERE 1 = 2"
						+ whereQuery,
					con
				)
			)
		...
		public static List<NalogZaPrevoz> ListByMagacinID(FbConnection con, int magacinID)
		{
			...
			using (
				FbCommand cmd = new FbCommand(
					"SELECT ID, DATUM, MAGACINID, KORISNIKID, NACIN_PLACANJA, CENA_MARSUTE_BEZ_PDV, PREVOZNIK, TAG, STATUS FROM NALOG_ZA_PREVOZ WHERE MAGACINID = @MID",
					con
				)
			)
			{
