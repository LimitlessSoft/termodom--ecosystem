	...
	public partial class SpecijalniCenovnik
	{
		public class Item
		{
			...
			public void Update(FbConnection con)
			{
				using (
					FbCommand cmd = new FbCommand(
						"UPDATE SPECIJALNI_CENOVNIK_ITEM SET CENOVNIKID = @CID, ROBAID = @RID, USLOV_TIP = @UT,"
							+ "MAX_RABAT = @MR, NABAVNA_CENA_MARGINA = @NCM, USLOV_VREDNOST = @UV WHERE ID = @ID",
						con
					)
				)
				{
			...
			)
			{
				using (
					FbCommand cmd = new FbCommand(
						@"INSERT INTO SPECIJALNI_CENOVNIK_ITEM (ID, CENOVNIKID, ROBAID, USLOV_TIP, USLOV_VREDNOST, MAX_RABAT, NABAVNA_CENA_MARGINA)
VALUES
...
RETURNING ID",
			...
			public static Item Get(FbConnection con, int id)
			{
				using (
					FbCommand cmd = new FbCommand(
						"SELECT ID, CENOVNIKID, ROBAID, USLOV_TIP, USLOV_VREDNOST, MAX_RABAT, NABAVNA_CENA_MARGINA FROM SPECIJALNI_CENOVNIK_ITEM WHERE ID = @ID",
						con
					)
				)
				{
			...
			public static List<Item> List(FbConnection con, string whereQuery = null)
			{
				...
				using (
					FbCommand cmd = new FbCommand(
						"SELECT ID, CENOVNIKID, ROBAID, USLOV_TIP, USLOV_VREDNOST, MAX_RABAT, NABAVNA_CENA_MARGINA FROM SPECIJALNI_CENOVNIK_ITEM "
							+ whereQuery,
						con
					)
				)
						...
						while (dr.Read())
							list.Add(
								new Item()
								{
									...
									UslovModifikator = Convert.ToDouble(dr["USLOV_VREDNOST"]),
									MaxRabat = Convert.ToDouble(dr["MAX_RABAT"]),
									NabavnaCenaMargina = Convert.ToDouble(
										dr["NABAVNA_CENA_MARGINA"]
									)
								}
							);
				}
			...
			public static void Delete(FbConnection con, int itemID)
			{
				using (
					FbCommand cmd = new FbCommand(
						"DELETE FROM SPECIJALNI_CENOVNIK_ITEM WHERE ID = @ID",
						con
					)
				)
				{
