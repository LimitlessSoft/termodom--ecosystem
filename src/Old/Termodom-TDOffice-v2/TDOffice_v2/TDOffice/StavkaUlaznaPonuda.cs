...
WHERE ID = @ID",
		...
		public static StavkaUlaznaPonuda Get(FbConnection con, int id)
		{
			using (
				FbCommand cmd = new FbCommand(
					"SELECT ID, ROBAID, NABAVNA_CENA, BRDOK FROM STAVKA_ULAZNA_PONUDA WHERE ID = @ID",
					con
				)
			)
			{
		...
		public static List<StavkaUlaznaPonuda> List(FbConnection con, string whereQuery = null)
		{
			...
			using (
				FbCommand cmd = new FbCommand(
					"SELECT ID, ROBAID, NABAVNA_CENA, BRDOK FROM STAVKA_ULAZNA_PONUDA WHERE 1 = 1 "
						+ whereQuery,
					con
				)
			)
		...
		public static int Insert(FbConnection con, int brDok, int robaID, double nabavnaCena)
		{
			using (
				FbCommand cmd = new FbCommand(
					"INSERT INTO STAVKA_ULAZNA_PONUDA (ID, ROBAID, NABAVNA_CENA, BRDOK) VALUES (((SELECT COALESCE(MAX(ID), 0) FROM STAVKA_ULAZNA_PONUDA) + 1), @RID, @NC, @BRDOK) RETURNING ID",
					con
				)
			)
			{
		...
		public static void Delete(FbConnection con, int id)
		{
			using (
				FbCommand cmd = new FbCommand(
					"DELETE FROM STAVKA_ULAZNA_PONUDA WHERE ID = @ID",
					con
				)
			)
			{
