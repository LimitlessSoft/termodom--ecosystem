...
namespace TDOffice_v2.TDOffice
{
	...
	public class PartnerGrupaItem
	{
		...
		public static bool Exists(FbConnection con, int partnerID, int partnerGrupaID)
		{
			using (
				FbCommand cmd = new FbCommand(
					"SELECT COUNT(PARTNER_ID) FROM PARTNER_GRUPA_ITEM WHERE PARTNER_GRUPA_ID = @PGID AND PARTNER_ID = @PID",
					con
				)
			)
			{
		...
		public static List<PartnerGrupaItem> ListByPartner(FbConnection con, int partnerID)
		{
			...
			using (
				FbCommand cmd = new FbCommand(
					"SELECT PARTNER_ID, PARTNER_GRUPA_ID FROM PARTNER_GRUPA_ITEM WHERE PARTNER_ID = @PID",
					con
				)
			)
			{
		...
		public static List<PartnerGrupaItem> ListByGrupa(FbConnection con, int partnerGrupaID)
		{
			...
			using (
				FbCommand cmd = new FbCommand(
					"SELECT PARTNER_ID, PARTNER_GRUPA_ID FROM PARTNER_GRUPA_ITEM WHERE PARTNER_GRUPA_ID = @PID",
					con
				)
			)
			{
		...
		public static void Insert(FbConnection con, int partnerID, int partnerGrupaID)
		{
			...
			using (
				FbCommand cmd = new FbCommand(
					"INSERT INTO PARTNER_GRUPA_ITEM (PARTNER_ID, PARTNER_GRUPA_ID) VALUES (@PID, @PGID)",
					con
				)
			)
			{
		...
		public static void Delete(FbConnection con, int partnerID)
		{
			using (
				FbCommand cmd = new FbCommand(
					"DELETE FROM PARTNER_GRUPA_ITEM WHERE PARTNER_ID = @PID",
					con
				)
			)
			{
		...
		public static void Delete(FbConnection con, int partnerID, int partnerGrupaID)
		{
			using (
				FbCommand cmd = new FbCommand(
					"DELETE FROM PARTNER_GRUPA_ITEM WHERE PARTNER_ID = @PID AND PARTNER_GRUPA_ID = @PGID",
					con
				)
			)
			{
