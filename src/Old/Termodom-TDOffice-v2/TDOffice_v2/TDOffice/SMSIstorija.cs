		...
		public static string GenerateRandomSetHash()
		{
			...
			using (SHA256 sha256Hash = SHA256.Create())
			{
				...
				byte[] bytes = sha256Hash.ComputeHash(
					Encoding.UTF8.GetBytes(Random.Next(0, 10000000).ToString())
				);
		...
		)
		{
			using (
				FbCommand cmd = new FbCommand(
					@"INSERT INTO SMS_ISTORIJA (ID, PARTNER_ID, TEKST, DATUM, HASH) VALUES
(((SELECT COALESCE(MAX(ID), 0) FROM SMS_ISTORIJA) + 1), @PID, @TEKST, @DATUM, @HASH) RETURNING ID",
					con
				)
			)
			{
		...
		public static SMSIstorija Get(FbConnection con, int id)
		{
			using (
				FbCommand cmd = new FbCommand(
					"SELECT ID, PARTNER_ID, TEKST, DATUM, HASH FROM SMS_ISTORIJA WHERE ID = @ID",
					con
				)
			)
			{
		...
		public static List<SMSIstorija> List(FbConnection con)
		{
			...
			using (
				FbCommand cmd = new FbCommand(
					"SELECT ID, PARTNER_ID, TEKST, DATUM, HASH FROM SMS_ISTORIJA",
					con
				)
			)
			{
