...
namespace TDOffice_v2.TDOffice
{
	public class Modul
	{
		...
		public void Update(FbConnection con)
		{
			using (
				FbCommand cmd = new FbCommand(
					@"UPDATE MODUL SET 
                KOMENTAR = @K, INTERNI_KOMENTAR = @IK WHERE ID = @ID",
					con
				)
			)
			{
		...
		public static Modul GetWithInsert(FbConnection con, int id)
		{
			using (
				FbCommand cmd = new FbCommand(
					"SELECT ID, KOMENTAR, INTERNI_KOMENTAR FROM MODUL WHERE ID = @ID",
					con
				)
			)
			{
				...
				using (FbDataReader dr = cmd.ExecuteReader())
				{
					...
					{
						using (
							FbCommand cmd1 = new FbCommand(
								"INSERT INTO MODUL (ID, KOMENTAR, INTERNI_KOMENTAR) VALUES (@ID, @K, @IK )",
								con
							)
						)
						{
