...
namespace TDOffice_v2.TDOffice
{
	public partial class Taskboard
	{
		public class Item
		{
			...
			public static readonly Dictionary<ItemStatus, Color> COLORS_BY_ITEM_STATUS =
				new Dictionary<ItemStatus, Color>()
				{
			...
			public void Update(FbConnection con)
			{
				using (
					FbCommand cmd = new FbCommand(
						"UPDATE TASKBOARD_TASK SET NASLOV = @N, TEXT = @T, STATUS = @S WHERE ID = @ID",
						con
					)
				)
				{
			...
			public static Item Get(FbConnection con, int id)
			{
				using (
					FbCommand cmd = new FbCommand(
						"SELECT ID, TASKBOARD_ID, NASLOV, TEXT, STATUS, KORISNIKID, DATUM FROM TASKBOARD_TASK WHERE ID = @ID",
						con
					)
				)
				{
			...
			public static List<Item> List(FbConnection con)
			{
				...
				using (
					FbCommand cmd = new FbCommand(
						"SELECT ID, TASKBOARD_ID, NASLOV, TEXT, STATUS, KORISNIKID, DATUM FROM TASKBOARD_TASK",
						con
					)
				)
				{
			...
			public static List<Item> ListByTaskboard(FbConnection con, int taskboardID)
			{
				...
				using (
					FbCommand cmd = new FbCommand(
						"SELECT ID, TASKBOARD_ID, NASLOV, TEXT, STATUS, KORISNIKID, DATUM FROM TASKBOARD_TASK WHERE TASKBOARD_ID = @TID",
						con
					)
				)
				{
			...
			)
			{
				using (
					FbCommand cmd = new FbCommand(
						"INSERT INTO TASKBOARD_TASK (ID, TASKBOARD_ID, NASLOV, TEXT, STATUS, KORISNIKID, DATUM) VALUES (((SELECT COALESCE(MAX(ID), 0) FROM TASKBOARD_TASK) + 1), @TID, @NASLOV,  @TEXT, 0, @KID, @DATUM) RETURNING ID",
						con
					)
				)
				{
