...
namespace TDOffice_v2.TDOffice
{
	public class Kurs
	{
		...
		public void Update(FbConnection con)
		{
			using (
				FbCommand cmd = new FbCommand(
					"UPDATE KURS SET EUR = @E, RSD = @R, USD = @U WHERE DATUM = @DAT",
					con
				)
			)
			{
		...
		)
		{
			using (
				FbCommand cmd = new FbCommand(
					"INSERT INTO KURS (DATUM, EUR, RSD, DOLAR) VALUES (@DATUM, @E, @R, @D)",
					con
				)
			)
			{
		...
		public static Kurs Get(FbConnection con, DateTime datum)
		{
			using (
				FbCommand cmd = new FbCommand(
					"SELECT DATUM, RSD, EUR, DOLAR FROM KURS WHERE DATUM = @DAT",
					con
				)
			)
			{
			...
			return Task.Run(() =>
			{
				...
				if (danasnjiKurs == null && true == false)
				{
					HttpRequestMessage request = new HttpRequestMessage(
						HttpMethod.Get,
						"https://freecurrencyapi.net/api/v2/latest?apikey=2b2bc880-567f-11ec-9556-a3a5d919fa42&base_currency=EUR"
					);

					HttpClient client = new HttpClient();
					HttpResponseMessage response = client.SendAsync(request).Result;

					DTO.KursGetDTO resp = JsonConvert.DeserializeObject<DTO.KursGetDTO>(
