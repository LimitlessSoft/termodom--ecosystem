...
namespace TDOffice_v2.TDOffice
{
	public partial class SpecijalniCenovnik
	{
		...
		public void Update(FbConnection con)
		{
			using (
				FbCommand cmd = new FbCommand(
					"UPDATE SPECIJALNI_CENOVNIK SET NAZIV = @NAZ WHERE ID = @ID",
					con
				)
			)
			{
		...
		public static SpecijalniCenovnik Get(FbConnection con, int id)
		{
			using (
				FbCommand cmd = new FbCommand(
					"SELECT ID, NAZIV FROM SPECIJALNI_CENOVNIK WHERE ID = @ID",
					con
				)
			)
			{
		...
		public static void Insert(FbConnection con, string naziv)
		{
			using (
				FbCommand cmd = new FbCommand(
					"INSERT INTO SPECIJALNI_CENOVNIK (ID, NAZIV) VALUES (((SELECT COALESCE(MAX(ID), 0) FROM SPECIJALNI_CENOVNIK) + 1),@NAZIV)",
					con
				)
			)
			{
