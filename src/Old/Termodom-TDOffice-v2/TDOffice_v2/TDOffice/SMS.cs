...
namespace TDOffice_v2.TDOffice
{
	public class SMS
	{
		...
		public void Update(FbConnection con)
		{
			using (
				FbCommand cmd = new FbCommand(
					"UPDATE SMS SET STATUS = @S, TEXT = @T WHERE ID = @ID",
					con
				)
			)
			{
		...
		public static int Insert(FbConnection con, string broj, string text)
		{
			using (
				FbCommand cmd = new FbCommand(
					"INSERT INTO SMS (ID, STATUS, BROJ, TEXT) VALUES (((SELECT COALESCE(MAX(ID), 0) FROM SMS) + 1), 0, @BROJ, @TEXT) RETURNING ID",
					con
				)
			)
			{
		...
		public static SMS Get(FbConnection con, int id)
		{
			using (
				FbCommand cmd = new FbCommand(
					"SELECT ID, STATUS, BROJ, TEXT FROM SMS WHERE ID = @ID",
					con
				)
			)
			{
