...
namespace TDOffice_v2.TDOffice
{
	public class ZaposleniUgovorORadu
	{
		...
		public void Update(FbConnection con)
		{
			using (
				FbCommand cmd = new FbCommand(
					"UPDATE ZAPOSLENI_UGOVOR_O_RADU SET ZAPOSLENI_ID = @ZI, FIRMA = @F, POCETAK_TRAJANJA = @PT, KRAJ_TRAJANJA = @KT WHERE ID = @ID",
					con
				)
			)
			{
		...
		public static int Insert(
			int firma,
			int zaposleniID,
			DateTime pocetaktrajanja,
			DateTime krajtrajanja
		)
		{
			using (FbConnection con = new FbConnection(TDOffice.connectionString))
			{
		...
		)
		{
			using (
				FbCommand cmd = new FbCommand(
					"INSERT INTO ZAPOSLENI_UGOVOR_O_RADU (ID, FIRMA, POCETAK_TRAJANJA, KRAJ_TRAJANJA, ZAPOSLENI_ID) VALUES (((SELECT COALESCE(MAX(ID), 0) from ZAPOSLENI_UGOVOR_O_RADU) + 1), @F, @PT, @KT, @ZI) RETURNING ID",
					con
				)
			)
			{
		...
		public static ZaposleniUgovorORadu Get(FbConnection con, int id)
		{
			using (
				FbCommand cmd = new FbCommand(
					"SELECT ID, ZAPOSLENI_ID, FIRMA, POCETAK_TRAJANJA, KRAJ_TRAJANJA FROM ZAPOSLENI_UGOVOR_O_RADU WHERE ID = @ID",
					con
				)
			)
			{
		...
		public static List<ZaposleniUgovorORadu> List(FbConnection con, string whereQuery = null)
		{
			...
			using (
				FbCommand cmd = new FbCommand(
					"SELECT ID, ZAPOSLENI_ID, FIRMA, POCETAK_TRAJANJA, KRAJ_TRAJANJA FROM ZAPOSLENI_UGOVOR_O_RADU WHERE 1 = 1"
						+ whereQuery,
					con
				)
			)
		...
		public static void Delete(FbConnection con, int id)
		{
			using (
				FbCommand cmd = new FbCommand(
					"DELETE FROM ZAPOSLENI_UGOVOR_O_RADU WHERE ID = @ID",
					con
				)
			)
			{
