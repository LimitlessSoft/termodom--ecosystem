...
namespace TDOffice_v2.TDOffice
{
	public class WebUredjivanjeCenaStavka
	{
		...
		public Enums.WebUredjivanjeCenaUslov Uslov { get; set; } =
			Enums.WebUredjivanjeCenaUslov.None;
		...
		public void Update(FbConnection con)
		{
			using (
				FbCommand cmd = new FbCommand(
					@"UPDATE WEB_UREDJIVANJE_CENA_STAVKA SET
                USLOV = @U,
                USLOV_MODIFIKATOR = @M,
                MAGACINID = @MID,
		...
		public static void Insert(
			int robaID,
			Enums.WebUredjivanjeCenaUslov uslov,
			double modifikator,
			int magacinID
		)
		{
		...
		)
		{
			using (
				FbCommand cmd = new FbCommand(
					@"INSERT INTO WEB_UREDJIVANJE_CENA_STAVKA
                (ROBAID, USLOV, USLOV_MODIFIKATOR, MAGACINID) VALUES (@RID, @U, @UM, @M)",
					con
				)
			)
			{
		...
		public static WebUredjivanjeCenaStavka Get(FbConnection con, int robaID)
		{
			using (
				FbCommand cmd = new FbCommand(
					"SELECT ROBAID, USLOV, USLOV_MODIFIKATOR, MAGACINID, REFERENTNI_PROIZVOD FROM WEB_UREDJIVANJE_CENA_STAVKA WHERE ROBAID = @RID",
					con
				)
			)
			{
				...
				using (FbDataReader dr = cmd.ExecuteReader())
					if (dr.Read())
						return new WebUredjivanjeCenaStavka()
						{
							...
							ReferentniProizvod =
								dr["REFERENTNI_PROIZVOD"] is DBNull
									? null
									: (int?)Convert.ToInt32(dr["REFERENTNI_PROIZVOD"])
						};
			}
		...
		public static List<WebUredjivanjeCenaStavka> List(FbConnection con)
		{
			...
			using (
				FbCommand cmd = new FbCommand(
					"SELECT ROBAID, USLOV, USLOV_MODIFIKATOR, MAGACINID, REFERENTNI_PROIZVOD FROM WEB_UREDJIVANJE_CENA_STAVKA",
					con
				)
			)
			{
