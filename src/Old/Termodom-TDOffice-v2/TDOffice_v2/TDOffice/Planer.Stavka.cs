	...
	public partial class Planer
	{
		public class Stavka
		{
			...
			public static int Insert(FbConnection con, int userID, DateTime datum, string body)
			{
				using (
					FbCommand cmd = new FbCommand(
						@"INSERT INTO PLANER_STAVKA
                (ID, USERID, DATUM, BODY)
                VALUES (((SELECT COALESCE(MAX(ID), 0) FROM PLANER_STAVKA) + 1), @USERID, @DATUM, @BODY) RETURNING ID",
						con
					)
				)
				{
			...
			{
				using (
					FbCommand cmd = new FbCommand("DELETE FROM PLANER_STAVKA WHERE ID = @SID", con)
				)
				{
					cmd.Parameters.AddWithValue("@SID", id);
