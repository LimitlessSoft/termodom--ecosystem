...
namespace TDOffice_v2.TDOffice
{
	...
	public class Pravo
	{
		public static List<PravoDefinicija> definicijePrava = new List<PravoDefinicija>()
		{
			...
			new PravoDefinicija(
				700804,
				"7 TDPopis - Moze da menja koju informaciju unosi prilikom izbora stavke"
			),
			new PravoDefinicija(
				700805,
				"7 TDPopis - Moze da generise novi popis na osnovu neslaganja - svih razlika"
			),
			new PravoDefinicija(
				700806,
				"7 TDPopis - Moze da stampa listu stavki koje se ne slazu - svih razlika"
			),
			new PravoDefinicija(
				700807,
				"7 TDPopis - Moze da stampa listu stavki koje se ne slazu - visak / manjak"
			),
			new PravoDefinicija(700808, "7 TDPopis - Moze da upravlja specijal stampom"),
			new PravoDefinicija(
				700809,
				"7 TDPopis - Moze da pokrene akciju kreiranja dokumenta zaduzenja"
			),
			...
			new PravoDefinicija(
				133605,
				"1336 Razduzenje Magacina - Akcija Razduzi Magacin / Razduzenje magacina"
			),
			...
			new PravoDefinicija(
				137010,
				"Definisanje Prodajne Cene Po Specijalnim Cenovnicima - Modul"
			),
			new PravoDefinicija(137020, "Partner Lista (Komercijalno) - Modul"),
			new PravoDefinicija(137030, "Specijalni Cenovnik - Modul"),
			new PravoDefinicija(
				137040,
				"Prodaja > Unesi partnera u dokument komercijalnog poslovanja - Modul"
			),
			new PravoDefinicija(
				137041,
				"Prodaja > Unesi partnera u dokument komercijalnog poslovanja - Rad sa tudjim dokumentima"
			),
			...
			new PravoDefinicija(
				1839500,
				"Administacija > Komercijalno > Dokument > Oslobodi web proracun"
			),
		...
		public void Update(FbConnection con)
		{
			using (
				FbCommand cmd = new FbCommand(
					"UPDATE PRAVA_ZAP SET VREDNOST = @V WHERE ID = @ID",
					con
				)
			)
			{
		...
		public static void NematePravoObavestenje(int modulID)
		{
			...
			using (
				fm_PravoPristupaModulu_Zatrazi ppm = new fm_PravoPristupaModulu_Zatrazi(
					"Obavestenje",
					"Nema pravo pristupa modulu " + modulID.ToString(),
					modulID
				)
			)
				ppm.ShowDialog();
		...
		public static Pravo Get(FbConnection con, int pravoID)
		{
			using (
				FbCommand cmd = new FbCommand(
					"SELECT ID, USER_ID, MODUL_ID, VREDNOST FROM PRAVA_ZAP WHERE ID = @PID",
					con
				)
			)
			{
		...
		public static Pravo Get(FbConnection con, int modulID, int userID)
		{
			using (
				FbCommand cmd = new FbCommand(
					"SELECT ID, USER_ID, MODUL_ID, VREDNOST FROM PRAVA_ZAP WHERE USER_ID = @UID AND MODUL_ID = @MID",
					con
				)
			)
			{
		...
		public static List<Pravo> List(FbConnection con, string whereQuery = null)
		{
			...
			using (
				FbCommand cmd = new FbCommand(
					"SELECT ID, USER_ID, MODUL_ID, VREDNOST FROM PRAVA_ZAP WHERE 1 = 1 "
						+ whereQuery,
					con
				)
			)
		...
		public static int Insert(FbConnection con, int modul, int status, int userID)
		{
			using (
				FbCommand cmd = new FbCommand(
					"INSERT INTO PRAVA_ZAP (ID, USER_ID, MODUL_ID, VREDNOST) VALUES (((SELECT COALESCE(MAX(ID), 0) FROM PRAVA_ZAP) + 1), @U, @M, @V) RETURNING ID",
					con
				)
			)
			{
