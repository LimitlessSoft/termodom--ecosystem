...
namespace TDOffice_v2.TDOffice
{
    ...
    {
        private static readonly Buffer<List<DokumentPopis>> _buffer = new Buffer<List<DokumentPopis>>(List);
        ...
        public DataTable ZabelezenoStanjePopisa
        {
            get
            {
                if (_zabelezenoStanjePopisaDT == null)
                    _zabelezenoStanjePopisaDT = string.IsNullOrWhiteSpace(_zabelezenoStanjePopisaRaw) ? new DataTable() : JsonConvert.DeserializeObject<DataTable>(_zabelezenoStanjePopisaRaw);
        ...
        {
            using (FbCommand cmd = new FbCommand(@"UPDATE DOKUMENT_POPIS SET
                                                        USERID = @U,
                                                        MAGACINID = @M,
                                                        STATUS = @S,
                                                        KOMENTAR = @K,
                ...
                using (FbDataReader dr = cmd.ExecuteReader())
                    if (dr.Read())
                        return new DokumentPopis()
                        {
                            ...
                            _zabelezenoStanjePopisaRaw = dr["ZABELEZENO_STANJE_POPISA"].ToStringOrDefault(),
...
WHERE 1 = 1 " + whereQuery, con))
        ...
        public static List<DokumentPopis> ListByMagacinID(FbConnection con, int magacinID)
        {
            ...
            using (FbCommand cmd = new FbCommand(@"SELECT ID, DATUM, USERID, MAGACINID, STATUS, KOMENTAR,
                    INTERNI_KOMENTAR, TIP, KOMERCIJALNO_POPIS_BRDOK, KOMERCIJALNO_NARUDZBENICA_BRDOK,
                    SPECIJAL_STAMPA, ZADUZENJE_BRDOK_KOMERCIJALNO, ZABELEZENO_STANJE_POPISA, NAPOMENA FROM DOKUMENT_POPIS WHERE MAGACINID = @MID", con))
            {
        ...
        /// <returns></returns>
        public static int Insert(int userID, int magacinID, int status, string komentar, string interniKomentar, PopisType tip, int? komercijalnoPopisBrDok, int? komercijalnoNarudzbenicaBrDok)
        {
            using (FbConnection con = new FbConnection(TDOffice.connectionString))
            {
                con.Open();
        ...
        /// <param name="komercijalnoNarudzbenicaBrDok"></param>
        /// <returns></returns>
        public static int Insert(FbConnection con, int userID, int magacinID, int status, string komentar, string interniKomentar, PopisType tip, int? komercijalnoPopisBrDok, int? komercijalnoNarudzbenicaBrDok)
        {
        ...
        private static Task AzurirajStanjeMagacinaUPopisuAsync(int magacinID)
        {
            return Task.Run(() =>
            {
                bool imaOtvorenihPopisa = ListByMagacinID(magacinID).Any(x => x.Status == DokumentStatus.Otkljucan);
                ...
                MagacinClan magacinVlasnik = magacinVlasnik = MagacinClan.ListByMagacinID(magacinID).Where(x => x.Tip == Enums.MagacinClanTip.Vlasnik).FirstOrDefault();
