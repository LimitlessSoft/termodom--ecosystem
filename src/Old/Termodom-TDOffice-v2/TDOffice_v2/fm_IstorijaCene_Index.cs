...
namespace TDOffice_v2
{
    public partial class fm_IstorijaCene_Index : Form
    {
        ...
        private Task<List<Komercijalno.Dokument>> _dokumentiNabavke { get; set; } = Task.Run<List<Komercijalno.Dokument>>(() =>
        {
            List<Komercijalno.Dokument> list = new List<Komercijalno.Dokument>();
            list.AddRange(Komercijalno.Dokument.ListByMagacinID(DateTime.Now.Year, 50).Where(x => _vrDokNabavke.Contains(x.VrDok)).ToList());
            return list;
        });

        private Task<List<Komercijalno.Dokument>> _dokumentiNivelacije { get; set; } = Task.Run<List<Komercijalno.Dokument>>(() =>
        {
            List<Komercijalno.Dokument> list = new List<Komercijalno.Dokument>();
            list.AddRange(Komercijalno.Dokument.ListByMagacinID(DateTime.Now.Year, 50).Where(x => new[] { 0, 21 }.Contains(x.VrDok)).ToList());
            return list;
        });
        private Task<List<Komercijalno.Dokument>> _dokumentiProdaje { get; set; } = Task.Run<List<Komercijalno.Dokument>>(() =>
         {
             List<Komercijalno.Dokument> list = new List<Komercijalno.Dokument>();
        ...
        public fm_IstorijaCene_Index(int robaID)
        {
            ...
            {
                return _stavkeRobe.Result.Where(x => x.MagacinID == 50 && new[] { 21, 0 }.Contains(x.VrDok)).ToList();
            });
            stavkeNabavke = Task.Run<List<Komercijalno.Stavka>>(() => { return _stavkeRobe.Result.Where(x => x.MagacinID == 50 && _vrDokNabavke.Contains(x.VrDok)).ToList(); });
            stavkeProdaje = Task.Run<List<Komercijalno.Stavka>>(() => { return _stavkeRobe.Result.Where(x => new[] { 15 }.Contains(x.VrDok)).ToList(); });
            UcitajPodatke();
        }
        ...
        private void UcitajPodatke()
        {
            Task<Dictionary<int, double>> prodajaUperiodu = Task.Run<Dictionary<int, double>>(() =>
            {
                ...
                Parallel.ForEach(stavkeProdaje.Result, new ParallelOptions() { MaxDegreeOfParallelism = 100 }, s =>
                {
                    Komercijalno.Dokument dok = _dokumentiProdaje.Result.FirstOrDefault(x => x.VrDok == s.VrDok && x.BrDok == s.BrDok);

                    int nPer = _isNedeljniGrafikon ? (int)((dok.Datum.Date - new DateTime(DateTime.Now.Year, 1, 1)).TotalDays / 7) : dok.Datum.Month;
            ...
            chart1.Series["NabavnaCena"].ChartType = System.Windows.Forms.DataVisualization.Charting.SeriesChartType.StepLine;
            ...
            chart1.Series["NabavnaCena"].Font = new Font(chart1.Series["NabavnaCena"].Font, FontStyle.Bold);
            ...
            chart1.Series["ProdajnaCena"].ChartType = System.Windows.Forms.DataVisualization.Charting.SeriesChartType.StepLine;
            ...
            chart1.Series["ProdajnaCena"].Font = new Font(chart1.Series["NabavnaCena"].Font, FontStyle.Bold);
            ...
            chart1.Series["ProdajaKolicina"].ChartType = System.Windows.Forms.DataVisualization.Charting.SeriesChartType.Column;
            ...
            chart1.Series["ProdajaKolicina"].Font = new Font(chart1.Series["ProdajaKolicina"].Font, FontStyle.Bold);
            chart1.Series["ProdajaKolicina"].ChartArea = "Prodaja";
            chart1.Series["ProdajaKolicina"].LabelFormat = "#,##0";

            chart1.ChartAreas["Default"].AxisX.ScaleView.Zoomable = true;
            chart1.ChartAreas["Default"].AxisX.ScaleView.SizeType = System.Windows.Forms.DataVisualization.Charting.DateTimeIntervalType.Number;
            ...
            Task.Run(async () => {
                ...
                {
                    chart1.ChartAreas["Default"].AxisY.Maximum = (int)stavkeNivelacija.Result.Max(x => x.ProdajnaCena) * 1.2;
                });
            });
            ...
            chart1.ChartAreas["Prodaja"].AxisX.ScaleView.SizeType = System.Windows.Forms.DataVisualization.Charting.DateTimeIntervalType.Number;
            ...
            chart1.ChartAreas["Prodaja"].AxisY.Title = "Ukupno prodato kolicinski VRDOK 15 (" + _roba.JM + ")";

            int nNedelja = (int)((new DateTime(DateTime.Now.Year, 12, 31) - new DateTime(DateTime.Now.Year, 1, 1)).TotalDays / 7);
            ...
            for (int i = 0; i < (_isNedeljniGrafikon ? nNedelja : 12); i++)
            {
                ...
                chart1.ChartAreas[0].AxisX.CustomLabels.Add(new CustomLabel(i, i + 1, _isNedeljniGrafikon ? "Nedelja " + (i + 1) + Environment.NewLine + start.ToString("dd.MM") +" - " + end.ToString("dd.MM") : _meseci[i], 0, LabelMarkStyle.Box));
                chart1.ChartAreas[1].AxisX.CustomLabels.Add(new CustomLabel(i, i + 1, _isNedeljniGrafikon ? "Nedelja " + (i + 1) + Environment.NewLine + start.ToString("dd.MM") + " - " + end.ToString("dd.MM") : _meseci[i], 0, LabelMarkStyle.Box));
            }
            ...
            {
                Komercijalno.Dokument dok = _dokumentiNabavke.Result.FirstOrDefault(x => x.VrDok == s.VrDok && x.BrDok == s.BrDok);
                ...
                {
                    items.Add(new Tuple<double, double>((dok.Datum.Date - new DateTime(DateTime.Now.Year, 1, 1)).TotalDays / (_isNedeljniGrafikon ? 7 : 30.5), s.NabavnaCena));
            ...
            {
                Komercijalno.Dokument dok = _dokumentiNivelacije.Result.FirstOrDefault(x => x.BrDok == s.BrDok);
                ...
                if (poslednjaCena < minCen || poslednjaCena > maxCena)
                {
                    items1.Add(new Tuple<double, double>((dok.Datum.Date - new DateTime(DateTime.Now.Year, 1, 1)).TotalDays / (_isNedeljniGrafikon ? 7 : 30.5), s.ProdajnaCena));
                    poslednjaCena = s.NabavnaCena;
                }
            }

            chart1.ChartAreas["Prodaja"].AxisY.Maximum = prodajaUperiodu.Result.Count == 0 ? 0 :(int)(prodajaUperiodu.Result.Max(x => x.Value) * 1.5);

            for (int i = 0; i < nNedelja; i++)
                chart1.Series["ProdajaKolicina"].Points.AddXY(i - 0.5, prodajaUperiodu.Result.ContainsKey(i) ? prodajaUperiodu.Result[i] : 0);

            foreach (Tuple<double, double> it in items.OrderBy(x => x.Item1))
                chart1.Series["NabavnaCena"].Points.AddXY(it.Item1, it.Item2);
            chart1.Series["NabavnaCena"].Points.AddXY((DateTime.Now - new DateTime(DateTime.Now.Year, 1, 1)).TotalDays / (_isNedeljniGrafikon ? 7 : 30.5), realnaPoslednjaNabavna);

            foreach (Tuple<double, double> it in items1.OrderBy(x => x.Item1))
                chart1.Series["ProdajnaCena"].Points.AddXY(it.Item1, it.Item2);
            chart1.Series["ProdajnaCena"].Points.AddXY((DateTime.Now - new DateTime(DateTime.Now.Year, 1, 1)).TotalDays / (_isNedeljniGrafikon ? 7 : 30.5), realnaPoslednjaProdajna);
            ...
            dataGridView1.Columns["VrDok"].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter;

            dataGridView1.Columns["BrDok"].Width = 50;
            dataGridView1.Columns["BrDok"].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter;
            ...
            this.dataGridView1.Sort(this.dataGridView1.Columns["Datum"], ListSortDirection.Ascending);
        ...
        private void modGrafikona_btn_Click(object sender, EventArgs e)
        {
            ...
            modGrafikona_btn.Text = _isNedeljniGrafikon ? "Promeni na mesecni prikaz" : "Promeni na nedeljni prikaz";
            UcitajPodatke();
        }
