...
namespace TDOffice_v2
{
	public partial class fm_Izvestaj_Prodaja_Roba_PosaljiKaoIzvestaj_Setup : Form
	{
		...
		public fm_Izvestaj_Prodaja_Roba_PosaljiKaoIzvestaj_Setup(
			DataTable data,
		...
		private void fm_Izvestaj_Prodaja_Roba_PosaljiKaoIzvestaj_Setup_Load(
			object sender,
			EventArgs e
		) { }
				...
				TDOffice.Poruka.Insert(
					new TDOffice.Poruka()
					{
						...
						Tag = new TDOffice.PorukaAdditionalInfo()
						{
							...
							AdditionalInfo = new Tuple<DataTable, Models.Periodi, string>(
								izvestaj,
								_periodi,
								_tipIzvestaja
							)
					...
					DataTable reperniRezultati =
						_tipIzvestaja == "RSD"
							? Models.Izvestaj.ProdajaRobePoRealnimCenama(
								_periodi,
								_magaciniUIzvestaju,
								_listaRobe.Count > 0 ? _listaRobe : null,
								CheckedValues
							)
