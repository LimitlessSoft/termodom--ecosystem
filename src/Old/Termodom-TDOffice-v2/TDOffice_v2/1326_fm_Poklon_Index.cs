...
namespace TDOffice_v2
{
	...
	{
		private static List<Poklon> pokloni = TDOffice
			.Config<List<Poklon>>.Get(TDOffice.ConfigParameter.Pokloni)
			.Tag;
		...
		{
			_robaMRE.Wait();
			Poklon poklon = pokloni
				.Where(x => x.ID == (int)comboBox1.SelectedValue)
				.FirstOrDefault();
			...
			{
				MessageBox.Show(
					"Vrednost MP Racuna mora biti veca od "
						+ poklon.VrednostMPRacuna.ToString("#,##0.00 RSD")
				);
				return;
			}

			List<Komercijalno.Stavka> stavkeRacuna = Komercijalno.Stavka.ListByDokument(
				DateTime.Now.Year,
				15,
				mpRacun
			);
			...
			Komercijalno.Stavka.Insert(
				...
				_robaUMagacnu
					.Where(x => x.RobaID == poklon.RobaID && x.MagacinID == dokument.MagacinID)
					.FirstOrDefault(),
				poklon.Kolicina,
				99.9
			);
