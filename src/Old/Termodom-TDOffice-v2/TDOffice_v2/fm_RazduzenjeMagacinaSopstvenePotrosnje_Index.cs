...
namespace TDOffice_v2
{
	public partial class fm_RazduzenjeMagacinaSopstvenePotrosnje_Index : Form
	{
		...
		/// </summary>
		private Dictionary<int, StavkaUslugeDto> _stavkeUsluge { get; set; } =
			new Dictionary<int, StavkaUslugeDto>();
			...
			try
			{
				...
				_izborUsluge.PotvrdaUsluge += async (robaId, kolicina, cena, naziv, jm) =>
				{
					...
					preostalaVrednostRobe_txt.Text = (
						await GetPreostaluVrednostRobeZaKojuTrebaNapravitiUslugeAsync()
					).ToString("#,##0.00 RSD");
				};
		...
		private async Task UcitajIzvornuInternuKalkulacijeAsync()
		{
			...
			try
			{
				...
				_izvornaInternaKalkulacijaTask = DokumentManager.GetAsync(
					izvornaBazaId,
					_izvorVrDok,
					izvorKalkulacijaBrDok,
					DateTime.Now.Year
				);
				...
				{
					MessageBox.Show(
						"Kalkulacija sa datim brojem nije pronadjena u izabranoj bazi!"
					);
					ToggleUi(true);
					return;
				}
				...
				{
					MessageBox.Show(
						"Kalkulacija je vec razduzena (vrDokOut), ne mozete ponovo razduziti po njoj!"
					);
					_izvornaInternaKalkulacijaTask = null;
					ToggleUi(true);
					return;
				...
				var firme = await FirmaManager.DictionaryAsync();
				var magacinRazduzenja = firme
					.Values.First(x => x.GlavniMagacin == izvornaBazaId)
					.MagacinRazduzenja;
				if (izvornaInternaKalkulacija.MagacinID != magacinRazduzenja)
				{
					MessageBox.Show(
						$"Magacin razduzenaj za izabranu firmu je {magacinRazduzenja}. Ova kalkulacija ne pripada datom magacinu!"
					);
					_izvornaInternaKalkulacijaTask = null;
					ToggleUi(true);
					return;
		...
		private async Task<double> GetPreostaluVrednostRobeZaKojuTrebaNapravitiUslugeAsync()
		{
			var izvornaInternaKalkulacija = await _izvornaInternaKalkulacijaTask;
			int izvornaBazaId = Convert.ToInt32(izvornaBaza_cmb.SelectedValue);
			await DokumentManager.PresaberiAsync(
				izvornaBazaId,
				_izvorVrDok,
				izvornaInternaKalkulacija.BrDok,
				DateTime.Now.Year
			);
		...
		private void ToggleUi(bool state)
		{
			...
			ToggleUfuUi(
				_izvornaInternaKalkulacijaTask != null
					&& _izvornaInternaKalkulacijaTask.Result != null
			);
		}
		...
		private async Task RazduziMagacinAsync()
		{
			...
			try
			{
				...
				var povezanaInternaOtpremnica = await DokumentManager.GetAsync(
					izvornaBazaId,
					(int)izvornaInternaKalkulacija.VrDokIn,
					(int)izvornaInternaKalkulacija.BrDokIn,
					izvornaInternaKalkulacija.Datum.Year
				);
				...
				var magacinRazduzenja = firme
					.Values.First(x => x.GlavniMagacin == izvornaBazaId)
					.MagacinRazduzenja;
				var novokreiraniDokument28BrDok = await DokumentManager.InsertAsync(
					new DokumentManager.DokumentInsertRequestBody()
					{
				...
				var novokreiraniDokument28 = await DokumentManager.GetAsync(
					magacinRazduzenja,
					28,
					novokreiraniDokument28BrDok,
					DateTime.Now.Year
				);

				var stavkeIzvora = await StavkaManager.DictionaryAsync(
					izvornaBazaId,
					izvornaInternaKalkulacija.VrDok,
					izvornaInternaKalkulacija.BrDok,
					izvornaInternaKalkulacija.Datum.Year
				);
				...
				{
					await StavkaManager.InsertAsync(
						new StavkaManager.StavkaInsertRequestBody()
						{
							BazaId = magacinRazduzenja,
							GodinaBaze = DateTime.Now.Year,
				...
				var novokreiraniDokument6BrDok = await DokumentManager.InsertAsync(
					new DokumentManager.DokumentInsertRequestBody()
					{
				...
				{
					await StavkaManager.NapraviUsluguAsync(
						new StavkaManager.NapraviUsluguRequestBody()
						{
