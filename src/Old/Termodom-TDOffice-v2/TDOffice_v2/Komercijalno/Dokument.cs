...
namespace TDOffice_v2.Komercijalno
{
	public class Dokument
	{
		...
		/// <returns></returns>
		public double OstvarenaMarza(
			List<Dokument> dokumentiNabavke,
			List<Stavka> stavkeNabavke,
			OstvarenaMarzaTipNabavneCene tipCene = OstvarenaMarzaTipNabavneCene.NaDanDokumenta
		)
		{
			List<Stavka> stavkeOvogDokumenta = Stavka.ListByDokument(
				DateTime.Now.Year,
				VrDok,
				BrDok
			);
			...
			if (stavkeNabavke == null)
				stavkeNabavke = Stavka.List(
					DateTime.Now.Year,
					$"ROBAID IN ("
						+ string.Join(", ", stavkeOvogDokumenta.Select(x => x.RobaID))
			...
			using (FbConnection con = new FbConnection(Komercijalno.CONNECTION_STRING[Datum.Year]))
			{
				...
				foreach (Stavka stavka in stavkeOvogDokumenta)
				{
					...
					double nabavnaCena =
						stavka.Vrsta != 1
							? prodajnaCena
							: tipCene == OstvarenaMarzaTipNabavneCene.NaDanDokumenta
								? Komercijalno.GetRealnaNabavnaCena(
									stavka.RobaID,
									Datum,
									dokumentiNabavke,
									stavkeNabavke
								)
								: Komercijalno.GetProsecnaNabavnaCena(
									con,
									robaID,
					...
					double prodajnaVrednostSaPopustom =
						(prodajnaCena * (1 - (rabat / 100))) * kolicina;
			...
			return ukupnaNabavnaVrednostDokumenta == 0 || ukupnaVrednostDokumentaSaPopustom == 0
				? 0
				: ((ukupnaVrednostDokumentaSaPopustom / ukupnaNabavnaVrednostDokumenta) - 1)
					* (100);
		}
...
WHERE VRDOK = @VRDOK AND BRDOK = @BRDOK",
				...
				using (FbDataReader dr = cmd.ExecuteReader())
					if (dr.Read())
						dok = new Dokument()
						{
							...
							DatRoka =
								dr["DATROKA"] is DBNull
									? null
									: (DateTime?)Convert.ToDateTime(dr["DATROKA"]),
							...
							Potrazuje =
								dr["POTRAZUJE"] is DBNull ? 0 : Convert.ToDouble(dr["POTRAZUJE"]),
							...
							VrDokOut =
								dr["VRDOKOUT"] is DBNull
									? null
									: (int?)Convert.ToInt32(dr["VRDOKOUT"]),
							VrDokIn =
								dr["VRDOKIN"] is DBNull
									? null
									: (int?)Convert.ToInt32(dr["VRDOKIN"]),
							BrDokOut =
								dr["BRDOKOUT"] is DBNull
									? null
									: (int?)Convert.ToInt32(dr["BRDOKOUT"]),
							BrDokIn =
								dr["BRDOKIN"] is DBNull
									? null
									: (int?)Convert.ToInt32(dr["BRDOKIN"]),
							RefID = Convert.ToInt32(dr["REFID"]),
							ZapID = Convert.ToInt32(dr["ZAPID"]),
							Placen = Convert.ToInt32(dr["PLACEN"]),
							MagID =
								dr["MAGID"] is DBNull ? null : (Int16?)Convert.ToInt16(dr["MAGID"]),
							AliasU =
								dr["ALIASU"] is DBNull ? null : (int?)Convert.ToInt32(dr["ALIASU"]),
							Popust1Dana =
								dr["POPUST1DANA"] is DBNull
									? null
									: (int?)Convert.ToInt32(dr["POPUST1DANA"]),
						};
			}
			return dok;
		}
		...
		)
		{
			...
			var response = await TDBrain_v3.GetAsync(
				$"/komercijalno/dokument/get?bazaID={bazaID}&vrdok={vrDok}&brdok={brDok}&godina={godina}"
			);
...
FROM DOKUMENT WHERE 1 = 1 " + queryString,
					con
				)
			)
			{
					...
					while (dr.Read())
						dok.Add(
							new Dokument()
							{
								...
								Razlika =
									dr["RAZLIKA"] is DBNull ? 0 : Convert.ToDouble(dr["RAZLIKA"]),
								NUID = (NacinUplate)Convert.ToInt32(dr["NUID"]),
								PPID =
								...
								MagID =
									dr["MAGID"] is DBNull
										? null
										: (Int16?)Convert.ToInt16(dr["MAGID"]),
								AliasU =
									dr["ALIASU"] is DBNull
										? null
										: (int?)Convert.ToInt32(dr["ALIASU"]),
								Popust1Dana =
									dr["POPUST1DANA"] is DBNull
										? null
										: (int?)Convert.ToInt32(dr["POPUST1DANA"]),
		...
		public static async Task<List<Dokument>> ListAsync(
			int? magacinID = null,
			int[] vrDok = null,
		...
		)
		{
			...
			{
				return JsonConvert.DeserializeObject<List<Dokument>>(
		...
		public static List<Dokument> ListByVrDok(FbConnection con, int vrDok)
		{
			...
			using (
				FbCommand cmd = new FbCommand(
					@"SELECT VRDOK, BRDOK, INTBROJ, FLAG, 
                    DATUM, MAGACINID, KODDOK, DUGUJE, MTID, POTRAZUJE, NUID, RAZLIKA, PPID, TROSKOVI, VRDOKOUT, VRDOKIN, BRDOKOUT, BRDOKIN, REFID, ZAPID, PLACEN, MAGID, DATROKA, ALIASU FROM DOKUMENT WHERE VRDOK = @V",
					con
				)
			)
			{
						...
						dok.Add(
							new Dokument()
							{
								...
								PPID =
									dr["PPID"] is DBNull ? null : (int?)Convert.ToInt32(dr["PPID"]),
								Troskovi =
									dr["TROSKOVI"] is DBNull ? 0 : Convert.ToDouble(dr["TROSKOVI"]),
								VrDokOut =
									dr["VRDOKOUT"] is DBNull ? 0 : Convert.ToInt32(dr["VRDOKOUT"]),
								VrDokIn =
									dr["VRDOKIN"] is DBNull ? 0 : Convert.ToInt32(dr["VRDOKIN"]),
								BrDokOut =
									dr["BRDOKOUT"] is DBNull ? 0 : Convert.ToInt32(dr["BRDOKOUT"]),
								BrDokIn =
									dr["BRDOKIN"] is DBNull ? 0 : Convert.ToInt32(dr["BRDOKIN"]),
								RefID = Convert.ToInt32(dr["REFID"]),
								ZapID = Convert.ToInt32(dr["ZAPID"]),
								Placen = Convert.ToInt32(dr["PLACEN"]),
								MagID =
		...
		public static List<Dokument> ListByMagacinID(FbConnection con, int magacinID)
		{
			...
			using (
				FbCommand cmd = new FbCommand(
					@"SELECT VRDOK, BRDOK, INTBROJ, FLAG, 
                    DATUM, MAGACINID, KODDOK, DUGUJE, MTID, POTRAZUJE, NUID, RAZLIKA, PPID, TROSKOVI, VRDOKOUT, VRDOKIN, BRDOKOUT, BRDOKIN, REFID, ZAPID, PLACEN, MAGID, DATROKA, ALIASU FROM DOKUMENT WHERE MAGACINID = @MID",
					con
				)
			)
			{
		...
		public static List<Dokument> ListPPIDNotNull(FbConnection con)
		{
			...
			using (
				FbCommand cmd = new FbCommand(
					@"SELECT VRDOK, BRDOK, INTBROJ, FLAG, DATUM,
                MAGACINID, KODDOK, DUGUJE, MTID, POTRAZUJE, NUID,
                PPID, TROSKOVI, VRDOKOUT, VRDOKIN, BRDOKIN, BRDOKOUT, RAZLIKA,
                REFID, ZAPID, PLACEN, MAGID, DATROKA, ALIASU FROM DOKUMENT WHERE PPID IS NOT NULL",
					con
				)
			)
			{
		...
		)
		{
			try
			{
				...
				using (
					FbCommand cmd = new FbCommand(
						@"SELECT POSLEDNJI FROM VRSTADOKMAG WHERE VRDOK = @VRDOK AND MAGACINID = @MAGACINID",
						con
					)
				)
				{
				...
				if (poslednjiBrDok == 0)
				{
					using (
						FbCommand cmd = new FbCommand(
							@"SELECT POSLEDNJI FROM VRSTADOK WHERE VRDOK = @VRDOK",
							con
						)
					)
					{
				...
				using (
					FbCommand cmd = new FbCommand(
						@"INSERT INTO DOKUMENT (VRDOK, BRDOK, INTBROJ, KODDOK, FLAG, DATUM, LINKED, MAGACINID, PPID, PLACEN, DATROKA,
				...
				)
				{
					...
					cmd.Parameters.AddWithValue(
						"@LINKED",
						Procedure.NextLinked(DateTime.Now, magacinID)
					);
					cmd.Parameters.AddWithValue("@MAGACINID", magacinID);
					cmd.Parameters.AddWithValue(
						"@MTID",
						Magacin.Get(DateTime.Now.Year, magacinID).MTID
					);
					cmd.Parameters.AddWithValue("@PPID", ppid);
					cmd.Parameters.AddWithValue("@NUID", nacinUplateID);
					cmd.Parameters.AddWithValue(
						"@ZAPID",
						komercijalnoKorisnikID == null ? 1 : komercijalnoKorisnikID
					);
					cmd.Parameters.AddWithValue("@PREVOZROBE", ppid == null ? 0 : 1);
					cmd.Parameters.AddWithValue(
						"@VRDOKIN",
						dozvoliDaljeIzmeneUKomercijalnom ? (int?)null : 8
					);
		...
		public static void SetKomentar(int godina, int vrDok, int brDok, string komentar)
		{
			using (FbConnection con = new FbConnection(Komercijalno.CONNECTION_STRING[godina]))
			{
				...
				using (
					FbCommand cmd = new FbCommand(
						"SELECT COUNT(VRDOK) AS BROJ FROM KOMENTARI WHERE VRDOK = @V AND BRDOK = @B",
						con
					)
				)
				{
				...
				if (postoji)
				{
					using (
						FbCommand cmd = new FbCommand(
							"UPDATE KOMENTARI SET KOMENTAR = @KOM WHERE VRDOK = @VRDOK AND BRDOK = @BRDOK",
							con
						)
					)
					{
				...
				else
				{
					using (
						FbCommand cmd = new FbCommand(
							"INSERT INTO KOMENTARI (VRDOK, BRDOK, KOMENTAR) VALUES (@VRDOK, @BRDOK, @KOM)",
							con
						)
					)
					{
		...
		public static void SetInterniKomentar(int godina, int vrDok, int brDok, string komentar)
		{
			using (FbConnection con = new FbConnection(Komercijalno.CONNECTION_STRING[godina]))
			{
				...
				if (postoji)
				{
					using (
						FbCommand cmd = new FbCommand(
							"UPDATE KOMENTARI SET INTKOMENTAR = @KOM WHERE VRDOK = @VRDOK AND BRDOK = @BRDOK",
							con
						)
					)
					{
				...
				else
				{
					using (
						FbCommand cmd = new FbCommand(
							"INSERT INTO KOMENTARI (VRDOK, BRDOK, INTKOMENTAR) VALUES (@VRDOK, @BRDOK, @KOM)",
							con
						)
					)
					{
