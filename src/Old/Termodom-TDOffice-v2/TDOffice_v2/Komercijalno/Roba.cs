...
namespace TDOffice_v2.Komercijalno
{
	public class Roba
	{
			...
			)
			{
				...
				using (
					FbCommand cmd = new FbCommand(
						"UPDATE ROBA SET NAZIV = @N, KATBR = @KB, TRKOL = @TRKOL, KATBRPRO = @KBP, JM = @JM, NAZIVZASTAMPU = @NZS WHERE ROBAID = @RID",
						con
					)
				)
				{
		...
		public static Roba Get(FbConnection con, int robaID)
		{
			using (
				FbCommand cmd = new FbCommand(
					"SELECT ROBAID, NAZIV, KATBR, KATBRPRO, JM, TRKOL, TARIFAID, VRSTA, NAZIVZASTAMPU, GRUPAID, PODGRUPA, PROID, DOB_PPID FROM ROBA WHERE ROBAID = @RID",
					con
				)
			)
			{
				...
				using (FbDataReader dr = cmd.ExecuteReader())
					if (dr.Read())
						return new Roba()
						{
							...
							Podgrupa =
								dr["PODGRUPA"] is DBNull
									? null
									: (int?)Convert.ToInt32(dr["PODGRUPA"]),
							TrKol =
								dr["TRKOL"] is DBNull
									? null
									: (double?)Convert.ToDouble(dr["TRKOL"]),
							ProID = dr["PROID"].ToString(),
							DOB_PPID =
								dr["DOB_PPID"] is DBNull
									? null
									: (int?)Convert.ToInt32(dr["DOB_PPID"])
						};
			}
		...
		public static Dictionary<int, Roba> DictList(FbConnection con)
		{
			...
			using (
				FbCommand cmd = new FbCommand(
					"SELECT ROBAID, NAZIV, KATBR, TRKOL, KATBRPRO, JM, TARIFAID, VRSTA, NAZIVZASTAMPU, GRUPAID, PODGRUPA, PROID, DOB_PPID FROM ROBA",
					con
				)
			)
			{
		...
		public static async Task<Termodom.Data.Entities.Komercijalno.RobaDictionary> Dictionary(
			int? godina = null
		)
		{
			var response = await TDBrain_v3.GetAsync(
				$"/komercijalno/roba/dictionary?godina={godina ?? DateTime.Now.Year}"
			);

			if ((int)response.StatusCode == 200)
				return JsonConvert.DeserializeObject<Termodom.Data.Entities.Komercijalno.RobaDictionary>(
