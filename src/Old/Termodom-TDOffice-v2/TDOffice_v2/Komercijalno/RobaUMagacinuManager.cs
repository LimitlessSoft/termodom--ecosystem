	...
	public static class RobaUMagacinuManager
	{
		public static async Task<RobaUMagacinuDictionary> Dictionary(int[] magacinId = null)
		{
			var response = await TDBrain_v3.GetAsync(
				$"/Komercijalno/RobaUMagacinu/Dictionary?{(magacinId == null || magacinId.Length == 0 ? "" : "magacinId=" + string.Join("&magacinId=", magacinId))}"
			);

			switch ((int)response.StatusCode)
			{
				case 200:
					return JsonConvert.DeserializeObject<RobaUMagacinuDictionary>(
