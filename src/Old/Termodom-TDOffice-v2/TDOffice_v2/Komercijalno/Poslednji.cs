			...
			)
			{
				...
				using (
					FbCommand cmd = new FbCommand(
						"SELECT NAZIV, ID, OPSEGOD, OPSEGDO, KORAK, OBJEKATID FROM POSLEDNJI",
						con
					)
				)
				{
					using (FbDataReader dr = cmd.ExecuteReader())
					{
						while (dr.Read())
						{
							list.Add(
								...
								{
									ID =
										dr["ID"] is DBNull ? (int?)null : Convert.ToInt32(dr["ID"]),
									Naziv = dr["NAZIV"].ToString(),
									OpsegDo =
										dr["OPSEGDO"] is DBNull
											? (int?)null
											: Convert.ToInt32(dr["OPSEGDO"]),
									OpsegOd =
										dr["OPSEGOD"] is DBNull
											? (int?)null
											: Convert.ToInt32(dr["OPSEGOD"]),
