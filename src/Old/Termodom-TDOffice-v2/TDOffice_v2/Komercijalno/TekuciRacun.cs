...
namespace TDOffice_v2.Komercijalno
{
	public class TekuciRacun
	{
		...
		{
			throw new Exception(
				"Nije sinhronizovano da radi sa svim bazama! Kontaktirajte administratora!"
			);
			using (
				FbCommand cmd = new FbCommand(
					"UPDATE TEKUCIRACUN SET RACUN = @R, BANKAID = @BID, VALUTA = @V, STANJE =@S, MAGACINID =@M WHERE PPID = @ID",
					con
				)
			)
			{
		...
		)
		{
			...
			using (
				FbCommand cmd = new FbCommand(
					"INSERT INTO TEKUCIRACUN (RACUN, PPID, BANKAID, VALUTA, STANJE, MAGACINID) VALUES (@R, @P, @BID, @V, @S, @M)",
					con
				)
			)
			{
		...
		public static TekuciRacun Get(FbConnection con, int ppid)
		{
			...
			using (
				FbCommand cmd = new FbCommand(
					"SELECT RACUN, PPID, BANKAID, VALUTA, STANJE, MAGACINID FROM TEKUCIRACUN WHERE PPID = @ID",
					con
				)
			)
			{
		...
		public static async Task<TekuciRacunList> ListAsync(int bazaId, int godinaBaze)
		{
			var response = await TDBrain_v3.GetAsync(
				$"/komercijalno/tekuciracun/list?bazaId={bazaId}&godinaBaze={godinaBaze}"
			);

			switch ((int)response.StatusCode)
			{
				case 200:
					return JsonConvert.DeserializeObject<TekuciRacunList>(
