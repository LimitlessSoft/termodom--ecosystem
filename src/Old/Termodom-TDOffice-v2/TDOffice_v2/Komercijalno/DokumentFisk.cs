		...
		public static DokumentFisk Get(FbConnection con, int dfid)
		{
			using (
				FbCommand cmd = new FbCommand(
					"SELECT DFID, VRDOK, BRDOK, TRANSACTIONTYPE, INVOICETYPE, REFERENTDOCUMENTNUMBER, REFERENTDOCUMENTDT, POSLAT, ZAPID, STATUS, NAPOMENA, LINKED_REFERENTDOCUMENTNUMBER, LINKED_REFERENTDOCUMENTDT, BYERID, BYEROP"
						+ " FROM DOKUMENT_FISK"
						+ " WHERE DFID = @DFID",
					con
				)
				...
				using (FbDataReader dr = cmd.ExecuteReader())
					if (dr.Read())
						return new DokumentFisk()
						{
							...
							ReferentDocumentNumber = dr["REFERENTDOCUMENTNUMBER"]
								.ToStringOrDefault(),
							...
							LinkedReferentDocumentNumber = dr["LINKED_REFERENTDOCUMENTNUMBER"]
								.ToStringOrDefault(),
							LinkedReferentDocumentDT = dr["LINKED_REFERENTDOCUMENTDT"]
								.ToStringOrDefault(),
