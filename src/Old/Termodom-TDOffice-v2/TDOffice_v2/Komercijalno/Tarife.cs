...
namespace TDOffice_v2.Komercijalno
{
	public class Tarife
	{
			...
			)
			{
				...
				using (
					FbCommand cmd = new FbCommand(
						"SELECT TARIFAID, NAZIV, STOPA, F_KOD, VRSTA FROM TARIFE WHERE TARIFAID = @TID",
						con
					)
				)
				{
		...
		public static async Task<Termodom.Data.Entities.Komercijalno.TarifaDictionary> Dictionary(
			int? godina = null
		)
		{
			var response = await TDBrain_v3.GetAsync(
				$"/komercijalno/tarifa/dictionary?godina={godina ?? DateTime.Now.Year}"
			);

			if ((int)response.StatusCode == 200)
				return new Termodom.Data.Entities.Komercijalno.TarifaDictionary(
					JsonConvert.DeserializeObject<
						Dictionary<string, Termodom.Data.Entities.Komercijalno.Tarifa>
