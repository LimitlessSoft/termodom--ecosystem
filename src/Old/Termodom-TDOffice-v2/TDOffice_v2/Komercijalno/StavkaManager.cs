...
namespace TDOffice_v2.Komercijalno
{
	public static class StavkaManager
	{
		...
		public static async Task<StavkaDictionary> DictionaryAsync(
			int bazaId,
			int vrDok,
			int brDok,
			int? godinaBaze
		...
		{
			string epString =
				$"/komercijalno/stavka/dictionary?bazaid={bazaId}&vrdok={vrDok}&brdok={brDok}&godina={godinaBaze ?? DateTime.Now.Year}";

			var response = await TDBrain_v3.GetAsync(epString);

			if ((int)response.StatusCode == 200)
				return JsonConvert.DeserializeObject<StavkaDictionary>(
					await response.Content.ReadAsStringAsync()
				);
			else if ((int)response.StatusCode == 500)
				throw new Termodom.Data.Exceptions.APIServerException();
