	...
	public class VrstaDokManager
	{
		public static async Task<VrstaDok> GetAsync(int godina, int vrDok)
		{
			var response = await TDBrain_v3.GetAsync(
				$"/Komercijalno/VrstaDok/Get?vrDok={vrDok}&godina={godina}"
			);
			switch ((int)response.StatusCode)
			{
				case 200:
					return JsonConvert.DeserializeObject<VrstaDok>(
		...
		{
			var response = await TDBrain_v3.GetAsync(
				$"/komercijalno/vrstadok/dictionary?godinaBaze={godina}"
			);

			switch ((int)response.StatusCode)
			{
				case 200:
					return JsonConvert.DeserializeObject<VrstaDokDictionary>(
						await response.Content.ReadAsStringAsync()
					);
				case 500:
					throw new Termodom.Data.Exceptions.APIServerException();
