...
namespace TDOffice_v2.Komercijalno
{
    public class Partner
    {
        ...
        public void Update()
        {
            ...
            {
                con.Open();
                using (FbCommand cmd = new FbCommand(@"UPDATE OR INSERT INTO PARTNER (PPID, NAZIV, ADRESA, POSTA, TELEFON, MOBILNI, FAX, EMAIL, KONTAKT, PIB, KATEGORIJA,
                    ...
                    @IMAUGOVOR, @VRSTACENOVNIKA, @DRZAVLJANSTVOID, @ZANIMANJEID, @WEB_STATUS, @GPPID, @CENE_OD_GRUPE, @VPCID, @PROCPC) MATCHING (PPID)", con))
                {
        ...
        public async static Task<Partner> GetAsync(int ppid)
        {
            var result = await TDBrain_v3.GetAsync("/komercijalno/partner/get?ppid=" + ppid);
            if((int)result.StatusCode == 200)
            {
                string responseString = await result.Content.ReadAsStringAsync();
                DTO.TDBrain_v3.Komercijalno.PartnerGetDTO dto = JsonConvert.DeserializeObject<DTO.TDBrain_v3.Komercijalno.PartnerGetDTO>(responseString);
        ...
        public async static Task<Partner> GetAsync(string pib)
        {
        ...
        public async static Task<List<Partner>> ListAsync(int? godina = null)
        {
            List<string> par = new List<string>();
            if (godina != null)
                par.Add("godina=" + godina);

            var result = await TDBrain_v3.GetAsync("/komercijalno/partner/list?" + (par.Count > 0 ? string.Join("&", par) : ""));
            if ((int)result.StatusCode == 200)
            {
                string responseString = await result.Content.ReadAsStringAsync();
                List<PartnerGetDTO> dto = JsonConvert.DeserializeObject<List<PartnerGetDTO>>(responseString);
        ...
        public static async Task<int> Insert(string naziv, string adresa, string posta, string telefon, string mobilni,
            ...
            int webStatus, int gppID, int ceneOdGrupe, int? vpcID, double? procpc)
        {
            var response = await TDBrain_v3.PostAsync("/komercijalno/partner/insert", new Dictionary<string, string>()
            {
                { "naziv", naziv },
                { "nazivZaStampu", naziv },
                { "adresa", adresa },
