...
namespace TDOffice_v2
{
	static class Program
	{
		...
		static void Main()
		{
			...
			{
				MessageBox.Show(
					"Putanja do TDOffice_v2 baze nije ispravna! Ispravite je unutar %appdata%/TDoffice_v2/Settings.td fajla"
				);
				return;
			}
			...
			TDOffice.Config<string> configConnectionStringConfig = TDOffice.Config<string>.Get(
				TDOffice.ConfigParameter.ConnectionStringConfig
			);
			TDOffice.Config<Dictionary<int, string>> komercijalnoConnectionStringsConfig =
				TDOffice.Config<Dictionary<int, string>>.Get(
			...
			{
				configConnectionStringConfig.Tag =
					"data source=localhost; initial catalog = C:\\Poslovanje\\Baze\\ConfigPDV\\CONFIG.FDB; user=SYSDBA; password=masterkey";
			...
			if (komercijalnoConnectionStringsConfig.Tag == null)
			{
				komercijalnoConnectionStringsConfig.Tag = new Dictionary<int, string>()
				{
					{
						2022,
						"data source=localhost; initial catalog = C:\\Poslovanje\\Baze\\2022\\FIRMA2022.FDB; user=SYSDBA; password=masterkey"
					},
					{
						2021,
						"data source=localhost; initial catalog = C:\\Poslovanje\\Baze\\2021\\FIRMA2021.FDB; user=SYSDBA; password=masterkey"
					},
					{
						2020,
						"data source=localhost; initial catalog = C:\\Poslovanje\\Baze\\2020\\FIRMA2020.FDB; user=SYSDBA; password=masterkey"
					},
					{
						2019,
						"data source=localhost; initial catalog = C:\\Poslovanje\\Baze\\2019\\FIRMA2019.FDB; user=SYSDBA; password=masterkey"
					},
					{
						2018,
						"data source=localhost; initial catalog = C:\\Poslovanje\\Baze\\2018\\FIRMA2018.FDB; user=SYSDBA; password=masterkey"
					},
					{
						2017,
						"data source=localhost; initial catalog = C:\\Poslovanje\\Baze\\2017\\FIRMA2017.FDB; user=SYSDBA; password=masterkey"
					}
				};
			...
			TDOffice.Config<List<int>> c = TDOffice.Config<List<int>>.Get(
			...
			TDOffice.Config<List<int>> cmp = TDOffice.Config<List<int>>.Get(
				TDOffice.ConfigParameter.MagacinUPopisu
			);
			if (cmp.Tag == null)
			{
		...
		public static async Task<Version> GetMinVerzijaAsync()
		{
			var response = await _client.GetAsync(
				"http://dev.api.termodom.rs/software/info?id=TDOffice_v2"
			);
			DTO_FV dto = JsonConvert.DeserializeObject<DTO_FV>(
		...
		{
			var response = await _client.GetAsync(
				"http://dev.api.termodom.rs/software/info?id=TDOffice_Installer"
			);
			DTO_IV dto = JsonConvert.DeserializeObject<DTO_IV>(
				await response.Content.ReadAsStringAsync()
			);
			return new Version(dto.minimal_version);
		}
		...
		public static async Task DownloadAndInstallNewVersionAsync()
		{
			...
			if (!File.Exists(Path.Combine(tempPath, "TDOffice_Installer.exe")))
			{
				var response1 = await _client.GetAsync(
					"http://dev.api.termodom.rs/software/download?id=TDOffice_Installer"
				);
				if ((int)response1.StatusCode == 200)
				{
					Stream str = await response1.Content.ReadAsStreamAsync();
					FileStream fs = new FileStream(
						Path.Combine(tempPath, "TDOffice_Installer.exe"),
