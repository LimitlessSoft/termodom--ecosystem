...
namespace TDOffice_v2
{
    public partial class fm_UlaznaPonuda_Index : Form
    {
        ...
        private Task<List<Komercijalno.Roba>> _komercijalnoRoba = Komercijalno.Roba.ListAsync(DateTime.Now.Year);
        ...
        private void UcitajStavke()
        {
            ...
            {
                Komercijalno.Roba r = _komercijalnoRoba.Result.FirstOrDefault(x => x.ID == s.RobaID);
        ...
        {
            int noviID = TDOffice.DokumentUlaznaPonuda.Insert(Program.TrenutniKorisnik.ID, DateTime.Now, DateTime.Now.AddMonths(3));
                ...
                ir.OnRobaClickHandler += (Komercijalno.RobaUMagacinu[] args) =>
                {
                    foreach(Komercijalno.RobaUMagacinu r in args)
                    {
                        if (_stavke.Any(x => x.RobaID == r.RobaID))
                            MessageBox.Show("Ulazna ponuda vec sadrzi robu " + _komercijalnoRoba.Result.FirstOrDefault(x => x.ID == r.RobaID).Naziv);
                        else
                        {
                            TDOffice.StavkaUlaznaPonuda.Get(TDOffice.StavkaUlaznaPonuda.Insert(_dokument.ID, r.RobaID, 0));
                            UcitajStavke();
                        }
                    }
                };
        ...
        {
            (sender as Control).BackColor = (sender as Control).Enabled ? TDColor.ControlaActive : TDColor.ControlInactive;
        }
