...
namespace TDOffice_v2
{
	public partial class fm_MagacinUPopisu_Index : Form
	{
		...
		private Task<List<TDOffice.MagacinClan>> _clanoviMagacina { get; set; } =
			TDOffice.MagacinClan.ListAsync();
		private Task<TDOffice.Config<List<int>>> _config { get; set; } =
			TDOffice.Config<List<int>>.GetAsync(TDOffice.ConfigParameter.MagacinUPopisu);
		...
		private void LoadMagacinUPopisu()
		{
			...
			dataGridView1.Columns["MAGACINID"].AutoSizeMode =
		...
		private void btnPrimeni_Click(object sender, EventArgs e)
		{
			...
			{
				foreach (
					TDOffice.MagacinClan clan in clanoviMagacina.Where(x => x.MagacinID == m.ID)
				)
				{
					Komercijalno.PravaZap pzap = Komercijalno.PravaZap.Get(
						clan.KorisnikID,
						1,
						10017
					);
