...
namespace TDOffice_v2
{
	public partial class fm_PravoPristupaModulu_OdobriZabrani : Form
	{
		...
		{
			TDOffice.Pravo lp = TDOffice
				.Pravo.List(
					$"USER_ID = {_poruka.Posiljalac} AND MODUL_ID = {Convert.ToInt32(_poruka.Tag.AdditionalInfo)}"
				)
				.FirstOrDefault();
			if (lp == null)
				lp = TDOffice.Pravo.Get(
					TDOffice.Pravo.Insert(
			...
			{
				naslovPoruke =
					"Odobreno pravo pristupa modula " + Convert.ToInt32(_poruka.Tag.AdditionalInfo);
				tekstPoruke =
					"Odobreno ti je trazeno pravo pristupa modulu "
					+ Convert.ToInt32(_poruka.Tag.AdditionalInfo);
			}
			else if (statusPoruke == -1)
			{
				naslovPoruke =
					"Zauvek zabranjeno pravo pristupa modula "
					+ Convert.ToInt32(_poruka.Tag.AdditionalInfo);
				tekstPoruke =
					"Zauvek ti je zabranjeno pravo pristupa modulu "
					+ Convert.ToInt32(_poruka.Tag.AdditionalInfo);
			}
			...
			Task.Run(() =>
			{
				...
				{
					foreach (
						TDOffice.Poruka ps in poruke.Where(x =>
							x.Tag != null
							&& x.Tag.Action == TDOffice.PorukaAction.PravoPristupaModulu
						)
					)
					{
