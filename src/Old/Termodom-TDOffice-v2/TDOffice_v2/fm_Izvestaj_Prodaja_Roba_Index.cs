...
namespace TDOffice_v2
{
	public partial class fm_Izvestaj_Prodaja_Roba_Index : Form
	{
		...
		{
			get { return prikaziZbirnoPoMagacinima_btn.Enabled; }
			set { prikaziZbirnoPoMagacinima_btn.Enabled = value; }
		}
		public bool DozvoliPrikazZbiraPoRobi
		{
			get { return prikaziZbirnoPoRobi_btn.Enabled; }
			set { prikaziZbirnoPoRobi_btn.Enabled = value; }
		}

		public fm_Izvestaj_Prodaja_Roba_Index(
			DataTable dataTable,
		...
		)
		{
			...
			foreach (int godina in _periodi.Godine.OrderBy(x => x))
			{
				...
				if (_dataTable.Columns.Contains("kGOD" + godina))
				{
					...
					dataGridView1.Columns["kGOD" + godina].DefaultCellStyle.Alignment =
						DataGridViewContentAlignment.MiddleRight;
					dataGridView1.Columns["kGOD" + godina].Width = 100;
					if (_dataTable.Columns.Contains("vGOD" + godina))
						dataGridView1.Columns["kGOD" + godina].DisplayIndex = 3 + y;
					dataGridView1.Columns["kGOD" + godina].HeaderText =
						$"KOLICINA GOD {godina} [{_periodi[godina].OdDatuma.ToString("dd.MM.yyyy")} - {_periodi[godina].DoDatuma.ToString("dd.MM.yyyy")}]";
				...
				if (_dataTable.Columns.Contains("vGOD" + godina))
				{
					...
					if (_dataTable.Columns.Contains("kGOD" + godina))
						dataGridView1.Columns["vGOD" + godina].DisplayIndex =
							3 + y + _periodi.Count;
					dataGridView1.Columns["vGOD" + godina].HeaderText =
						$"VREDNOST GOD {godina} [{_periodi[godina].OdDatuma.ToString("dd.MM.yyyy")} - {_periodi[godina].DoDatuma.ToString("dd.MM.yyyy")}]";
		...
		private void fm_Izvestaj_Prodaja_Roba_Index_Load(object sender, EventArgs e)
		{
			Task.Run(() =>
			{
				List<int> magaciniUTabeli = _dataTable
					...
					.Distinct()
					.ToList();
				List<Komercijalno.Magacin> komercijalnoMagacini = Komercijalno
					.Magacin.ListAsync()
					.Result;
				...
				this.Invoke(
					(MethodInvoker)
						delegate
						{
							filterMagacini_clb.DataSource = komercijalnoMagacini
								.Where(x => magaciniUTabeli.Contains(x.ID))
								.ToList();
		...
		{
			using (
				fm_Izvestaj_Prodaja_Roba_PosaljiKaoIzvestaj_Setup s =
					new fm_Izvestaj_Prodaja_Roba_PosaljiKaoIzvestaj_Setup(
						_dataTable,
						_periodi,
						_tipIzvestaja,
						_listaRobe
		...
		private void prikaziZbirnoPoMagacinima_btn_Click(object sender, EventArgs e)
		{
			...
			{
				using (
					fm_Izvestaj_Prodaja_Roba_ZbirnoPoMagacinima gm =
		...
		private void filterMagacini_clb_SelectedIndexChanged(object sender, EventArgs e)
		{
			...
			DataRow[] rows = _dataTable.Select(
				$"MAGACINID IN ({string.Join(", ", filterMagacini_clb.CheckedItems.OfType<Komercijalno.Magacin>().Select(x => x.ID))})"
			);

			dataGridView1.DataSource =
				rows == null || rows.Length == 0 ? null : rows.CopyToDataTable();
			;
		}
		...
		{
			using (
				fm_Izvestaj_Prodaja_Roba_ZbirnoPoRobi fm =
					new fm_Izvestaj_Prodaja_Roba_ZbirnoPoRobi(_dataTable, _periodi)
			)
			{
