...
namespace TDOffice_v2
{
	public partial class fm_IzborUsluga_Index : Form
	{
		...
		public delegate void OnPotvrdaUsluge(
			int robaID,
			double kolicina,
			double cena,
			string naziv,
		...
		private void fm_IzborUsluga_Index_Shown(object sender, System.EventArgs e)
		{
			if (PotvrdaUsluge == null)
				throw new Exception(
					$"Ne mozete prikazati Izbor Robe bez i jednog event handlera na {nameof(PotvrdaUsluge)} eventu!"
				);
		}
		...
		private async Task PopuniDGVAsync()
		{
			...
			foreach (
				Termodom.Data.Entities.Komercijalno.Roba r in (await _robaTask).Values.Where(x =>
					new int[] { 2, 4 }.Contains(x.Vrsta)
				)
			)
			{
