	...
	public partial class fm_Izvestaj_Prodaja_Roba_Setup : Form
	{
			...
			Task.Run(() =>
			{
				...
				foreach (DataGridViewRow row in godine_dgv.Rows)
					if (Convert.ToBoolean(row.Cells["KoristiSe"].Value))
						periodi[Convert.ToInt32(row.Cells["Godina"].Value)] =
							new Models.Periodi.Period(
				...
				if (_listaRobe.Count > 0 && !listaRobe_cb.Checked)
				{
					if (
						MessageBox.Show(
							"Lista robe ima stavke ali nije cekirano da se koristi lista. Da li zelite nastaviti?",
							"Potvrdi",
				...
				DataTable dt = Models.Izvestaj.ProdajaRobePoRealnimCenama(
					periodi,
					magacini,
					listaRobe_cb.Checked ? _listaRobe : null
				);
				...
				this.Invoke(
					(MethodInvoker)
						...
						{
							using (
								fm_Izvestaj_Prodaja_Roba_Index ind =
									new fm_Izvestaj_Prodaja_Roba_Index(
										dt,
										periodi,
										"RSD",
										_listaRobe,
								...
								Task.Run(() =>
								{
									if ((DateTime.Now - checkPoint).TotalSeconds > 15)
									{
										List<TDOffice.CheckListItem> korisnikZadaci =
											TDOffice.CheckListItem.ListByKorisnikID(
												Program.TrenutniKorisnik.ID
											);
										TDOffice.CheckListItem item = korisnikZadaci.FirstOrDefault(
											x =>
												x.Job == TDOffice.CheckList.Jobs.IzvestajProdajeRobe
										);
										if (item != null)
										{
		...
		private void UcitajPodatkePoslednjeCene()
		{
			Task.Run(() =>
			{
				...
				DataTable dt = Models.Izvestaj.ProdajaRobePoPoslednjimCenama(
					periodi,
					magacini,
					listaRobe_cb.Checked ? _listaRobe : null
				);
