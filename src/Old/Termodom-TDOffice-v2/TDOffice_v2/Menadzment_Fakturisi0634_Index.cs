...
namespace TDOffice_v2
{
	public partial class Menadzment_Fakturisi0634_Index : Form
	{
		...
		private Task<List<Komercijalno.Magacin>> _magaciniKomercijalno =
		...
		private Task<Dictionary<int, double>> UcitajNabavneCeneAsync()
		{
			return Task.Run(() =>
			{
				...
				List<Komercijalno.Dokument> dokumentiNabavke = Komercijalno.Dokument.List(
					$"MAGACINID = 50 AND VRDOK IN (0, 1, 2, 36)"
				);
				List<Komercijalno.Stavka> stavkeNabavke = Komercijalno.Stavka.List(
					DateTime.Now.Year,
					$"MAGACINID = 50 AND VRDOK IN (0, 1, 2, 36)"
				);
				...
				foreach (int key in keys)
				{
					try
					{
						dict[key] = Komercijalno.Komercijalno.GetRealnaNabavnaCena(
							key,
							DateTime.Now,
							dokumentiNabavke,
							stavkeNabavke
					...
					{
						if (
							ex.Message.ToLower().Contains("invoke")
							|| ex.Message.ToLower().Contains("disposed")
						)
							return null;
		...
		private void NamestiUI()
		{
			...
			this.status_btn.BackgroundImage =
				_dokument.Status == 0 && _dokument.KomercijalnoFaktura == null
			...
			magacin_txt.Text = _magaciniKomercijalno
				.Result.FirstOrDefault(x => x.ID == _dokument.MagacinID)
				.Naziv;
			poveziSaKomercijalim_btn.Enabled =
				_dokument.Status == 1 && _dokument.KomercijalnoFaktura == null;
		}
		...
		private void UcitajStavke()
		{
			...
			{
				Komercijalno.Roba r = _robaKomercijalno.Result.FirstOrDefault(x =>
					x.ID == s.RobaID
				);
			...
			Task.Run(() =>
			{
				using (Menadzment_Lager0634_Index l = new Menadzment_Lager0634_Index())
				{
					...
					l.OnRobaSelect += (robaID, kolicina) =>
					{
						if (
							TDOffice
								.Stavka0634.List("BRDOK = " + _dokument.ID)
								.FirstOrDefault(x => x.RobaID == robaID) != null
						)
						{
			...
			Task.Run(() =>
			{
				...
				using (
					InputBox ib = new InputBox(
						"Cilj nabavne vrednosti",
						"Unesite ciljanu nabavnu vrednost"
					)
				)
				{
					ib.ShowDialog();
				...
				while (trenutnaNabavna < ciljNabavneVrednosti)
				{
					...
					if (
						TDOffice
							.Stavka0634.List("BRDOK = " + _dokument.ID)
							.FirstOrDefault(x => x.RobaID == keys[k]) != null
					)
						continue;
		...
		private void poveziSaKomercijalim_btn_Click(object sender, EventArgs e)
		{
			...
			)
			{
				...
				foreach (
					TDOffice.Stavka0634 s in TDOffice
						.Stavka0634.List("BRDOK = " + _dokument.ID)
					...
					Komercijalno.Stavka.Insert(
						con,
						dok,
						_robaKomercijalno.Result.FirstOrDefault(x => x.ID == s.RobaID),
						_robaUMagacinuKomercijalno.Result.FirstOrDefault(x => x.RobaID == s.RobaID),
						s.Kolicina,
						0
					);
		...
		private void obrisiSveStavkeToolStripMenuItem_Click(object sender, EventArgs e)
		{
			if (
				MessageBox.Show(
					"Da li zelite ukloniti sve stavke iz dokumenta?",
					"Potvrdi",
