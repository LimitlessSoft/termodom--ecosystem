...
namespace TDOffice_v2
{
	public partial class _1365_fm_SastavnicaRastavnica_Index : Form
	{
		...
		private void SetUI()
		{
			List<TDOffice.DokumentSastavnica> pravilaList = TDOffice
				.DokumentSastavnica.ListByTip(TDOffice.Enums.DokumentSastavnicaTip.Pravilo)
				.ToList();
			pravilaList.Add(
				new TDOffice.DokumentSastavnica() { ID = -1, Tag = "< Izaberi pravilo > " }
			);
			...
			this.BackColor =
				_sastavnica.Status == TDOffice.DokumentStatus.Zakljucan ? Color.Red : Color.Green;
		}
		...
		private void ReloadData()
		{
			...
			List<TDOffice.StavkaSastavnica> stavkeSastavnice =
				TDOffice.StavkaSastavnica.ListBySastavnicaID(id);
			...
			foreach (TDOffice.StavkaSastavnica ss in stavkeSastavnice)
			{
				...
				dr["KOLICINA"] =
					_sastavnica.Status == TDOffice.DokumentStatus.Zakljucan
						? ss.Kolicina
						: ss.Kolicina * (double)numericUpDown1.Value;
			...
			this.Text =
				"Sastavnica ["
				+ _sastavnica.ID.ToString()
				+ "] za magacin ID["
				+ _sastavnica.MagacinID.ToString()
				+ "]";
		}
		...
		private void button1_Click(object sender, EventArgs e)
		{
			...
			if (
				MessageBox.Show(
					"Sigurno zelite da kreirate "
						+ numericUpDown1.Value.ToString()
						+ " setova po sastavnici "
						+ this.pravilaSastavnica_cmb.Text
						+ "?",
					"Kreiranje satavnice",
			...
			)
			{
				using (FbConnection con = new FbConnection(TDOffice.TDOffice.connectionString))
				{
					...
					foreach (DataGridViewRow row in dataGridView1.Rows)
						TDOffice.StavkaSastavnica.Insert(
							con,
							_sastavnica.ID,
							Convert.ToInt32(row.Cells["Robaid"].Value),
				...
				{
					con.Open();
					int komercijalnoBrDok = Komercijalno.Dokument.Insert(
						con,
						16,
						_sastavnica.ID.ToString(),
						null,
					...
					List<Komercijalno.RobaUMagacinu> rumKom =
						Komercijalno.RobaUMagacinu.ListByMagacinID(con, _sastavnica.MagacinID);
					...
					foreach (DataGridViewRow row in dataGridView1.Rows)
					{
						...
						Komercijalno.Stavka.Insert(
							DateTime.Now.Year,
							dok,
							robaKom.Where(x => x.ID == robaID).FirstOrDefault(),
							rumKom.Where(x => x.RobaID == robaID).FirstOrDefault(),
							Convert.ToDouble(row.Cells["KOLICINA"].Value),
							0
						);
					}
