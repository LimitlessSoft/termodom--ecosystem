...
namespace TDOffice_v2
{
	public partial class _1301_fm_Poruka_Index : Form
	{
			...
			get
			{
				if (_poruka == null)
					throw new NullReferenceException(
						"Objekat TDOffice.Poruka koji je vezan za ovu formu je null!"
					);
		...
		{
			if (
				(_poruka.Status == TDOffice.PorukaTip.Sticky && !ForceClose)
				|| _poruka.Status == TDOffice.PorukaTip.Expanding
			)
				e.Cancel = true;
		}
		...
		private void SetUI()
		{
			...
			this.panel1.BackColor =
				_poruka.Status == TDOffice.PorukaTip.Sticky
					? Color.Purple
					: _poruka.Status == TDOffice.PorukaTip.Expanding
						? Color.Red
						: this.panel1.BackColor;
			...
			TDOffice.User posiljalac = _korisnici
				.Result.Where(x => x.ID == _poruka.Posiljalac)
				.FirstOrDefault();
			...
			this.label4.ForeColor =
				_poruka.Tag != null && !string.IsNullOrWhiteSpace(_poruka.Tag.Beleska)
					? Color.Orange
					: Color.Black;
			...
			if (_poruka.Tag != null)
			{
				switch (_poruka.Tag.Action)
				{
					...
					case TDOffice.PorukaAction.RealizovanoRazduzenjeMagacina:
						action_btn.Visible = true;
						action_btn.Text =
							"Idi na razduzenje magacina br: " + _poruka.Tag.AdditionalInfo;
						break;
			...
			{
				List<TDOffice.Poruka> porukeCC = TDOffice.Poruka.List(
					$"PAKET = {(int)_poruka.Paket} AND TIPPRIMAOCA = {(int)TDOffice.Enums.PorukaTipPrimaoca.CC}"
				);
				string stCC = "CC: ";
				foreach (TDOffice.Poruka pcc in porukeCC)
				{
					TDOffice.User primalac = _korisnici
						.Result.Where(x => x.ID == pcc.Primalac)
						.FirstOrDefault();
			...
			else if (_poruka.Status == TDOffice.PorukaTip.Expanding)
			{
				...
				this.Size = new Size(
					(int)Math.Min(currentScreen.Width, 800 * Math.Pow(1.2, days)),
					(int)Math.Min(currentScreen.Height, 400 * Math.Pow(1.2, days))
				);
				...
				int oc = Math.Max(
					0,
					Convert.ToInt32(220 - Math.Min(240, Math.Pow(1.5, Math.Pow(days + 1, 2.2))))
				);
		...
		private void PozicionirajPoruku()
		{
			...
			this.Location = new System.Drawing.Point(
				pivotX + (System.DateTime.Now.Millisecond / 50),
				pivotY + (System.DateTime.Now.Millisecond / 50)
			);
		}
		...
		private void btnOdgovor_Click(object sender, EventArgs e)
		{
			...
			TDOffice.User trenutnaPorukaKorisnik = _korisnici.Result.FirstOrDefault(x =>
				x.ID == trenutnaPoruka.Posiljalac
			);
			string oldText = trenutnaPoruka.Tekst;
			trenutnaPoruka.Tekst = "\n\n========================\n";
			trenutnaPoruka.Tekst += trenutnaPoruka.Datum.ToString("[ dd.MM.yyyy ]\n");
			trenutnaPoruka.Tekst +=
				trenutnaPorukaKorisnik == null
					? "nepoznati korisnik"
					: trenutnaPorukaKorisnik.Username + " je napisao:\n";
			trenutnaPoruka.Tekst += "========================\n";
			...
			switch (_poruka.Tag.Action)
			{
				case TDOffice.PorukaAction.NULL:
					...
					)
					{
						...
						var trenutnaPoruka = TDOffice.Poruka.Get(idPoruke);
						TDOffice.User trenutnaPorukaKorisnik = _korisnici
							.Result.Where(x => x.ID == trenutnaPoruka.Posiljalac)
							.FirstOrDefault();
						string oldText = trenutnaPoruka.Tekst;
						trenutnaPoruka.Tekst = "\n\n========================\n";
						trenutnaPoruka.Tekst += trenutnaPoruka.Datum.ToString("[ dd.MM.yyyy ]\n");
				...
				case TDOffice.PorukaAction.NoviTDOfficePopis:
					...
					using (
						_7_fm_TDPopis_Index pi = new _7_fm_TDPopis_Index(
							TDOffice.DokumentPopis.Get(Convert.ToInt32(_poruka.Tag.AdditionalInfo))
						)
					)
						pi.ShowDialog();
				...
				case TDOffice.PorukaAction.NoviPartner:
					using (
						_1348_fm_Partner_Index ppnew = new _1348_fm_Partner_Index(
							Convert.ToInt32(_poruka.Tag.AdditionalInfo)
						)
					)
						ppnew.ShowDialog();
					break;
				case TDOffice.PorukaAction.OdgovorNaStickyPoruku:
					TDOffice.Poruka porukaPosiljaoca = TDOffice.Poruka.Get(
						Convert.ToInt32(_poruka.Tag.AdditionalInfo)
					);
				...
				case TDOffice.PorukaAction.RealizovanaZamenaRobe:
					using (
						fm_ZamenaRobe2_Index zi = new fm_ZamenaRobe2_Index(
							Convert.ToInt32(_poruka.Tag.AdditionalInfo)
						)
					)
						zi.ShowDialog();
				...
				case TDOffice.PorukaAction.RealizovanoRazduzenjeMagacina:
					using (
						_1336_fm_RazduzenjeMagacina_Index zi =
							new _1336_fm_RazduzenjeMagacina_Index(
								TDOffice.DokumentRazduzenjaMagacina.Get(
									Convert.ToInt32(_poruka.Tag.AdditionalInfo)
								)
							)
					)
				...
				case TDOffice.PorukaAction.IzvestajProdajeRobe:
					...
					DataTable dtt = JsonConvert.DeserializeObject<DataTable>(
						JsonConvert.SerializeObject((obj.First() as IEnumerable<object>).First())
					);
					Models.Periodi periodi = JsonConvert.DeserializeObject<Models.Periodi>(
						JsonConvert.SerializeObject(
							(obj.ElementAt(1) as IEnumerable<object>).First()
						)
					);
					string tipizv = JsonConvert.DeserializeObject<string>(
						JsonConvert.SerializeObject(
							(obj.ElementAt(2) as IEnumerable<object>).FirstOrDefault()
						)
					);
				...
				case TDOffice.PorukaAction.IzvestajProdajeRobeZbirnoPoMagacinima:
					...
					DataTable dtt1 = JsonConvert.DeserializeObject<DataTable>(
						JsonConvert.SerializeObject((obj1.First() as IEnumerable<object>).First())
					);
					Models.Periodi periodi1 = JsonConvert.DeserializeObject<Models.Periodi>(
						JsonConvert.SerializeObject(
							(obj1.ElementAt(1) as IEnumerable<object>).First()
						)
					);
					...
					using (
						fm_Izvestaj_Prodaja_Roba_ZbirnoPoMagacinima ind =
							new fm_Izvestaj_Prodaja_Roba_ZbirnoPoMagacinima(
								dtt1,
								periodi1,
								st,
								nList1
				...
				case TDOffice.PorukaAction.IzvestajPrometaMagacina:
					...
					using (
						fm_PrikazIzvestajPrometMagacina ind = new fm_PrikazIzvestajPrometMagacina(
							dtt2
						)
					)
					{
				...
				case TDOffice.PorukaAction.PravoPristupaModulu:
					using (
						fm_PravoPristupaModulu_OdobriZabrani ppm =
							new fm_PravoPristupaModulu_OdobriZabrani(_poruka)
					)
					{
						ppm.ShowDialog();
						ppm.BringToFront();
		...
		private void btn_Pin_Click(object sender, EventArgs e)
		{
			...
			for (int i = 0; i < Program.TrenutniKorisnik.Tag.Pinovi.Count; i++)
			{
				...
				{
					Program.TrenutniKorisnik.Tag.Pinovi.Remove(
						Program
							.TrenutniKorisnik.Tag.Pinovi.Where(x => x.PinID == pid)
							.FirstOrDefault()
					);
					pinovana = true;
					break;
			...
			if (pinovana == false)
			{
				TDOffice.User.Pin pin = new TDOffice.User.Pin() { PinID = pid };
