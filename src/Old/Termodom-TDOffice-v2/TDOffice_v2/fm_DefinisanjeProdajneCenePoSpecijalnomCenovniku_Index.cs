...
namespace TDOffice_v2
{
	public partial class fm_DefinisanjeProdajneCenePoSpecijalnomCenovniku_Index : Form
	{
		...
		private void fm_DefinisanjeProdajneCenePoSpecijalnomCenovniku_Index_Load(
			object sender,
			EventArgs e
		) { }
		...
		private void button1_Click(object sender, EventArgs e)
		{
			...
			)
			{
				...
				{
					MessageBox.Show(
						"Dokument sa datim brojem nije pronadjen (ni MP Racun ni Proracun)!"
					);
					return;
				}
				...
				List<Komercijalno.Stavka> stavke = Komercijalno.Stavka.List(
					con,
					$"VRDOK = {dokument.VrDok} AND BRDOK = {dokument.BrDok}"
				);
				...
				TDOffice.PartnerKomercijalno partnerKomercijalno = TDOffice.PartnerKomercijalno.Get(
					(int)dokument.PPID
				);
				if (
					partnerKomercijalno == null
				...
				if (
					...
					|| partnerKomercijalno
						...
						.NaciniUplateZaKojiVazeSpecijalniCenovnici == null
					|| !partnerKomercijalno.SpecijalniCenovnikPars.NaciniUplateZaKojiVazeSpecijalniCenovnici.Contains(
						(int)dokument.NUID
					)
				)
				{
					MessageBox.Show(
						"Partner nema definisane uslove za zadati nacin placanja dokumenta!"
					);
					return;
				}

				List<TDOffice.SpecijalniCenovnik.Item> specijalniCenovniciItems =
					TDOffice.SpecijalniCenovnik.Item.List(
						$"CENOVNIKID IN ({string.Join(", ", partnerKomercijalno.SpecijalniCenovnikPars.SpecijalniCenovnikList)})"
					);

				foreach (Komercijalno.Stavka s in stavke)
				{
					TDOffice.SpecijalniCenovnik.Item it = specijalniCenovniciItems.FirstOrDefault(
						x => x.RobaID == s.RobaID
					);
					double zeljenaProdajna,
						rabat = 0;
					if (it == null)
					{
						it = new TDOffice.SpecijalniCenovnik.Item()
						{
							...
							RobaID = s.RobaID,
							UslovModifikator = partnerKomercijalno
								.SpecijalniCenovnikPars
								.OtherModifikator,
							UslovTip = partnerKomercijalno.SpecijalniCenovnikPars.OtherTip
						};
						zeljenaProdajna = Math.Round(
							fm_SpecijalniCenovnik_Index.IzracunajSpecijalnuCenu(
								con,
								it,
								dokument.MagacinID,
								_dokumentiNabavke.Result,
								_stavkeNabavke.Result
							),
							4
						);
						rabat = Math.Min(
							it.MaxRabat,
							(((zeljenaProdajna / s.ProdCenaBP) - 1) * -100)
						);
						s.Rabat = radioButton2.Checked ? Math.Floor(rabat) : Math.Round(rabat, 2);
						s.Update(con);
						continue;
					}
					zeljenaProdajna = fm_SpecijalniCenovnik_Index.IzracunajSpecijalnuCenu(
						con,
						it,
						dokument.MagacinID,
						_dokumentiNabavke.Result,
