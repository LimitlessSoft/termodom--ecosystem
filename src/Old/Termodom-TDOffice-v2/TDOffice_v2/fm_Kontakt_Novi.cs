...
namespace TDOffice_v2
{
	public partial class fm_Kontakt_Novi : Form
	{
		...
		private void button1_Click(object sender, EventArgs e)
		{
			...
			TDOffice.Partner postojeciPartner = _partneri.Result.FirstOrDefault(x =>
				x.Mobilni.Contains(validMobileNumber)
			);
			if (postojeciPartner != null)
			{
				MessageBox.Show(
					$"Mobilni vec postoji u partneru [({postojeciPartner.ID}) {postojeciPartner.Naziv}]"
				);
				return;
			}
