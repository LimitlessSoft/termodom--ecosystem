...
namespace TDOffice_v2
{
	public partial class fm_ZamenaRobe2_Index : Form
	{
		...
		private double _vracaSeSumNeto
		{
			get => _vracaSeSum * ((100 - (double)trosakZamene_nud.Value) / 100);
		}
		...
		private void UcitajZamenuRobe(int brojZamene)
		{
			...
			Komercijalno.Magacin mag = _magacini.Result.FirstOrDefault(x =>
				x.ID == _zamenaRobe.MagacinID
			);
				...
				ir.OnRobaClickHandler += (Komercijalno.RobaUMagacinu[] rums) =>
				{
					...
					Task<double> minKolicinaRobeUKartici = Task.Run<double>(() =>
					{
						using (
							FbConnection con = new FbConnection(
								Komercijalno.Komercijalno.CONNECTION_STRING[_MPRacun.Datum.Year]
							)
						)
						{
							con.Open();
							Komercijalno.Procedure.SrediKarticu(
								con,
								_MPRacun.MagacinID,
								rums[0].RobaID,
							...
							#region
							DateTime maxDatumDokumentaZaRobuIDUnutarMagacina = new DateTime(
								DateTime.Now.Year,
								1,
								1
							);
							...
							using (
								FbCommand cmd = new FbCommand(
									@"SELECT MAX(DOKUMENT.DATUM) FROM DOKUMENT
                                LEFT OUTER JOIN STAVKA ON DOKUMENT.VRDOK = STAVKA.VRDOK AND DOKUMENT.BRDOK = STAVKA.BRDOK
                                WHERE DOKUMENT.MAGACINID = @MAG AND STAVKA.ROBAID = @RID",
									con
								)
							)
							{
								...
								using (FbDataReader dr = cmd.ExecuteReader())
									if (dr.Read())
										if (!(dr[0] is DBNull))
											maxDatumDokumentaZaRobuIDUnutarMagacina =
							...
							{
								Komercijalno.RobaUMagacinu rum = _robaUMagacinu.FirstOrDefault(x =>
									x.RobaID == rums[0].RobaID
								);
							...
							else
							{
								using (
                                    ...
                                    WHERE DOKUMENT.MAGACINID = @MID AND DOKUMENT.DATUM >= @DAT AND ROBAID = @RID",
										con
									)
								)
								{
									...
									throw new Exception(
										"Greska prilikom hvatanja minimalnog stanja iz kartice robe!"
									);
								}
							}
						}
					...
					using (
						InputBox ib = new InputBox(
							"Kolicina",
							"Unesite kolicinu koju zelite uzeti!"
						)
					)
					{
						ib.ShowDialog();
					...
					{
						MessageBox.Show(
							$"Kolicina bi odvela magacin u minus u nekom trenutku tokom godine!"
					...
					double razlika =
						_vracaSeSumNeto
						- (_uzimaSeSum + (rums[0].ProdajnaCena * kolicinaKojaSeUzima));
					if (razlika < 0)
					{
						double prvaNizaKolicinaKojuMoguUnetiUDokument = Math.Round(
							((_vracaSeSumNeto - _uzimaSeSum) / rums[0].ProdajnaCena),
							2
						);
						MessageBox.Show(
							$"Ne mogu uneti stavku, vrednost robe koja se uzima ce premasiti vrednost vracene robe za {Math.Abs(razlika).ToString("#,##0.00")}!"
						);
						MessageBox.Show(
							$"Najveca validna kolicina za unos je {prvaNizaKolicinaKojuMoguUnetiUDokument}"
						);
						return;
					}

					_zamenaRobe.Tag.UzimaSe.Add(
						new Tuple<int, double>(rums[0].RobaID, kolicinaKojaSeUzima)
					);
					_zamenaRobe.Update();
					this.Invoke(
						(MethodInvoker)
		...
		private void PrilagodiUI()
		{
			...
			this.dokumentStatus_btn.Text =
				_zamenaRobe.Status == TDOffice.DokumentStatus.Zakljucan ? "Otkljucaj" : "Zakljucaj";
			this.BackColor =
				_zamenaRobe.Status == TDOffice.DokumentStatus.Zakljucan
					? _zamenaRobe.Realizovana
						? Color.Purple
						: Color.Red
					: Color.Green;
		}
		...
		private void UcitajMPRacun(int mpRacun)
		{
			...
			)
			{
				...
				_stavkeMPRacuna = _zamenaRobe.Realizovana
					? _zamenaRobe.Tag.StaroStanjeStavkiDokumenta
					: Komercijalno.Stavka.ListByDokument(con, 15, mpRacun);
		...
		private void OsveziVracaSeDGV()
		{
			...
			)
			{
				...
				foreach (Tuple<int, double> r in _zamenaRobe.Tag.VracaSe)
				{
					...
					Komercijalno.Stavka stavka = _stavkeMPRacuna.FirstOrDefault(x =>
						x.RobaID == r.Item1
					);

					double prodajnaCenaDanas = rum == null ? double.MaxValue : rum.ProdajnaCena;
					double prodajnaCenaNaDanMPRacuna = Komercijalno.Procedure.ProdajnaCenaNaDan(
						con,
						_MPRacun.MagacinID,
						r.Item1,
						_MPRacun.Datum
					...
					double prodajnaCenaMPRacunaSaPopustom =
						prodajnaCenaNaDanMPRacuna * ((100 - stavka.Rabat) / 100);
					double manjaProdajnaCena =
						prodajnaCenaMPRacunaSaPopustom > prodajnaCenaDanas
							? prodajnaCenaDanas
							: prodajnaCenaMPRacunaSaPopustom;
			...
			vracaSe_dgv.Columns["Prodajna Cena Racun Sa Popustom"].DefaultCellStyle.Format =
				"#,##0.00";
		...
		private void OsveziZbirove()
		{
			...
			vracaSeVrednost_txt.Text =
				$"(-{trosakZamene_nud.Value}%) {_vracaSeSumNeto.ToString("#,##0.00")}";
		}
		...
		private void mpRacun_dgv_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
		{
			if (
				e.RowIndex < 0
				|| _zamenaRobe.Status != TDOffice.DokumentStatus.Otkljucan
				|| _zamenaRobe.Realizovana
			)
				return;
			...
			{
				MessageBox.Show(
					"Kolicina koja se vraca ne moze biti veca od kolicine iz MP racuna!"
				);
				return;
			}
			...
			)
			{
				...
				foreach (Tuple<int, double> v in _zamenaRobe.Tag.VracaSe)
				{
					...
					Komercijalno.Procedure.SrediKarticu(
						...
						_MPRacun.MagacinID,
						st.RobaID,
						new DateTime(_MPRacun.Datum.Year, 1, 1)
					);
				}
				...
				foreach (Tuple<int, double> u in _zamenaRobe.Tag.UzimaSe)
				{
					...
					{
						MessageBox.Show(
							"Doslo je do greske u nalasku robe prilikom insertovanja robe koja se uzima!"
						);
						return;
					}
					...
					{
						MessageBox.Show(
							"Doslo je do greske u nalasku robe u magacinu prilikom insertovanja robe koja se uzima!"
						);
						return;
					}
				...
				Komercijalno.Procedure.NapraviUslugu(
					15,
					_MPRacun.BrDok,
					6635,
					Math.Round(trosakZamene, 2, MidpointRounding.AwayFromZero),
					1,
					0
				);
				Komercijalno.Procedure.PresaberiDokument(con, 15, _MPRacun.BrDok);
				...
				PrilagodiUI();
				MessageBox.Show(
					"Uspesno izvrsena zamena! Razlika: " + trosakZamene.ToString("#,##0.00")
				);
			}
		}
		...
		private void dokumentStatus_btn_Click(object sender, EventArgs e)
		{
			if (_zamenaRobe.Status == TDOffice.DokumentStatus.Otkljucan)
			{
				...
				if (
					MessageBox.Show(
						"Da li sigurno zelite zakljucati ovu zamenu robe?",
						"Zakljucavanje zamene robe",
			...
			else
			{
				...
				if (
					MessageBox.Show(
						"Da li sigurno zelite otkljucati ovu zamenu robe?",
						"Otkljucavanje zamene robe",
						MessageBoxButtons.YesNo
					) != DialogResult.Yes
				)
					return;
		...
		private void blankoFizickiPovracaj_btn_Click(object sender, EventArgs e)
		{
			...
			gfx.DrawString(
				"Referent: "
					+ _users
						.Result.Where(x => x.ID == _zamenaRobe.UserID)
						.FirstOrDefault()
						.Username,
				h2,
				bb,
			...
			gfx.DrawLine(
				XPens.Black,
				new XPoint(marginLeft, top),
				new XPoint(page.Width - marginLeft, top)
			);
			gfx.DrawLine(
				XPens.Black,
				new XPoint(marginLeft, top),
				new XPoint(marginLeft, top + tableLineHeight)
			);
			gfx.DrawLine(
				XPens.Black,
				new XPoint(page.Width - marginLeft, top),
				new XPoint(page.Width - marginLeft, top + tableLineHeight)
			);
			gfx.DrawLine(
				XPens.Black,
				new XPoint(page.Width - marginLeft - kolicinaCellWidth - jmCellWidth, top),
				new XPoint(
					page.Width - marginLeft - kolicinaCellWidth - jmCellWidth,
					top + tableLineHeight
				)
			);
			gfx.DrawLine(
				XPens.Black,
				new XPoint(page.Width - marginLeft - jmCellWidth, top),
				new XPoint(page.Width - marginLeft - jmCellWidth, top + tableLineHeight)
			);
			...
			gfx.DrawString(
				...
				h1,
				bb,
				new XPoint(page.Width - kolicinaCellWidth - marginLeft * 4, top + 22)
			);
		...
		private void stampaj_btn_Click(object sender, EventArgs e)
		{
			...
			top += 20;
			gfx.DrawString(
				"Datum zamene: " + _zamenaRobe.Datum.ToString("dd.MM.yyyy"),
				h2,
				bb,
				new XPoint(marginLeft, top)
			);
			top += 20;
			gfx.DrawString(
				"Zamena izvrsena po MP racunu: " + _zamenaRobe.MPRacun.ToString(),
				h2,
				bb,
				new XPoint(marginLeft, top)
			);
			if (_zamenaRobe.NovokreiraniMPRacun != null)
			{
				top += 16;
				gfx.DrawString(
					"Uzeta roba ima vecu vrednost od vracene s toga je za razliku kreiran MP racun broj: "
						+ _zamenaRobe.NovokreiraniMPRacun.ToString(),
					h3,
					bb,
					new XPoint(marginLeft, top)
				);
			...
			foreach (var stavka in _zamenaRobe.Tag.VracaSe)
			{
				...
				gfx.DrawString(
					Komercijalno.Roba.Get(DateTime.Now.Year, stavka.Item1).Naziv,
					h3,
					bb,
					new XPoint(marginLeft * 2, top + 12)
				);
				gfx.DrawString(
					stavka.Item2.ToString(),
					h3,
					bb,
					new XPoint(page.Width - kolicinaCellWidth, top + 12)
				);
				gfx.DrawString(
					Komercijalno.Roba.Get(DateTime.Now.Year, stavka.Item1).JM,
					h3,
					bb,
					new XPoint(page.Width - jmCellWidth - 5, top + 12)
				);
				top += tableLineHeight;
			}
			gfx.DrawLine(
			...
			p.StartInfo = new ProcessStartInfo(
				System.IO.Path.GetTempPath() + "/_temp_ZamenaRobe.pdf"
			)
			{
		...
		private void obrisiToolStripMenuItem_Click(object sender, EventArgs e)
		{
			...
			int robaID = Convert.ToInt32(
				vracaSe_dgv.Rows[vracaSe_dgv.SelectedCells[0].RowIndex].Cells["RobaID"].Value
			);
		...
		private void toolStripMenuItem1_Click(object sender, EventArgs e)
		{
			...
			int robaID = Convert.ToInt32(
				uzimaSe_dgv.Rows[uzimaSe_dgv.SelectedCells[0].RowIndex].Cells["RobaID"].Value
			);
		...
		{
			int robaID = Convert.ToInt32(
				mpRacun_dgv.Rows[mpRacun_dgv.SelectedCells[0].RowIndex].Cells["RobaID"].Value
			);

			using (
				_7_fm_Komercijalno_Roba_Kartica k = new _7_fm_Komercijalno_Roba_Kartica(
					robaID,
					_zamenaRobe.MagacinID
				)
			)
				if (!k.IsDisposed)
					k.ShowDialog();
