...
namespace TDOffice_v2
{
	public partial class fm_ObracunPoreza_Index : Form
	{
		...
		private async Task CreateDGV(int magacinID, int godina)
		{
			...
			Komercijalno.Promena.PromenaCollection pocetnoStanjePromene =
				await Komercijalno.Promena.CollectionAsync(
					godina,
					osnovaKonta + magacinID.ToString("00"),
					new int[] { 0 }
				);
			Komercijalno.Promena.PromenaCollection ulazRobePromene =
				await Komercijalno.Promena.CollectionAsync(
					godina,
					osnovaKonta + magacinID.ToString("00"),
					new int[] { 1, 6, 8, 11, 12, 15, 20 }
				);
			Komercijalno.Promena.PromenaCollection izlazRobePromene =
				await Komercijalno.Promena.CollectionAsync(
					godina,
					osnovaKonta + magacinID.ToString("00"),
					new int[] { 13, 14 }
			...
			dgv.ColumnHeadersHeightSizeMode = System
				.Windows
				.Forms
			...
			double ukupanUlaz = (
				ulazRobePromene.AsQueryable().Sum(x => (double)x.Duguje)
				+ izlazRobePromene.AsQueryable().Sum(y => (double)y.Duguje)
			);
			double ukupanIzlaz =
				izlazRobePromene.AsQueryable().Sum(x => (double)x.Potrazuje)
				+ ulazRobePromene.AsQueryable().Sum(x => (double)x.Potrazuje);
		...
		{
			Dictionary<int, Tuple<double, double>> stanja =
				new Dictionary<int, Tuple<double, double>>();
			foreach (DataGridView dgv in flowLayoutPanel1.Controls.OfType<DataGridView>())
				stanja.Add(
					Convert.ToInt32(dgv.Tag),
					new Tuple<double, double>(
						Convert.ToDouble(dgv.Rows[1].Cells["Column2"].Value),
						Convert.ToDouble(dgv.Rows[1].Cells["Column11"].Value)
					)
				);
			...
			foreach (DataGridView dgv in flowLayoutPanel1.Controls.OfType<DataGridView>())
			{
				...
				double razlikaKrajnje = !stanja.ContainsKey(dgvGodina + 1)
					? 0
					: stanja[dgvGodina].Item2 - stanja[dgvGodina + 1].Item1;
				double razlikaPocetno = !stanja.ContainsKey(dgvGodina - 1)
					? 0
					: stanja[dgvGodina].Item1 - stanja[dgvGodina - 1].Item2;
		...
		private void flowLayoutPanel1_Resize(object sender, EventArgs e)
		{
			...
			foreach (DataGridView dgv in this.flowLayoutPanel1.Controls)
			{
				dgv.Size = new System.Drawing.Size(
					this.flowLayoutPanel1.Width - 9,
					dgv.Rows.GetRowsHeight(DataGridViewElementStates.Visible) + 20
				);
			}
			this.flowLayoutPanel1.ResumeLayout(false);
			this.ResumeLayout(false);
