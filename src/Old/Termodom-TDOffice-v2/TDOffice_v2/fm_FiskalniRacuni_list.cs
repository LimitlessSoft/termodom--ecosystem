...
namespace TDOffice_v2
{
	public partial class fm_FiskalniRacuni_list : Form
	{
		...
		{
			Dictionary<string, Komercijalno.DokumentFisk> dict =
				new Dictionary<string, Komercijalno.DokumentFisk>();
		...
		{
			return FiskalniRacunManager.DictionaryAsync(
				odDatuma_dtp.Value.ToString("MM-dd-yyyy"),
				doDatuma_dtp.Value.AddDays(1).ToString("MM-dd-yyyy")
			);
		...
		private Task<FiskalniRacunTaxItemDictionary> UcitajFiskalneRacuneTaxItemsAsync(
		...
		private async void openFileDialog1_FileOk(object sender, CancelEventArgs e)
		{
			...
			foreach (string f in openFileDialog1.FileNames)
			{
				...
				List<FiskalniRacunDto> fiskalniRacuniFromFile = JsonConvert.DeserializeObject<
					List<FiskalniRacunDto>
				>(text, dateTimeConverter);
				List<FiskalniRacun> fiskalniRacuni = fiskalniRacuniFromFile
					.Select(x => (FiskalniRacun)(new FiskalniRacun()).InjectFrom(x))
					.ToList();
				...
				foreach (var fiskalniRacunFromFile in fiskalniRacuniFromFile)
					if (insertedInvoiceNumbers.Any(x => x == fiskalniRacunFromFile.InvoiceNumber))
						taxItems.AddRange(
							fiskalniRacunFromFile.TaxItems.Select(x =>
							{
		...
		private async void button1_Click(object sender, EventArgs e)
		{
			...
			foreach (var fr in _fiskalniRacuni.Values)
			{
				cFiskalniRacuni++;
				status_lbl.Text =
					$"Obrada fiskalnih racuna ({cFiskalniRacuni}/{nFisklaniRacuni}) ...";

				if (fr.SignedBy != selektovanPfr.JID)
					continue;

				if (
					!tipTransakcije_clb.GetItemChecked(0)
					&& fr.TransactionType == "Продаја"
					&& fr.InvoiceType == "Промет"
				)
					continue;
				if (
					!tipTransakcije_clb.GetItemChecked(1)
					&& fr.TransactionType == "Продаја"
					&& fr.InvoiceType == "Копија"
				)
					continue;
				if (
					!tipTransakcije_clb.GetItemChecked(2)
					&& fr.TransactionType == "Продаја"
					&& fr.InvoiceType == "Обука"
				)
					continue;
				if (
					!tipTransakcije_clb.GetItemChecked(3)
					&& fr.TransactionType == "Продаја"
					&& fr.InvoiceType == "Аванс"
				)
					continue;
				if (
					!tipTransakcije_clb.GetItemChecked(4)
					&& fr.TransactionType == "Рефундација"
					&& fr.InvoiceType == "Промет"
				)
					continue;
				if (
					!tipTransakcije_clb.GetItemChecked(5)
					&& fr.TransactionType == "Рефундација"
					&& fr.InvoiceType == "Копија"
				)
					continue;
				if (
					!tipTransakcije_clb.GetItemChecked(6)
					&& fr.TransactionType == "Рефундација"
					&& fr.InvoiceType == "Обука"
				)
					continue;
				if (
					!tipTransakcije_clb.GetItemChecked(7)
					&& fr.TransactionType == "Рефундација"
					&& fr.InvoiceType == "Аванс"
				)
					continue;
				...
				var pdv = taxItems.ContainsKey(fr.InvoiceNumber)
					? taxItems[fr.InvoiceNumber].Sum(x => x.Amount)
					: 0;
				...
				switch (fr.TransactionType)
				{
					...
					default:
						MessageBox.Show(
							$"Nepoznat tip transakcije '{fr.TransactionType}'. Ovaj racun nece biti uzet u zbir racuna."
						);
						break;
				}
				Komercijalno.DokumentFisk df = _dokumentFisk.Result.ContainsKey(fr.InvoiceNumber)
					? _dokumentFisk.Result[fr.InvoiceNumber]
					: null;

				if (df == null)
					MessageBox.Show(
						$"Fiskali racun identifikacije {fr.InvoiceNumber} ne postoji u komercijalnom poslovanju!"
					);
		...
		{
			var doks = await DokumentManager.DictionaryAsync(
				magacinID,
				datum.Year,
				new int[] { 22 },
				new int[] { magacinID },
		...
		private async void button2_Click(object sender, EventArgs e)
		{
			...
			for (int i = odDatuma_dtp.Value.Year; i <= doDatuma_dtp.Value.Year; i++)
			{
				...
				dokumenti.Add(
					i,
					await DokumentManager.DictionaryAsync(
						150,
						i,
						new int[] { 15, 22 },
						null,
			...
			for (int i = 0; i <= (doDatuma_dtp.Value.Date - odDatuma_dtp.Value.Date).TotalDays; i++)
			{
				...
				foreach (int magacinId in magacini.Keys.Where(x => _magaciniZaIzvestaj.Contains(x)))
				{
					try
					{
						...
						double prometNaDan =
							prometiNaDan.FirstOrDefault(x => x.Item1 == magacinId).Item2
							+ await GetVrednostPovratnicaNaDanAsync(magacinId, datum);
						...
						if (
							Math.Abs(
								(
									prometNaDan
									- (ukupanFiskalizovanPometProdaja + ukupanFiskalizovanRefund)
								)
							) > 1
						)
						{
							var dokumentiUDanu =
								new List<Termodom.Data.Entities.Komercijalno.Dokument>();
							if (dokumenti[datum.Year].ContainsKey(15))
								dokumenti[datum.Year]
									[15]
									.Values.Where(x =>
										x.Datum.Date == datum.Date && x.MagacinID == magacinId
									)
									.ToList();
							if (dokumenti[datum.Year].ContainsKey(22))
								dokumentiUDanu.AddRange(
									dokumenti[datum.Year]
										[22]
										.Values.Where(x =>
											x.Datum.Date == datum.Date && x.MagacinID == magacinId
							...
							List<Komercijalno.DokumentFisk> dfs =
								new List<Komercijalno.DokumentFisk>(_dokumentFisk.Result.Values)
									.Where(x => x.InvoiceType == 0)
									.ToList();
							dokumentiUDanu.RemoveAll(x =>
								dfs.Any(y => y.VrDok == x.VrDok && y.BrDok == x.BrDok)
							);
							//List<Komercijalno.Stavka> stavkeUDanu = _stavke.Result.Where(x => dokumentiUDanu.Any(y => y.VrDok == x.VrDok && y.BrDok == x.BrDok)).ToList();
							List<FiskalniRacun> ffs = new List<FiskalniRacun>(
								_fiskalniRacuni.Values
							)
								.Where(x =>
									x.SignedBy == pfrMagacina
									&& x.SDCTime_ServerTimeZone.Date == datum.Date
								...
								.ToList();
							ffs.RemoveAll(x =>
								dfs.Any(y => y.ReferentDocumentNumber == x.InvoiceNumber)
							);
			...
			{
				using (
					_1344_fm_SpecifikacijaNovca_Index si = new _1344_fm_SpecifikacijaNovca_Index()
				)
				{
					si.Shown += (a1, a2) =>
					{
