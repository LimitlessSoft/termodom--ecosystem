...
namespace TDOffice_v2
{
	public partial class _1301_fm_Poruka_List : Form
	{
		...
		private static List<TDOffice.Poruka> ListByDatumIncludingCurrentUser(
			DateTime odDatuma,
			DateTime doDatuma
		)
		{
			return TDOffice.Poruka.List(
				$@"(PRIMALAC = {Program.TrenutniKorisnik.ID} OR
		...
		/// </summary>
		private Task<
			Dictionary<DateTime, Dictionary<int, TDOffice.Poruka>>
		> _porukeBuffer { get; set; }
		private List<TDOffice.User> _korisnici { get; set; }
		private Dictionary<int, TDOffice.User> _korisniciDict { get; set; } =
			new Dictionary<int, TDOffice.User>();
		...
		public _1301_fm_Poruka_List()
		{
			...
			foreach (
				TDOffice.PorukaTip pt in (TDOffice.PorukaTip[])
					Enum.GetValues(typeof(TDOffice.PorukaTip))
			)
				tipoviPoruka.Add(new Tuple<int, string>((int)pt, pt.ToString()));

			tipoviPoruka = tipoviPoruka
				.Prepend(new Tuple<int, string>(-1, " < svi tipovi > "))
				.ToList();
			...
			tdtaskovi = tdtaskovi
				.Prepend(new Tuple<int, string>(-2, " < Izbor akcije > "))
				.ToList();
		...
		private void PrikaziPoruke()
		{
			try
			{
				...
				int? primalac = primljene_rb.Checked
					? (int?)Program.TrenutniKorisnik.ID
					: korespodent == -2
						? null
						: (int?)korespodent;
				int? posiljalac = poslate_rb.Checked
					? (int?)Program.TrenutniKorisnik.ID
					: korespodent == -2
						? null
						: (int?)korespodent;
				...
				int? tipPoruke =
					Convert.ToInt32(cmb_Tip.SelectedValue) == -1
						? null
						: (int?)Convert.ToInt32(cmb_Tip.SelectedValue);
				...
				if (!_porukeBuffer.IsCompleted)
				{
					...
					{
						whereQuery +=
							$" AND (PRIMALAC = {Program.TrenutniKorisnik.ID} OR POSILJALAC = {Program.TrenutniKorisnik.ID})";
					}
					else
					{
						if (primalac != null)
				...
				else
				{
					...
					{
						poruke.RemoveAll(x =>
							x.Primalac != Program.TrenutniKorisnik.ID
							&& x.Posiljalac != Program.TrenutniKorisnik.ID
						);
					}
					else
					{
				...
				switch (akcija)
				{
					...
					case 11:
						poruke.RemoveAll(x =>
							x.Naslov
							!= "<TASK> Izvestaj Ugovora o radu koji isticu u narednih 15 dana"
						);
						break;
					case 12:
				...
				for (int i = 0; i < Program.TrenutniKorisnik.Tag.Pinovi.Count; i++)
				{
					...
					if (Program.TrenutniKorisnik.Tag.Pinovi[i].prikazana == 0)
					{
						...
						{
							int id = pp.ID;
							_1301_fm_Poruka_Index p = new _1301_fm_Poruka_Index(
								TDOffice.Poruka.Get(id)
							);
							p.SkupiPoruku();
							p.ShowDialog();
						});
		...
		private void InitializePorukeBuffer()
		{
			...
			{
				Dictionary<DateTime, Dictionary<int, TDOffice.Poruka>> dict =
					new Dictionary<DateTime, Dictionary<int, TDOffice.Poruka>>();
