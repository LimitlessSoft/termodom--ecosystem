...
namespace TDOffice_v2
{
	public partial class Menadzment : Form
	{
		...
		{
			using (
				Menadzment_SvediPocetnoStanje_Index i = new Menadzment_SvediPocetnoStanje_Index()
			)
				i.ShowDialog();
		}
		...
		{
			using (
				Mendazment_SvediPocetnoStanjeCene_Index i =
					new Mendazment_SvediPocetnoStanjeCene_Index()
			)
				i.ShowDialog();
		}
		...
		{
			AnimationBox ab = AnimationBox.Show(
				"Brisanje roba sa **** koja je neaktivna je u toku!"
			);
			...
			Task.Run(() =>
			{
				...
				using (
					FbConnection con = new FbConnection(
						Komercijalno.Komercijalno.CONNECTION_STRING[2022]
					)
				)
				{
					con.Open();
					using (
						FbCommand cmd = new FbCommand("DELETE FROM STAVKA WHERE ROBAID = @RID", con)
					)
					{
		...
		{
			using (
				Menadzment_SvediRazlikuMPRacuna_Index sr =
					new Menadzment_SvediRazlikuMPRacuna_Index()
			)
				sr.ShowDialog();
		}
		...
		{
			using (
				fm_UporedjivanjeProdajnihCena_Index u = new fm_UporedjivanjeProdajnihCena_Index()
			)
				u.ShowDialog();
		}
		...
		private void svediKolicinePocetnogStanjaNaMinimalnoMoguceToolStripMenuItem_Click(
			object sender,
			EventArgs e
		)
		{
			...
			{
				MessageBox.Show(
					"Ovu akciju ne treba pokretati jer se treba prebaciti u poseban prozor! Da bi je sada pokrenuli, "
			...
			block = true;
			string zakucaniConnString =
				"data source=4monitor; initial catalog = c:\\poslovanje\\baze\\2023\\TERMODOM2023.FDB; user=SYSDBA; password=m; pooling=True";
		...
		private void kopirajPodatkeTabeleSTAVKAIzDokumentaUDokumentToolStripMenuItem_Click(
			object sender,
			EventArgs e
		)
		{
			...
			{
				using (
					fm_KopirajPodatkeTabeleSTAVKAIzDokumentaUDokument k =
						new fm_KopirajPodatkeTabeleSTAVKAIzDokumentaUDokument()
				)
					k.ShowDialog();
			});
		}
		...
		private void razduzenjeMagacinaPoOtpremnicamaToolStripMenuItem_Click(
			object sender,
			EventArgs e
		)
		{
			...
			{
				using (
					fm_Menadzment_RazduzenjeMagacinaPoOtpremnicama i =
						new fm_Menadzment_RazduzenjeMagacinaPoOtpremnicama()
				)
					i.ShowDialog();
			});
		}
		...
		private void svediKolicinePocetnogStanjaNaMinimalnoMoguceToolStripMenuItem_Click_1(
			object sender,
			EventArgs e
		)
		{
		...
		private void kopirajPodatkeTabeleSTAVKAIzDokumentaUDokumentToolStripMenuItem_Click_1(
			object sender,
			EventArgs e
		) { }
		...
		private void NalaziNajmanjeStanjeUTokuGodineBezStanja0IInsertujeUTrazeniDokument()
		{
			...
			)
			{
				...
				{
					var dokument = dokumenti.First(x =>
						x.VrDok == stavka.VrDok && x.BrDok == stavka.BrDok
					);
					if (dokument.Datum < new DateTime(2023, 1, 31, 23, 59, 59))
						continue;

					if (
						!validniVrDok.Contains(stavka.VrDok)
						|| !new List<int>() { 0 }.Contains(dokument.KodDok)
					)
						continue;
				...
				foreach (var k in stavkaMaxKolicina.Keys)
				{
					...
					Stavka.Insert(
						con,
						destinacioniDokument,
						roba.First(x => x.ID == rum.RobaID),
						rum,
						stavkaMaxKolicina[k] * -1,
						0
					);
				}
		...
		private void Temp3()
		{
			...
			)
			{
				...
				var destinacioneStavke = Stavka.ListByDokument(
					con,
					destinacioniVrDok,
					destinacioniBrDok
				);
