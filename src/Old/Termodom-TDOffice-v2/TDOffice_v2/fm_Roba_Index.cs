...
namespace TDOffice_v2
{
	public partial class fm_Roba_Index : Form
	{
		...
		private async Task UcitajRobuAsync(int robaID)
		{
			...
			{
				DTO.TDBrain_v3.Komercijalno.RobaDTO roba =
					JsonConvert.DeserializeObject<DTO.TDBrain_v3.Komercijalno.RobaDTO>(
		...
		private async void button1_Click(object sender, EventArgs e)
		{
			...
			string trPakJM = transportnoPakovanje_cb.Checked
				? transportnoPakovanjeJM_cmb.SelectedValue.ToString()
				: null;
			double? trPakKol = transportnoPakovanje_cb.Checked
				? (double?)Convert.ToDouble(transportnoPakovanjeKolicina_txt.Text)
				: null;
			var res = await TDBrain_v3.PostAsync(
				"/komercijalno/roba/update",
