...
namespace TDOffice_v2
{
	public partial class _1370_fm_DefinisanjeProdajneCene_Index : Form
	{
		...
		private _7_fm_Komercijalno_Roba_Kartica _fmKarticaRobe { get; set; } =
			new _7_fm_Komercijalno_Roba_Kartica()
			{
		...
		{
			List<Termodom.Data.Entities.Komercijalno.VrstaDok> vrdokList = (
				await VrstaDokManager.DictionaryAsync()
			)
				.Values.Where(x => new int[] { 4, 13, 15, 32, 34 }.Contains(x.VrDok))
				.ToList();
			vrdokList.Add(
			...
			List<Termodom.Data.Entities.Komercijalno.VrstaDok> vrdokListPd = (
				await VrstaDokManager.DictionaryAsync()
			)
				.Values.Where(x => new int[] { 1, 2 }.Contains(x.VrDok))
				.ToList();
			vrdokListPd.Add(
				new Termodom.Data.Entities.Komercijalno.VrstaDok()
				{
			...
			dataGridView1.Columns["Rabat"].DefaultCellStyle.Alignment =
				DataGridViewContentAlignment.MiddleRight;
			...
			dataGridView1.Columns["Marza"].DefaultCellStyle.Alignment =
				DataGridViewContentAlignment.MiddleRight;
			...
			dataGridView1.Columns["MaxRabat"].DefaultCellStyle.Alignment =
				DataGridViewContentAlignment.MiddleRight;
			...
			dataGridView1.Columns["BuducaMarza"].DefaultCellStyle.Alignment =
				DataGridViewContentAlignment.MiddleRight;
			...
			dataGridView1.Columns["BuduciRabat"].DefaultCellStyle.Alignment =
				DataGridViewContentAlignment.MiddleRight;
			...
			dataGridView1.Columns["BuducaCena"].DefaultCellStyle.Alignment =
				DataGridViewContentAlignment.MiddleRight;
			...
			dataGridView1.Columns["NabavnaVrednost"].DefaultCellStyle.Alignment =
				DataGridViewContentAlignment.MiddleRight;
			...
			dataGridView1.Columns["RazlikaUCeni"].DefaultCellStyle.Alignment =
				DataGridViewContentAlignment.MiddleRight;
			...
			dataGridView1.Columns["ProdajnaVrednost"].DefaultCellStyle.Alignment =
				DataGridViewContentAlignment.MiddleRight;
		...
		private void btnAnaliziraj_Click(object sender, EventArgs e)
		{
			...
			if (
				dokument.MagacinID != Program.TrenutniKorisnik.MagacinID
				&& !Program.TrenutniKorisnik.ImaPravo(137001)
			)
			{
			...
			)
			{
                ...
                WHERE s.VRDOK = @VRDOK AND s.BRDOK = @BRDOK",
						con
					)
				)
				{
			...
			foreach (DataRow dr in stavkeDokumentaDT.Rows)
			{
				...
				if (
					Math.Abs(
						Procedure.ProdajnaCenaNaDan(150, robaID, dokument.Datum)
							- Convert.ToDouble(dr["PRODAJNACENA"])
					) > 0.1
				)
				{
					MessageBox.Show(
						"U dokumentu postoje cene koje se ne slazu sa realnim cenama na taj dan!"
					);
					break;
				}
			}
			...
			List<Komercijalno.Dokument> dokumentiNabavke = Dokument.List(
				$"MAGACINID = 150 AND VRDOK IN (0, 1, 2, 36)"
			);
			List<Komercijalno.Stavka> stavkeNabavke = Stavka.List(
				DateTime.Now.Year,
				$"ROBAID IN ("
					+ string.Join(
						", ",
			...
			)
			{
				...
				foreach (DataRow row in stavkeDokumentaDT.Rows)
				{
					...
					double nabavnaCena = rb_PoslednjaNavavnaCena.Checked
						? Komercijalno.Komercijalno.GetRealnaNabavnaCena(
							robaID,
							DateTime.Now,
							dokumentiNabavke,
							stavkeNabavke
						)
						: Komercijalno.Komercijalno.GetProsecnaNabavnaCena(
							con,
							robaID,
							dtp_Od.Value,
							dtp_Do.Value
			...
			rucVrednost_txt.Text = (
				ukupnaVrednostDokumentaSaPopustom - ukupnaNabavnaVrednostDokumenta
			).ToString("#,##0.00 RSD");
			prodajnaVrednost_txt.Text = ukupnaVrednostDokumentaSaPopustom.ToString("#,##0.00 RSD");
			vrednostBezPopusta_txt.Text = ukupnaVrednostDokumentaBezPopusta.ToString(
				"#,##0.00 RSD"
			);

			tb_DatiRabat.Text = (
				((ukupnaVrednostDokumentaSaPopustom / ukupnaVrednostDokumentaBezPopusta) - 1)
				* (-100)
			).ToString("0.##");
			tb_Marza.Text = (
				((ukupnaVrednostDokumentaBezPopusta / ukupnaNabavnaVrednostDokumenta) - 1) * (100)
			).ToString("0.##");
			tb_PreracunataMarza.Text = (
				((ukupnaNabavnaVrednostDokumenta / ukupnaVrednostDokumentaBezPopusta) - 1) * (-100)
			).ToString("0.##");
			realizovanaMarza_txt.Text = (
				((ukupnaVrednostDokumentaSaPopustom / ukupnaNabavnaVrednostDokumenta) - 1) * (100)
			).ToString("0.##");
...
",
		...
		{
			using (
				_1370_fm_DefinisanjeProdajneCene_UnosProcentaZeljeneCene u =
					new _1370_fm_DefinisanjeProdajneCene_UnosProcentaZeljeneCene()
			)
			{
				u.ShowDialog();
				double vrednost = u.returnValue;
				_1370_fm_DefinisanjeProdajneCene_UnosProcentaZeljeneCene.PraviloDefinisanjaCene pravilo =
					u.returnPravilo;
				...
				foreach (DataRow dr in dtTemp.Rows)
				{
					...
					double buducaCena =
						pravilo
						== _1370_fm_DefinisanjeProdajneCene_UnosProcentaZeljeneCene
							.PraviloDefinisanjaCene
							.PlusZeljenaMarza
							? nabavnaCena * (1 + (double)(vrednost / 100))
							: prodajnaCena * ((double)1 - (udeoRabata / 100));

					if (
						(((buducaCena / prodajnaCena) - 1) * -100)
						> LocalSettings.Settings.DefinisanjeProdajneCene_MaximalniRabat
					)
					{
						buducaCena =
							prodajnaCena
		...
		{
			_1370_fm_DefinisanjeProdajneCene_UnosRabataFormatiranje.FormatRabata formatRabata =
				_1370_fm_DefinisanjeProdajneCene_UnosRabataFormatiranje.FormatRabata.Standard;

			using (
				_1370_fm_DefinisanjeProdajneCene_UnosRabataFormatiranje formatiranje =
					new _1370_fm_DefinisanjeProdajneCene_UnosRabataFormatiranje()
			)
			{
			...
			{
				if (
					Convert.ToDouble(row["ProdajnaCena"]) > 0
					&& Convert.ToDouble(row["NabavnaCena"]) > 0
				)
				{
					int stavkaID;
					try
					{
						...
						double buduciRabat = Math.Round(
							Convert.ToDouble(row["BuduciRabat"]),
							formatRabata
							== _1370_fm_DefinisanjeProdajneCene_UnosRabataFormatiranje
								.FormatRabata
								.BezDecimala
								? 0
								: 2
					...
					{
						MessageBox.Show(
							"Doslo je do greske prilikom unosa rabata za stavku: " + row["Naziv"]
						);
					}
				}
			}
