...
namespace TDOffice_v2
{
	public partial class fm_Izvestaj_Prodaja_Roba_ZbirnoPoMagacinima_PosaljiKaoIzvestaj_Setup : Form
	{
		...
		private Task<List<Komercijalno.RobaUMagacinu>> _komercijalnoRobaUMagacinu50 { get; set; } =
			Komercijalno.RobaUMagacinu.ListByMagacinIDAsync(50);
		private Task<List<Komercijalno.Tarife>> _tarife { get; set; } =
			Task.Run(() =>
			{
				return Komercijalno.Tarife.List();
			});

		public fm_Izvestaj_Prodaja_Roba_ZbirnoPoMagacinima_PosaljiKaoIzvestaj_Setup(
			DataTable data,
		...
		private void fm_Izvestaj_Prodaja_Roba_ZbirnoPoMagacinima_PosaljiKaoIzvestaj_Setup_Load(
			object sender,
			EventArgs e
		) { }
				...
				TDOffice.Poruka.Insert(
					new TDOffice.Poruka()
					{
						...
						Tag = new TDOffice.PorukaAdditionalInfo()
						{
							...
							AdditionalInfo = new Tuple<DataTable, Models.Periodi>(
								izvestaj,
								_periodi
							)
						},
						Tekst = string.Join(
							Environment.NewLine,
							komentar_rtb.Text,
							"Izvestaj Prometa Robe. Pogledaj prilog gore desno!"
						)
					}
				);
			}
		...
		private void btn_ReperniMagacini_Click(object sender, EventArgs e)
		{
			...
			using (Input_CheckedListBox cb = new Input_CheckedListBox())
			{
				...
				Task.Run(() =>
				{
					...
					DataTable reperniRezultati =
						_tipIzvestaja == "RSD"
							? Models.Izvestaj.ProdajaRobePoRealnimCenama(
								_periodi,
								CheckedValues,
								_listaRobe
							)
							: Models.Izvestaj.ProdajaRobePoPoslednjimCenama(
								_periodi,
								CheckedValues,
								_listaRobe
							);

					DataTable reperniDT = reperniRezultati
						.AsEnumerable()
