...
namespace TDOffice_v2
{
	public partial class _1301_fm_Poruka_Nova : Form
	{
		...
		{
			get { return _vidljivostPorukeIzbor; }
			set
			{
		...
		{
			InitializeComponent();
			_poruka = new TDOffice.Poruka()
			{
				Tag = new TDOffice.PorukaAdditionalInfo() { Action = TDOffice.PorukaAction.NULL }
			};
			SetUI();
		}
					...
					case TDOffice.PorukaAction.NULL:
					{
						TDOffice.Poruka.Insert(
							new TDOffice.Poruka()
							{
								...
								Status =
									vidljivostPoruke_cmb.SelectedIndex == 0
										? TDOffice.PorukaTip.Standard
										: vidljivostPoruke_cmb.SelectedIndex == 1
											? TDOffice.PorukaTip.Sticky
											: TDOffice.PorukaTip.Expanding,
			...
			if (menuItem != null)
			{
				...
				if (owner != null)
				{
					...
					if (sourceControl.Name != "bccPrimalac_clb")
					{
						...
						{
							if (
								(primalac_clb.Items[i] as TDOffice.User).ID
					...
					else
					{
						...
						{
							if (
								(bccPrimalac_clb.Items[i] as TDOffice.User).ID
								!= Program.TrenutniKorisnik.ID
							)
							{
		...
		private void bccPrimalac_clb_ItemCheck(object sender, ItemCheckEventArgs e)
		{
			...
			if (e.NewValue == CheckState.Checked)
			{
				...
				!primalac_clb.Items.OfType<TDOffice.User>().Select(x => x.ID).Contains(current.ID)
			)
