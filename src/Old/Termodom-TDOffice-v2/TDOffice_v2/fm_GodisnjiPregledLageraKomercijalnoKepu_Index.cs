...
namespace TDOffice_v2
{
	public partial class fm_GodisnjiPregledLageraKomercijalnoKepu_Index : Form
	{
		...
		private void UcitajPodatke()
		{
					...
					)
					{
						...
						using (
							FbCommand cmd = new FbCommand(
								"SELECT BRDOK FROM DOKUMENT WHERE VRDOK = 0 AND MAGACINID = @MID ORDER BY BRDOK ASC",
								con
							)
						)
						{
						...
						else
						{
							using (
								FbCommand cmd = new FbCommand(
									"SELECT COALESCE(SUM(KOLICINA * PRODAJNACENA), 0) FROM STAVKA WHERE VRDOK = 0 AND BRDOK = @BRDOK",
									con
								)
							)
							{
								...
								using (FbDataReader dr = cmd.ExecuteReader())
									if (dr.Read())
										dataRowKomercijalno[godina.ToString() + "_Pocetno_Stanje"] =
											Convert.ToDouble(dr[0]);
							}
						}
						using (
							FbCommand cmd = new FbCommand(
								"SELECT COALESCE(SUM(STANJE * PRODAJNACENA), 0) FROM ROBAUMAGACINU WHERE MAGACINID = @MID",
								con
							)
						)
						{
							...
							using (FbDataReader dr = cmd.ExecuteReader())
								if (dr.Read())
									dataRowKomercijalno[godina.ToString() + "_Krajnje_Stanje"] =
										Convert.ToDouble(dr[0]);
						}
					}
				}
			...
			foreach (int godina in Komercijalno.Komercijalno.CONNECTION_STRING.Keys)
			{
				dgv.Columns[godina.ToString() + "_Pocetno_Stanje"].DefaultCellStyle.Alignment =
					DataGridViewContentAlignment.MiddleRight;
				dgv.Columns[godina.ToString() + "_Pocetno_Stanje"].DefaultCellStyle.Format =
					"#,##0.00 RSD";

				dgv.Columns[godina.ToString() + "_Krajnje_Stanje"].DefaultCellStyle.Alignment =
					DataGridViewContentAlignment.MiddleRight;
				dgv.Columns[godina.ToString() + "_Krajnje_Stanje"].DefaultCellStyle.Format =
					"#,##0.00 RSD";
			}
		...
		private void ObojiPodatke()
		{
			...
			foreach (DataGridViewRow row in dgv.Rows)
			{
				...
				)
				{
					...
					double krajnjeStanje = Convert.ToDouble(
						row.Cells[godina + "_Krajnje_Stanje"].Value
					);
					double pocetnoStanje = Convert.ToDouble(
						row.Cells[(godina + 1) + "_Pocetno_Stanje"].Value
					);
					...
					if (Math.Abs(krajnjeStanje - pocetnoStanje) > 1000)
					{
						...
						row.Cells[(godina + 1) + "_Pocetno_Stanje"].Style.Font = new Font(
