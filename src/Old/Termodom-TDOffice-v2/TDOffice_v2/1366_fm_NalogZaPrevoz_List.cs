...
namespace TDOffice_v2
{
	public partial class _1366_fm_NalogZaPrevoz_List : Form
	{
		...
		private PartialBuffer<TDOffice.NalogZaPrevoz> _partialBuffer { get; set; } =
			new PartialBuffer<TDOffice.NalogZaPrevoz>(
				TDOffice.NalogZaPrevoz.List,
				TDOffice.NalogZaPrevoz.MinDatum,
				60
			);
		...
		private async void FillDGV()
		{
			...
			List<TDOffice.NalogZaPrevoz> nalozi = _partialBuffer.Data(
				odDatuma_dtp.Value.Date.AddDays(-1)
			);

			foreach (
				TDOffice.NalogZaPrevoz np in nalozi.Where(x =>
			...
			)
			{
				...
				dr["Magacin"] = (await _komercijalnoMagacini)
					.FirstOrDefault(x => x.ID == np.MagacinID)
					.Naziv;
		...
		private void nova_btn_Click(object sender, EventArgs e)
		{
			...
			int nalogID = TDOffice.NalogZaPrevoz.Insert(
				Program.TrenutniKorisnik.MagacinID,
				Program.TrenutniKorisnik.ID,
				new TDOffice.NalogZaPrevoz.Detalji()
			);
		...
		private void dataGridView1_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
		{
			...
			using (
				_1366_fm_NalogZaPrevoz_Index np = new _1366_fm_NalogZaPrevoz_Index(
					TDOffice.NalogZaPrevoz.Get(nalogID)
				)
			)
			{
		...
		private void btn_Stampaj_Click(object sender, EventArgs e)
		{
			...
			gfx.DrawString(
				"Nalozi za prevoz magacina "
					+ magacini_cmb.Text
					+ "   "
					+ odDatuma_dtp.Value.ToString("dd.MM.yyyy")
					+ " - "
					+ doDatuma_dtp.Value.ToString("dd.MM.yyyy"),
				p1,
				XBrushes.Black,
				new Point(30, 30)
			);
			...
			foreach (DataGridViewRow r in dataGridView1.Rows)
			{
				...
				foreach (TDOffice.NalogZaPrevoz.Destinacija dest in _nalog.Tag.Destinacije)
				{
					...
					if ((int)dest.NacinPlacanja == 1)
					{
						List<Komercijalno.Stavka> stavke = Komercijalno
							...
							.Where(x => x.MagacinID == _nalog.MagacinID)
							.ToList();
						if (
							stavke.Count(x =>
								_1366_fm_NalogZaPrevoz_Odrediste.uslugaIDPrevoza.Contains(x.RobaID)
							) != 0
						)
						{
							miKupNapVirmanom += stavke
								.Where(x =>
									_1366_fm_NalogZaPrevoz_Odrediste.uslugaIDPrevoza.Contains(
										x.RobaID
									)
								)
				...
				gfx.DrawString(
					_nalog.NamaNaplacenPrevozBezPDV.ToString("#,##0.00 RSD"),
					p2,
					XBrushes.Black,
					new Point(150, y)
				);
				gfx.DrawString(
					miKupNapVirmanom.ToString("#,##0.00 RSD"),
					p2,
					XBrushes.Black,
					new Point(280, y)
				);
				gfx.DrawString(
					miKupNapKesom.ToString("#,##0.00 RSD"),
					p2,
					XBrushes.Black,
					new Point(420, y)
				);
				gfx.DrawString(
					miKupNap.ToString("#,##0.00 RSD"),
					p2,
					XBrushes.Black,
					new Point(530, y)
				);
				gfx.DrawString(
					miPlacamo.ToString("#,##0.00 RSD"),
					p2,
					XBrushes.Black,
					new Point(610, y)
				);
				...
				if (y + 30 > page.Height)
				{
					...
					gfx.DrawString("Datum", p2, XBrushes.Black, new Point(80, y));
					gfx.DrawString(
						"Prevoznik nama naplatio ",
						p2,
						XBrushes.Black,
						new Point(150, y)
					);
					gfx.DrawString(
						"Kupcu naplatili virmanom ",
						p2,
						XBrushes.Black,
						new Point(280, y)
					);
			...
			gfx.DrawString(
				sumPrevoznikNamaNaplatio.ToString("#,##0.00 RSD"),
				p2,
				XBrushes.Black,
				new Point(150, y + 30)
			);
			gfx.DrawString(
				sumKupNapKesom.ToString("#,##0.00 RSD"),
				p2,
				XBrushes.Black,
				new Point(420, y + 30)
			);
			gfx.DrawString(
				sumMiKupnaplatili.ToString("#,##0.00 RSD"),
				p2,
				XBrushes.Black,
				new Point(530, y + 30)
			);
			gfx.DrawString(
				sumMiPlacamo.ToString("#,##0.00 RSD"),
				p2,
				XBrushes.Black,
				new Point(610, y + 30)
			);
