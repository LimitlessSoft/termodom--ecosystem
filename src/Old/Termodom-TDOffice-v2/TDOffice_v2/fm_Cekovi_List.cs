...
namespace TDOffice_v2
{
	...
	{
		private Task<List<Komercijalno.Magacin>> magacini = Komercijalno.Magacin.ListAsync(
			DateTime.Now.Year
		);
		...
		public fm_Cekovi_List()
		{
			...
			magacini.Result.Add(
				new Komercijalno.Magacin() { Naziv = " < svi magacini >", ID = -1 }
			);
			...
			dodajUPripremuZaStampuToolStripMenuItem.Enabled = Program.TrenutniKorisnik.ImaPravo(
				136103
			);
		...
		private void UcitajCekove()
		{
			...
			List<TDOffice.Cek> cekovi =
				magacinID > 0 ? TDOffice.Cek.ListByMagacinID(magacinID) : TDOffice.Cek.List();
			...
			foreach (
				TDOffice.Cek c in cekovi.Where(x =>
					x.DatumValute.Date >= odDatuma_dtp.Value.Date
					&& x.DatumValute.Date <= doDatuma_dtp.Value.Date
				)
			)
			{
				if (statusFilter_cmb.SelectedIndex > 0)
					if (
						statusFilter_cmb.SelectedIndex == 1
							&& c.Status == TDOffice.Enums.CekStatus.Nerealizovan
						|| statusFilter_cmb.SelectedIndex == 2
							&& c.Status == TDOffice.Enums.CekStatus.Realizovan
					)
						continue;
				...
				dr["MagacinText"] = magacini
					.Result.Where(x => x.ID == c.MagacinID)
					.FirstOrDefault()
					.Naziv;
		...
		{
			for (
				int i = rowID == null ? 0 : (int)rowID;
				i < (rowID == null ? dataGridView1.Rows.Count : ((int)rowID + 1));
				i++
			)
			{
				...
				if (
					Convert.ToInt32(dataGridView1.Rows[i].Cells["Status"].Value)
					== (int)TDOffice.Enums.CekStatus.Realizovan
				)
		...
		private void zaduziToolStripMenuItem_Click(object sender, EventArgs e)
		{
			...
			dataGridView1.Rows[dataGridView1.SelectedCells[0].RowIndex].Cells["Zaduzen"].Value =
				true;
			dataGridView1.Rows[dataGridView1.SelectedCells[0].RowIndex].Cells["ZaduzenText"].Value =
				"Zaduzen";
		...
		{
			realizujToolStripMenuItem.Enabled =
				Convert.ToInt32(
					dataGridView1
			...
			if (
				...
				zaduziToolStripMenuItem.Enabled =
					Convert.ToInt32(
						dataGridView1
							.Rows[dataGridView1.SelectedCells[0].RowIndex]
							.Cells["Status"]
		...
		private void pretraga_txt_TextChanged(object sender, EventArgs e)
		{
			...
			DataRow[] result = _listaCekovaDT
				.Copy()
				.Select($"BrojCeka LIKE '%{input}%' OR TRGradjana LIKE '%{input}%'");
