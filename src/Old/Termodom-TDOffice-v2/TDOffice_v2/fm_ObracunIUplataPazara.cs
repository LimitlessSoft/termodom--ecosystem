...
namespace TDOffice_v2
{
	...
	{
		private Task<MagacinDictionary> _komercijalnoMagacini { get; set; } =
		...
		{
			_komercijalnoMagacini.ContinueWith(
				async (prev) =>
				{
					List<Tuple<int, string>> list = new List<Tuple<int, string>>();

					foreach (
						Termodom.Data.Entities.Komercijalno.Magacin m in (
							await _komercijalnoMagacini
						).Values
					)
			...
			Task.Run(async () =>
			{
				try
				{
					...
					foreach (Tuple<int, string> tup in clb_Magacini.CheckedItems)
					{
						...
						{
							IzvodDictionary izvodi = await IzvodManager.DictionaryAsync(
								magacin.ID,
								godina
							);
							DokumentDictionary dokumenti = await DokumentManager.DictionaryAsync(
								magacin.ID,
								godina,
								new int[] { 15, 22 },
								new int[] { magacin.ID },
							...
							while (datumObrade <= doDatuma)
							{
									...
									(MethodInvoker)
										delegate
										{
											toolStripStatusLabel1.Text =
												$"Ucitavanje {datumObrade.ToString("dd.MM.yyyy")}";
										}
								);
								...
								var izvodiNaDan_N = izvodi.Values.Where(x =>
									...
									&& !string.IsNullOrWhiteSpace(x.PozNaBroj)
									&& x.PozNaBroj.Length == 7
									&& Convert.ToInt32(x.PozNaBroj.Substring(0, 2))
										== datumObrade.Month
									&& Convert.ToInt32(x.PozNaBroj.Substring(2, 2))
										== datumObrade.Day
								...
								mpRacuni_N = !dokumenti.ContainsKey(15)
									? 0
									: dokumenti[15]
										.Values.Where(x =>
											x.Datum.Date == datumObrade.Date
								...
								povratnice_N = !dokumenti.ContainsKey(22)
									? 0
									: dokumenti[22]
										.Values.Where(x =>
											x.Datum.Date == datumObrade.Date
								...
								)
								{
									...
									drn["Za Uplatu (Mp. Racuni - Povratnice)"] =
										mpRacuni_N - povratnice_N;
					...
					this.Invoke(
						(MethodInvoker)
							delegate
							{
								...
								dataGridView1.Columns["Potrazuje"].DefaultCellStyle.Format =
									"#,##0.00";
								dataGridView1.Columns["Potrazuje"].DefaultCellStyle.Alignment =
									DataGridViewContentAlignment.MiddleRight;
								dataGridView1.Columns["Mp Racuni"].DefaultCellStyle.Format =
									"#,##0.00";
								dataGridView1.Columns["Mp Racuni"].DefaultCellStyle.Alignment =
									DataGridViewContentAlignment.MiddleRight;
								dataGridView1.Columns["Povratnice"].DefaultCellStyle.Format =
									"#,##0.00";
								dataGridView1.Columns["Povratnice"].DefaultCellStyle.Alignment =
									DataGridViewContentAlignment.MiddleRight;
								dataGridView1
									.Columns["Za Uplatu (Mp. Racuni - Povratnice)"]
									.DefaultCellStyle
									.Format = "#,##0.00";
								dataGridView1
									.Columns["Za Uplatu (Mp. Racuni - Povratnice)"]
								...
								dataGridView1.Columns["Razlika"].DefaultCellStyle.Format =
									"#,##0.00";
		...
		private void ObojiDGV()
		{
			...
			{
				if (
					dataGridView1.Rows[i].Cells["Konto"].Value.ToString() == "Bez Uplata"
					...
					Math.Abs(Convert.ToDouble(dataGridView1.Rows[i].Cells["Razlika"].Value))
					< tolerancija
				)
		...
		private async void analitikaToolStripMenuItem_Click(object sender, EventArgs e)
		{
			...
			string pozNaBroj = dataGridView1
