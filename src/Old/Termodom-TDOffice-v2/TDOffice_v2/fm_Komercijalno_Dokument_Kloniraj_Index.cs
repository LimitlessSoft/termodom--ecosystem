		...
		private void btn_Kloniraj_Click(object sender, EventArgs e)
		{
			...
			using (
				FbConnection con = new FbConnection(
					Komercijalno.Komercijalno.CONNECTION_STRING[godinaSource]
				)
			)
			{
				con.Open();
				...
				{
					MessageBox.Show($"Dokument vrdok: {vrDok}, brdok: {brDok} nije pronadjen");
					return;
				}
			...
			using (
				FbConnection con = new FbConnection(
					Komercijalno.Komercijalno.CONNECTION_STRING[godinaDest]
				)
			)
			{
				con.Open();
				int newBrDok = Komercijalno.Dokument.Insert(
					con,
					izvorniDokument.VrDok,
					izvorniDokument.IntBroj,
					izvorniDokument.PPID,
					"Klon",
					Convert.ToInt32(izvorniDokument.NUID),
					izvorniDokument.MagacinID,
					izvorniDokument.RefID,
					izvorniDokument.MagID
				);
				...
				foreach (Stavka s in izvorneStavke)
				{
					...
					{
						MessageBox.Show(
							"U ovoj godini ne postoji roba u sifarniku sa IDem: " + s.RobaID
						);
						continue;
					}
					if (rum == null)
					{
						MessageBox.Show(
							"U ovoj godini ne postoji roba u magacinu sa IDem: " + s.RobaID
						);
						continue;
					}
