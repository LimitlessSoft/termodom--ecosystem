...
namespace TDOffice_v2
{
	public partial class _7_fm_Komercijalno_Roba_Kartica : Form
	{
		...
		private Task<List<Komercijalno.Partner>> _komercijalnoPartneri =
		...
		private Task<List<Komercijalno.Magacin>> _magacini = Task.Run(async () =>
		{
			...
			list.Add(new Komercijalno.Magacin() { ID = -1, Naziv = "Svi magacini" });
		...
		private Task<Termodom.Data.Entities.Komercijalno.VrstaDokDictionary> _komercijalnoVrstaDokumenta { get; set; } =
			VrstaDokManager.DictionaryAsync();
		...
		{
			racunajVazecuNabavnuCenuToolStripMenuItem.Text = LocalSettings
				.Settings
				.KarticaRobe_RacunajVazecuNabavnuCenu
				? "Nemoj Racunati Vazecu Nabavnu Cenu"
				: "Racunaj Vazecu Nabavnu Cenu";
			racunajVazecuNabavnuCenuToolStripMenuItem.BackColor = LocalSettings
				.Settings
				.KarticaRobe_RacunajVazecuNabavnuCenu
				? Color.LightGreen
				: Color.Coral;

			boldujDokument36ToolStripMenuItem.Text = LocalSettings
				.Settings
				.KarticaRobe_BoldujDokument36
				? "Nemoj Dodatno Bojiti Dokument Ulazne Ponude 36"
				: "Dodatno Oboji Dokument Ulazne Ponude 36";
			boldujDokument36ToolStripMenuItem.BackColor = LocalSettings
				.Settings
				.KarticaRobe_BoldujDokument36
				? Color.LightGreen
				: Color.Coral;
		...
		public void UcitajKarticu(int robaID, int magacinID)
		{
			...
			_komercijalnoStavke = Task.Run(() =>
			{
				...
				dict[DateTime.Now.Year] = Stavka.List(
					DateTime.Now.Year,
					$"ROBAID = {robaID} AND MAGACINID = {magacinID} AND VRDOK IN (0, 1, 2, 7, 13, 14, 15, 16, 17, 18, 19, 21, 22, 23, 25, 26, 27, 28, 29, 30, 35, 36)"
				);
				using (
					FbConnection con = new FbConnection(
						Komercijalno.Komercijalno.CONNECTION_STRING[DateTime.Now.Year - 1]
					)
				)
				{
					con.Open();
					dict[DateTime.Now.Year - 1] = Stavka.List(
						con,
						$"ROBAID = {robaID} AND MAGACINID = {magacinID} AND VRDOK IN (0, 1, 2, 7, 13, 14, 15, 16, 17, 18, 19, 21, 22, 23, 25, 26, 27, 28, 29, 30, 35, 36)"
					);
				}
			...
			{
				List<Dokument> all = Dokument.List(
					$"MAGACINID = {magacinID} AND VRDOK IN (0, 1, 2, 7, 13, 14, 15, 16, 17, 18, 19, 21, 22, 23, 25, 26, 27, 28, 29, 30, 35, 36)"
				);
				...
				foreach (Stavka s in _komercijalnoStavke.Result[godina])
					doks.Add(
						all.FirstOrDefault(x =>
							x.VrDok == s.VrDok && x.BrDok == s.BrDok && x.Datum.Year == godina
						)
					);
			...
			foreach (int godina in _komercijalnoStavke.Result.Keys)
			{
				...
				)
				{
					...
					foreach (Stavka s in _komercijalnoStavke.Result[godina])
					{
						Komercijalno.Dokument dok =
							_komercijalnoDokumentiMagacina.Result.FirstOrDefault(x =>
								x != null
								&& x.VrDok == s.VrDok
								&& x.BrDok == s.BrDok
								&& x.Datum != null
						...
						Partner PPNaziv = _komercijalnoPartneri.Result.FirstOrDefault(x =>
							x.PPID == partnerID
						);
						if (PPNaziv == null)
						...
						trenutnoStanje +=
							s.Kolicina
							* (
								_komercijalnoDokumentiMagacina
						...
						DateTime dtemp = _komercijalnoDokumentiMagacina
							.Result.FirstOrDefault(x =>
								x.VrDok == s.VrDok
								&& x.BrDok == s.BrDok
								&& x.Datum != null
						...
						row["VrDok"] = s.VrDok;
						row["Dokument"] =
							_komercijalnoVrstaDokumenta.Result[s.VrDok].NazivDok
							+ (
								s.VrDok == 36
									? " ( " + ((DateTime)dok.DatRoka).ToString("dd.MM.yyyy") + " ) "
									: ""
							);
						row["BrDok"] = s.BrDok;
						...
						row["ProdajnaCena"] =
							s.ProdCenaBP == 0
								? s.ProdajnaCena
								: (s.ProdCenaBP + (s.Korekcija == null ? 0 : s.Korekcija));
			...
			if (LocalSettings.Settings.KarticaRobe_RacunajVazecuNabavnuCenu)
			{
				...
				dataGridView1.Columns["PoslednjaNabavnaCena"].DefaultCellStyle.Alignment =
					DataGridViewContentAlignment.MiddleRight;
			...
			bool oduzimajPDVSaPcMag =
				_magacini.Result.FirstOrDefault(x => x.ID == magacinID).Vrsta == 2;
			...
			drMag["CenaBezPDV"] = oduzimajPDVSaPcMag
				? ((double)(pcMag * (1 - (double)((double)t.Stopa / (double)(100 + t.Stopa)))))
				: pcMag;
		...
		public void UcitajKarticuMagacinID(int robaID, int magacinID)
		{
			...
			drMag["CenaSaPDV"] = oduzimajPDVSaPcMag
				? ((double)(pcMag * (1 - (double)((double)t.Stopa / (double)(100 + t.Stopa)))))
				: pcMag;
		...
		{
			Dokument dokument36 = _komercijalnoDokumentiMagacina.Result.FirstOrDefault(x =>
				x.VrDok == 36 && datum >= x.Datum && datum <= x.DatRoka
			);

			if (dokument36 != null)
				return _komercijalnoStavke
					.Result[dokument36.Datum.Year]
					.FirstOrDefault(x => x.VrDok == dokument36.VrDok && x.BrDok == dokument36.BrDok)
					.NabavnaCena;

			List<Dokument> dokumentiKojiDolazeUObzir = new List<Dokument>(
				_komercijalnoDokumentiMagacina.Result
			);
			dokumentiKojiDolazeUObzir.RemoveAll(x =>
				x.Datum > datum || !new int[] { 0, 1, 2, 3 }.Contains(x.VrDok)
			);
			...
			return _komercijalnoStavke
				.Result[vazeciDokumentNabavke.Datum.Year]
				.FirstOrDefault(x =>
					x.VrDok == vazeciDokumentNabavke.VrDok && x.BrDok == vazeciDokumentNabavke.BrDok
				)
				.NabavnaCena;
		}
		...
		{
			LocalSettings.Settings.KarticaRobe_RacunajVazecuNabavnuCenu = !LocalSettings
				.Settings
				.KarticaRobe_RacunajVazecuNabavnuCenu;
		...
		{
			LocalSettings.Settings.KarticaRobe_BoldujDokument36 = !LocalSettings
				.Settings
				.KarticaRobe_BoldujDokument36;
			LocalSettings.Update();
			PodesiUI();
		...
		{
			string kom = dataGridView1
				.SelectedRows[0]
				.Cells["InterniKomentar"]
				.Value.ToStringOrDefault();
