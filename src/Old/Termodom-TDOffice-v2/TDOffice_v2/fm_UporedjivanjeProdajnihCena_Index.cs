...
namespace TDOffice_v2
{
    public partial class fm_UporedjivanjeProdajnihCena_Index : Form
    {
        ...
        private void StartUcitavanjeBazaAsync()
        {
            ...
            _ucitavanjeBaza.ContinueWith((prev) =>
            {
        ...
        private async Task UcitajBazeAsync()
        {
            ...
            if(result.StatusCode == System.Net.HttpStatusCode.OK)
            {
                ...
                _connectionInfos = JsonConvert.DeserializeObject<List<Termodom.Data.Entities.DBSettings.ConnectionInfo>>(response);
                List<string> paths = _connectionInfos.Select(x => x.PutanjaDoBaze).Distinct().ToList();
        ...
        private async void uporedi_btn_ClickAsync(object sender, EventArgs e)
        {
            ...
            Termodom.Data.Entities.DBSettings.ConnectionInfo connInfo = _connectionInfos.First(x => x.PutanjaDoBaze == bazaPath);
            ...
            {
                await UporediIzvorneIDestinacioneDokumenteZaPeriod(vrDok, this.odDatuma_dtp.Value, this.doDatuma_dtp.Value, connInfo);
            }
        ...
        private async Task UporediIzvorniIDestinacioniDokumentAsync(int vrDok, int brDok, Termodom.Data.Entities.DBSettings.ConnectionInfo connInfo)
        {
            ...
            Termodom.Data.Entities.Komercijalno.Dokument izvorniDokument = await Komercijalno.Dokument.GetAsync(connInfo.MagacinID, vrDok, brDok, connInfo.Godina);
            ...
            Termodom.Data.Entities.Komercijalno.Dokument destinacioniDokument = await Komercijalno.Dokument.GetAsync(connInfo.MagacinID, (int)izvorniDokument.VrDokOut, (int)izvorniDokument.BrDokOut, connInfo.Godina);

            if (destinacioniDokument == null)
            {
                MessageBox.Show("Destinacioni dokument je zabelezen u izvornom ali nije pronadjen!");
                return;
            }

            Termodom.Data.Entities.Komercijalno.StavkaDictionary izvorneStavke = await Komercijalno.Stavka.DictionaryAsync(connInfo.MagacinID, izvorniDokument.VrDok, izvorniDokument.BrDok, connInfo.Godina);
            Termodom.Data.Entities.Komercijalno.StavkaDictionary destinacioneStavke = await Komercijalno.Stavka.DictionaryAsync(connInfo.MagacinID, destinacioniDokument.VrDok, destinacioniDokument.BrDok, connInfo.Godina);
            ...
            {
                Termodom.Data.Entities.Komercijalno.Stavka destinacionaStavka = destinacioneStavke.Values.FirstOrDefault(x => x.RobaID == izvornaStavka.RobaID);
            ...
            foreach (Termodom.Data.Entities.Komercijalno.Stavka destinacionaStavka in destinacioneStavke.Values)
            {
                Termodom.Data.Entities.Komercijalno.Stavka izvornaStavka = izvorneStavke.Values.FirstOrDefault(x => x.RobaID == destinacionaStavka.RobaID);
            ...
            else
            {
                ...
                {
                    Termodom.Data.Entities.Komercijalno.Stavka izvornaStavka = izvorneStavke.Values.First(x => x.RobaID == robaid);
                    Termodom.Data.Entities.Komercijalno.Stavka destinacionaStavka = destinacioneStavke.Values.First(x => x.RobaID == robaid);
                    ...
                    if (this.uporediKolicine_cb.Checked)
                        if (izvornaStavka.Kolicina != destinacionaStavka.Kolicina)
                        {
                            ...
                            dr["RobaID"] = robaid;
                            dr["Naziv"] = izvornaStavka.Naziv;
                            dr["Opis"] = $"Izvorna kolicina: {izvornaStavka.Kolicina.ToString("#,##0.00")} - Destinaciona kolicina: {destinacionaStavka.Kolicina.ToString("#,##0.00")}";
                            dt.Rows.Add(dr);
                        }

                    if (this.uporediNabavneCene_cb.Checked)
                        razlika = Math.Abs(izvornaStavka.NabavnaCena - destinacionaStavka.NabavnaCena);
                    if (razlika > 0.01)
                    {
                        DataRow dr = dt.NewRow();
                        dr["RobaID"] = robaid;
                        dr["Naziv"] = izvornaStavka.Naziv;
                        dr["Opis"] = $"Izvorna nabavna cena: {izvornaStavka.NabavnaCena.ToString("#,##0.00")} - Destinaciona nabavna cena: {destinacionaStavka.NabavnaCena.ToString("#,##0.00")}";
                        dt.Rows.Add(dr);
                    }

                    if (this.uporediProdajneCene_cb.Checked)
                    {
                        double izvornaProdajna = izvornaStavka.ProdajnaCena * (_VPDokumenti.Contains(izvornaStavka.VrDok) ? (1d + ((double)tarifa.Stopa / 100d)) : 1);
                        double destinacionaProdajna = destinacionaStavka.ProdajnaCena * (_VPDokumenti.Contains(destinacionaStavka.VrDok) ? (1d + ((double)tarifa.Stopa / 100d)) : 1);
                        ...
                        if (razlika > 0.01)
                        {
                            ...
                            dr["RobaID"] = robaid;
                            dr["Naziv"] = izvornaStavka.Naziv;
                            dr["Opis"] = $"Izvorna prodajna cena: {izvornaProdajna.ToString("#,##0.0000")} - Destinaciona prodajna cena: {destinacionaProdajna.ToString("#,##0.0000")}";
                            dt.Rows.Add(dr);
                        }
                    }
                }
        ...
        private async Task UporediIzvorneIDestinacioneDokumenteZaPeriod(int vrDok, DateTime odDatuma, DateTime doDatuma, Termodom.Data.Entities.DBSettings.ConnectionInfo connInfo)
        {
            ...
            foreach (int v in dokumenti.Keys)
            {
                ...
                {
                    Termodom.Data.Entities.Komercijalno.StavkaDictionary izvorneStavke = await Komercijalno.Stavka.DictionaryAsync(connInfo.MagacinID, v, b, connInfo.Godina);
                    Termodom.Data.Entities.Komercijalno.StavkaDictionary destinacioneStavke = await Komercijalno.Stavka.DictionaryAsync(connInfo.MagacinID, v, b, connInfo.Godina);
        ...
        private void proveriDaliImaDestinacioniDokument_cb_CheckedChanged(object sender, EventArgs e)
        {
