...
namespace TDOffice_v2
{
	public partial class Menadzment_Fakturisi0634_List : Form
	{
		...
		private void nova_btn_Click(object sender, EventArgs e)
		{
			...
			int novaFaktura = TDOffice.Dokument0634.Insert(
			...
			using (
				Menadzment_Fakturisi0634_Index f = new Menadzment_Fakturisi0634_Index(novaFaktura)
			)
				f.ShowDialog();
		}
		...
		private void Ucitaj_Click(object sender, EventArgs e)
		{
			...
			foreach (
				TDOffice.Dokument0634 dok in TDOffice.Dokument0634.List("MAGACINID = " + magacinID)
			)
			{
		...
		{
			int brDok = Convert.ToInt32(
				dataGridView1.Rows[dataGridView1.SelectedCells[0].RowIndex].Cells["BrDok"].Value
			);
