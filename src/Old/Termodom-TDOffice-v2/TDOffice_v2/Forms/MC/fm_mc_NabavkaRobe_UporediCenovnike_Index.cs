...
namespace TDOffice_v2.Forms.MC
{
	public partial class fm_mc_NabavkaRobe_UporediCenovnike_Index : Form
	{
		...
		private Task<List<Komercijalno.Partner>> partneriTask = Komercijalno.Partner.ListAsync(
		...
		private readonly TDOffice.Config<NabavkaRobeDobavljaciSettings> _dobavljaciSettings =
			TDOffice.Config<NabavkaRobeDobavljaciSettings>.Get(
		...
		private async Task LoadDataAsync()
		{
			try
			{
				...
				uporediCenovnikeResponse.Payload.ForEach(
					(item) =>
					{
						ppidsInCenovnici.AddRange(
							item.SubItems.Select(x => x.DobavljacPPID).Distinct()
						);
					}
				);
				...
				foreach (var ppid in ppidsInCenovnici)
				{
					...
					dt.Columns.Add(
						$"Dobavljac ({partner?.Naziv ?? ppid.ToString()}) Kat Br",
						typeof(string)
					);
					dt.Columns.Add(
						$"VP Cena sa popustom: {partner?.Naziv ?? ppid.ToString()}",
						typeof(double)
					);
				}
				...
				foreach (var item in uporediCenovnikeResponse.Payload)
				{
					...
					foreach (var ppid in ppidsInCenovnici)
					{
						...
						dr[$"Dobavljac ({partner?.Naziv ?? ppid.ToString()}) Kat Br"] =
							c == null ? "None" : c.DobavljacKatBr;

						var vpCena = c?.VPCenaSaPopustom ?? 0;
						var razmeraItem = _dobavljaciSettings
							.Tag.Dobavljaci.FirstOrDefault(x => x.PPID == ppid)
							.JMs.FirstOrDefault(x => x.RobaId == item.RobaId);
						...
						dr[$"VP Cena sa popustom: {partner?.Naziv ?? ppid.ToString()}"] =
							c == null ? 0 : vpCena;
					}
					dt.Rows.Add(dr);
				}
		...
		private void UpdateDGV()
		{
			...
			foreach (var ppid in ppidsInCenovnici)
			{
				var partner = partneriTask
					.GetAwaiter()
					.GetResult()
					.FirstOrDefault(x => x.PPID == ppid);
				dataGridView1
					.Columns[$"VP Cena sa popustom: {partner?.Naziv ?? ppid.ToString()}"]
					.DefaultCellStyle
					.Format = "#,##0.00 RSD";
			}
