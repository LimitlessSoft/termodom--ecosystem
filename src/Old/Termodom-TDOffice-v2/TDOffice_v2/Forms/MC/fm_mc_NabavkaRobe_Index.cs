...
namespace TDOffice_v2.Forms.MC
{
	public partial class fm_mc_NabavkaRobe_Index : Form
	{
		...
		public fm_mc_NabavkaRobe_Index()
		{
			...
			_dobavljaciSettings = TDOffice.Config<NabavkaRobeDobavljaciSettings>.Get(
		...
		private class MCNabavkaRobeUporediCenovnikeItemDto
		{
			...
			public string KatBr { get; set; }
			public string Naziv { get; set; }
			public List<MCNabavkaRobeUporediCenovnikeSubItemDto> SubItems { get; set; } =
				new List<MCNabavkaRobeUporediCenovnikeSubItemDto>();
		}
		...
		private async void ReloadAsync()
		{
			...
			var uporediCenovnikeResponse = await TDAPI.GetAsync<
				List<MCNabavkaRobeUporediCenovnikeItemDto>
			>("/mc-nabavka-robe-uporedi-cenovnike");
			...
			{
				var dob = _dobavljaciSettings.Tag.Dobavljaci.First(x =>
					x.PPID == dobavljacSelect.Item1
				);
				...
				foreach (var d in comboBox1.Items)
				{
					...
					var dCena = uporediCenovnikeResponse
						.Payload.FirstOrDefault(x => x.RobaId == item.KomercijalnoRobaId.Value)
						?.SubItems.FirstOrDefault(x =>
							x.DobavljacPPID == (d as Tuple<int, string>).Item1
						);

					dr["Dobavljac: " + (d as Tuple<int, string>).Item2] =
						dCena?.VPCenaSaPopustom ?? 0;
				}
		...
		private void poveziSaRobomToolStripMenuItem_Click(object sender, EventArgs e)
		{
			...
			var dobavljacSelect = comboBox1.SelectedItem as Tuple<int, string>;
			var dobavljac = _dobavljaciSettings.Tag.Dobavljaci.First(x =>
				x.PPID == dobavljacSelect.Item1
			);
			_izborRobe.OnRobaClickHandler = (Komercijalno.RobaUMagacinu[] robaUMagacinu) =>
			{
				...
				Task.Run(() =>
				{
					var response = TDAPI
						.PutAsync<
							MCPartnerCenovnikKatBrRobaIdSaveRequest,
							MCPartnerCenovnikKatBrRobaIdEntity
						>(
							"mc-nabavka-robe-sacuvaj-partner-cenovnik-item",
							new MCPartnerCenovnikKatBrRobaIdSaveRequest()
							{
		...
		private void button1_Click(object sender, EventArgs e)
		{
			...
			Task.Run(async () =>
			{
				try
				{
					...
					var postojeCeneNaDanResponse = await TDAPI.GetAsync<
						MCNabavkaRobeProveriPostojanjeCenovnikaNaDanRequest,
						bool
					>(
						"/mc-nabavka-robe-proveri-postojanje-cenovnika-na-dan",
						new MCNabavkaRobeProveriPostojanjeCenovnikaNaDanRequest()
						{
							PPID = dobavljac.PPID,
					...
					if (
						postojeCeneNaDanResponse.Payload
						&& MessageBox.Show(
							"Za ovog dobavljaca vec postoji cenovnik na ovaj dan. Nastavljanjem akcije ce sve cene na ovaj dan biti uklonjene. Da li zelite nastaviti?",
							"Nastaviti?",
		...
		private ListResponse<CenovnikItem> UvuciFajl(bool sacuvajUBazu = false)
		{
			...
			formData.Add(
				new StringContent(dobavljac.KolonaKatBr.ToString()),
				"KolonaKataloskiBroj"
			);
			...
			formData.Add(
				new StringContent(dobavljac.KolonaCena.ToString()),
				"KolonaVPCenaBezRabata"
			);
			...
			if (sacuvajUBazu)
				formData.Add(
					new StringContent(
						DateTime
							.SpecifyKind(doDatuma_dtp.Value, DateTimeKind.Utc)
			...
			var response = httpClient
				.PostAsync(
					Path.Join(
						TDAPI.HttpClient.BaseAddress.ToString(),
						"mc-nabavka-robe-uvuci-fajl"
					),
					formData
				)
				.Result;
		...
		private void UpdateDGV()
		{
			...
			dataGridView1.Columns["LocalKolicina"].DefaultCellStyle.Alignment =
			...
			dataGridView1.Columns["DobavljacKolicina"].DefaultCellStyle.Alignment =
				DataGridViewContentAlignment.MiddleCenter;
			dataGridView1.Columns["DobavljacKolicina"].DefaultCellStyle.BackColor =
				Color.LightYellow;

			foreach (var d in comboBox1.Items)
			{
				dataGridView1.Columns["Dobavljac: " + (d as Tuple<int, string>).Item2].Width = 80;
				dataGridView1.Columns["Dobavljac: " + (d as Tuple<int, string>).Item2].ReadOnly =
					true;
				dataGridView1
					.Columns["Dobavljac: " + (d as Tuple<int, string>).Item2]
					.DefaultCellStyle
				...
				dataGridView1
					...
					.DefaultCellStyle
					.Format = "#,##0.00";
				dataGridView1.Columns["Dobavljac: " + (d as Tuple<int, string>).Item2].Visible =
					!_prikaziKoloneZaStelovanjeJM;
			}
		...
		)
		{
			...
			var currValue = Convert.ToDouble(
			...
			var robaIdKomercijalno = Convert.ToInt32(
				dataGridView1.Rows[e.RowIndex].Cells["RobaIdKomercijalno"].Value
			);
