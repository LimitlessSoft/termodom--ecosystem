...
namespace TDOffice_v2
{
	public partial class fm_Partner_Analiza_Neaktivni_Index : Form
	{
		...
		private Task<DataTable> _data { get; set; } =
			Task.Run(() =>
			{
...
ORDER BY DIFF DESC",
		...
		{
			_komercijalnoMagacini.Result.Add(
				new Komercijalno.Magacin() { ID = -1, Naziv = " <<< Svi Magacini >>> " }
			);
...
ORDER BY NEAKTIVNOST DESC",
				...
				)
				{
					...
					using (FbDataAdapter da = new FbDataAdapter(cmd))
					{
						...
						dataGridView1.Columns["NEAKTIVNOST"].AutoSizeMode =
							DataGridViewAutoSizeColumnMode.AllCells;
					}
				}
				...
				Task.Run(() =>
				{
					...
					if (!this.IsDisposed)
					{
						...
						TDOffice.CheckListItem item = korisnikZadaci.FirstOrDefault(x =>
							x.Job == TDOffice.CheckList.Jobs.NeaktivniPartneri
						);
						if (item != null)
						{
