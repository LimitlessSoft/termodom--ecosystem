				...
				using (FbConnection con = new FbConnection(zakucaniConnString))
				{
					...
					)
					{
						...
						Komercijalno.Dokument dokumentPocetnogStanja = Komercijalno
							.Dokument.List(con, "VRDOK = 0 AND MAGACINID = " + magacin.ID)
							.FirstOrDefault();

						UpdateStatusLabel(magacin.Naziv + " / Ucitavam stavke pocetnog stanja");
						List<Komercijalno.Stavka> stavkePocetnogStanja = Komercijalno.Stavka.List(
							con,
							"VRDOK = 0 AND BRDOK = " + dokumentPocetnogStanja.BrDok.ToString()
						);

						UpdateStatusLabel(magacin.Naziv + " / Ucitavam robu u magacinu");
						List<Komercijalno.RobaUMagacinu> robaUMagacinu =
							Komercijalno.RobaUMagacinu.ListByMagacinID(con, magacin.ID);
						...
						foreach (Komercijalno.RobaUMagacinu rum in robaUMagacinu)
						{
							...
							{
								UpdateStatusLabel(
									magacin.Naziv + " / Azuriranje kolicina " + i + " od " + total
								);
								List<
									Tuple<int, double>
								...
								using (
									FbCommand cmd = new FbCommand(
										@"SELECT D.VRDOK, S.KOLICINA FROM STAVKA S
...
ORDER BY D.DATUM ASC, D.LINKED",
								...
								else
								{
									Komercijalno.Stavka.Insert(
										...
										Komercijalno.Roba.Get(con, rum.RobaID),
										rum,
										minimalnoStanje * -1,
										0
									);
