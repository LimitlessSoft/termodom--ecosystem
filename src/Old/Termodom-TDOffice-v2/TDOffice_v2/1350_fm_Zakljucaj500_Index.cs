...
namespace TDOffice_v2
{
	public partial class _1350_fm_Zakljucaj500_Index : Form
	{
		...
		{
			Termodom.Data.Entities.Komercijalno.VrstaDokDictionary vrsteDokumenta =
				await Komercijalno.VrstaDokManager.DictionaryAsync();
			List<VrstaDok> vrdokList = vrsteDokumenta
				.Values.Where(x => x.VrDok == 13 || x.VrDok == 15)
				.ToList();
		...
		private async void btn_Zakljucaj_Click(object sender, EventArgs e)
		{
			...
			if (dokument.NUID != Komercijalno.NacinUplate.Virman)
			{
				MessageBox.Show($"Dokument nije virmanski!");
			}
			if (dokument.PPID <= 0)
			{
				MessageBox.Show($"Dokument nema dodeljenog partnera!");
				return;
			}
			...
			if (dugovanje < tolerancija)
			{
				...
				string tekst =
					"<<Bonus zakljucavanje 500>','Dokument <"
					+ brDok.ToString()
