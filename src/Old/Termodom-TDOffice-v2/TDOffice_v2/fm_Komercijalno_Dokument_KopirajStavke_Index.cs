...
namespace TDOffice_v2
{
	public partial class fm_Komercijalno_Dokument_KopirajStavke_Index : Form
	{
		...
		private void fm_Komercijalno_Dokument_KopirajStavke_Index_Load(object sender, EventArgs e)
		{
			...
			_distinctPutanjeDoBaza.ContinueWith(
				async (prev) =>
				{
		...
		private void kopiraj_btn_Click(object sender, EventArgs e)
		{
			...
			Task.Run(() =>
			{
				...
				try
				{
					this.Invoke(
						(MethodInvoker)
							...
							{
								izvorniString =
									$"data source=4monitor; initial catalog = {izGodine_cmb.SelectedValue.ToString()}; user=SYSDBA; password=m";
								destinacioniString =
									$"data source=4monitor; initial catalog = {uGodinu_cmb.SelectedValue.ToString()}; user=SYSDBA; password=m";
				...
				{
					con.Open();
					destinacioniDokument = Komercijalno.Dokument.Get(
						con,
						destinacija_vrDok,
						destinacija_brDok
					);
				...
				if (
					destinactioniDokumentMoraBitiOtkljucan_cb.Checked
					&& destinacioniDokument.Flag != 0
				)
				{
					this.Invoke(
						(MethodInvoker)
				...
				{
					con.Open();
					izvorneStavke = Komercijalno.Stavka.ListByDokument(
						con,
						izvor_vrDok,
						izvor_brDok
					);
				...
				//using(FbConnection con = new FbConnection(Komercijalno.Komercijalno.CONNECTION_STRING[destinacija_godina]))
				{
					...
					if (destinacioniDokumentMoraBitiPrazan_cb.Checked)
					{
						this.Invoke(
							(MethodInvoker)
								...
								{
									status_lbl.Text =
										$"Proveravam da li je destinacioni dokument prazan!";
								}
						);

						if (
							Komercijalno
								.Stavka.ListByDokument(con, destinacija_vrDok, destinacija_brDok)
								.Count > 0
						)
						{
							this.Invoke(
					...
					this.Invoke(
						(MethodInvoker)
							...
							{
								status_lbl.Text =
									$"Ucitavam robu magacina iz destinacionog dokumenta ({destinacioniDokument.MagacinID})";
							}
					);
					List<Komercijalno.RobaUMagacinu> robaUMagacinu =
						Komercijalno.RobaUMagacinu.ListByMagacinID(
							con,
							destinacioniDokument.MagacinID
						);
					...
					List<Komercijalno.Stavka> postojeceStavkePrePocetkaInserta =
						Komercijalno.Stavka.ListByDokument(
							con,
					...
					foreach (Komercijalno.Stavka s in izvorneStavke)
					{
						...
						int newStavkaID = Komercijalno.Stavka.Insert(
							con,
							destinacioniDokument,
							robaKomercijalno.FirstOrDefault(x => x.ID == s.RobaID),
							robaUMagacinu.FirstOrDefault(x => x.RobaID == s.RobaID),
							s.Kolicina,
							s.Rabat
						);
						...
						{
							Komercijalno.Stavka novaStavka = Komercijalno.Stavka.Get(
								con,
								newStavkaID
							);
						...
						if (prodajneCeneOstaviKaoUIzvornomDokumentu_cb.Checked)
						{
							...
							double prodajnaCenaBP = VPDokumenti.Contains(izvorniDokument.VrDok)
								? s.ProdajnaCena
								: s.ProdCenaBP;
							novaStavka.ProdajnaCena = VPDokumenti.Contains(
								destinacioniDokument.VrDok
							)
								? prodajnaCenaBP
								: s.ProdajnaCena;
		...
		private void PoveziDokumente(
			Komercijalno.Dokument izvorniDokument,
			Komercijalno.Dokument destinacioniDokument,
			string izvorniString,
			string destinactioniString
		)
		{
			if (
				MessageBox.Show(
					"Da li zelite povezati ove dokumente?",
					"Povezi dokumente?",
			...
			)
			{
				if (izvorniDokument.VrDokOut.HasValue)
					if (
						MessageBox.Show(
							"Izvorni dokument vec ima vrednost u polju 'VrDokOut'. Povezivanje dokumenata ce obrisati staru vrednost. Da li zelite nastaviti?",
							"Oprez!",
							MessageBoxButtons.YesNo
						) != DialogResult.Yes
					)
				...
				if (destinacioniDokument.VrDokIn.HasValue)
					if (
						MessageBox.Show(
							"Destinacioni dokument vec ima vrednost u polju 'VrDokIn'. Povezivanje dokumenata ce obrisati staru vrednost. Da li zelite nastaviti?",
							"Oprez!",
							MessageBoxButtons.YesNo
						) != DialogResult.Yes
					)
		...
		private void destinacioniDokumentMoraBitiPrazan_cb_CheckedChanged(
			object sender,
			EventArgs e
		)
		{
