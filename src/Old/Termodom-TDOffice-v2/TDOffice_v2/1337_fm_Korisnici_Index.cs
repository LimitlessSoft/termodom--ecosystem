...
namespace TDOffice_v2
{
	public partial class _1337_fm_Korisnici_Index : Form
	{
		...
		private Task<fm_Help> _helpForm { get; set; } =
			fm_Help.GenerateAsync(TDOffice.Modul.GetWithInsert((int)Modul.Korisnici_Index));
		...
		{
			List<Tuple<int, string>> listaTipovaAutomatskogObavestenja =
				new List<Tuple<int, string>>();
			foreach (
				TDOffice.User.TipAutomatskogObavestenja to in (TDOffice.User.TipAutomatskogObavestenja[])
					Enum.GetValues(typeof(TDOffice.User.TipAutomatskogObavestenja))
			)
			{
				listaTipovaAutomatskogObavestenja.Add(
					new Tuple<int, string>((int)to, to.ToString().DivideOnCapital())
				);
			}
			...
			foreach (
				TDOffice.User.TipAutomatskogObavestenja t in korisnik.Tag.PrimaObavestenja.Keys
			)
			{
				...
				Task.Run(() =>
					{
						...
						foreach (TDOffice.TDTaskItem item in _tdTaskItems.Result)
						{
							...
							dr["Uradjen"] =
								item.Done == null
									? "Nikada"
									: ((DateTime)item.Done).ToString("dd.MM.yyyy [HH:mm]");
							dt.Rows.Add(dr);
						}
						...
						this.Invoke(
							(MethodInvoker)
								delegate
								{
									...
									taskovi_dgv.Columns["Task"].AutoSizeMode =
										DataGridViewAutoSizeColumnMode.Fill;
									taskovi_dgv.Columns["Task"].ReadOnly = true;

									taskovi_dgv.Columns["Interval"].AutoSizeMode =
										DataGridViewAutoSizeColumnMode.AllCells;
									taskovi_dgv.Columns["Interval"].ReadOnly = false;
									taskovi_dgv.Columns["Interval"].DefaultCellStyle.BackColor =
										Color.LightYellow;
									taskovi_dgv.Columns["Interval"].HeaderText =
										"Interval ( minuti )";

									taskovi_dgv.Columns["Uradjen"].AutoSizeMode =
										DataGridViewAutoSizeColumnMode.AllCells;
		...
		private void SetujCheckStateTipaPrimanjaObavestenja(
			TDOffice.User.TipAutomatskogObavestenja tip,
			bool value
		)
		{
		...
		{
			List<TDOffice.Pravo> prava = TDOffice
				.Pravo.List()
				.Where(x => x.UserID == korisnik.ID)
				.ToList();
		...
		)
		{
			...
			TDOffice.Config<List<int>> config = TDOffice.Config<List<int>>.Get(
		...
		private void clb_PrimalacObavestenja_SelectedIndexChanged(object sender, EventArgs e)
		{
			...
			Tuple<int, string> item =
				clb_PrimalacObavestenja.Items[clb_PrimalacObavestenja.SelectedIndex]
				as Tuple<int, string>;

			korisnik.Tag.PrimaObavestenja[(TDOffice.User.TipAutomatskogObavestenja)item.Item1] =
				clb_PrimalacObavestenja.GetItemChecked(clb_PrimalacObavestenja.SelectedIndex);
			korisnik.Update();
		}
		...
		private void taskovi_dgv_CellValidating(
			object sender,
			DataGridViewCellValidatingEventArgs e
		)
		{
		...
		private async void ukloniToolStripMenuItem_Click(object sender, EventArgs e)
		{
			...
			if (
				MessageBox.Show(
					"Da li sigurno zelite ukloniti ovaj task korisniku?",
					"Potvrdi",
			...
			int itemID = Convert.ToInt32(
				taskovi_dgv.Rows[taskovi_dgv.SelectedCells[0].RowIndex].Cells["ID"].Value
			);
