...
namespace TDOffice_v2
{
	public partial class fm_Magacin_Index : Form
	{
		...
		private Task<List<Config.Zaposleni>> _configZaposleni { get; set; } =
			Config.Zaposleni.ListAsync();
		private Task<List<TDOffice.User>> _tdOfficeKorisnici { get; set; } =
			TDOffice.User.ListAsync();
		...
		public fm_Magacin_Index(int magacinID)
		{
			...
			if (_tdMagacin == null)
			{
				TDOffice.Magacin.Insert(
					_komercijalnoMagacin.Result.ID,
					_komercijalnoMagacin.Result.ID,
					50,
					1,
					"NN",
					"NN"
				); // Poslednja 3 parametra treba izmeniti na korisnikov unos
				_tdMagacin = TDOffice.Magacin.Get(_komercijalnoMagacin.Result.ID);
			}
		}
		...
		private void fm_Magacin_Index_Load(object sender, EventArgs e)
		{
			Task.Run(() =>
			{
				...
				List<TDOffice.User> vlasnikCmbData = new List<TDOffice.User>(
					_tdOfficeKorisnici.Result
				);
				vlasnikCmbData.Add(
					new TDOffice.User() { ID = -1, Username = " < nema vlasnika >" }
				);
				vlasnikCmbData.Sort((x, y) => x.ID.CompareTo(y.ID));

				List<TDOffice.User> noviKorisnikCmbData = new List<TDOffice.User>(
					_tdOfficeKorisnici.Result
				);
				noviKorisnikCmbData.Add(
					new TDOffice.User() { ID = -1, Username = " < izaberi korisnika >" }
				);
				...
				this.Invoke(
					(MethodInvoker)
						delegate
						{
							TDOffice.MagacinClan vlasnik = TDOffice
								.MagacinClan.ListByMagacinID(_tdMagacin.ID)
								.FirstOrDefault(x =>
									x.Tip == TDOffice.Enums.MagacinClanTip.Vlasnik
								);
				...
				this.Invoke(
					(MethodInvoker)
						delegate
						{
							...
							nadredjeniMagacin_cmb.DataSource = new List<Komercijalno.Magacin>(
								_komercijalnoMagacini.Result
							);

							magacinRazduzenja_cmb.DisplayMember = "Naziv";
							magacinRazduzenja_cmb.ValueMember = "ID";
							magacinRazduzenja_cmb.DataSource = new List<Komercijalno.Magacin>(
								_komercijalnoMagacini.Result
							);
		...
		private Task UcitajKorisnikeMagacinaAsync()
		{
			return Task.Run(() =>
			{
				List<TDOffice.MagacinClan> mcs = TDOffice.MagacinClan.ListByMagacinID(
					_tdMagacin.ID
				);
				...
				foreach (TDOffice.MagacinClan mc in mcs)
				{
					...
					TDOffice.User tdUser = _tdOfficeKorisnici.Result.FirstOrDefault(x =>
						x.ID == mc.KorisnikID
					);
					Config.Zaposleni configZap =
						tdUser.KomercijalnoUserID != null
							? _configZaposleni.Result.FirstOrDefault(x =>
								x.ZapID == tdUser.KomercijalnoUserID
							)
							: new Config.Zaposleni() { ZapID = -1, Naziv = "NIJE DEFINISAN" };
				...
				this.Invoke(
					(MethodInvoker)
						delegate
						{
							...
							korisnici_dgv.Columns["ID"].AutoSizeMode =
								DataGridViewAutoSizeColumnMode.AllCells;
							korisnici_dgv.Columns["Korisnik"].AutoSizeMode =
								DataGridViewAutoSizeColumnMode.AllCells;
							korisnici_dgv.Columns["KomercijalnoID"].AutoSizeMode =
								DataGridViewAutoSizeColumnMode.AllCells;
							korisnici_dgv.Columns["KomercijalnoIme"].AutoSizeMode =
								DataGridViewAutoSizeColumnMode.AllCells;
						}
				);
			});
		...
		private void vlasnik_cmb_SelectedIndexChanged(object sender, EventArgs e)
		{
			...
			TDOffice.MagacinClan magacinClan = mcs.Where(x =>
					x.Tip == TDOffice.Enums.MagacinClanTip.Vlasnik
				)
				.FirstOrDefault();
			if (magacinClan != null)
			...
			if (korisnikID > 0)
				TDOffice.MagacinClan.Insert(
					_tdMagacin.ID,
					korisnikID,
					TDOffice.Enums.MagacinClanTip.Vlasnik
				);
		...
		private void noviKorisnik_btn_Click(object sender, EventArgs e)
		{
			...
			TDOffice.MagacinClan.Insert(
				_tdMagacin.ID,
				korisnikID,
				TDOffice.Enums.MagacinClanTip.Clan
			);
			UcitajKorisnikeMagacinaAsync();
