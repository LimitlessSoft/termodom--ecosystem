...
namespace TDOffice_v2
{
    public partial class _1336_fm_RazduzenjeMagacina_Index : Form
    {
        ...
        private void SetUI()
        {
            TDOffice.User k = _korisnici.Result.Where(x => x.ID == _dokument.KorisnikID).FirstOrDefault();

            Komercijalno.Magacin m = _magacini.Result.Where(x => x.ID == _dokument.MagacinID).FirstOrDefault();
            Komercijalno.Dokument d = Komercijalno.Dokument.ListByMagacinID(_dokument.Datum.Year, _dokument.MagacinID).ToList().Where(x => x.VrDok == 19 && x.BrDok ==_dokument.KomercijalnoBrDokOut).FirstOrDefault();
            ...
            this.izvrsiRazduzenje_btn.Enabled = !(_dokument.Status == TDOffice.DokumentRazduzenjaMagacinaStatus.Izvrsen);

            this.dataGridView1.Enabled = !(_dokument.Status == TDOffice.DokumentRazduzenjaMagacinaStatus.Izvrsen);

            this.BackColor = _dokument.Status == TDOffice.DokumentRazduzenjaMagacinaStatus.Zakljucan ? Color.Red : _dokument.Status == TDOffice.DokumentRazduzenjaMagacinaStatus.Izvrsen ? Color.OrangeRed : Color.Green;

            this.status_btn.BackgroundImage = _dokument.Status == TDOffice.DokumentRazduzenjaMagacinaStatus.Zakljucan ? TDOffice_v2.Properties.Resources.key_green : TDOffice_v2.Properties.Resources.key_red;
            this.status_btn.Enabled = _dokument.Status == TDOffice.DokumentRazduzenjaMagacinaStatus.Izvrsen ? false : _dokument.Status == TDOffice.DokumentRazduzenjaMagacinaStatus.Otkljucan ? Program.TrenutniKorisnik.ImaPravo(133602) : Program.TrenutniKorisnik.ImaPravo(133603);
            ...
            this.komentar_btn.Enabled = _dokument.Status == TDOffice.DokumentRazduzenjaMagacinaStatus.Otkljucan ? true : false;
            this.interniKomentar_btn.Enabled = _dokument.Status == TDOffice.DokumentRazduzenjaMagacinaStatus.Otkljucan ? true : false;
        }
        private void SetDGV()
        {
        ...
        private void OnIzborRobeTrigger(Komercijalno.RobaUMagacinu[] args)
        {
            ...
            TDOffice.StavkaRazduzenjaMagacina ss = _stavke.Where(x => x.RobaID == r.RobaID).FirstOrDefault();
        ...
        private void status_btn_Click(object sender, EventArgs e)
        {
            ...
            _dokument.Status = _dokument.Status == TDOffice.DokumentRazduzenjaMagacinaStatus.Otkljucan ? TDOffice.DokumentRazduzenjaMagacinaStatus.Zakljucan : TDOffice.DokumentRazduzenjaMagacinaStatus.Otkljucan;
            _dokument.Update();
        ...
        private void izvrsiRazduzenje_btn_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Magacin nema definisan svoj Magacin Razduzenja!\nPodesavanja > Magacini > Magacin > Magacin Razduzenja");
                return;
            }
            ...
            if (MessageBox.Show("Sigurno zelite da izvrsite razduzenje u magacin " + _magacini.Result.FirstOrDefault(x => x.ID == _tdMagacin.Result.ID).Naziv + "?", "Razduzenje magacina", MessageBoxButtons.YesNo) != DialogResult.Yes)
                return;

            using (FbConnection con = new FbConnection(Komercijalno.Komercijalno.CONNECTION_STRING[DateTime.Now.Year]))
            {
                con.Open();
                int komercijalnoBrDok19 = Komercijalno.Dokument.Insert(con, 19, null, null, "TDOffice_v2", 1, _dokument.MagacinID, Program.TrenutniKorisnik.KomercijalnoUserID, _tdMagacin.Result.MagacinRazduzenja);
                Komercijalno.Dokument dok19 = Komercijalno.Dokument.Get(con, 19, komercijalnoBrDok19);

                int komercijalnoBrDok18 = Komercijalno.Dokument.Insert(con, 18, null, null, "TDOffice_v2", 1, _tdMagacin.Result.MagacinRazduzenja, Program.TrenutniKorisnik.KomercijalnoUserID, _dokument.MagacinID);
                Komercijalno.Dokument dok18 = Komercijalno.Dokument.Get(con, 18, komercijalnoBrDok18);
                ...
                Komercijalno.Dokument.SetKomentar(dok19.Datum.Year, 19, dok19.BrDok, _dokument.Komentar);
                Komercijalno.Dokument.SetInterniKomentar(dok19.Datum.Year, 19, dok19.BrDok, _dokument.InterniKomentar);
                Komercijalno.Dokument.SetKomentar(dok19.Datum.Year, 18, dok18.BrDok, _dokument.Komentar);
                Komercijalno.Dokument.SetInterniKomentar(dok19.Datum.Year, 18, dok18.BrDok, _dokument.InterniKomentar);

                _dokument.KomercijalnoBrDokOut = komercijalnoBrDok19;
                _dokument.Update();

                Task<List<Komercijalno.Roba>> robaKom = Komercijalno.Roba.ListAsync(DateTime.Now.Year);
                Task<List<Komercijalno.RobaUMagacinu>> rumKom = Komercijalno.RobaUMagacinu.ListByMagacinIDAsync(_dokument.MagacinID);

                foreach (DataGridViewRow row in dataGridView1.Rows)
                {
                    int robaID = Convert.ToInt32(row.Cells["Robaid"].Value);

                    Komercijalno.Stavka.Insert(DateTime.Now.Year, dok19, robaKom.Result.Where(x => x.ID == robaID).FirstOrDefault(), rumKom.Result.Where(x => x.RobaID == robaID).FirstOrDefault(), Convert.ToDouble(row.Cells["KOLICINA"].Value), 0);
                    Komercijalno.Stavka.Insert(DateTime.Now.Year, dok18, robaKom.Result.Where(x => x.ID == robaID).FirstOrDefault(), rumKom.Result.Where(x => x.RobaID == robaID).FirstOrDefault(), Convert.ToDouble(row.Cells["KOLICINA"].Value), 0);
                }
                ...
                // Slanje poruka
                string tekstPoruke = "Korisnik " + Program.TrenutniKorisnik.Username + " je razduzio lager magacina " + _dokument.MagacinID + " po dokumentu br." + _dokument.ID + " cija je vrednost \nKreirana je interna otpremnica br." + dok18.BrDokIn;
                string naslovPoruke = "Razduzenje magacina" + _dokument.MagacinID + " po dokumentu broj " + _dokument.ID;
                TDOffice.Poruka.Insert(TDOffice.User.List().Where(x => x.Tag != null && x.Tag.PrimaObavestenja[TDOffice.User.TipAutomatskogObavestenja.NakonRazduzenjaRobe] == true).Select(x => x.ID).ToArray(), naslovPoruke, tekstPoruke, new TDOffice.PorukaAdditionalInfo() { Action = TDOffice.PorukaAction.RealizovanoRazduzenjeMagacina, AdditionalInfo = _dokument.ID });
            }
        ...
        private void dataGridView1_CellValidating(object sender, DataGridViewCellValidatingEventArgs e)
        {
            ...
            {
                int id = Convert.ToInt32(dataGridView1.Rows[dataGridView1.SelectedCells[0].RowIndex].Cells["ID"].Value);
                TDOffice.StavkaRazduzenjaMagacina _stavka = TDOffice.StavkaRazduzenjaMagacina.Get(id);
