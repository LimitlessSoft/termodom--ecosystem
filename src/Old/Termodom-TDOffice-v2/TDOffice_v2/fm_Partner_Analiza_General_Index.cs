...
namespace TDOffice_v2
{
    ...
    {
        private static int[] _profiliHobiList { get; set; } = new int[12] { 6101, 6102, 6103, 6104, 6105, 6106, 6107, 6108, 6109, 6111, 6112, 6113 };
        private static int[] _profiliStandardList { get; set; } = new int[29] { 90, 91, 570, 572, 573, 574, 577, 579, 580, 582, 583, 588, 590, 592, 597, 1094, 1096, 1097, 2034, 2198, 2281, 2282, 2284, 2342, 2343, 2497, 3213, 3342, 3486 };
        private static int[] _profiliProfiList { get; set; } = new int[13] { 836, 862, 1807, 1808, 2331, 2496, 3005, 5286, 6977, 7397, 7398, 7399, 7400 };

        private static int[] _revizijeList { get; set; } = new int[7] { 167, 2953, 3410, 4741, 6303, 7020, 4136 };

        private static int[] _gkpList { get; set; } = new int[18] { 30, 32, 34, 38, 39, 42, 51, 56, 62, 65, 66, 838, 1131, 3120, 5097, 6968, 7259, 7273 };
        private static int[] _gkpIzolacijaList { get; set; } = new int[21] { 7017, 7016, 6185, 6186, 6144, 6143, 1919, 2204, 305, 301, 2185, 294, 2006, 6972, 2482, 1725, 398, 400, 1506, 5957, 6183 };

        private static int[] _lepakFasadniHobiList { get; set; } = new int[1] { 453 };
        private static int[] _lepakFasadniStandardList { get; set; } = new int[4] { 437, 2732, 6817, 6543 };
        private static int[] _lepakFasadniProfiList { get; set; } = new int[1] { 3253 };

        private int? predefinedPPID { get; set; } = null;
        private Task<List<Komercijalno.Partner>> _komercijalnoPartneri { get; set; } = Komercijalno.Partner.ListAsync(DateTime.Now.Year);
        ...
        private void UpdateDGV()
        {
            ...
            roba_dgv.Columns["KOLICINE"].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
        ...
        public static Double OstvarenaRazlika(int godina, int ppid)
        {
            ...
            {
                con.Open();
                using (FbCommand cmd = new FbCommand("Select SUM((s.PRODCENABP - s.NABAVNACENA) * s.KOLICINA) AS RAZLIKA from Dokument d join stavka s on d.brdok = s.brdok and d.vrdok = s.vrdok where d.VRDOK = 15 and D.PPID = @PPID", con))
                {
                    cmd.Parameters.AddWithValue("@PPID", ppid);
                    using (FbDataReader dr = cmd.ExecuteReader())
                    {
                        if (dr.Read())
                            R = dr["RAZLIKA"] is DBNull ? 0 : (double)Convert.ToDouble(dr["RAZLIKA"]);
                    }
                }
            }
            return R;
...
ORDER BY s.ROBAID", con))
            ...
            foreach (int godina in Komercijalno.Komercijalno.CONNECTION_STRING.Keys.OrderBy(x => x))
            {
                ...
                {
                    con.Open();
                    using (FbCommand cmd = new FbCommand("SELECT COALESCE(SUM(DUGUJE - POTRAZUJE), 0) FROM DOKUMENT WHERE (VRDOK = 15 OR VRDOK = 13) AND PPID = @PPID", con))
                    {
                        cmd.Parameters.AddWithValue("@PPID", PPID);
            ...
            foreach (int godina in Komercijalno.Komercijalno.CONNECTION_STRING.Keys.OrderBy(x => x))
            {
                double ORP = rabatDR[godina.ToString()] is DBNull || prometDR[godina.ToString()] is DBNull ? 0 : ((Convert.ToDouble(rabatDR[godina.ToString()])) / Convert.ToDouble(prometDR[godina.ToString()]));// (((double)rabatDR[godina.ToString()]) / (double)prometDR[godina.ToString()]) * 100;
            ...
            foreach (int godina in Komercijalno.Komercijalno.CONNECTION_STRING.Keys.OrderBy(x => x))
            {
                ...
                {
                    con.Open();
                    using (FbCommand cmd = new FbCommand("SELECT count(*) as brk FROM DOKUMENT WHERE VRDOK = 15 AND PPID = @PPID", con))
                    {
                        cmd.Parameters.AddWithValue("@PPID", PPID);

                        using (FbDataReader dr = cmd.ExecuteReader())
                            brojKupovina[godina.ToString()] = dr.Read() ? Convert.ToInt32(dr["brk"]) : 0;
                    }
                }
            }
            ...
            foreach (int godina in Komercijalno.Komercijalno.CONNECTION_STRING.Keys.OrderBy(x => x))
            {
                ...
                {
                    con.Open();
                    using (FbCommand cmd = new FbCommand("SELECT MAX(DATUM) as dat FROM DOKUMENT WHERE VRDOK = 15 AND PPID = @PPID", con))
                    {
                        cmd.Parameters.AddWithValue("@PPID", PPID);
            ...
            foreach (int godina in Komercijalno.Komercijalno.CONNECTION_STRING.Keys.OrderBy(x => x))
            {
                ...
                promet_dgv.Columns[godina.ToString()].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
                promet_dgv.Columns[godina.ToString()].AutoSizeMode = DataGridViewAutoSizeColumnMode.AllCells;
            }

            double ukupnoHobiProfila = robaDT.Select($"ROBAID IN ({string.Join(", ", _profiliHobiList)})").AsEnumerable().Sum(x => Convert.ToDouble(x["KOLICINE"]));
            double ukupnoStandardProfila = robaDT.Select($"ROBAID IN ({string.Join(", ", _profiliStandardList)})").AsEnumerable().Sum(x => Convert.ToDouble(x["KOLICINE"]));
            double ukupnoProfiProfila = robaDT.Select($"ROBAID IN ({string.Join(", ", _profiliProfiList)})").AsEnumerable().Sum(x => Convert.ToDouble(x["KOLICINE"]));
            ...
            if (ukupnoProfila > 0)
            {
                ...
                output.Add("Udeo HOBI profila: " + (ukupnoHobiProfila / ukupnoProfila).ToString("0.00%"));
                output.Add("Udeo STANDARD profila: " + (ukupnoStandardProfila / ukupnoProfila).ToString("0.00%"));
                output.Add("Udeo PROFI profila: " + (ukupnoProfiProfila / ukupnoProfila).ToString("0.00%"));
                output.Add("");
                output.Add("Minimalni odnos (Hobi / Standard / Profi) = 0% / 30% / 70%");
                output.Add("======================");
            }

            double ukupnoGKP = robaDT.Select($"ROBAID IN ({string.Join(", ", _gkpList)})").AsEnumerable().Sum(x => Convert.ToDouble(x["KOLICINE"]));
            double ukupnoRevizija = robaDT.Select($"ROBAID IN ({string.Join(", ", _revizijeList)})").AsEnumerable().Sum(x => Convert.ToDouble(x["KOLICINE"]));
            double ukupnoGKPSrafova = robaDT.Select($"NAZIV LIKE '%FOSFAT%'").AsEnumerable().Sum(x => Convert.ToDouble(x["KOLICINE"])) +
                robaDT.Select($"NAZIV LIKE '%1000/1%'").AsEnumerable().Sum(x => Convert.ToDouble(x["KOLICINE"]) * 1000);
            ...
            double ukupnoGKPIzolacije = robaDT.Select($"ROBAID IN ({string.Join(", ", _gkpIzolacijaList)})").AsEnumerable().Sum(x => Convert.ToDouble(x["KOLICINE"]));
            ...
            double ukupnoHobiFasadnogLepka = robaDT.Select($"ROBAID IN ({string.Join(", ", _lepakFasadniHobiList)})").AsEnumerable().Sum(x => Convert.ToDouble(x["KOLICINE"]));
            double ukupnoStandardFasadnogLepka = robaDT.Select($"ROBAID IN ({string.Join(", ", _lepakFasadniStandardList)})").AsEnumerable().Sum(x => Convert.ToDouble(x["KOLICINE"]));
            double ukupnoProfiFasadnogLepka = robaDT.Select($"ROBAID IN ({string.Join(", ", _lepakFasadniProfiList)})").AsEnumerable().Sum(x => Convert.ToDouble(x["KOLICINE"]));
            double ukupnoFasadnogLepka = ukupnoHobiFasadnogLepka + ukupnoStandardFasadnogLepka + ukupnoProfiFasadnogLepka;

            if (ukupnoFasadnogLepka > 0)
            {
                output.Add("======================");

                output.Add("Udeo HOBI fasadnog lepka: " + (ukupnoHobiFasadnogLepka / ukupnoFasadnogLepka).ToString("0.00%"));
                output.Add("Udeo STANDARD fasadnog lepka: " + (ukupnoStandardFasadnogLepka / ukupnoFasadnogLepka).ToString("0.00%"));
                output.Add("Udeo PROFI fasadnog lepka: " + (ukupnoProfiFasadnogLepka / ukupnoFasadnogLepka).ToString("0.00%"));
                output.Add("");
                output.Add("Minimalni odnos (Hobi / Standard / Profi) = 0% / 30% / 70%");
                output.Add("======================");
            }
            ...
            Task.Run(() =>
            {
                ...
                if (!this.IsDisposed)
                {
                    ...
                    TDOffice.CheckListItem item = korisnikZadaci.FirstOrDefault(x => x.Job == TDOffice.CheckList.Jobs.DetaljnaAnalizaPartnera);
                    if (item != null)
                    {
        ...
        {
            MessageBox.Show("Jos uvek nije uradjeno. Za sada su zakucani proizvodi sa kojima radim analizu ali ce biti u listama");
        }

        private void karticaRobeToolStripMenuItem_Click(object sender, EventArgs e)
        {
            int robaID = Convert.ToInt32(roba_dgv.Rows[roba_dgv.SelectedCells[0].RowIndex].Cells["ROBAID"].Value);
