...
namespace TDOffice_v2
{
	public partial class fm_PrenosRobeDopuna_Index : Form
	{
		...
		// Obeleziti 150 da NE moze u minus i srediti kartice309309
		...
		private void DopunaRobePoProdaji()
		{
			try
			{
				...
				)
				{
					...
					using (
						FbCommand cmd = new FbCommand(
							$"select S.ROBAID, SUM(S.KOLICINA) FROM STAVKA S LEFT OUTER JOIN DOKUMENT D ON D.VRDOK = S.VRDOK AND D.BRDOK = S.BRDOK WHERE S.VRDOK IN (15, 19) AND S.MAGACINID = {magacinID} AND D.DATUM >= '{odDatuma.Date.ToString("dd.MM.yyyy")}' AND "
								+ $"D.DATUM <= '{doDatuma.Date.ToString("dd.MM.yyyy")}' GROUP BY S.ROBAID",
							con
						)
					)
					using (FbDataReader dr = cmd.ExecuteReader())
						while (dr.Read())
							izlazRobePoRobaIDZaPeriod.Add(
				...
				)
				{
					...
					var destinacioniDokument = Komercijalno.Dokument.Get(
						con,
						destinacioniVrDok,
						destinacioniBrDok
					);
					...
					{
						MessageBox.Show(
							$"Dokument {destinacioniVrDok}, {destinacioniBrDok} nije pronadjen!"
						);
						return;
					}
					...
					using (
						FbCommand cmd = new FbCommand(
							$"SELECT COUNT(VRDOK) FROM STAVKA WHERE VRDOK = {destinacioniVrDok} AND BRDOK = {destinacioniBrDok}",
							con
						)
					)
					{
					...
					foreach (int key in izlazRobePoRobaIDZaPeriod.Keys)
					{
						...
						{
							MessageBox.Show(
								$"Greska ucitavanja Robe {key} prilikom inserta u dokument, preskacem i nastavljam dalje!"
							);
							continue;
						}
						...
						{
							MessageBox.Show(
								$"Greska ucitavanja RobeUMagacinu {key} prilikom inserta u dokument, preskacem i nastavljam dalje!"
							);
							continue;
						}
		...
		private void DopunaRobePoStanjuAsync()
		{
			try
			{
				...
				using (
					FbConnection conIzvor = new FbConnection(
				...
				)
				{
					conIzvor.Open();
					using (
						FbConnection conDest = new FbConnection(
							"data source=4monitor; initial catalog = c:\\poslovanje\\baze\\2024\\TERMODOM2024.FDB; user=SYSDBA; password=m; pooling=True"
						)
					)
					{
						...
						var robaUMagacinu = Komercijalno.RobaUMagacinu.ListByMagacinID(
							conIzvor,
							magacinID
						);
						var destinacioniDokument = Komercijalno.Dokument.Get(
						...
						var stavkeDestinacionogDokumenta = Komercijalno.Stavka.ListByDokument(
							conDest,
							destinacioniVrDok,
							destinacioniBrDok
						);

						var dokumentiOdDatumaDoDatuma = Komercijalno.Dokument.ListByMagacinID(
							conDest,
							magacinID
						);
						dokumentiOdDatumaDoDatuma.RemoveAll(x =>
							x.Datum.Date < odDatuma_dtp.Value.Date
							|| x.Datum > doDatuma_dtp.Value.Date
						);

						stavkeDestinacionogDokumenta.RemoveAll(x =>
							!dokumentiOdDatumaDoDatuma.Any(y =>
						...
						foreach (var rum in robaUMagacinu)
						{
							if (rum.Stanje < 0)
							{
								var stavkaDokumenta = stavkeDestinacionogDokumenta.FirstOrDefault(
									x => x.RobaID == rum.RobaID
								);
		...
		private void DopunaRobePoNeispravnojKartici()
		{
			try
			{
				...
				)
				{
					...
					dokumentiOdDatumaDoDatuma.RemoveAll(x =>
						x.Datum.Date < odDatuma_dtp.Value.Date || x.Datum > doDatuma_dtp.Value.Date
					);
					...
					foreach (var rum in robaUMagacinu)
					{
						...
						var stavkeRobe = stavke
							.Where(x => x.RobaID == rum.RobaID && x.TrenStanje != null)
							.ToList();
						...
						if (kolicina < 0)
						{
							var stavkaDokumenta = stavkeDestinacionogDokumenta.FirstOrDefault(x =>
								x.RobaID == rum.RobaID
							);
