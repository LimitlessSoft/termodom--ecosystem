...
namespace TDOffice_v2
{
	public partial class fm_BiznisPlan_Index : Form
	{
		...
		private void UcitajBiznisPlan()
		{
			...
			Task<List<Komercijalno.Stavka>> troskoviMagacina = Komercijalno.Stavka.ListAsync(
				DateTime.Now.Year,
				$"MAGACINID = {magacinID} AND VRDOK = 6"
			);
			...
			)
			{
				...
				using (
					FbCommand cmd = new FbCommand(
						"SELECT COALESCE(SUM(POTRAZUJE), 0) FROM DOKUMENT WHERE VRDOK = 15 AND MAGACINID = @MAGID",
						con
					)
				)
				{
			...
			#region Troskovi
			TDOffice.Config<Dictionary<string, List<int>>> _grupeTroskova = TDOffice.Config<
			...
			foreach (string grupa in _grupeTroskova.Tag.Keys)
			{
				...
				if (
					biznisPlan.PlaniraniTroskovi != null
				...
				dr["Trosak"] = grupa;
				dr["Vrednost"] = troskoviMagacina
					.Result.Where(x => _grupeTroskova.Tag[grupa].Contains(x.RobaID))
					.Sum(x => x.NabCenSaPor * x.Kolicina);
			...
			troskovi_dgv.Columns["Vrednost"].DefaultCellStyle.Alignment =
				DataGridViewContentAlignment.MiddleRight;

			troskovi_dgv.Columns["Planirano"].DefaultCellStyle.Format = "#,##0.00 RSD";
			troskovi_dgv.Columns["Planirano"].DefaultCellStyle.Alignment =
				DataGridViewContentAlignment.MiddleRight;
			...
			foreach (DataGridViewRow row in troskovi_dgv.Rows)
			{
				row.DefaultCellStyle.BackColor =
					Convert.ToDouble(row.Cells["Vrednost"].Value)
					< Convert.ToDouble(row.Cells["Planirano"].Value)
						? Color.Green
						: Color.LightCoral;
			}
			#endregion
			...
			troskoviSumVrednost_txt.Text =
				ukupniTroskovi.ToString("#,##0.00 RSD")
				+ " / "
				+ ukupnoPlaniranoTroskova.ToString("#,##0.00 RSD");
			troskoviSumVrednost_txt.BackColor =
				ukupniTroskovi < ukupnoPlaniranoTroskova ? Color.Green : Color.LightCoral;
		...
		private void postaviNovuVrednostToolStripMenuItem_Click(object sender, EventArgs e)
		{
			using (InputBox ib = new InputBox("Nova Vrednost", "Nova Vrednost"))
			{
				...
				string Tag = (
					(ContextMenuStrip)((ToolStripItem)sender).Owner
				).SourceControl.Tag.ToString();
		...
		{
			string grupaTroska = troskovi_dgv
				.Rows[troskovi_dgv.SelectedCells[0].RowIndex]
				.Cells["Trosak"]
				.Value.ToString();
