...
namespace TDOffice_v2
{
	public static class TDAPI
	{
		...
		public static Response<TPayload> HandleResponse<TPayload>(
		...
		)
		{
			...
			return JsonConvert.DeserializeObject<Response<TPayload>>(
				responseMessage.Content.ReadAsStringAsync().GetAwaiter().GetResult()
			);
		}

		public static async Task<Response<TPayload>> HandleResponseAsync<TPayload>(
			HttpResponseMessage responseMessage
		)
		{
			if (responseMessage.NotOk())
		...
		public static Response<TPayload> HandleRawResponse<TPayload>(
			HttpResponseMessage responseMessage
		)
		{
			var response = new Response<TPayload>();
			response.Status = responseMessage.StatusCode;
			response.Payload = JsonConvert.DeserializeObject<TPayload>(
				responseMessage.Content.ReadAsStringAsync().GetAwaiter().GetResult()
			);
			return response;
		}

		public static async Task<Response<TPayload>> HandleRawResponseAsync<TPayload>(
			HttpResponseMessage responseMessage
		)
		{
			var response = new Response<TPayload>();
		...
		public static async Task<Response<TPayload>> GetAsync<TRequest, TPayload>(
			string endpoint,
			TRequest request
		)
		{
			return await HandleResponseAsync<TPayload>(
				await HttpClient.GetAsJsonAsync<TRequest>(endpoint, request)
			);
		}
		...
		public static async Task<Response<TPayload>> GetRawAsync<TRequest, TPayload>(
			string endpoint,
			TRequest request
		)
		{
			return await HandleRawResponseAsync<TPayload>(
				await HttpClient.GetAsJsonAsync<TRequest>(endpoint, request)
			);
		}
		#endregion
		...
		public static async Task<Response<TPayload>> PostAsync<TRequest, TPayload>(
			string endpoint,
			TRequest request
		)
		{
			return await HandleResponseAsync<TPayload>(
				await HttpClient.PostAsJsonAsync<TRequest>(endpoint, request)
			);
		}

		public static async Task<Response> PostAsync<TRequest>(
		...
		public static async Task<Response<TPayload>> PutAsync<TRequest, TPayload>(
			string endpoint,
			TRequest request
		)
		{
			return await HandleResponseAsync<TPayload>(
				await HttpClient.PutAsJsonAsync<TRequest>(endpoint, request)
			);
		}

		public static async Task<Response> PutAsync<TRequest>(
			HttpClient httpClient,
			string endpoint,
			TRequest request
		)
