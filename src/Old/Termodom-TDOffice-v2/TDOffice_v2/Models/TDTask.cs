...
namespace TDOffice_v2.Models
{
	public static class TDTask
	{
		...
		public static void GenerisiPopis(int magacinID, int vlasnikPopisa)
		{
			...
			Task<List<TDOffice.DokumentPopis>> skorasnjiPopisi = Task.Run(() =>
			{
				return TDOffice.DokumentPopis.List(
					"DATUM >= '" + DateTime.Now.AddDays(15).ToString("dd.MM.yyyy") + "'"
				);
			});
			...
			Task<List<Komercijalno.RobaUMagacinu>> rums =
				Komercijalno.RobaUMagacinu.ListByMagacinIDAsync(magacinID);
			...
			while (ubacenoStavki < 5 && nTries < 10)
			{
				...
				while (popisnaGrupa.Length == 1)
				{
					try
					{
						popisnaGrupa += roba.Result[Random.Next(roba.Result.Count - 1)]
							.KatBrPro.Split('?')[1];
					}
					catch { }
				}
				...
				List<Komercijalno.Roba> robaZaPopis = roba
					.Result.Where(x => x.KatBrPro.Contains(popisnaGrupa))
					.ToList();
				...
				foreach (Komercijalno.Roba r in robaZaPopis)
				{
					...
					Komercijalno.RobaUMagacinu rum = rums.Result.FirstOrDefault(x =>
						x.RobaID == r.ID
					);
			...
			TDOffice.Poruka.Insert(
				new TDOffice.Poruka()
				{
					...
					Tekst =
						"Popisi i dopuni robu u popisu. Pristupi popisu klikom na dugme gore desno."
				}
			);
		}
	}
