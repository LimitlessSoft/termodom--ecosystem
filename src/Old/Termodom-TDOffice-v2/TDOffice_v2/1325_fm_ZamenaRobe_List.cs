...
namespace TDOffice_v2
{
	public partial class _1325_fm_ZamenaRobe_List : Form
	{
		...
		{
			this.cmb_Magacin.SelectedValue =
				Program.TrenutniKorisnik.MagacinID > 0 ? Program.TrenutniKorisnik.MagacinID : -1;
		...
		private void ReloadData()
		{
			...
			List<TDOffice.DokumentZamenaRobe> zameneRobe = TDOffice.DokumentZamenaRobe.List(
				$@"DATUM >= '{odDatuma_dtp.Value.Date.ToString("dd.MM.yyyy")}'
                    AND DATUM <= '{doDatuma_dtp.Value.Date.AddDays(1).ToString("dd.MM.yyyy")}'"
					+ (mid > 0 ? $" AND MAGACINID = {mid}" : "")
			);
			...
			foreach (TDOffice.DokumentZamenaRobe p in zameneRobe)
			{
				...
				Komercijalno.Magacin mag = _magaciniKomercijalno.FirstOrDefault(x =>
					x.ID == p.MagacinID
				);
				dr["Magacin"] =
				...
				dr["UserID"] = p.UserID;
				dr["Referent"] = _korisnici
					.Result.Where(x => x.ID == p.UserID)
					.FirstOrDefault()
					.Username;
		...
		private void nova_btn_Click(object sender, EventArgs e)
		{
			...
			if (
				MessageBox.Show(
					"Da li zelite da kreirate nalog za magacin \n " + this.cmb_Magacin.Text,
					"Zamena robe ",
			...
			int newID = TDOffice.DokumentZamenaRobe.Insert(
				Program.TrenutniKorisnik.ID,
				Convert.ToInt32(cmb_Magacin.SelectedValue),
				0,
				null,
				null
			);
