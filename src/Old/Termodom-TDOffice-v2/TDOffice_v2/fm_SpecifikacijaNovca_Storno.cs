...
namespace TDOffice_v2
{
	public partial class fm_SpecifikacijaNovca_Storno : Form
	{
		...
		private TDOffice.Config<Dictionary<int, Dictionary<int, int>>> _brojeviStornoNaloga =
			TDOffice.Config<Dictionary<int, Dictionary<int, int>>>.Get(
				TDOffice.ConfigParameter.NalogZaIspravkuPoslednjiBroj
			); // magacin brMpRacuna dodeljeniBroj
		...
		public fm_SpecifikacijaNovca_Storno(
			TDOffice.SpecifikacijaNovca specifikacijaNovca,
			StornoTip stornoTip
		)
		{
			InitializeComponent();
		...
		{
			_danasnjiMPRacuni = Komercijalno
				.Dokument.ListByMagacinID(DateTime.Now.Year, _specifikacijaNovca.MagacinID)
				.Where(x => x.VrDok == 15 && x.Datum == _specifikacijaNovca.Datum)
				.ToList();
			...
			foreach (Komercijalno.Dokument dok in _danasnjiMPRacuni)
			{
				...
				dr["NacinPlacanja"] =
					dok.NUID == Komercijalno.NacinUplate.Gotovina
						? "Gotovina"
						: dok.NUID == Komercijalno.NacinUplate.Kartica
							? "Kartica"
							: dok.NUID == Komercijalno.NacinUplate.Odlozeo
								? "Odlozeno"
								: "Virman";
		...
		{
			List<int> stornoMPRacuni =
				_tip == StornoTip.KasaDuplo
					? _specifikacijaNovca.Tag.StornoKasaDuplo
					: _specifikacijaNovca.Tag.StorniraniMPRacuni;

			DataTable dt = _mpRacuniDT.Clone();

			foreach (
				Komercijalno.Dokument dok in _danasnjiMPRacuni.Where(x =>
					!stornoMPRacuni.Contains(x.BrDok)
				)
			)
			{
		...
		private void PrikaziStorniraneMPRacune()
		{
			...
			dataGridView2.Columns["Vrednost"].DefaultCellStyle.Alignment =
				DataGridViewContentAlignment.MiddleRight;
		}
			...
			if (!stornoMPRacuni.Contains(brDok))
			{
				...
				if (Komercijalno.Dokument.Get(DateTime.Now.Year, 15, brDok).Placen == 1)
				{
					...
					if (!_brojeviStornoNaloga.Tag.ContainsKey(_specifikacijaNovca.MagacinID))
						_brojeviStornoNaloga.Tag[_specifikacijaNovca.MagacinID] =
							new Dictionary<int, int>();

					int poslednji =
						_brojeviStornoNaloga.Tag[_specifikacijaNovca.MagacinID].Values.Count == 0
							? 0
							: _brojeviStornoNaloga.Tag[_specifikacijaNovca.MagacinID].Values.Max();
