...
namespace TDOffice_v2
{
	public static class SMSGateway
	{
		...
		public static bool PosaljiSms(string mobilni, string text, string set = null)
		{
			try
			{
				TDOffice.Partner partner = TDOffice
					.Partner.List($"MOBILNI = '{MobileNumber.Collate(mobilni)}'")
					.FirstOrDefault();
				...
				byte[] bytes = Encoding.ASCII.GetBytes(
					JsonConvert.SerializeObject(new SMS() { Mobile = mobilni, Text = text })
				);
				...
				{
					Debug.Log(
						"Error sending new message to socket! "
				...
				Debug.Log(
					"Socket Waiting Response... " + DateTime.Now.ToString("[ dd.MM.yyyy HH:mm:ss ]")
				);
