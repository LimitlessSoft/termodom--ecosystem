...
namespace TDOffice_v2
{
	public partial class fm_TabelarniPregledIzvoda : Form
	{
		private class TabelarniPregledIzvodaGetDto
		{
			...
			public decimal NovoStanje
			{
				get => UnosPotrazuje - UnosDuguje;
			}
		...
		private void UcitajPodatke()
		{
			...
			{
				var response = TDAPI
					.GetAsync<List<TabelarniPregledIzvodaGetDto>>("/tabelarni-pregled-izvoda")
					.GetAwaiter()
					.GetResult();
				...
				{
					MessageBox.Show(
						"Doslo je do greske prilikom getovanja '/tabelarni-pregled-izvoda'"
					);
					this.Enabled = true;
					return;
				}
				...
				foreach (var item in response.Payload)
				{
					...
					dataRow["Korisnik"] =
						item.TagId == null ? Program.TrenutniKorisnik.ID : item.Korisnik;
					dt.Rows.Add(dataRow);
				}
		...
		)
		{
			...
			{
				var response = TDAPI
					.PutAsync<DokumentTagizvodPutRequest, DokumentTagIzvodGetDto>(
						"/tabelarni-pregled-izvoda",
				...
				if (response.NotOk)
					MessageBox.Show(
						"Greska prilikom azuriranja podataka u bazi! PUT '/tabelarni-pregled-izvoda'"
					);
