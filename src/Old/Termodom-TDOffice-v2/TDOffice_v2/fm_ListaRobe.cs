	...
	public partial class fm_ListaRobe : Form
	{
		...
		private void NapuniDGV()
		{
			...
			foreach (int rid in robaUListi)
			{
				...
				{
					MessageBox.Show(
						"Doslo je do greske. U listi postoji roba koje nema u sifarniku!"
					);
					return;
				}
		...
		private void obrisiSveStavkeToolStripMenuItem_Click(object sender, EventArgs e)
		{
			if (
				MessageBox.Show(
					"Da li zelite da obrisete sve stavke iz liste?",
					"Lista robe brisanje stavki",
		...
		private void tsbtn_SacuvajListu_Click(object sender, EventArgs e)
		{
			...
			using (InputBox box = new InputBox("Ime liste", "Unesite ime liste"))
			{
				...
				if (File.Exists(destinationPath + listName + ".rlt"))
					if (
						MessageBox.Show(
							"Lista sa ovim imenom vec postoji. Da li ipak zelite sacuvati? Stara lista ce biti obrisana!"
						) != DialogResult.Yes
					)
						return;

				File.WriteAllLines(
					destinationPath + listName + ".rlt",
					Komercijalno.File.GenerisiRLT(robaUListi)
				);
			}
		}
		...
		private void tsbtn_OtvoriListu_Click(object sender, EventArgs e)
		{
			...
			System
				.Windows.Forms.Application.OpenForms.OfType<Main>()
				.FirstOrDefault()
				.Invoke(
					(MethodInvoker)
						delegate
			...
			if (this.dataGridView1.RowCount > 0)
			{
				if (
					MessageBox.Show(
						"Da li zelite ukloniti postojece stavke pre otvaranja liste?",
						"Ukloniti stavke?",
						MessageBoxButtons.YesNo
					) == DialogResult.Yes
				)
