...
namespace TDOffice_v2
{
	public partial class fm_Brain_Index : Form
	{
		...
		private async Task UcitajFtpPodatke()
		{
			...
			{
				FTPDTO dto = JsonConvert.DeserializeObject<FTPDTO>(
		...
		private async Task UcitajOdredistaBazaKomercijalnogPoslovanja()
		{
			...
			List<Termodom.Data.Entities.DBSettings.ConnectionInfo> putanjeDoBaza =
		...
		private async void button1_Click(object sender, EventArgs e)
		{
			...
			var response = await TDBrain_v3.PostAsync(
				"/dbsettings/firebird/servername/set",
				new Dictionary<string, string>() { { "serverName", txt_fbdbServerName.Text } }
			);
		...
		private async void button3_Click(object sender, EventArgs e)
		{
			...
			var response = await TDBrain_v3.PostAsync(
				"/dbsettings/firebird/password/set",
				new Dictionary<string, string>() { { "password", txt_fbdbPassword.Text } }
			);
		...
		private async void button4_Click(object sender, EventArgs e)
		{
			...
			foreach (DataGridViewRow row in dataGridView1.Rows)
			{
				...
				var response1 = await TDBrain_v3.PostAsync(
					"/dbsettings/baza/komercijalno/addorupdate",
		...
		private async void button6_Click(object sender, EventArgs e)
		{
			...
			var response = await TDBrain_v3.PostAsync(
				$"/dbsettings/komercijalno/config/path/set",
				new Dictionary<string, string>() { { "connString", vudimConfigBaza_txt.Text } }
			);
			if ((int)response.StatusCode != 200)
			{
				MessageBox.Show(
					"Greska prilikom azuriranja connection string komercijalno config!"
				);
				return;
			}
		...
		private async void button5_Click(object sender, EventArgs e)
		{
			...
			var response = await TDBrain_v3.PostAsync(
				$"/ftpsettings/password/set?password={vrednost}"
			);
			if ((int)response.StatusCode == 200)
			{
		...
		private async void button8_Click(object sender, EventArgs e)
		{
			...
			var response = await TDBrain_v3.PostAsync(
				$"/ftpsettings/username/set?username={vrednost}"
			);
			if ((int)response.StatusCode == 200)
			{
