	...
	partial class Main
	{
		...
		private void StartTaskCheckerLoopAsync()
		{
			...
			taskCheckerLoop = Task.Run(async () =>
			{
				...
				{
					List<TDOffice.TDTaskItem> taskoviTrenutnogKorisnika =
						TDOffice.TDTaskItem.ListByUserID(Program.TrenutniKorisnik.ID);

					foreach (TDOffice.TDTaskItem item in taskoviTrenutnogKorisnika)
					{
						if (
							item.Done != null
							&& (DateTime.Now - (DateTime)item.Done).TotalMinutes < item.Interval
						)
							continue;
						...
						switch ((TDOffice.TDTask)item.TDTaskID)
						{
							...
							case TDOffice.TDTask.ProveraDaLiImaPraznihFakturaUPoslednjih15Dana:
								if (
									Komercijalno
										.Dokument.List(
											"MAGACINID = 50 AND VRDOK = 13 AND FLAG = 0 AND DATUM >= '"
									...
									for (int i = 0; i < 2; i++)
										Komercijalno.Dokument.Insert(
											DateTime.Now.Year,
											13,
											"TDAuto",
											null,
											null,
											(int)Komercijalno.NacinUplate.Virman,
											50,
											null,
											50
										);
							...
							case TDOffice.TDTask.PrometMagacina:
								List<int> magacini = new List<int>()
								{
									12,
									13,
									14,
								...
								TDOffice.Poruka.Insert(
									new TDOffice.Poruka()
									{
										...
										Tekst = string.Join(
											Environment.NewLine,
											" ",
											"<TASK> Promet Magacina. Pogledaj prilog gore desno!"
										)
									}
								);
								break;
							case TDOffice.TDTask.MinimalneZalihe:
								Task<List<Komercijalno.Roba>> roba = Komercijalno.Roba.ListAsync(
									DateTime.Now.Year
								);
								Task<List<Komercijalno.RobaUMagacinu>> robaUMagacinu =
									Komercijalno.RobaUMagacinu.ListAsync();
								List<int> magaciniPP = new List<int>()
								{
									12,
									13,
									14,
								...
								TDOffice.Config<double> koeficijentConfig =
									TDOffice.Config<double>.Get(
										TDOffice.ConfigParameter.KoeficijentMinZalihe
									);
								if (koeficijentConfig.Tag <= 0)
								{
									MessageBox.Show(
										$"Koeficijent u globalnim podseavanjima ima vrednost {koeficijentConfig.Tag} koja nije validna. Vrednost mora biti veca od 0!"
									);
									return;
								}

								DataTable pt = Models.Izvestaj.ProdajaRobeZaDatiPeriod_Kolicinski(
									magaciniPP,
								...
								finalDT.Columns.Add(
									"ProsecnaProdajaPoslednja3Meseca",
									typeof(double)
								);
								...
								foreach (DataRow dr in pt.Rows)
								{
									...
									Komercijalno.Roba r = roba.Result.FirstOrDefault(x =>
										x.ID == robaID
									);
									Komercijalno.RobaUMagacinu rum =
										robaUMagacinu.Result.FirstOrDefault(x =>
											x.MagacinID == magacinID && x.RobaID == robaID
										);

									if (
										rum == null
										|| prosekProdaje < rum.Stanje * koeficijentConfig.Tag
									)
										continue;
								...
								TDOffice.Poruka.Insert(
									new TDOffice.Poruka()
									{
										...
										Tekst = string.Join(
											Environment.NewLine,
											" ",
											"<TASK> U prilogu se nalazi izvestaj minimalnih zaliha magacina. Pogledaj prilog gore desno!"
										)
									}
								);
								break;
							case TDOffice.TDTask.PrekomerneZalihe:
								Task<List<Komercijalno.Roba>> robapz = Komercijalno.Roba.ListAsync(
									DateTime.Now.Year
								);
								Task<List<Komercijalno.RobaUMagacinu>> robaUMagacinupz =
									Komercijalno.RobaUMagacinu.ListAsync();
								List<int> magacinipz = new List<int>()
								{
									12,
									13,
									14,
								...
								TDOffice.Config<double> koeficijentConfigpz =
									TDOffice.Config<double>.Get(
								...
								{
									MessageBox.Show(
										$"Koeficijent u globalnim podseavanjima ima vrednost {koeficijentConfigpz.Tag} koja nije validna. Vrednost mora biti veca od 0!"
									);
									return;
								}
								DataTable pz = Models.Izvestaj.ProdajaRobeZaDatiPeriod_Kolicinski(
									magacinipz,
									DateTime.Now.AddMonths(-3),
									DateTime.Now
								);
								...
								foreach (DataRow dr in pz.Rows)
								{
									...
									Komercijalno.Roba r = robapz.Result.FirstOrDefault(x =>
										x.ID == robaID
									);
									Komercijalno.RobaUMagacinu rum =
										robaUMagacinupz.Result.FirstOrDefault(x =>
											x.MagacinID == magacinID && x.RobaID == robaID
										);

									if (
										rum == null
										|| prosekProdaje >= rum.Stanje * koeficijentConfigpz.Tag
									)
										continue;
								...
								TDOffice.Poruka.Insert(
									new TDOffice.Poruka()
									{
										...
										Tekst = string.Join(
											Environment.NewLine,
											" ",
											"<TASK> U prilogu se nalazi izvestaj prekomernih zaliha magacina. Pogledaj prilog gore desno!"
										)
									}
								);
								break;
							case TDOffice.TDTask.NedovrsenaRazduzenja:
								...
								List<Komercijalno.Magacin> magaciniNR = Komercijalno
									.Magacin.ListAsync()
									.Result;
								...
								{
									Komercijalno.Magacin m = magaciniNR.FirstOrDefault(x =>
										x.ID == drz.MagacinID
									);
									TDOffice.User k = korisnici.FirstOrDefault(x =>
										x.ID == drz.KorisnikID
									);
								...
								TDOffice.Poruka.Insert(
									new TDOffice.Poruka()
									{
										...
										Tekst = string.Join(
											Environment.NewLine,
											" ",
											"<TASK> U prilogu se nalazi izvestaj nedovrsenih razduzenja magacina. Pogledaj prilog gore desno!"
										)
									}
								);
								break;
							case TDOffice.TDTask.TimeKeeper:
								var res = await TDBrain_v3.PostAsync(
									"/Komercijalno/TimeKeeper/Update"
								);
								break;
		...
		private void PosaljiIzvestajNezakljucanihMPKalkulacijaKaoPoruku()
		{
			...
			output.Add("=======================");
			output.Add(
				"Izvestaj nezakljucanih MP kalkulacija ( neisporucena dokumenta stovarista )"
			);
			output.Add("=======================");
			for (int z = 0; z < 2; z++)
			{
				...
				using (
					FbConnection con = new FbConnection(
						Komercijalno.Komercijalno.CONNECTION_STRING[DateTime.Now.Year - z]
					)
				)
				{
					con.Open();

					List<Komercijalno.Dokument> mpKalkulacije = Komercijalno.Dokument.ListByVrDok(
						con,
						18
					);
					...
					for (int i = 0; i < mpKalkulacije.Count; i++)
					{
						List<Komercijalno.Stavka> stavke = Komercijalno.Stavka.ListByDokument(
							con,
							mpKalkulacije[i].VrDok,
							mpKalkulacije[i].BrDok
						);
						if (stavke.Count == 0)
					...
					else
					{
						output.Add("----------------------");
						output.Add(
							"[Magacin]: [ukupno nezakljucanih kalkulacija] --- [nezakljucane kalkulacije starije od 7 dana]"
						);
						output.Add("----------------------");
						for (int i = 12; i <= 28; i++)
							output.Add(
								$"M{i}: {mpKalkulacije.Count(x => x.MagacinID == i)} --- {mpKalkulacije.Count(x => x.MagacinID == i && x.Datum.AddDays(7) < DateTime.Now)}"
							);
						output.Add("----------------------");
					}
				}
		...
		public static void PosaljiIzvestajDokumenataUKojimaJeMarzaManjaOdProsecneKaoPoruku()
		{
			...
			int dana = 5;
			List<string> output = new List<string>();
			List<Dokument> dokumentiKomercijalno = Komercijalno.Dokument.List(
				"DATUM >= '"
					+ DateTime.Now.AddDays(-dana).ToString("dd.MM.yyyy")
					+ "' AND (VRDOK = 15 OR VRDOK = 13 OR VRDOK = 32)"
			);

			// vrdok, brdok, ostvarena marza, prodajna vrednost, dati rabat
			List<Tuple<int, int, double, double, double>> marze =
				new List<Tuple<int, int, double, double, double>>();

			List<Dokument> dokumentiNabavke = Dokument.List(
				$"MAGACINID = 50 AND VRDOK IN (0, 1, 2, 36)"
			);
			List<Stavka> stavkeNabavke = Komercijalno.Stavka.List(
				DateTime.Now.Year,
				"MAGACINID = 50 AND VRDOK IN (0, 1, 2, 36)"
			);

			Program.IzvestajNelogicnihMarziMaxStage = dokumentiKomercijalno.Count(x =>
				x.Potrazuje != 0
			);
			Program.IzvestajNelogicnihMarziTrenutniStage = 0;
			Parallel.ForEach(
				dokumentiKomercijalno.Where(x => x.Potrazuje != 0),
				dok =>
				{
					double realizovanaMarza = dok.OstvarenaMarza(
						dokumentiNabavke,
						stavkeNabavke,
						Dokument.OstvarenaMarzaTipNabavneCene.NaDanDokumenta
					);
					double rabat = ((dok.Potrazuje / dok.Duguje) - 1) * -100;
					marze.Add(
						new Tuple<int, int, double, double, double>(
							dok.VrDok,
							dok.BrDok,
							realizovanaMarza,
							dok.Potrazuje,
			...
			if (marze.Count != 0)
			{
				double ukupnaProdajnaVrednost = marze
					.Where(x => x.Item1 == 15 || x.Item1 == 13)
					.Sum(x => x.Item4);
				double ukupnaNabavnaVerdnost = marze
					.Where(x => x.Item1 == 15 || x.Item1 == 13)
					.Sum(x => x.Item4 - (x.Item4 * (x.Item3 / (100 + x.Item3))));
				double ukupanRUCProcenat =
					(ukupnaProdajnaVrednost / ukupnaNabavnaVerdnost - 1) * 100;

				//double prosecnaMarza = marze.Where(x => x.Item1 == 15 || x.Item1 == 13).Sum(x => x.Item3) / marze.Count();

				output.Add(
					$"Realizovana RUC u validnim dokumentima (15, 13) u proteklih {dana} dana je {ukupanRUCProcenat.ToString("0.00")}%"
				);
				...
				foreach (
					var m in marze.Where(x =>
						x.Item3 >= 0 && x.Item3 < ukupanRUCProcenat && x.Item5 != 0
					)
				)
					output.Add(
						$"VRDOK: {m.Item1} - BRDOK: {m.Item2} - MARZA:  {m.Item3.ToString("0.00\\%")} - RABAT: {m.Item5.ToString("0.00\\%")}"
					);
