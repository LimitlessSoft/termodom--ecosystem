...
public class PromenaManager(ILogger<PromenaManager> logger, KomercijalnoDbContext dbContext)
    : LSCoreManagerBase<PromenaManager>(logger, dbContext), IPromenaManager
{
    public List<PromenaDto> GetMultiple(PromenaGetMultipleRequest request) =>
        dbContext.Promene
        .Where(x =>
            (string.IsNullOrWhiteSpace(request.KontoStartsWith) || x.Konto.StartsWith(request.KontoStartsWith))
