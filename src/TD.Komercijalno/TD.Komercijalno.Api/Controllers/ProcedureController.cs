...
namespace TD.Komercijalno.Api.Controllers
{
    ...
    public class ProcedureController (IProcedureManager procedureManager) : Controller
    {
        ...
        public List<ProdajnaCenaNaDanDto> ProdajnaCenaNaDanOptimized([FromQuery] ProceduraGetProdajnaCenaNaDanOptimizedRequest request) =>
            procedureManager.GetProdajnaCenaNaDanOptimized(request);

        [HttpGet]
        [Route("/procedure/nabavna-cena-na-dan")]
        public List<NabavnaCenaNaDanDto> NabavnaCenaNaDan([FromQuery] ProceduraGetNabavnaCenaNaDanRequest request) =>
