...
namespace TDOffice_v2
{
    public partial class Menadzment_RazduziLager0634_Index : Form
    {
        ...
        private Task<TDOffice.Config<Dictionary<int, Dictionary<int, double>>>> _lager { get; set; } = TDOffice.Config<Dictionary<int, Dictionary<int, double>>>.GetAsync(TDOffice.ConfigParameter.Lager0634);
        // magacin, List<brDok34>
        private Task<TDOffice.Config<Dictionary<int, List<int>>>> _razduzene34 { get; set; } = TDOffice.Config<Dictionary<int, List<int>>>.GetAsync(TDOffice.ConfigParameter.Lager0634RazduzenaDokumenta);
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            Task.Run(() =>
            {
                ...
                using (FbConnection con = new FbConnection(Komercijalno.Komercijalno.CONNECTION_STRING[_godina]))
                {
                    con.Open();
                    this.Invoke((MethodInvoker)delegate
                    {
                    ...
                    List<Komercijalno.Dokument> nerazduzene34 = Komercijalno.Dokument.List(con, "VRDOK = 34 AND MAGACINID = " + magacinID);
                    ...
                    if (!_razduzene34.Result.Tag.ContainsKey(magacinID) || _razduzene34.Result.Tag[magacinID] == null)
                    ...
                    if (_razduzene34.Result.Tag[magacinID].Count > 0) // Ovo samo zbog perfomansi
                        nerazduzene34.RemoveAll(x => _razduzene34.Result.Tag[magacinID].Contains(x.BrDok));

                    if(_lager.Result.Tag == null)
                        _lager.Result.Tag = new Dictionary<int, Dictionary<int, double>>();

                    if (!_lager.Result.Tag.ContainsKey(magacinID) || _lager.Result.Tag[magacinID] == null)
                    ...
                    List<Komercijalno.RobaUMagacinu> robaUMagacinu = Komercijalno.RobaUMagacinu.ListByMagacinID(con, magacinID);
                    ...
                    this.Invoke((MethodInvoker)delegate
                    {
                        status_lbl.Text = "Prolazim kroz trenutni lager i punim buffer stavki za dodati u lager...";
                    });
                    ...
                    this.Invoke((MethodInvoker)delegate
                    {
                        status_lbl.Text = "Prolazim kroz nerazduzene 34 i dodajem u buffer stavki za dodati u lager...";
                    });
                    foreach (Komercijalno.Dokument dok in nerazduzene34)
                    {
                        _razduzene34.Result.Tag[magacinID].Add(dok.BrDok);

                        foreach(Komercijalno.Stavka s in Komercijalno.Stavka.ListByDokument(con, 34, dok.BrDok))
                            if (s.Vrsta == 1)
