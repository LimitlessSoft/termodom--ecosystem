...
namespace TDOffice_v2
{
    public partial class _1366_fm_NalogZaPrevoz_Odrediste : Form
    {
        ...
        public _1366_fm_NalogZaPrevoz_Odrediste(TDOffice.NalogZaPrevoz.Destinacija destinacija, TDOffice.NalogZaPrevoz nalogZaPrevoz)
        {
            InitializeComponent();
        ...
        private void SetUI()
        {
            ...
            if ((int)_destinacija.NacinPlacanja == 1)
            {
                try
                {
                    List<Komercijalno.Stavka> stavke = Komercijalno.Stavka.ListByDokument(DateTime.Now.Year, 15, Convert.ToInt32(miKupcuNaplatili_txt.Text)).Where(x => x.MagacinID == _nalogZaPrevoz.MagacinID).ToList();
                    ...
                    miKupcuNaplatili_lbl.Text = stavke.Where(x => uslugaIDPrevoza.Contains(x.RobaID)).Sum(x => x.ProdCenaBP * x.Kolicina).ToString("#,##0.00 RSD + PDV");
                }
                catch (Exception)
                {
                    miKupcuNaplatili_lbl.Text = "Nije pronadjena stavka prevoza u datom mp racunu";
        ...
        private void _1366_fm_NalogZaPrevoz_Odrediste_FormClosed(object sender, FormClosedEventArgs e)
        {
            if (_izmena == true)
            {
        ...
        {
            if ((veza_cmb.SelectedItem as Tuple<int, string>).Item1 == Convert.ToInt32(TDOffice.Enums.NalogZaPrevozOsnov.MPRacun))
            {
                try
                {
                    Komercijalno.Dokument dok = Komercijalno.Dokument.Get(_nalogZaPrevoz.Datum.Year, 15, Convert.ToInt32(veza_txt.Text.ToString()));

                    if (dok == null) { MessageBox.Show("Dokument veza" + this.veza_txt.Text + " nije pronadjen"); return; }
                    if (dok.MagacinID != _nalogZaPrevoz.MagacinID) { MessageBox.Show("Dokument veza" + this.veza_txt.Text + " ne pripada vasem magacinu"); return; }
                    if (dok.Flag != 1) { MessageBox.Show("Dokument veza" + this.veza_txt.Text + " nije zakljucan"); return; }
            ...
            if ((miKupcuNaplatili_cmb.SelectedItem as Tuple<int, string>).Item1 == (int)TDOffice.Enums.NalogZaPrevozOdredisteNacinPlacanja.KrozRacun)
            {
                // Korisnik je slektovao da je prevoz u mp racunu
                try
                {
        ...
        private void veza_cmb_SelectedIndexChanged(object sender, EventArgs e)  
        {
            ...
            _destinacija.Osnov = (TDOffice.Enums.NalogZaPrevozOsnov)(veza_cmb.SelectedItem as Tuple<int, string>).Item1;
            _izmena = true;
        }
        ...
        private void miKupcuNaplatili_cmb_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            _destinacija.NacinPlacanja = (TDOffice.Enums.NalogZaPrevozOdredisteNacinPlacanja)(miKupcuNaplatili_cmb.SelectedItem as Tuple<int, string>).Item1;
            _izmena = true;
