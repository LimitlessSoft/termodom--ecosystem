...
namespace TDOffice_v2
{
    ...
    {
        private TDOffice.Config<Dictionary<string, List<int>>> _grupeTroskova = TDOffice.Config<Dictionary<string, List<int>>>.Get(TDOffice.ConfigParameter.GrupeTroskova);
        ...
        {
            using (InputBox ib = new InputBox("Nova Grupa Troskova", "Unesite naziv nove grupe troskova"))
            {
                ib.ShowDialog();
        ...
        private void dataGridView1_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            ...
            foreach (Komercijalno.Roba r in _roba.Result.Where(x => x.Vrsta != 1 && x.Naziv.ToLower().Contains("trosak")))
            {
                DataRow dr = dt.NewRow();
                dr["ID"] = r.ID;
                dr["Naziv"] = r.Naziv;
        ...
        {
            string trenutnaGrupa = grupa_cmb.SelectedValue.ToString();
            if (MessageBox.Show("Da li sigurno zelite obrisati grupu: " + trenutnaGrupa, "Potvrdi!", MessageBoxButtons.YesNo) != DialogResult.Yes)
                return;
