...
namespace TDOffice_v2.Forms
{
    public partial class fm_WebUredjivanjeProizvoda : Form
    {
        ...
        private Task LoadDataAsync()
        {
            ...
            {
                var tableData = await TDAPI.GetAsync<List<WebUredjivanjeProizvodaProizvodiGetDto>>("/web-uredjivanje-proizvoda/proizvodi");
                if (tableData.Status != System.Net.HttpStatusCode.OK)
                {
                    MessageBox.Show($"Greska u komunikaciji sa API serverom! [{tableData.Status.ToString()}]");
                ...
                this.Invoke((MethodInvoker)delegate
                {
                    ...
                    dataGridView1.Columns["KatBr Komercijalno"].DefaultCellStyle.BackColor = KomercijalnoInfoBoja;
                    dataGridView1.Columns["Naziv Komercijalno"].DefaultCellStyle.BackColor = KomercijalnoInfoBoja;
                    dataGridView1.Columns["JM Komercijalno"].DefaultCellStyle.BackColor = KomercijalnoInfoBoja;
