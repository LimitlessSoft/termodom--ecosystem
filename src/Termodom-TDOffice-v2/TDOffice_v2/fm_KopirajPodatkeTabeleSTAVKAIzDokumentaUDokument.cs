...
namespace TDOffice_v2
{
    public partial class fm_KopirajPodatkeTabeleSTAVKAIzDokumentaUDokument : Form
    {
        ...
        private void fm_KopirajPodatkeTabeleSTAVKAIzDokumentaUDokument_Load(object sender, EventArgs e)
        {
        ...
        private async void LoadAsync()
        {
            ...
            foreach (DistinctConnectionInfo csi in await distinctPutanjeDoBazaTask)
            {
                ...
                putanjeDoBazaTuple.Add(new Tuple<string, string>(csi.PutanjaDoBaze, $"{csi.Godina} - {putanjaParts[putanjaParts.Length - 1]}"));
            }
        ...
        {
            if (MessageBox.Show("Da li sigurno zelite pokrenuti ovu akciju? Proverite jos jednom baze!!!", "Potvrdi", MessageBoxButtons.YesNo) != DialogResult.Yes)
            {
            ...
            try
            {
                ...
                var izvorniString = $"data source=4monitor; initial catalog = {izvornaBaza}; user=SYSDBA; password=m";
                var destinacioniString = $"data source=4monitor; initial catalog = {destinacionaBaza}; user=SYSDBA; password=m";
                ...
                using (FbConnection con = new FbConnection(izvorniString))
                {
                    ...
                    using (FbCommand cmd = new FbCommand("SELECT COUNT(*) FROM DOKUMENT WHERE VRDOK = @VRDOK AND BRDOK = @BRDOK", con))
                    {
                    ...
                    using (FbCommand cmd = new FbCommand(@$"SELECT ROBAID, {izvornoPolje}
                        FROM STAVKA
                        WHERE VRDOK = @VRDOK AND BRDOK = @BRDOK", con))
                    {
                        ...
                        using (FbDataReader dr = cmd.ExecuteReader())
                            while (dr.Read())
                                izvorneVrednosti.Add(Convert.ToInt32(dr["ROBAID"]), dr[izvornoPolje] is DBNull ?
                                    null :
                ...
                using (FbConnection con = new FbConnection(destinacioniString))
                {
                    ...
                    SetStatus("Proveravam da li destinacioni dokument postoji...");
                    using (FbCommand cmd = new FbCommand("SELECT COUNT(*) FROM STAVKA WHERE VRDOK = @VRDOK AND BRDOK = @BRDOK", con))
                    {
                        cmd.Parameters.AddWithValue("@VRDOK", destinacioniVrDok.VrDok);
                        cmd.Parameters.AddWithValue("@BRDOK", destinacioniBrDok);
                    ...
                    using (FbCommand cmd = new FbCommand(@$"UPDATE STAVKA SET
                        {destinacionoPolje} = @VREDNOST WHERE VRDOK = @VRDOK AND BRDOK = @BRDOK AND ROBAID = @ROBAID", con))
                    {
                        cmd.Parameters.AddWithValue("@VRDOK", destinacioniVrDok.VrDok);
                        cmd.Parameters.AddWithValue("@BRDOK", destinacioniBrDok);
