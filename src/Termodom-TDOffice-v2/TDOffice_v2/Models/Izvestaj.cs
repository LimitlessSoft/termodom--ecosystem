...
namespace TDOffice_v2.Models
{
    public static class Izvestaj
    {
        ...
        /// </summary>
        /// <returns></returns>
        public static DataTable ProdajaRobePoRealnimCenama(Models.Periodi periodi, List<int> magacini, List<int> listaRobe)
        {
            // Primer
            ...
            if (listaRobe != null && listaRobe.Count == 0)
                throw new Exception("Lista robe nije null ali je prazna. Morate je popuniti ili deklarisati kao null za svu robu!");

            DataTable dt = new DataTable();
            foreach (int godina in periodi.Godine.OrderByDescending(x => x))
            {
                string realnaVrednostCommand = @"
                    SELECT s.MAGACINID, r.NAZIV, s.ROBAID, COALESCE(SUM(s.KOLICINA), 0) as KGOD" + godina + @", r.JM,
                    COALESCE(SUM((s.KOLICINA * s.PRODCENABP * (1 + (t.STOPA / 100)))), 0) AS VGOD" + godina + @"
                    FROM STAVKA s
                ...
                using (FbConnection con = new FbConnection(Komercijalno.Komercijalno.CONNECTION_STRING[godina]))
                {
                    ...
                    using (FbCommand cmd = new FbCommand(realnaVrednostCommand, con))
                    {
                        ...
                        using (FbDataAdapter da = new FbDataAdapter(cmd))
                        {
                            ...
                            {
                                da.Fill(dt);
                                dt.PrimaryKey = new DataColumn[] { dt.Columns["ROBAID"], dt.Columns["MAGACINID"] };
                                continue;
                            }

                            da.Fill(tempDT);
                            tempDT.PrimaryKey = new DataColumn[] { tempDT.Columns["ROBAID"], tempDT.Columns["MAGACINID"] };
        ...
        public static DataTable ProdajaRobePoRealnimCenama(Models.Periodi periodi, List<int> magacini, List<int> listaRobe, List<int> reperniMagacini)
        {
            ...
            DataTable reperniDT = reperniTemp.AsEnumerable().GroupBy(x => x.Field<int>("ROBAID")).Select(g =>
            {
                ...
                {
                    row["KGOD" + godina] = g.Sum(r => r.Field<decimal?>("KGOD" + godina) is null ? 0 : r.Field<decimal>("KGOD" + godina));
                    row["VGOD" + godina] = g.Sum(r => r.Field<decimal?>("VGOD" + godina) is null ? 0 : r.Field<decimal>("VGOD" + godina));
                }
        ...
        public static DataTable ProdajaRobePoPoslednjimCenama(Models.Periodi periodi, List<int> magacini, List<int> listaRobe)
        {
            // Primer
            // MagacinID    Naziv   RobaID      kGOD2021    kGOD2022    JM      vGOD2021     vGOD2022   
            // 12           GIPS    39          100         200         m2      10000        20000
            ...
            List<Komercijalno.RobaUMagacinu> _komercijalnoRobaUMagacinu50 = Komercijalno.RobaUMagacinu.ListByMagacinID(50);
    ...
    LEFT OUTER JOIN DOKUMENT d ON d.VRDOK = s.VRDOK AND d.BRDOK = s.BRDOK
    LEFT OUTER JOIN ROBA r on s.ROBAID = r.ROBAID
    WHERE d.MAGACINID IN (" + string.Join(", ", magacini) + @") AND (d.VRDOK = 13 OR d.VRDOK = 15) AND DATUM >= @ODDATUMA AND DATUM <= @DODATUMA
    ...
    ORDER BY s.ROBAID";
            ...
            {
                Komercijalno.RobaUMagacinu rum = _komercijalnoRobaUMagacinu50.FirstOrDefault(x => x.RobaID == Convert.ToInt32(row["ROBAID"]));
                Komercijalno.Roba r = _komercijalnoRoba.FirstOrDefault(x => x.ID == Convert.ToInt32(row["ROBAID"]));
                ...
                foreach (int god in periodi.Godine)
                {
                    ...
                    row["VGOD" + god] = err ? 0 : Convert.ToDecimal(Convert.ToDecimal(kol) * Convert.ToDecimal((rum.ProdajnaCena * (1 + (tar.Stopa / 100)))));
                }
            }
            return dt;
        }
        public static DataTable ProdajaRobePoPoslednjimCenama(Models.Periodi periodi, List<int> magacini, List<int> listaRobe, List<int> reperniMagacini)
        {
            DataTable baseDT = ProdajaRobePoPoslednjimCenama(periodi, magacini, listaRobe);
            DataTable reperniTemp = ProdajaRobePoPoslednjimCenama(periodi, reperniMagacini, listaRobe);
            DataTable reperniDT = reperniTemp.AsEnumerable().GroupBy(x => x.Field<int>("ROBAID")).Select(g =>
            {
                var row = reperniTemp.NewRow();
        ...
        public static DataTable OdobreniRabati(List<int> magacini, int godina, DateTime odDat, DateTime doDat)
        {
            DataTable dt = new DataTable();
            using (FbConnection con = new FbConnection(Komercijalno.Komercijalno.CONNECTION_STRING[godina]))
            {
    ...
    FROM DOKUMENT
    WHERE MAGACINID IN (" + string.Join(", ", magacini) + @") AND FLAG = 1 AND (VRDOK = 13 OR VRDOK = 15) AND DATUM >= @ODDATUMA AND DATUM <= @DODATUMA
    GROUP BY MAGACINID", con))
                {
        ...
        public static DataTable ProdajaRobeZaDatiPeriod_Kolicinski(int magacinID, DateTime odDatuma, DateTime doDatuma)
        {
            DataTable dt = new DataTable();
            ...
            foreach(int god in godine)
            {
                using(FbConnection con = new FbConnection(Komercijalno.Komercijalno.CONNECTION_STRING[god]))
                {
                    ...
                    using(FbCommand cmd = new FbCommand(@"SELECT s.ROBAID, COALESCE(SUM(s.KOLICINA), 0) AS KOLICINA
FROM STAVKA s
LEFT OUTER JOIN DOKUMENT d ON d.VRDOK = s.VRDOK AND d.BRDOK = s.BRDOK
WHERE d.VRDOK IN (15, 13) AND d.MAGACINID = @MAGACINID AND d.DATUM >= @ODDATUMA AND d.DATUM <= @DODATUMA
GROUP BY s.ROBAID", con))
                    {
        ...
        /// <returns></returns>
        public static DataTable ProdajaRobeZaDatiPeriod_Kolicinski(List<int> magacini, DateTime odDatuma, DateTime doDatuma)
        {
            List<int> godine = new List<int>();
            for (int i = odDatuma.Year; i <= doDatuma.Year; i++)
                godine.Add(i);
            ...
            foreach (int god in godine)
            {
                using (FbConnection con = new FbConnection(Komercijalno.Komercijalno.CONNECTION_STRING[god]))
                {
                    ...
                    using (FbCommand cmd = new FbCommand(@"SELECT s.MAGACINID, s.ROBAID, COALESCE(SUM(s.KOLICINA), 0) AS KOLICINA
FROM STAVKA s
LEFT OUTER JOIN DOKUMENT d ON d.VRDOK = s.VRDOK AND d.BRDOK = s.BRDOK
WHERE d.VRDOK IN (15, 13) AND d.MAGACINID IN (" + string.Join(", ", magacini) + @") AND d.DATUM >= @ODDATUMA AND d.DATUM <= @DODATUMA
GROUP BY s.MAGACINID, s.ROBAID", con))
                    {
                        cmd.Parameters.AddWithValue("@ODDATUMA", odDatuma);
                        cmd.Parameters.AddWithValue("@DODATUMA", doDatuma);
