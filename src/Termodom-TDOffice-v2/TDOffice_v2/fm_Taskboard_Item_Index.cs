...
namespace TDOffice_v2
{
    public partial class fm_Taskboard_Item_Index : Form
    {
        ...
        private void UcitajKomentare()
        {
            ...
            List<TDOffice.TaskboardTaskKomentar> komentari = TDOffice.TaskboardTaskKomentar.ListByTaskID(_item.ID).OrderBy(x=> x.Datum).ToList();
            foreach (TDOffice.TaskboardTaskKomentar k in komentari)
            {
                ...
                rtb_Komentari.AppendText(k.Datum.ToString() + "  Komentar korisnika  <" + TDOffice.User.Get(k.KorisnikID).Username + ">   <<" + k.Text + ">>\n");
            }

        }
        private void btn_Sacuvaj_Click(object sender, EventArgs e)
        {
            _item.Status = (TDOffice.Taskboard.ItemStatus)Convert.ToInt32(itemStatus_cmb.SelectedValue);
            _item.Update();
        ...
        {
            if (MessageBox.Show("Sigurno zelite da dodate komentar ", "Dodavanje komentara", MessageBoxButtons.YesNo) == DialogResult.No)
            {
                tb_NoviKomentar.Clear();
                return;
            }
                int newID = TDOffice.TaskboardTaskKomentar.Insert(Program.TrenutniKorisnik.ID, _item.ID, DateTime.Now, tb_NoviKomentar.Text.ToString());
            UcitajKomentare();
            tb_NoviKomentar.Clear();
        }
    }
