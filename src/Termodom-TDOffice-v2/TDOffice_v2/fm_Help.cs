...
namespace TDOffice_v2
{
    public partial class fm_Help : Form
    {
        ...
        {
            if (MessageBox.Show("Da li sigurno zelite da sacuvate uradjene izmene?", "Potvrdi", MessageBoxButtons.YesNo) == DialogResult.No)
            {
                UcitajModul();
        ...
        private void fm_Help_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            {
                if (MessageBox.Show("Imate izmene, zelite li da ih sacuvate ?", "Potvrdi", MessageBoxButtons.YesNo) == DialogResult.No)
                {
                    UcitajModul();
                    odustaniOdCuvanjaHelpa_btn.Hide();
                    sacuvajHelp_btn.Hide();
