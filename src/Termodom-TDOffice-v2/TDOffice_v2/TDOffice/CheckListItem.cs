...
namespace TDOffice_v2.TDOffice
{
    public class CheckListItem
    {
        ...
        {
            using(FbCommand cmd = new FbCommand("UPDATE CHECKLIST_ITEM SET DATUM_IZVRSENJA = @DI, INTERVAL_DANA = @INTERVAL WHERE ID = @ID", con))
            {
        ...
        public static int Insert(FbConnection con, CheckList.Jobs job, int korisnikID, int intervalDana)
        {
            using(FbCommand cmd = new FbCommand(@"INSERT INTO CHECKLIST_ITEM (ID, POSAO, KORISNIK_ID, DATUM_IZVRSENJA, INTERVAL_DANA) VALUES (
((SELECT COALESCE(MAX(ID), 0) FROM CHECKLIST_ITEM) + 1),
@POSAO, @KORISNIK, NULL, @INTERVAL) RETURNING ID", con))
            {
        ...
        {
            using(FbCommand cmd = new FbCommand("SELECT ID, POSAO, KORISNIK_ID, DATUM_IZVRSENJA, INTERVAL_DANA FROM CHECKLIST_ITEM WHERE ID = @ID", con))
            {
                cmd.Parameters.AddWithValue("@ID", id);
                ...
                using (FbDataReader dr = cmd.ExecuteReader())
                    if (dr.Read())
                        return new CheckListItem()
                        {
                            ...
                            DatumIzvrsenja = dr["DATUM_IZVRSENJA"] is DBNull ? null : (DateTime?)Convert.ToDateTime(dr["DATUM_IZVRSENJA"]),
        ...
        public static List<CheckListItem> List(FbConnection con)
        {
            ...
            using (FbCommand cmd = new FbCommand("SELECT ID, POSAO, KORISNIK_ID, DATUM_IZVRSENJA, INTERVAL_DANA FROM CHECKLIST_ITEM", con))
            {
                using (FbDataReader dr = cmd.ExecuteReader())
                    while (dr.Read())
        ...
        {
            List<CheckListItem> list = new List<CheckListItem>();
            using (FbCommand cmd = new FbCommand("SELECT ID, POSAO, KORISNIK_ID, DATUM_IZVRSENJA, INTERVAL_DANA FROM CHECKLIST_ITEM WHERE KORISNIK_ID = @KID", con))
            {
