...
namespace TDOffice_v2.TDOffice
{
    public class Magacin
    {
        ...
        {
            using(FbCommand cmd = new FbCommand("UPDATE MAGACIN SET NADREDJENI_MAGACIN = @NM, MAGACIN_RAZDUZENJA = @MR, FIRMAID = @FID, ADRESA = @ADRESA, OPSTINA = @OPSTINA WHERE ID = @ID", con))
            {
        ...
        public static void Insert(int id, int nadredjeniMagacin, int magacinRazduzenja, int firmaID, string adresa, string opstina)
        {
            using(FbConnection con = new FbConnection(TDOffice.connectionString))
            {
                con.Open();
        ...
        public static void Insert(FbConnection con, int id, int nadredjeniMagacin, int magacinRazduzenja, int firmaID, string adresa, string opstina)
        {
            using(FbCommand cmd = new FbCommand("INSERT INTO MAGACIN (ID, NADREDJENI_MAGACIN, MAGACIN_RAZDUZENJA, FIRMAID, ADRESA, OPSTINA) VALUES (@ID, @NM, @MR, @FID, @ADRESA, @OPSTINA)", con))
            {
                cmd.Parameters.AddWithValue("@ID", id);
                cmd.Parameters.AddWithValue("@NM", nadredjeniMagacin);
                cmd.Parameters.AddWithValue("@MR", magacinRazduzenja);
        ...
        {
            using(FbCommand cmd = new FbCommand("SELECT ID, NADREDJENI_MAGACIN, MAGACIN_RAZDUZENJA, FIRMAID, ADRESA, OPSTINA FROM MAGACIN WHERE ID = @ID", con))
            {
                cmd.Parameters.AddWithValue("@ID", id);
        ...
        public static List<Magacin> List(FbConnection con)
        {
            ...
            using(FbCommand cmd = new FbCommand("SELECT ID, NADREDJENI_MAGACIN, MAGACIN_RAZDUZENJA, FIRMAID, ADRESA, OPSTINA FROM MAGACIN", con))
            {
                using (FbDataReader dr = cmd.ExecuteReader())
                    while (dr.Read())
