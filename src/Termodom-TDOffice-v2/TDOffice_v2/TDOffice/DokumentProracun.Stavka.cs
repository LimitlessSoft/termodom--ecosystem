...
namespace TDOffice_v2.TDOffice
{
    public partial class DokumentProracun
    {
        public class Stavka
        {
            ...
            {
                using(FbCommand cmd = new FbCommand("UPDATE DOKUMENT_PRORACUN_STAVKA SET KOLICINA = @K, PRODAJNA_CENA_BEZ_PDV = @PC WHERE ID = @ID", con))
                {
            ...
            {
                using (FbCommand cmd = new FbCommand("SELECT ID, DOKUMENT_ID, ROBAID, KOLICINA, PRODAJNA_CENA_BEZ_PDV FROM DOKUMENT_PRORACUN_STAVKA WHERE ID = @ID", con))
                {
            ...
            public static int Insert(DokumentProracun dokument, int robaID, double kolicina, double prodajnaCenaBezPDV)
            {
                using(FbConnection con = new FbConnection(TDOffice.connectionString))
                {
                    con.Open();
            ...
            public static int Insert(FbConnection con, DokumentProracun dokument, int robaID, double kolicina, double prodajnaCenaBezPDV)
            {
                _updateMRE.Wait();
                _updateMRE.Reset();
                ...
                using(FbCommand cmd = new FbCommand(@"INSERT INTO DOKUMENT_PRORACUN_STAVKA
(ID, DOKUMENT_ID, ROBAID, KOLICINA, PRODAJNA_CENA_BEZ_PDV) VALUES (@ID, @DID, @ROBAID, @KOLICINA, @PCBP)", con))
                {
            ...
            public static List<Stavka> List(FbConnection con)
            {
                ...
                using(FbCommand cmd = new FbCommand("SELECT ID, DOKUMENT_ID, ROBAID, KOLICINA, PRODAJNA_CENA_BEZ_PDV FROM DOKUMENT_PRORACUN_STAVKA", con))
                {
                    using(FbDataReader dr = cmd.ExecuteReader())
                    {
                        while(dr.Read())
            ...
            public static List<Stavka> ListByDokument(FbConnection con, DokumentProracun dokument)
            {
                ...
                using (FbCommand cmd = new FbCommand("SELECT ID, DOKUMENT_ID, ROBAID, KOLICINA, PRODAJNA_CENA_BEZ_PDV FROM DOKUMENT_PRORACUN_STAVKA WHERE DOKUMENT_ID = @DID", con))
                {
                    cmd.Parameters.AddWithValue("@DID", dokument.ID);
                    using (FbDataReader dr = cmd.ExecuteReader())
                    {
            ...
            {
                using(FbCommand cmd = new FbCommand("SELECT COALESCE(MAX(ID), 0) FROM DOKUMENT_PRORACUN_STAVKA", con))
                    using(FbDataReader dr = cmd.ExecuteReader())
                        if (dr.Read())
            ...
            {
                using (FbCommand cmd = new FbCommand("DELETE FROM DOKUMENT_PRORACUN_STAVKA WHERE ID = @ID", con))
                {
            ...
            {
                using (FbCommand cmd = new FbCommand("DELETE FROM DOKUMENT_PRORACUN_STAVKA WHERE DOKUMENT_ID = @ID", con))
                {
