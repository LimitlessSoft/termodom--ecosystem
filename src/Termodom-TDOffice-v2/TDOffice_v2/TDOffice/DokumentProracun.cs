...
namespace TDOffice_v2.TDOffice
{
    public partial class DokumentProracun
    {
        ...
        {
            using(FbCommand cmd =new FbCommand(@"UPDATE DOKUMENT_PRORACUN SET
                USERID = @U,
                PPID = @PPID,
                MAGACINID = @M,
                STATUS = @S,
        ...
        public static int Insert(DateTime datum, int? ppid, int magacinID, int userID, string komentar, string interniKomentar, int nuid)
        {
            using(FbConnection con = new FbConnection(TDOffice.connectionString))
            {
                con.Open();
        ...
        public static int Insert(FbConnection con, DateTime datum, int? ppid, int magacinID, int userID, string komentar, string interniKomentar, int nuid)
        {
            _updateMRE.Wait();
            _updateMRE.Reset();
            ...
            using(FbCommand cmd = new FbCommand(@"INSERT INTO DOKUMENT_PRORACUN (ID, DATUM, PPID, MAGACINID, USERID, STATUS, KOMENTAR, INTERNI_KOMENTAR, NUID) VALUES
                (@ID, @DATUM, @PPID, @MAGACINID, @USERID, @STATUS, @KOMETNAR, @IK, @NUID)", con))
            {
        ...
        {
            _updateMRE.Wait();
            using(FbCommand cmd = new FbCommand(@"SELECT ID, DATUM, PPID, MAGACINID,
                USERID, STATUS, KOMENTAR, INTERNI_KOMENTAR, KOMERCIJALNO_PRORACUN_BROJ, NUID FROM DOKUMENT_PRORACUN WHERE ID = @ID", con))
            {
                cmd.Parameters.AddWithValue("@ID", id);
                ...
                using(FbDataReader dr = cmd.ExecuteReader())
                {
                    if (dr.Read())
                        return new DokumentProracun()
                        {
                            ...
                            KomercijalnoProracunBroj = dr["KOMERCIJALNO_PRORACUN_BROJ"] is DBNull ? null : (int?)Convert.ToInt32(dr["KOMERCIJALNO_PRORACUN_BROJ"]),
        ...
        public static List<DokumentProracun> List(FbConnection con)
        {
            ...
            using (FbCommand cmd = new FbCommand(@"SELECT ID, DATUM, PPID, MAGACINID,
                USERID, STATUS, KOMENTAR, INTERNI_KOMENTAR, KOMERCIJALNO_PRORACUN_BROJ, NUID FROM DOKUMENT_PRORACUN", con))
            {
                using (FbDataReader dr = cmd.ExecuteReader())
                {
                    while (dr.Read())
                        list.Add(new DokumentProracun()
                        {
                            ID = Convert.ToInt32(dr["ID"]),
                            Datum = Convert.ToDateTime(dr["DATUM"]),
                            PPID = dr["PPID"] is DBNull ? null : (int?)Convert.ToInt32(dr["PPID"]),
        ...
        {
            using(FbCommand cmd = new FbCommand("SELECT COALESCE(MAX(ID), 0) FROM DOKUMENT_PRORACUN", con))
            {
                using(FbDataReader dr = cmd.ExecuteReader())
                {
                    if (dr.Read())
