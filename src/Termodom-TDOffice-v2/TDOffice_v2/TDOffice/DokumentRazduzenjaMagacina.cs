...
namespace TDOffice_v2.TDOffice
{
    public partial class DokumentRazduzenjaMagacina
    {
        ...
        {
            using (FbCommand cmd = new FbCommand(@"UPDATE DOKUMENT_RAZDUZENJE_MAGACINA SET
                MAGACINID = @M,
                DATUM = @D,
                KORISNIKID = @K,
                STATUS = @S,
        ...
        public static int Insert(int magacinID, DateTime datum, int korisnikID, DokumentRazduzenjaMagacinaStatus status)
        {
            using (FbConnection con = new FbConnection(TDOffice.connectionString))
            {
                con.Open();
        ...
        public static int Insert(int magacinID, DateTime datum, int korisnikID, DokumentRazduzenjaMagacinaStatus status, string interniKomentar, string komentar)
        {
            using (FbConnection con = new FbConnection(TDOffice.connectionString))
            {
                con.Open();
        ...
        public static int Insert(FbConnection con, int magacinID, DateTime datum,  int kID, DokumentRazduzenjaMagacinaStatus status, string komentar, string interniKomentar)
        {
            using (FbCommand cmd = new FbCommand(@"INSERT INTO DOKUMENT_RAZDUZENJE_MAGACINA
        ...
        {
            using (FbCommand cmd = new FbCommand(@"SELECT ID, MAGACINID, DATUM, KORISNIKID, STATUS, KOMENTAR, INTERNI_KOMENTAR, KOMERCIJALNO_BRDOKOUT FROM DOKUMENT_RAZDUZENJE_MAGACINA WHERE ID = @ID", con))
            {
                cmd.Parameters.AddWithValue("@ID", id);
                ...
                using (FbDataReader dr = cmd.ExecuteReader())
                {
                    if (dr.Read())
                        return new DokumentRazduzenjaMagacina()
                        {
                            ...
                            Status = (DokumentRazduzenjaMagacinaStatus)Convert.ToInt32(dr["STATUS"]),
                            InterniKomentar = dr["INTERNI_KOMENTAR"].ToStringOrDefault(),
                            Komentar = dr["KOMENTAR"].ToStringOrDefault(),
                            KomercijalnoBrDokOut = dr["KOMERCIJALNO_BRDOKOUT"] is DBNull ? 0 : Convert.ToInt32(dr["KOMERCIJALNO_BRDOKOUT"])
                        };
                }
            }
        ...
        public static List<DokumentRazduzenjaMagacina> List(FbConnection con, string whereQuery = null)
        {
            if (!string.IsNullOrWhiteSpace(whereQuery))
                whereQuery = " AND " + whereQuery;

            List<DokumentRazduzenjaMagacina> list = new List<DokumentRazduzenjaMagacina>();
            using (FbCommand cmd = new FbCommand(@"SELECT ID, MAGACINID, DATUM, KORISNIKID, STATUS, KOMENTAR, INTERNI_KOMENTAR, KOMERCIJALNO_BRDOKOUT
                ...
                WHERE 1 = 1", con))
            {
                ...
                {
                    while (dr.Read())
                        list.Add(new DokumentRazduzenjaMagacina()
                        {
                            ID = Convert.ToInt32(dr["ID"]),
                            Datum = Convert.ToDateTime(dr["DATUM"]),
                            MagacinID = Convert.ToInt32(dr["MAGACINID"]),
        ...
        public static List<DokumentRazduzenjaMagacina> ListByMagacinID(FbConnection con, int magacinID)
        {
            List<DokumentRazduzenjaMagacina> list = new List<DokumentRazduzenjaMagacina>();
            using (FbCommand cmd = new FbCommand(@"SELECT ID, MAGACINID, DATUM, KORISNIKID, STATUS, KOMENTAR, INTERNI_KOMENTAR, KOMERCIJALNO_BRDOKOUT FROM DOKUMENT_RAZDUZENJE_MAGACINA WHERE MAGACINID = @MID", con))
            {
