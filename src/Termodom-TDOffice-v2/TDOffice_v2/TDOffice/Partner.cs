...
namespace TDOffice_v2.TDOffice
{
    public class Partner
    {
        ...
        public void Update(FbConnection con)
        {
            ...
            using (FbCommand cmd = new FbCommand(@"UPDATE PARTNER SET NAZIV = @N, MOBILNI = @MOB, SMS_BLOK = @BLOK, MEJL = @MEJL,
                GRAD = @GRAD, KOMENTAR = @KOMENTAR, KOMERCIJALNO_PPID = @KPPID, WEB_ID = @WID,
                MAGACINID = @MID WHERE ID = @ID", con))
            {
        ...
        public static int Insert(string naziv, string mobilni, string mejl, int grad, string komentar, List<int> grupe, int? komercijalnoPPID, int? webID, int? magacinID)
        {
            using(FbConnection con = new FbConnection(TDOffice.connectionString))
            {
                con.Open();
                return Insert(con, naziv, mobilni, mejl, grad, komentar, grupe, komercijalnoPPID, webID, magacinID);
            }
        }
        public static int Insert(FbConnection con, string naziv, string mobilni, string mejl, int grad, string komentar, List<int> grupe, int? komercijalnoPPID, int? webID, int? magacinID)
        {
            if (!MobileNumber.IsValid(mobilni))
                throw new Exception("Morate uneti validan mobilni telefon!");

            using(FbCommand cmd = new FbCommand(@"INSERT INTO PARTNER (ID, NAZIV, MOBILNI, MEJL, GRAD, KOMENTAR, KOMERCIJALNO_PPID, WEB_ID, MAGACINID) VALUES
                (((SELECT COALESCE(MAX(ID), 0) FROM PARTNER) + 1), @NAZIV, @MOBILNI, @MEJL, @GRAD, @KOMENTAR, @KID, @WID, @MID) RETURNING ID", con))
            {
        ...
        public static int InsertBrziKontakt(string mobilni)
        {
            ...
            if (part == null)
                return Insert("*** brzi kontakt ***", MobileNumber.Collate(mobilni), null, 0, null, null, null, null, null);
        ...
        {
            using(FbCommand cmd = new FbCommand("SELECT ID, NAZIV, MOBILNI, SMS_BLOK, MEJL, GRAD, KOMENTAR, KOMERCIJALNO_PPID, WEB_ID, MAGACINID FROM PARTNER WHERE ID = @ID ", con))
            {
                cmd.Parameters.AddWithValue("@ID", id);
                ...
                using(FbDataReader dr = cmd.ExecuteReader())
                {
                    if (dr.Read())
                        return new Partner()
                        {
                            ...
                            Grupe = PartnerGrupaItem.ListByPartner(Convert.ToInt32(dr["ID"])).Select(x => x.PartnerGrupaID).ToList(),
                            KomercijalnoPPID = dr["KOMERCIJALNO_PPID"] is DBNull ? null : (int?)Convert.ToInt32(dr["KOMERCIJALNO_PPID"]),
                            WebID = dr["WEB_ID"] is DBNull ? null : (int?)Convert.ToInt32(dr["WEB_ID"]),
        ...
        public static List<Partner> List(FbConnection con, string whereQuery = null)
        {
            ...
            using (FbCommand cmd = new FbCommand("SELECT ID, NAZIV, MOBILNI, SMS_BLOK, MEJL, GRAD, KOMENTAR, KOMERCIJALNO_PPID, WEB_ID, MAGACINID FROM PARTNER" + whereQuery, con))
            {
                using (FbDataReader dr = cmd.ExecuteReader())
                {
                    while (dr.Read())
