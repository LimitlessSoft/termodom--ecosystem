...
namespace TDOffice_v2.TDOffice
{
    public class DokumentZamenaRobe
    {
        ...
        {
            using (FbCommand cmd = new FbCommand(@"UPDATE DOKUMENT_ZAMENA_ROBE SET
                                                        USERID = @U,
                                                        STATUS = @S,
                                                        KOMENTAR = @K,
                                                        INTERNI_KOMENTAR = @IK,
                                                        ...
                                                        WHERE ID = @ID", con))
            {
                ...
                cmd.Parameters.AddWithValue("@TAG", System.Text.Encoding.UTF8.GetBytes(Newtonsoft.Json.JsonConvert.SerializeObject(Tag)));
        ...
        public static DokumentZamenaRobe Get(FbConnection con, int id)
        {
            using (FbCommand cmd = new FbCommand(@"SELECT ID, DATUM, USERID, MAGACINID, STATUS, KOMENTAR,
                ...
                LOG, TAG FROM DOKUMENT_ZAMENA_ROBE WHERE ID = @id", con))
            {
                cmd.Parameters.AddWithValue("@id", id);
                using (FbDataReader dr = cmd.ExecuteReader())
                    if (dr.Read())
                        return new DokumentZamenaRobe()
                        {
                            ...
                            MPRacun = dr["MP_RACUN"] is DBNull ? (int?)null : Convert.ToInt32(dr["MP_RACUN"]),
                            NovokreiraniMPRacun = dr["NOVI_MP_RACUN"] is DBNull ? (int?)null : Convert.ToInt32(dr["NOVI_MP_RACUN"]),
                            Realizovana = Convert.ToInt32(dr["REALIZOVANA"]) != 0,
                            TrosakZamene = Convert.ToDouble(dr["TROSAK_ZAMENE"]),
                            FiskalniIsecak = dr["FISKALNI_ISECAK"] is DBNull ? null : dr["FISKALNI_ISECAK"].ToString(),
...
TAG FROM DOKUMENT_ZAMENA_ROBE WHERE 1 = 1 " + whereQuery, con))
            {
                using (FbDataReader dr = cmd.ExecuteReader())
                    while (dr.Read())
                        list.Add(new DokumentZamenaRobe()
                        {
                            ID = Convert.ToInt32(dr["ID"]),
                            Datum = Convert.ToDateTime(dr["DATUM"]),
                            UserID = Convert.ToInt32(dr["USERID"]),
        ...
        /// <returns></returns>
        public static int Insert(int userID, int magacinID, int status, string komentar, string interniKomentar)
        {
            using (FbConnection con = new FbConnection(TDOffice.connectionString))
            {
                con.Open();
        ...
        /// <param name="komentar"></param>
        /// <param name="interniKomentar"></param>
        public static int Insert(FbConnection con, int userID, int magacinID, int status, string komentar, string interniKomentar)
        {
            using (FbCommand cmd = new FbCommand(@"INSERT INTO DOKUMENT_ZAMENA_ROBE
