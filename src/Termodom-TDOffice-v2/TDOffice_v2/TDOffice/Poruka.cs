...
namespace TDOffice_v2.TDOffice
{
    ...
    public class Poruka
    {
        ...
        public PorukaAdditionalInfo Tag
        {
            ...
            {
                if (_rawTag == null)
                    _rawTag = Encoding.UTF8.GetBytes(JsonConvert.SerializeObject(new PorukaAdditionalInfo()));

                return JsonConvert.DeserializeObject<PorukaAdditionalInfo>(Encoding.UTF8.GetString(_rawTag));
            }
            set
            {
                _rawTag = Encoding.UTF8.GetBytes(JsonConvert.SerializeObject(value));
            }
        }
        ...
        {
            using(FbCommand cmd = new FbCommand("UPDATE PORUKA SET DATUM_CITANJA = @DC, STATUS = @S, TAG = @T, ARHIVIRANA = @ARH WHERE ID = @ID", con))
            {
        ...
        public static Poruka Get(int porukaID)
        {
            ...
            {
                con.Open();
                using (FbCommand cmd = new FbCommand("SELECT ID, POSILJALAC, PRIMALAC, DATUM, DATUM_CITANJA, NASLOV, TEKST, STATUS, TAG, ARHIVIRANA, TIPPRIMAOCA, PAKET FROM PORUKA WHERE ID = @ID", con))
                {
                    ...
                    using (FbDataReader dr = cmd.ExecuteReader())
                    {
                        if (dr.Read())
                        {
                            return new Poruka()
                            {
                                ...
                                DatumCitanja = dr["DATUM_CITANJA"] is DBNull ? (DateTime?)null : Convert.ToDateTime(dr["DATUM_CITANJA"]),
                                Naslov = dr["NASLOV"].ToString(),
                                Tekst = dr["TEKST"].ToString(),
                                Status = (PorukaTip)Convert.ToInt32(dr["STATUS"]),
                                _rawTag = dr["TAG"] is DBNull ? Encoding.UTF8.GetBytes(JsonConvert.SerializeObject(new PorukaAdditionalInfo())) : (byte[])dr["TAG"],
                                Arhivirana = Convert.ToInt32(dr["ARHIVIRANA"]) == 1 ? true : false,
                                TipPrimaoca = (Enums.PorukaTipPrimaoca)Convert.ToInt32(dr["TIPPRIMAOCA"]),
                                Paket = dr["PAKET"] is DBNull ? (int?)null : Convert.ToInt32(dr["PAKET"])
                            };
                        }
                    }
                }
        ...
        {
            return List("DATUM >= '" + odDatuma.ToSystemShortDateFormatString() + "' AND DATUM < '" + doDatuma.ToSystemShortDateFormatString() + "'");
        }
        ...
        public static List<Poruka> List(FbConnection con, string whereQuery = null)
        {
            ...
            using (FbCommand cmd = new FbCommand($@"SELECT ID, POSILJALAC, PRIMALAC, DATUM, DATUM_CITANJA, NASLOV, TEKST, STATUS, TAG,
                ARHIVIRANA, TIPPRIMAOCA, PAKET FROM PORUKA" + whereQuery, con))
            {
                using (FbDataReader dr = cmd.ExecuteReader())
                {
                    while (dr.Read())
        ...
        public static void Insert(Poruka poruka)
        {
            ...
            {
                con.Open();
                using (FbCommand cmd = new FbCommand(@"INSERT INTO PORUKA 
                    (ID, POSILJALAC, PRIMALAC, DATUM, DATUM_CITANJA, NASLOV, TEKST, STATUS, TAG, TIPPRIMAOCA, PAKET) 
                    VALUES 
                    (((SELECT COALESCE(MAX(ID), 0) FROM PORUKA) + 1), @POSILJALAC, @PRIMALAC, @DATUM, NULL, @NASLOV, @TEKST, @STATUS, @TAG, @TIPPRIMAOCA, @PAKET)", con))
                {
        ...
        public static void Insert(int[] primaoci, string naslov, string tekst, PorukaAdditionalInfo additionalInfo = null)
        {
            foreach (int i in primaoci)
                Insert(new Poruka()
                {
                    ...
                    _rawTag = Encoding.UTF8.GetBytes(JsonConvert.SerializeObject(additionalInfo)),
                    Tekst = tekst
                });
        }
        public static int MaxPaketID()
        {
            ...
            {
                con.Open();
                using (FbCommand cmd = new FbCommand("SELECT COALESCE(MAX(PAKET) + 1, 1) AS MAXID FROM PORUKA", con))
                {
                    using (FbDataReader dr = cmd.ExecuteReader())
                    {
                        if (dr.Read())
