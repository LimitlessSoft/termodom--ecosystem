...
namespace TDOffice_v2.TDOffice
{
    public partial class StavkaRazduzenjaMagacina
    {
        ...
        {
            using (FbCommand cmd = new FbCommand(@"UPDATE STAVKA_RAZDUZENJE_MAGACINA SET
                ROBAID = @rid,
                KOLICINA = @K,
                DOKUMENT_ID = @did
                WHERE ID = @ID", con))
        ...
        {
            using (FbCommand cmd = new FbCommand(@"INSERT INTO STAVKA_RAZDUZENJE_MAGACINA
                (ID, ROBAID, KOLICINA, DOKUMENT_ID)
                VALUES
                (((SELECT COALESCE(MAX(ID), 0) FROM STAVKA_RAZDUZENJE_MAGACINA) + 1), @RID, @Kol, @DID)", con))
            {
        ...
        {
            using (FbCommand cmd = new FbCommand(@"SELECT ID, ROBAID, KOLICINA, DOKUMENT_ID FROM STAVKA_RAZDUZENJE_MAGACINA WHERE ID = @ID", con))
            {
                cmd.Parameters.AddWithValue("@ID", id);
        ...
        public static List<StavkaRazduzenjaMagacina> List(FbConnection con)
        {
            ...
            using (FbCommand cmd = new FbCommand(@"SELECT ID, ROBAID, KOLICINA, DOKUMENT_ID FROM STAVKA_RAZDUZENJE_MAGACINA", con))
            {
                using (FbDataReader dr = cmd.ExecuteReader())
                {
                    while (dr.Read())
                        list.Add(new StavkaRazduzenjaMagacina()
                        {
                            ID = Convert.ToInt32(dr["ID"]),
                            RobaID = Convert.ToInt32(dr["ROBAID"]),
                            Kolicina = Convert.ToDouble(dr["KOLICINA"]),
        ...
        public static List<StavkaRazduzenjaMagacina> ListByDokumentID(FbConnection con, int DokumentID)
        {
            List<StavkaRazduzenjaMagacina> list = new List<StavkaRazduzenjaMagacina>();
            using (FbCommand cmd = new FbCommand(@"SELECT ID, ROBAID, KOLICINA, DOKUMENT_ID FROM STAVKA_RAZDUZENJE_MAGACINA WHERE DOKUMENT_ID = @DID", con))
            {
                cmd.Parameters.AddWithValue("@DID", DokumentID);
                using (FbDataReader dr = cmd.ExecuteReader())
                {
        ...
        {
            using (FbCommand cmd = new FbCommand("DELETE FROM STAVKA_RAZDUZENJE_MAGACINA WHERE ID = @ID", con))
            {
                cmd.Parameters.AddWithValue("@ID", id);
