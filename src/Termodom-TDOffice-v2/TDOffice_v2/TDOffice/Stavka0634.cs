...
namespace TDOffice_v2.TDOffice
{
    public class Stavka0634
    {
        ...
        {
            using(FbCommand cmd = new FbCommand("UPDATE STAVKA_0634 SET KOLICINA = @KOL WHERE ID = @ID", con))
            {
        ...
        {
            using(FbCommand cmd= new FbCommand("SELECT ID, ROBAID, BRDOK, KOLICINA FROM STAVKA0634 WHERE ID = @ID", con))
            {
                cmd.Parameters.AddWithValue("@ID", id);
        ...
        public static List<Stavka0634> List(FbConnection con, string queryString)
        {
            ...
            using (FbCommand cmd = new FbCommand("SELECT ID, ROBAID, BRDOK, KOLICINA FROM STAVKA_0634 WHERE 1 = 2 " + queryString, con))
            {
                using (FbDataReader dr = cmd.ExecuteReader())
                    while (dr.Read())
                        list.Add(new Stavka0634()
                        {
                            ID = Convert.ToInt32(dr["ID"]),
                            Kolicina = Convert.ToDouble(dr["KOLICINA"]),
                            RobaID = Convert.ToInt32(dr["ROBAID"]),
        ...
        {
            using(FbCommand cmd = new FbCommand("INSERT INTO STAVKA_0634 (ID, ROBAID, BRDOK, KOLICINA) VALUES (((SELECT COALESCE(MAX(ID), 0) FROM STAVKA_0634) + 1), @ROBAID, @BRDOK, @KOLICINA) RETURNING ID", con))
            {
