...
namespace TDOffice_v2.TDOffice
{
    public partial class Taskboard
    {
        ...
        {
            using(FbCommand cmd = new FbCommand("SELECT ID, NAZIV, KORISNIKID FROM TASKBOARD WHERE ID = @ID", con))
            {
        ...
        public static List<Taskboard> List(FbConnection con)
        {
            ...
            using (FbCommand cmd = new FbCommand("SELECT ID, NAZIV, KORISNIKID FROM TASKBOARD", con))
            {
                using (FbDataReader dr = cmd.ExecuteReader())
                    while (dr.Read())
                        list.Add(new Taskboard()
                        {
                            ID = Convert.ToInt32(dr["ID"]),
                            Naziv = dr["NAZIV"].ToString(),
                            KorisnikID = Convert.ToInt32(dr["KORISNIKID"])
        ...
        {
            using(FbCommand cmd = new FbCommand("INSERT INTO TASKBOARD (ID, NAZIV, KORISNIKID) VALUES (((SELECT COALESCE(MAX(ID), 0) FROM TASKBOARD) + 1), @N, @KID) RETURNING ID", con))
            {
