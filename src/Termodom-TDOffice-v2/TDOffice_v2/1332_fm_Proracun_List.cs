...
namespace TDOffice_v2
{
    ...
    {
        private Task<FbConnection> _conTDOfficeTask { get; set; } = Task.Run(() =>
        {
            FbConnection con = new FbConnection(TDOffice.TDOffice.connectionString);
            con.Open();
            return con;
        });
        private FbConnection _conTDOffice { get => _conTDOfficeTask.Result; }
        private Task<FbConnection> _conKomercijalnoTask { get; set; } = Task.Run(() =>
        {
            FbConnection con = new FbConnection(Komercijalno.Komercijalno.CONNECTION_STRING[DateTime.Now.Year]);
            con.Open();
            return con;
        });
        private FbConnection _conKomercijalno { get => _conKomercijalnoTask.Result; }
        ...
        private void SetDGV()
        {
            ...
            dataGridView1.Columns["Referent"].AutoSizeMode = DataGridViewAutoSizeColumnMode.AllCells;
            dataGridView1.Columns["Komercijalno"].Visible = false;
            dataGridView1.Columns["Status"].Visible = false;
            dataGridView1.Columns["KomProracun"].AutoSizeMode = DataGridViewAutoSizeColumnMode.AllCells;
        ...
        private void PopulateDGV()
        {
            ...
            string _sqlWhere = "Datum > '" + new DateTime(pocetak.Year, pocetak.Month, pocetak.Day, 0, 0, 0) + "' AND Datum < '" + new DateTime(kraj.Year, kraj.Month, kraj.Day, 23, 59, 59) + "'";
            ...
            foreach (DataGridViewRow r in dataGridView1.Rows)
            {
                ...
                r.DefaultCellStyle.ForeColor =
                    status == 1 && komercijalno > 0 ?
                        Color.Red :
                        Color.FromArgb(255, 125, 25);
            }
        }

        private void UcitajProracune()
        {
            List<TDOffice.DokumentProracun> dokumenti = TDOffice.DokumentProracun.List(_conTDOffice);

            Task<Dictionary<int, List<Komercijalno.Dokument>>> komDokBuffer = Task.Run(() =>
            {
                Dictionary<int, List<Komercijalno.Dokument>> dict = new Dictionary<int, List<Komercijalno.Dokument>>();
            ...
            Dictionary<int, Komercijalno.Magacin> magacini = Komercijalno.Magacin.Dict(_conKomercijalno);
            ...
            foreach (TDOffice.DokumentProracun p in dokumenti)
            {
                if(!magacini.ContainsKey(p.MagacinID))
                {
                    Task.Run(() =>
                    {
                        MessageBox.Show($"Greska prilikom ucitavanja magacina za dokument TD Proracuna broj {p.ID}, magacin {p.MagacinID}");
                    });
                    continue;
                }
                ...
                dr["Komercijalno"] = p.KomercijalnoProracunBroj == null ? -1 : p.KomercijalnoProracunBroj;
                ...
                {
                    Komercijalno.Dokument proracunKom = komDokBuffer.Result[p.Datum.Year].FirstOrDefault(x => x.BrDok == p.KomercijalnoProracunBroj);
                    dr["KomProracun"] = proracunKom == null ? "ERROR" : proracunKom.BrDok.ToString();
                    dr["MPRacun"] = proracunKom == null ? "ERROR" : proracunKom.BrDokOut.ToStringOrDefault();
                }
        ...
        private void nova_btn_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Nemate povezan nalog komercijalnog poslovanja pa ne mozete kreirati proracun!");
                return;
            }

            int newID = TDOffice.DokumentProracun.Insert(DateTime.Now, null, Convert.ToInt32(magacin_cmb.SelectedValue), Program.TrenutniKorisnik.ID, "Generisao: TDOffice_v2", null,-1);
