...
namespace TDOffice_v2
{
    public partial class fm_Izvestaj_Prodaja_Roba_ZbirnoPoRobi : Form
    {
        ...
        private void fm_Izvestaj_Prodaja_Roba_ZbirnoPoRobi_Load(object sender, EventArgs e)
        {
            ...
            List<int> robaUTabeli = _dataTable.AsEnumerable().Select(x => Convert.ToInt32(x["ROBAID"])).Distinct().ToList();
            foreach (int robaID in robaUTabeli)
            {
                zbirDR = zbirDT.NewRow();
                zbirDR["ROBAID"] = robaID;
                zbirDR["PROIZVOD"] = _komercijalnoRoba.Result.FirstOrDefault(x => x.ID == robaID).Naziv;
                foreach (int godina in _periodi.Godine.OrderBy(x => x))
                {
                    double zk = 0;
                    double zv = 0;
                    ...
                    {
                        if (_dataTable.Columns.Contains("kGOD" + godina))
                            zk = Convert.ToDouble(_dataTable.Select("ROBAID = " + robaID).CopyToDataTable().Compute("Sum(kGOD" + godina + ")", string.Empty));
                        if (_dataTable.Columns.Contains("vGOD" + godina))
                            zv = Convert.ToDouble(_dataTable.Select("ROBAID = " + robaID).CopyToDataTable().Compute("Sum(vGOD" + godina + ")", string.Empty));
                    }
                    catch
                    {
