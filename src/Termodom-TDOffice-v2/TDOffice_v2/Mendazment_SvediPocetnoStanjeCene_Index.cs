    ...
    public partial class Mendazment_SvediPocetnoStanjeCene_Index : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            Task.Run(() =>
            {
                ...
                using(FbConnection con = new FbConnection(Komercijalno.Komercijalno.CONNECTION_STRING[2020]))
                {
                    con.Open();

                    using(FbConnection con2 = new FbConnection(Komercijalno.Komercijalno.CONNECTION_STRING[2021]))
                    {
                        con2.Open();

                        UpdateStatusLabel(" / Ucitavam robu u magacinu prethodne godine");
                        List<Komercijalno.RobaUMagacinu> rums = Komercijalno.RobaUMagacinu.List(con);
                        ...
                        foreach (Komercijalno.Magacin magacin in checkedListBox1.CheckedItems.OfType<Komercijalno.Magacin>())
                        {
                            ...
                            Komercijalno.Dokument dokumentPocetnogStanja = Komercijalno.Dokument.List(con2, "VRDOK = 0 AND MAGACINID = " + magacin.ID).FirstOrDefault();

                            UpdateStatusLabel(magacin.Naziv + " / Ucitavam stavke pocetnog stanja");
                            List<Komercijalno.Stavka> stavkePocetnogStanja = Komercijalno.Stavka.List(con2, "VRDOK = 0 AND BRDOK = " + dokumentPocetnogStanja.BrDok.ToString());

                            UpdateStatusLabel(magacin.Naziv + " / Ucitavam robu za ovaj magacin");
                            List<Komercijalno.RobaUMagacinu> robaZaOvajMagacin = rums.Where(x => x.MagacinID == magacin.ID).ToList();
                            ...
                            {
                                i++;
                                UpdateStatusLabel(magacin.Naziv + " / Azuriranje cene " + i + " od " + total);

                                var sps = stavkePocetnogStanja.FirstOrDefault(x => x.RobaID == rum.RobaID);
                                if(sps == null)
                                {
                                    sps = Komercijalno.Stavka.Get(con2, Komercijalno.Stavka.Insert(con2, dokumentPocetnogStanja, Komercijalno.Roba.Get(con2, rum.RobaID), rum, 0, 0));
                                }
