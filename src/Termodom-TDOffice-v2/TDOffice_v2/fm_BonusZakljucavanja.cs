...
namespace TDOffice_v2
{
    public partial class fm_BonusZakljucavanja : Form
    {
        ...
        {
            var korisniciKojiPrimaju = TDOffice.User.List()
                .Where(x => x.Tag != null && x.Tag.PrimaObavestenja[TDOffice.User.TipAutomatskogObavestenja.PrimaObavestenjaOObnoviBonusaZakljucavanja] == true);
            foreach (TDOffice.User kp in korisniciKojiPrimaju)
            {
                TDOffice.Poruka.Insert(new TDOffice.Poruka()
                {
                    ...
                    Tekst = string.Join(Environment.NewLine, "", "Zahtev za reset bonusa zakljucavanja")
                });
            }
        ...
        private void btnZakljucajuzpomocbonusa_Click(object sender, EventArgs e)
        {
            ...
            if (dok == null) { MessageBox.Show("Dokument" + " nije pronadjen"); return; }
            if (dok.PPID == null) {MessageBox.Show("Dokument nema PPID"); return; }
            ...
            else
            {
                ...
                {
                    MessageBox.Show("Vrednost Dokumenta br." + dok.BrDok.ToString() + " premasuje limit");
                    return;
                }
            }
        }
