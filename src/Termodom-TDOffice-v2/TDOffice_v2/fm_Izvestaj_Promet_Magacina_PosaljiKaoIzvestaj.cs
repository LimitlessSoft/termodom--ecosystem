...
namespace TDOffice_v2
{
    public partial class fm_Izvestaj_Promet_Magacina_PosaljiKaoIzvestaj : Form
    {
        ...
        public fm_Izvestaj_Promet_Magacina_PosaljiKaoIzvestaj(DataTable data, Dictionary<int, Tuple<DateTime, DateTime>> periodi)
        {
            InitializeComponent();
...
FROM
    (
    SELECT s.MAGACINID, SUM(s.KOLICINA * s.PRODAJNACENA * (100 - s.rabat)/100) AS vGOD" + godina + @"
    FROM STAVKA s
    LEFT OUTER JOIN DOKUMENT d ON d.VRDOK = s.VRDOK AND d.BRDOK = s.BRDOK
    WHERE d.FLAG = 1 AND d.MAGACINID IN (" + string.Join(", ", CheckedValues) + @") AND (d.VRDOK = 13 OR d.VRDOK = 15) AND DATUM >= @ODDATUMA AND DATUM <= @DODATUMA
    ...
    FROM STAVKA s
    LEFT OUTER JOIN DOKUMENT d ON d.VRDOK = s.VRDOK AND d.BRDOK = s.BRDOK
    WHERE d.MAGACINID IN (" + string.Join(", ", CheckedValues) + @") AND (d.VRDOK = 22) AND DATUM >= @ODDATUMA AND DATUM <= @DODATUMA
    GROUP BY s.MAGACINID ORDER BY S.MAGACINID desc
    ...
    GROUP BY t1.MAGACINID", con))
                    ...
                    foreach (int godina in _periodi.Keys.OrderBy(x => x))
                    {
                        ...
                        drizv["vGOD" + godina] = finalDT.Compute($"Sum(vGOD{godina})", string.Empty);
                    }
                    finalDT.Rows.Add(dr);
