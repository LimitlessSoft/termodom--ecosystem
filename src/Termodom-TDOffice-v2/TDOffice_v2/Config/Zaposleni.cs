...
namespace TDOffice_v2.Config
{
    public class Zaposleni
    {
        ...
        {
            using (FbCommand cmd = new FbCommand(@"UPDATE ZAPOSLENI SET NAZIV = @NAZIV, ADRESA = @ADRESA, TELEFON = @TEL, EMAIL = @EMAIL,
        ...
        {
            using (FbCommand cmd = new FbCommand(@"SELECT ZAPID, NAZIV, ADRESA, TELEFON, EMAIL, ORGJED, KORISNIK, 
                EL_POTPIS, GRUPA, GRUPAID, PRAVA_GRUPE, NZAPID, EKSPID, MBROJ, BRLK, EMAIL_PASS, EMAIL_REPLY,
                WEB_USERID, CUST_ID, PPID FROM ZAPOSLENI  WHERE ZAPID = @ZID", con))
            {
                cmd.Parameters.AddWithValue("@ZID", zapID);
                ...
                using (FbDataReader dr = cmd.ExecuteReader())
                    if (dr.Read())
                        return new Zaposleni()
                        {
                            ...
                            ELPotpis = dr["EL_POTPIS"] is DBNull ? null : dr["EL_POTPIS"].ToString(),
                            Grupa = dr["GRUPA"] is DBNull ? (Int16?)null : Convert.ToInt16(dr["GRUPA"]),
                            GrupaID = dr["GRUPAID"] is DBNull ? (Int16?)null : Convert.ToInt16(dr["GRUPAID"]),
                            PravaGrupe = dr["PRAVA_GRUPE"] is DBNull ? (Int16?)null : Convert.ToInt16(dr["PRAVA_GRUPE"]),
                            NZapID = dr["NZAPID"] is DBNull ? (Int16?)null : Convert.ToInt16(dr["NZAPID"]),
                            EkspID = dr["EKSPID"] is DBNull ? (Int16?)null : Convert.ToInt16(dr["EKSPID"]),
                            MBroj = dr["MBROJ"] is DBNull ? null : dr["MBROJ"].ToString(),
                            BRLK = dr["BRLK"] is DBNull ? null : dr["BRLK"].ToString(),
                            EmailPass = dr["EMAIL_PASS"] is DBNull ? null : dr["EMAIL_PASS"].ToString(),
                            EmailReply = dr["EMAIL_REPLY"] is DBNull ? null : dr["EMAIL_REPLY"].ToString(),
                            WebUserID = dr["WEB_USERID"] is DBNull ? (Int32?)null : Convert.ToInt32(dr["WEB_USERID"]),
        ...
        public static List<Zaposleni> List(FbConnection con)
        {
            ...
            using (FbCommand cmd = new FbCommand(@"SELECT ZAPID, NAZIV, ADRESA, TELEFON, EMAIL, ORGJED, KORISNIK, 
                EL_POTPIS, GRUPA, GRUPAID, PRAVA_GRUPE, NZAPID, EKSPID, MBROJ, BRLK, EMAIL_PASS, EMAIL_REPLY,
                WEB_USERID, CUST_ID, PPID FROM ZAPOSLENI", con))
            {
                using (FbDataReader dr = cmd.ExecuteReader())
                    while (dr.Read())
