    ...
    public partial class Korisnik
    {
        ...
        public DateTime? PoslednjiPutVidjen
        {
            get
            {
                ...
                {
                    HttpRequestMessage request = new HttpRequestMessage(HttpMethod.Get, Program.BaseAPIUrl + "/Webshop/Korisnik/Poseta/Get?id=" + this.ID);
                    ...
                    failedRequestArgs.Add(new APIRequestTimeoutExceptionArguments()
                    {
                        Request = request,
                        Response = response,
                        Time = DateTime.Now
        ...
        public static Korisnik Get(int id)
        {
            ...
            for (int i = 0; i < Program.ForbiddenRequestMaxTries; i++)
            {
                ...
                failedRequestArguments.Add(new APIRequestTimeoutExceptionArguments()
                {
                    Request = request,
                    Response = response,
                    Time = DateTime.Now
        ...
        public static Korisnik Get(string ime)
        {
            ...
            for (int i = 0; i < Program.ForbiddenRequestMaxTries; i++)
            {
                HttpResponseMessage response = client.SendAsync(request).Result;
                if (response.StatusCode == HttpStatusCode.OK)
                    return response != null ? JsonConvert.DeserializeObject<Korisnik>(response.Content.ReadAsStringAsync().Result) : null;
                else if (response.StatusCode == HttpStatusCode.InternalServerError)
                    throw new APIRequestInternalServerErrorException();
