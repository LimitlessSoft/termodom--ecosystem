...
namespace TDOffice_v2
{
    public partial class _1336_fm_RazduzenjeMagacina_List : Form
    {
        ...
        private void SetDGV()
        {
            ...
            dataGridView1.Columns["Komentar"].AutoSizeMode = DataGridViewAutoSizeColumnMode.AllCells;
            dataGridView1.Columns["Komentar"].Visible = false;

            dataGridView1.Columns["InterniKomentar"].AutoSizeMode = DataGridViewAutoSizeColumnMode.AllCells;
        ...
        private void PopulateDGV()
        {
            ...
            foreach (DataGridViewRow r in dataGridView1.Rows)
            {
                ...
                if (Convert.ToInt32(r.Cells["Status"].Value) == 2)
                {
        ...
        private void UcitajRazduzenja()
        {
            ...
            List<TDOffice.DokumentRazduzenjaMagacina> dokumenti = magID == -1 ? TDOffice.DokumentRazduzenjaMagacina.List() : TDOffice.DokumentRazduzenjaMagacina.ListByMagacinID(magID);
            ...
            {
                Komercijalno.Magacin m = _magacini.Result.Where(x => x.ID == drz.MagacinID).FirstOrDefault();
                TDOffice.User k = _korisnici.Result.Where(x => x.ID == drz.KorisnikID).FirstOrDefault();
        ...
        private void nova_btn_Click(object sender, EventArgs e)
        {
            ...
            int newID = TDOffice.DokumentRazduzenjaMagacina.Insert(magID, DateTime.Now, Program.TrenutniKorisnik.ID, TDOffice.DokumentRazduzenjaMagacinaStatus.Otkljucan);

            using (_1336_fm_RazduzenjeMagacina_Index rm = new _1336_fm_RazduzenjeMagacina_Index(TDOffice.DokumentRazduzenjaMagacina.Get(newID)))
                rm.ShowDialog();
        ...
        private void dataGridView1_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            TDOffice.DokumentRazduzenjaMagacina dokument = TDOffice.DokumentRazduzenjaMagacina.Get(brDok);

            using (_1336_fm_RazduzenjeMagacina_Index rm = new _1336_fm_RazduzenjeMagacina_Index(dokument))
            {
                rm.ShowDialog();
