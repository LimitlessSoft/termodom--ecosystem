...
namespace TDOffice_v2.Komercijalno
{
    public class Kontakt
    {
        ...
        {
            using(FbCommand cmd = new FbCommand("SELECT PPID, KONID, NAZIV, TELEFON, MOBILNI, FAX, EMAIL, RMID, AKTIVAN FROM KONTAKTI WHERE PPID = @PPID AND KONID = @KONID", con))
            {
        ...
        {
            using (FbCommand cmd = new FbCommand("SELECT PPID, KONID, NAZIV, TELEFON, MOBILNI, FAX, EMAIL, RMID, AKTIVAN FROM KONTAKTI", con))
            {
