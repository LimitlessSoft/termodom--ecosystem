...
namespace TDOffice_v2
{
    public partial class fm_KontrolaLagera_Index : Form
    {
        ...
        private void SetBaseDataTable()
        {
            ...
            dataGridView1.Columns["OptZal"].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;

            dataGridView1.Columns["KritZal"].Width = 100;
            dataGridView1.Columns["KritZal"].DefaultCellStyle.Format = "#,##0.00";
            dataGridView1.Columns["KritZal"].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;

            dataGridView1.Columns["IzasloMes"].Width = 100;
            dataGridView1.Columns["IzasloMes"].DefaultCellStyle.Format = "#,##0.00";
            dataGridView1.Columns["IzasloMes"].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;

            dataGridView1.Columns["ProdatoMes"].Width = 100;
            dataGridView1.Columns["ProdatoMes"].DefaultCellStyle.Format = "#,##0.00";
            dataGridView1.Columns["ProdatoMes"].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;

            dataGridView1.Columns["VisakZaliha"].Width = 100;
            dataGridView1.Columns["VisakZaliha"].DefaultCellStyle.Format = "#,##0.00";
            dataGridView1.Columns["VisakZaliha"].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
        ...
        private void SetUI()
        {
            ...
            _magacini.Result.Add(new Komercijalno.Magacin() { ID = -1, Naziv = " < izaberi magacin > " });
        ...
        private void UpdateDGV()
        {
            ...
            slogova_lbl.Text = "Slogova: " + (_shownData == null ? 0 : _shownData.Rows.Count).ToString();

            if (dataGridView1.DataSource == null || _shownData == null || _shownData.Rows.Count == 0)
                return;
        ...
        private void prikazi_btn_Click(object sender, EventArgs e)
        {
            ...
            Task.Run(() =>
            {
                ...
                Task<List<Komercijalno.Stavka>> stavke = Task.Run<List<Komercijalno.Stavka>>(() =>
                {
                    ...
                    for (int y = 0; y <= yearsDiff; y++)
                    {
                        ...
                        tasks[y] = Task.Run(() =>
                        {
                            ...
                            if (i != 0 && !Komercijalno.Komercijalno.CONNECTION_STRING.ContainsKey(godina))
                            {
                            ...
                            {
                                con.Open();
                                List<Komercijalno.Stavka> mainList = Komercijalno.Stavka.ListByMagacinID(con, _trenutniMagacin);
                                lock (list)
                                {
                ...
                Task<List<Komercijalno.RobaUMagacinu>> robaUMagacinu = Task.Run<List<Komercijalno.RobaUMagacinu>>(() =>
                {
                    List<Komercijalno.RobaUMagacinu> rum = Komercijalno.RobaUMagacinu.ListByMagacinID(_trenutniMagacin);
                    if (listaRobe_cb.Checked)
                    {
                ...
                Task<List<Komercijalno.Stavka>> stavkeProdajeMagacina = Task.Run<List<Komercijalno.Stavka>>(() =>
                {
                    return stavke.Result.Where(x => x.MagacinID == _trenutniMagacin && new[] { 15, 13 }.Contains(x.VrDok)).ToList();
                });
                Task<List<Komercijalno.Stavka>> stavkeIzlazaMagacina = Task.Run<List<Komercijalno.Stavka>>(() =>
                {
                    return stavke.Result.Where(x => x.MagacinID == _trenutniMagacin && Komercijalno.Komercijalno.izlazniVrDok.Contains(x.VrDok)).ToList();
                });
                Task<List<Komercijalno.Dokument>> prodajniDokumentiPerioda = Task.Run<List<Komercijalno.Dokument>>(() =>
                {
                    List<Komercijalno.Dokument> doks = Komercijalno.Dokument.ListByVrDok(DateTime.Now.Year, 15);
                    doks.AddRange(Komercijalno.Dokument.ListByVrDok(DateTime.Now.Year, 13));
                    doks.RemoveAll(x => x.MagacinID != _trenutniMagacin || x.Datum.Date < odDatuma_dtp.Value.Date || x.Datum.Date > doDatuma_dtp.Value.Date);
                    return doks;
                });
                Task<List<Komercijalno.Dokument>> izlazniDokumentiPerioda = Task.Run<List<Komercijalno.Dokument>>(() =>
                {
                ...
                Task<Dictionary<int, double>> ukupnoProdato = Task.Run<Dictionary<int, double>>(() =>
                {
                    ...
                    Parallel.ForEach(robaUMagacinu.Result, r =>
                    {
                        double res = stavkeProdajeMagacina.Result.Where(x => x.RobaID == r.RobaID &&
                            prodajniDokumentiPerioda.Result.Any(y => y.VrDok == x.VrDok && y.BrDok == x.BrDok))
                            .Sum(x => x.Kolicina);
                        lock (dict)
                        {
                            dict[r.RobaID] = res;
                ...
                Task<Dictionary<int, double>> ukupnoIzaslo = Task.Run<Dictionary<int, double>>(() =>
                {
                    ...
                    Parallel.ForEach(robaUMagacinu.Result, r =>
                    {
                        double res = stavkeIzlazaMagacina.Result.Where(x => x.RobaID == r.RobaID &&
                            izlazniDokumentiPerioda.Result.Any(y => y.VrDok == x.VrDok && y.BrDok == x.BrDok))
                            .Sum(x => x.Kolicina);
                        lock (dict)
                        {
                            dict[r.RobaID] = res;
                ...
                double nMeseci = (doDatuma_dtp.Value.Date - odDatuma_dtp.Value.Date).TotalDays / 30.5;
                double ukupnaVrednostLagera = 0;

                foreach (Komercijalno.RobaUMagacinu rum in robaUMagacinu.Result)
                {
                    Komercijalno.Roba r = _roba.Result.Where(x => x.ID == rum.RobaID).FirstOrDefault();
                ...
                {
                    ukupnaVrednostLagera_txt.Text = ukupnaVrednostLagera.ToString("#,##0.00 RSD");
                    dataGridView1.Columns["Vrednost"].HeaderText = "Vrednost (Stanje * Prodajna Cena)";
                    ...
                    if (godineZaKojeStavkeNisuUcitane.Count > 0)
                        MessageBox.Show("Za godine " + string.Join(", ", godineZaKojeStavkeNisuUcitane) + " nisu ucitani podaci!");
        ...
        {
            if(filterStanjeOperacija_cmb.SelectedIndex == 0 || filterStanjeUslov_cmb.SelectedIndex == 0)
            {
            ...
            string stanjeOperacija = filterStanjeOperacija_cmb.SelectedIndex == 1 ? "=" : filterStanjeOperacija_cmb.SelectedIndex == 2 ? ">" : "<";
        ...
        private void comboBox1_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            switch (kolonaVrednsot_cmb.SelectedIndex)
            {
                case 0:
                    ...
                    break;
                case 1:
                    dataGridView1.Columns["Vrednost"].HeaderText = "Vrednost (Stanje - Izaslo mesecno)";
                    break;
                case 2:
                    dataGridView1.Columns["Vrednost"].HeaderText = "Vrednost (Stanje - Opt Zalihe)";
                    break;
                case 3:
                    dataGridView1.Columns["Vrednost"].HeaderText = "Vrednost (Opt Zalihe - Izaslo Mesecno)";
                    break;
                case 4:
                    dataGridView1.Columns["Vrednost"].HeaderText = "Vrednost (Prodato Mesecno * Prodajna cena)";
                    break;
                case 5:
                    dataGridView1.Columns["Vrednost"].HeaderText = "Vrednost (Stanje - Prodato Mesecno)";
                    break;
                case 6:
                    dataGridView1.Columns["Vrednost"].HeaderText = "Vrednost (Opt Zalihe - Prodato Mesecno)";
                    break;
                case 7:
                    dataGridView1.Columns["Vrednost"].HeaderText = "Vrednost (Visak Zaliha * Prodajna Cena)";
                    break;
                default:
