...
{
    public static partial class TDWebApi
    {
        ...
        {
            return GetRawAsync("/me")
                .GetAwaiter()
                .GetResult().Status == System.Net.HttpStatusCode.OK;
        }
        ...
        public static async Task AuthenticateAsync()
        {
            try
            {
                var response = await PostAsync<AuthenticateRequest, string>("/authenticate", new AuthenticateRequest
                {
                    Username = "ALEKS13",
                    Password = "kalozin"
                });
                ...
                if (response.Status != System.Net.HttpStatusCode.OK)
                    throw new HttpRequestException(response.Status.ToString());

                HttpClient.DefaultRequestHeaders.Add("Authorization", "bearer " + response.Payload.ToString());
            }
            catch(Exception ex)
            {
                var a = ex;
