        ...
        public Task<IActionResult> Update([Required] Models.Webshop.Proizvod o)
        {
            return Task.Run<IActionResult>(() =>
            {
                try
                {
                    ...
                    {
                        con.Open();
                        using (MySqlCommand cmd = new MySqlCommand(@"UPDATE PROIZVOD SET
SLIKA = @SLIKA,
...
WHERE ROBAID = @ROBAID", con))
                        {
                            ...
                            cmd.Parameters.Add("@KUPOVINA_SAMO_U_TRANSPORTNOM_PAKOVANJU", MySqlDbType.Int16);
                            cmd.Parameters.Add("@ISTAKNUTI_PROIZVODI", MySqlDbType.VarChar);
                            cmd.Parameters.Add("@UPOZORENJE_ZALIHA_MALIH_STOVARISTA", MySqlDbType.Int16);
                            ...
                            cmd.Parameters["@TRANSPORTNO_PAKOVANJE_JM"].Value = o.TransportnoPakovanjeJM;
                            cmd.Parameters["@KUPOVINA_SAMO_U_TRANSPORTNOM_PAKOVANJU"].Value = o.KupovinaSamoUTransportnomPakovanju;
                            cmd.Parameters["@ISTAKNUTI_PROIZVODI"].Value = o.IstaknutiProizvodi;
                            cmd.Parameters["@UPOZORENJE_ZALIHA_MALIH_STOVARISTA"].Value = o.UpozorenjeZalihaMalihStovarista;
                            ...
                            cmd.Parameters["@SUBGROUPS"].Value = JsonConvert.SerializeObject(o.Podgrupe);
        ...
        public Task<IActionResult> IDSet(int oldID, int newID)
        {
            return Task.Run<IActionResult>(() =>
            {
                try
                {
                    ...
                    {
                        con.Open();
                        using (MySqlCommand cmd = new MySqlCommand("UPDATE PROIZVOD SET ROBAID = @ID WHERE ROBAID = @OLD", con))
                        {
                    ...
                    {
                        con.Open();
                        using (MySqlCommand cmd = new MySqlCommand("UPDATE ROBA SET ROBAID = @NEWID WHERE ROBAID = @OLDID", con))
                        {
            ...
            return Task.Run<IActionResult>(() =>
            {
                try
                {
                    lock(_insertLock)
                    {
                        ...
                        {
                            con.Open();
                            using (MySqlCommand cmd = new MySqlCommand(@"INSERT INTO PROIZVOD
...
@KUPOVINA_SAMO_U_TRANSPORTNOM_PAKOVANJU, @ISTAKNUTI_PROIZVODI,
@UPOZORENJE_ZALIHA_MALIH_STOVARISTA, @CENOVNA_GRUPA_ID, @POVEZANI_PROIZVODI,
@AKTUELNI_RABAT, @SUBGROUPS, @PARENT, @BREND_ID)", con))
                            {
                                cmd.Parameters.Add("@ROBAID", MySqlDbType.Int32);
                                cmd.Parameters.Add("@SLIKA", MySqlDbType.VarChar);
                                cmd.Parameters.Add("@PODGRUPAID", MySqlDbType.Int32);
        ...
        public Task<IActionResult> Get(int id, string rel)
        {
            return Task.Run<IActionResult>(() =>
            {
                if (id <= 0 && string.IsNullOrWhiteSpace(rel) ||
                ...
                try
                {
                    ...
                    {
                        con.Open();
                        string cmdText = @"SELECT ROBAID, SLIKA, PODGRUPAID, AKTIVAN, PDV, DISPLAY_INDEX, KRATAK_OPIS,
...
BREND_ID FROM PROIZVOD WHERE ";
                        ...
                        using (MySqlCommand cmd = new MySqlCommand(cmdText, con))
                        {
                            ...
                            using (MySqlDataReader dr = cmd.ExecuteReader())
                                if (dr.Read())
                                    return StatusCode(200, new Models.Webshop.Proizvod()
                                    {
                ...
                try
                {
                    ...
                    using(MySqlConnection con = new MySqlConnection(Program.ConnectionStringWebshop))
                    {
                        ...
                        using(MySqlCommand cmd = new MySqlCommand(@"SELECT ROBAID, SLIKA, PODGRUPAID, AKTIVAN, PDV, DISPLAY_INDEX, KRATAK_OPIS,
                            ...
                            using(MySqlDataReader dr = cmd.ExecuteReader())
                                while (dr.Read())
                                    list.Add(new Models.Webshop.Proizvod()
                                    {
                                        RobaID = Convert.ToInt32(dr["ROBAID"]),
                                        Slika = dr["SLIKA"].ToString(),
                                        PodgrupaID = Convert.ToInt32(dr["PODGRUPAID"]),
