...
namespace TD.Web.Common.Repository.DbMappings
{
	public class UserEntityMap : LSCoreEntityMap<UserEntity>
	{
		...
		public override Action<EntityTypeBuilder<UserEntity>> Mapper { get; } =
			entityTypeBuilder =>
			{
				entityTypeBuilder.HasIndex(x => x.Username).IsUnique();
				...
				entityTypeBuilder.Property(x => x.FavoriteStoreId).IsRequired();
				...
				entityTypeBuilder.Property(x => x.Mail).HasMaxLength(_mailMaxCharacters);

				entityTypeBuilder.Property(x => x.Type).IsRequired();

				entityTypeBuilder.Property(x => x.Comment).HasMaxLength(_commentMaxLength);
