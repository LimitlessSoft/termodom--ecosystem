...
export default {
    beforeExecution: async () => {
        await webDbClient.productGroupRepository.create(expected.productGroup.name, null, expected.productGroup.src)
    },
    afterExecution: async () => {
        await webDbClient.productGroupRepository.hardDelete(expected.productGroup.name)
        await webDbClient.disconnect()
    },
    execution: async (driver) => {
        await driver.get(PROJECT_URL)
        
        const buttonGroupLocator = By.xpath(`/html/body/div/div/main/div[2]/div/div[1]//div[./button[text()="${expected.productGroup.name}"]]`)
        await driver.wait(until.elementLocated(buttonGroupLocator), WAIT_TIMEOUT)
        ...
        const backButtonLocator = By.xpath(`/html/body/div/div/main/div[2]/div/div[1]/div/button`)
        await driver.wait(until.elementLocated(backButtonLocator), WAIT_TIMEOUT)
        
        const backButtonText = await (await driver.findElement(backButtonLocator)).getText()
