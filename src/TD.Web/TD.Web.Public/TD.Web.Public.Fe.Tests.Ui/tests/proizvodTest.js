...
export default async (driver) => {
    ...
    await driver.wait(until.elementLocated(productLocator), WAIT_TIMEOUT * 3)
    ...
    const hasAnyChild = await page.findElements(By.xpath('./*')).then(elements => elements.length > 0)
    assert(hasAnyChild)
}