...
public class ProductGroupManager(IProductGroupRepository repository) : IProductGroupManager
{
	...
	public List<ProductsGroupsGetDto> GetMultiple() =>
		repository.GetMultiple().ToMappedList<ProductGroupEntity, ProductsGroupsGetDto>();

	public long Save(ProductsGroupsSaveRequest request)
	{
		var entity = request.Id == 0 ? new ProductGroupEntity() : repository.Get(request.Id!.Value);
