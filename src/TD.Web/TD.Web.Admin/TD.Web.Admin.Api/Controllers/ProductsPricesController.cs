...
public class ProductsPricesController(IProductPriceManager productPriceManager) : ControllerBase
{
	...
	public List<ProductsPricesGetDto> GetMultiple() => productPriceManager.GetMultiple();

	[HttpPut]
	[Route("/products-prices")]
	public long Save(SaveProductPriceRequest request) => productPriceManager.Save(request);

	[HttpDelete]
	[Route("/products-prices/{id}")]
	public void Delete([FromRoute] LSCoreIdRequest request) => productPriceManager.Delete(request);
}
