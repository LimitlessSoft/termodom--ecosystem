...
namespace Termodom.Controllers
{
    ...
    {
        private static string[] _allowedExtensions = new string[3]
        {
            ".jpg", ".jpeg", ".png"
        };
        ...
        public async Task<IActionResult> GetAllImagesDisplaySmall()
        {
            ...
            {
                List<string> images = new List<string>();
                images.AddRange(Directory.GetFiles(Path.Combine(Program.WebRootPath, "img\\gallery\\Source\\")));
                return View("/Views/Gallery/_pGalleryItemDisplaySmall.cshtml", images);
            });
        }
        #endregion
        ...
        public async Task<IActionResult> Upload(IFormFile file)
        {
            return await Task.Run<IActionResult>(async () =>
            {
                ...
                using (Stream fileStream = new FileStream(Path.Combine(Program.WebRootPath, "img\\gallery\\Source\\" + file.FileName), FileMode.Create))
                ...
                if (t1024 && t512 && t256 && t128 && t64 && t32)
                    return StatusCode(200, Path.Combine(Program.WebRootPath, "\\img\\gallery\\Source\\" + file.FileName));
        ...
        private bool SaveTo(IFormFile file, Quality quality)
        {
            try
            {
                using (Stream memoryStream = new MemoryStream())
                {
                    ...
                    int q = Convert.ToInt32(quality.ToString().Substring(1, quality.ToString().Length - 1));

                    double K = q / (double)Math.Min(origin.Height, origin.Width);

                    Bitmap newPic = new Bitmap(origin, new Size(Convert.ToInt32(origin.Width * K), Convert.ToInt32(origin.Height * K)));
                    newPic.Save(Path.Combine(Program.WebRootPath, "img\\gallery\\" + quality.ToString().Substring(1, quality.ToString().Length - 1) + "/" + file.FileName));
                }
