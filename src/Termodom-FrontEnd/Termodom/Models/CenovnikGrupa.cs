...
namespace Termodom.Models
{
    ...
    public class CenovnikGrupa
    {
        ...
        {
            using(MySqlCommand cmd = new MySqlCommand("SELECT MAX(CENOVNIK_GRUPAID) FROM CENOVNIK_GRUPA", con))
            {
        ...
        public static int Insert(MySqlConnection con, string Naziv)
        {
            lock (_insertLock)
            {
                using (MySqlCommand cmd = new MySqlCommand("INSERT INTO CENOVNIK_GRUPA(NAZIV) VALUES(@NAZIV)", con))
                {
        ...
        public static CenovnikGrupa Get(MySqlConnection con, int id)
        {
            ...
            // TODO: Implement
            using(MySqlCommand cmd = new MySqlCommand("SELECT NAZIV FROM CENOVNIK_GRUPA WHERE CENOVNIK_GRUPAID = @ID", con))
            {
                using(MySqlDataReader dt = cmd.ExecuteReader())
                {
                    if (dt.Read())
        ...
        {
            using(MySqlCommand cmd = new MySqlCommand("DELETE FROM CENOVNIK_GRUPA WHERE CENOVNIK_GRUPAID = @ID", con))
            {
                cmd.ExecuteNonQuery();
            }
