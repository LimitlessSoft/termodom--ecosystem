...
namespace Termodom.Models
{
    public partial class Porudzbina
    {
        ...
        public void Update()
        {
            HttpRequestMessage request = new HttpRequestMessage(HttpMethod.Post, Program.BaseAPIUrl + "/Webshop/Porudzbina/Update");
            request.Content = System.Net.Http.Json.JsonContent.Create<DTO.Webshop.PorudzbinaDTO>(DTO.Webshop.PorudzbinaDTO.ConvertPorudzbina(this)); // TODO: DTO.WebShop.PorudzbinaUpdateDTO
        ...
        public double MPVrednost()
        {
            ...
            return Stavke.Sum(x => x.Kolicina * x.VPCena * ((100 + proizvodi.Where(y => y.RobaID == x.RobaID).FirstOrDefault().PDV) / 100));
        }
        /// <summary>
        ...
        public double MPVrednost(List<Proizvod> proizvodiBuffer)
        {
            try
            {
                double ukupno = Stavke.Sum(x => x.Kolicina * x.VPCena * ((100 + proizvodiBuffer.Where(y => y.RobaID == x.RobaID).FirstOrDefault().PDV) / 100));
                return ukupno;
            }
            catch
            {
        ...
        public static string Insert(Porudzbina porudzbina) // TODO: PorudzbinaInsertDTO
        {
            ...
            HttpRequestMessage request = new HttpRequestMessage(HttpMethod.Post, Program.BaseAPIUrl + "/Webshop/Porudzbina/Insert");
            // TODO: DTO.WebShop.PorudzbinaInsertDTO
            APIRequestFailedLog failedLog = null;
            request.Content = System.Net.Http.Json.JsonContent.Create<DTO.Webshop.PorudzbinaDTO>(DTO.Webshop.PorudzbinaDTO.ConvertPorudzbina(porudzbina));
            HttpResponseMessage response = APIRequest.Send(request, out failedLog);
        ...
        {
            HttpRequestMessage request = new HttpRequestMessage(HttpMethod.Get, Program.BaseAPIUrl + "/Webshop/Porudzbina/Get?id=" + id);

            APIRequestFailedLog failedLog = null;
            HttpResponseMessage response = APIRequest.Send(request, out failedLog);

            if (response.StatusCode == System.Net.HttpStatusCode.OK)
                return Porudzbina.ConvertPorudzbinaDTO(JsonConvert.DeserializeObject<DTO.Webshop.PorudzbinaDTO>(response.Content.ReadAsStringAsync().Result));
            else if (response.StatusCode == System.Net.HttpStatusCode.GatewayTimeout)
                throw new APIRequestTimeoutException(failedLog);
            else if (response.StatusCode == System.Net.HttpStatusCode.InternalServerError)
                throw new APIRequestInternalServerErrorException();
        ...
        {
            HttpRequestMessage request = new HttpRequestMessage(HttpMethod.Get, Program.BaseAPIUrl + "/Webshop/Porudzbina/List");

            APIRequestFailedLog failedLog = null;
            HttpResponseMessage response = APIRequest.Send(request, out failedLog);

            if (response.StatusCode == System.Net.HttpStatusCode.OK)
                return Porudzbina.ConvertToListPorudzbinaDTO(JsonConvert.DeserializeObject<List<DTO.Webshop.PorudzbinaDTO>>(response.Content.ReadAsStringAsync().Result));
            else if (response.StatusCode == System.Net.HttpStatusCode.GatewayTimeout)
                throw new APIRequestTimeoutException(failedLog);
            else if (response.StatusCode == System.Net.HttpStatusCode.InternalServerError)
                throw new APIRequestInternalServerErrorException();
        ...
        public static List<Porudzbina> ConvertToListPorudzbinaDTO(List<DTO.Webshop.PorudzbinaDTO> list) // TODO: Remove
        {
            // TODO: Remove
            List<Porudzbina> newList = new List<Porudzbina>();
            foreach (DTO.Webshop.PorudzbinaDTO p in list)
                newList.Add(new Porudzbina()
                {
                    ID = p.ID,
                    BrDokKom = p.BrDokKomercijalno,
                    UserID = p.KorisnikID,
