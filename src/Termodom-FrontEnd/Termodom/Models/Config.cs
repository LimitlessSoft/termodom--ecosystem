...
namespace Termodom.Models
{
    ...
    public class Config<T>
    {
        ...
        {
            using (MySqlCommand cmd = new MySqlCommand("UPDATE CONFIG SET NAME = @N, VALUE = @V WHERE ID = @ID", con))
            {
        ...
        {
            using(MySqlCommand cmd = new MySqlCommand("SELECT ID, NAME, VALUE FROM CONFIG WHERE ID = @ID", con))
            {
                cmd.Parameters.AddWithValue("@ID", id);
        ...
        {
            name = name.ToUpper();
            using (MySqlCommand cmd = new MySqlCommand("SELECT ID, NAME, VALUE FROM CONFIG WHERE NAME = @NAME", con))
            {
        ...
        public static int Insert(MySqlConnection con, string name, T value)
        {
            ...
            lock (___insertLock)
            {
                ...
                using (MySqlCommand cmd = new MySqlCommand("INSERT INTO CONFIG (ID, NAME, VALUE) VALUES (@ID, @NAME, @VALUE)", con))
                {
                    cmd.Parameters.AddWithValue("@ID", id);
