...
namespace Termodom.Models
{
    public class CenovnikBuffer
    {
        ...
        private ConcurrentDictionary<int, CenovnikBufferItem> _items = new ConcurrentDictionary<int, CenovnikBufferItem>(); // KorisnikID, BufferItem
        ...
        public CenovnikBufferItem this[int korisnikID]
        {
            get
            {
                if (_items.ContainsKey(korisnikID))
                {
                    ...
                    if (c != null)
                        if ((DateTime.Now - c.Updated).TotalSeconds < OUTDATED_TIMESPAN.TotalSeconds)
