...
export const userRepository = {
    login: async (username, password) => new Promise(async (resolve, reject) => {
        if (!username || !password) {
            reject("Korisničko ime i lozinka su obavezni")
            return
        }
        if (username.length < 3 || password.length < 3) {
            reject("Korisničko ime i lozinka moraju imati najmanje 3 karaktera")
            return
        }
    ...
    create: (username, password, type = 'user') => new Promise((async (resolve, reject) => {
        ...
        if (username.length < 3 || password.length < 3) {
            const msg = "Korisničko ime i lozinka moraju imati najmanje 3 karaktera"
            console.warn(msg)
            reject(msg)
            return
        }
