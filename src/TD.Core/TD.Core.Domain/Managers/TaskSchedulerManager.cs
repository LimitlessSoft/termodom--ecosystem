...
namespace TD.Core.Domain.Managers
{
	...
	{
		private List<System.Threading.Tasks.Task> _asyncTasks { get; set; } =
			new List<System.Threading.Tasks.Task>();
		private List<Core.Contracts.Tasks.Task> _tasks { get; set; } =
			new List<Core.Contracts.Tasks.Task>();
		...
		public System.Threading.Tasks.Task RunTasksAsync(bool runTasksAsync)
		{
			return System.Threading.Tasks.Task.Run(() =>
			{
				if (_state == TaskSchedulerState.Running)
					throw new Exception(
						"Task scheduler is already running! Stop it before starting it again!"
					);
