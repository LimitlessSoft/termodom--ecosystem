	...
	public class MinioManager
	{
		...
		)
		{
			fileName = fileName.Replace(
				...
				Contracts.Constants.Minio.DictionarySeparatorChar
			);
			...
			var be = new BucketExistsArgs().WithBucket(_bucketBase);

			bool found = await client.BucketExistsAsync(be).ConfigureAwait(false);
			if (!found)
			{
				var mb = new MakeBucketArgs().WithBucket(_bucketBase);
		...
		public async Task<Response<FileDto>> DownloadAsync(string file)
		{
			...
			try
			{
				var statObjectArgs = new StatObjectArgs().WithBucket(_bucketBase).WithObject(file);
			...
			var tagsArgs = new GetObjectTagsArgs().WithBucket(_bucketBase).WithObject(file);
