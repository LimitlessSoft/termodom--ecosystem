...
namespace TD.Core.Framework
{
	public class BaseApiStartup : BaseStartup
	{
		...
		public Func<
			IApplicationBuilder,
			IApplicationBuilder
		>? AfterAuthenticationMiddleware { get; set; } = null;

		public BaseApiStartup(
			string projectName,
			bool addAuthentication = true,
			bool useCustomAuthorizationPolicy = false
		)
			: base(projectName)
		{
					...
					.AddJwtBearer(options =>
					{
						options.TokenValidationParameters = new TokenValidationParameters()
						{
							...
							IssuerSigningKey = new SymmetricSecurityKey(
								Encoding.UTF8.GetBytes(ConfigurationRoot["Jwt:Key"])
							),
#else
							ValidIssuer = Environment
								.GetEnvironmentVariable("JWT_ISSUER")
								.ToString(),
							ValidAudience = Environment
								.GetEnvironmentVariable("JWT_AUDIENCE")
								.ToString(),
							IssuerSigningKey = new SymmetricSecurityKey(
								Encoding.UTF8.GetBytes(
									Environment.GetEnvironmentVariable("JWT_KEY").ToString()
								)
							),
#endif
							ValidateIssuer = true,
		...
		)
		{
			if (!_addAuthentication && AfterAuthenticationMiddleware != null)
				throw new Exception(
					$"You must enable authentication if you want to use {nameof(AfterAuthenticationMiddleware)}!"
				);
