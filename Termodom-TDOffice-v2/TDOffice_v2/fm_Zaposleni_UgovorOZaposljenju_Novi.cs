...
namespace TDOffice_v2
{
    public partial class fm_Zaposleni_UgovorOZaposljenju_Novi : Form
    {
        ...
        private void btn_Sacuvaj_Click(object sender, EventArgs e)
        {
            ...
            List<TDOffice.ZaposleniUgovorORadu> zuorSortedByKrajTrajanjaDesc = TDOffice.ZaposleniUgovorORadu.List("ZAPOSLENI_ID = "+ _zaposleni.ID.ToString() + " order by KRAJ_TRAJANJA DESC");
            ...
            List<TDOffice.ZaposleniUgovorORadu> ugovoriUKontinuitetu = new List<TDOffice.ZaposleniUgovorORadu>();
            TDOffice.ZaposleniUgovorORadu poslednjiUgovor = zuorSortedByKrajTrajanjaDesc.FirstOrDefault();
            foreach(TDOffice.ZaposleniUgovorORadu zu in zuorSortedByKrajTrajanjaDesc)
            {
                // ========================================
                // Proverava preklapanje datuma
                // ========================================
                if (dtp_Pocetak.Value.Date <= zu.KrajTrajanja.Date && zu.PocetakTrajanja.Date <= dtp_Kraj.Value.Date)
                {
                    MessageBox.Show("Period trajanja ovog ugovora bi se preklopio sa postojecim ugovorom br." + zu.ID.ToString() + "\nNe mogu kreirati ovaj ugovor!", "Preklapanje datuma");
                    return;
                }
                // =========================================
                // =========================================
                ...
                // =========================================
                if (zu.Firma != poslednjiUgovor.Firma ||
            ...
            // Pripremam proveru perioda provedenog u istoj firmi
            // =========================================
            if ((dtp_Kraj.Value.Date - dtp_Pocetak.Value.Date).TotalDays + ugovoriUKontinuitetu.Sum(x => (x.KrajTrajanja.Date - x.PocetakTrajanja.Date).TotalDays) > 730)
            {
                if (MessageBox.Show("Zaposleni je vec imao ugvor u ovoj firmi i sa ovim ugovorom ce mu ukupno vreme preci 2 godine.\nDa li zelite nastaviti?",
            ...
            TDOffice.ZaposleniUgovorORadu.Insert(firma, _zaposleni.ID, dtp_Pocetak.Value, dtp_Kraj.Value);
            MessageBox.Show($"Uspesno kreiran novi Ugovor o radu za zaposlenog {_zaposleni.ToString()}");
