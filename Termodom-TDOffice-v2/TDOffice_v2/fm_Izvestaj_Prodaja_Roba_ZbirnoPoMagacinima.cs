...
namespace TDOffice_v2
{
    public partial class fm_Izvestaj_Prodaja_Roba_ZbirnoPoMagacinima : Form
    {
        ...
        public fm_Izvestaj_Prodaja_Roba_ZbirnoPoMagacinima(DataTable dt, Models.Periodi periodi, string tipIzv, List<int> listaRobe)
        {
            InitializeComponent();
            _dataTable = dt;
            _periodi = periodi;
            ...
            _help = this.InitializeHelpModulAsync(Modul.fm_Izvestaj_Prodaja_Roba_ZbirnoPoMagacinima);
        }
        ...
        private void fm_Izvestaj_Prodaja_Roba_ZbirnoPoMagacinima_Load(object sender, EventArgs e)
        {
            ...
            {
                if (_dataTable.Columns.Contains("KGOD" + godina))
                    zbirDR["ZbirKolicina" + godina] = Convert.ToDouble(_dataTable.Compute("Sum(KGOD" + godina + ")", string.Empty));
                if (_dataTable.Columns.Contains("VGOD" + godina))
                    zbirDR["ZbirVrednosti" + godina] = Convert.ToDouble(_dataTable.Compute("Sum(VGOD" + godina + ")", string.Empty));
            }
            ...
            foreach (int magacin in magaciniUTabeli)
            {
                ...
                foreach (int godina in _periodi.Godine.OrderBy(x => x))
                {
                    ...
                    {
                        if (_dataTable.Columns.Contains("KGOD" + godina))
                            zk = Convert.ToDouble(_dataTable.Select("MAGACINID = " + magacin).CopyToDataTable().Compute("Sum(KGOD" + godina + ")", string.Empty));
                        if (_dataTable.Columns.Contains("VGOD" + godina))
                            zv = Convert.ToDouble(_dataTable.Select("MAGACINID = " + magacin).CopyToDataTable().Compute("Sum(VGOD" + godina + ")", string.Empty));
                    }
                    catch
                    {
            ...
            foreach (int godina in _periodi.Godine.OrderBy(x => x))
            {
                ...
                if (_dataTable.Columns.Contains("KGOD" + godina))
                {
                    dataGridView1.Columns["ZbirKolicina" + godina].DefaultCellStyle.Format = "#,##0";
                    dataGridView1.Columns["ZbirKolicina" + godina].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
                    dataGridView1.Columns["ZbirKolicina" + godina].Width = 100;
                    dataGridView1.Columns["ZbirKolicina" + godina].DisplayIndex = y;
                    dataGridView1.Columns["ZbirKolicina" + godina].HeaderText = $"ZBIR KOLICINE GOD {godina} [{_periodi[godina].OdDatuma.ToString("dd.MM.yyyy")} - {_periodi[godina].DoDatuma.ToString("dd.MM.yyyy")}]";
                ...
                if (_dataTable.Columns.Contains("VGOD" + godina))
                {
                    dataGridView1.Columns["ZbirVrednosti" + godina].DefaultCellStyle.Format = "#,##0.00 RSD";
                    dataGridView1.Columns["ZbirVrednosti" + godina].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
                    dataGridView1.Columns["ZbirVrednosti" + godina].Width = 200;
                    dataGridView1.Columns["ZbirVrednosti" + godina].DisplayIndex = !_dataTable.Columns.Contains("KGOD" + godina) ? y : y + _periodi.Count;
                    dataGridView1.Columns["ZbirVrednosti" + godina].HeaderText = $"ZBIR VREDNOSTI GOD {godina} [{_periodi[godina].OdDatuma.ToString("dd.MM.yyyy")} - {_periodi[godina].DoDatuma.ToString("dd.MM.yyyy")}]";
        ...
        private void NamestiKolone()
        {
            ...
            {
                if (_dataTable.Columns.Contains("KGOD" + godina))
                    dataGridView1.Columns["ZbirKolicina" + godina].Visible = prikaziKolicine_cb.Checked;

                if (_dataTable.Columns.Contains("VGOD" + godina))
                    dataGridView1.Columns["ZbirVrednosti" + godina].Visible = prikaziVrednosti_cb.Checked;
            }
        }
        ...
        {
            using (fm_Izvestaj_Prodaja_Roba_ZbirnoPoMagacinima_PosaljiKaoIzvestaj_Setup st = new fm_Izvestaj_Prodaja_Roba_ZbirnoPoMagacinima_PosaljiKaoIzvestaj_Setup(_dataTable, _periodi, _tipIzvestaja))
                st.ShowDialog();
        }
        ...
        private void listRobe_btn_Click(object sender, EventArgs e)
        {
            ...
            using (fm_Izvestaj_Prodaja_Roba_Index ind = new fm_Izvestaj_Prodaja_Roba_Index(_dataTable, _periodi, "asd",nLista,nLista))
            {
        ...
        private void OsveziChart()
        {
            ...
            foreach(DataGridViewRow row in dataGridView1.Rows)
                if(!(row.Cells["GRAFIKON"].Value is DBNull) && Convert.ToBoolean(row.Cells["GRAFIKON"].Value) == true)
                {
