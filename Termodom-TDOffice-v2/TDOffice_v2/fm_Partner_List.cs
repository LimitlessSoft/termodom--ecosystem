...
namespace TDOffice_v2
{
    ...
    {
        private Task<List<TDOffice.Partner>> _tdOfficePartneri = TDOffice.Partner.ListAsync("NAZIV NOT LIKE '%undefined%' AND NAZIV NOT LIKE '%BLOKIRAN%'");
        ...
        private Task PrikaziPartnereAsync()
        {
            return Task.Run(() =>
            {
                try
                {
                    ...
                    {
                        Komercijalno.Magacin m = p.MagacinID == null ?
                            _magaciniKomercijalno.Result.FirstOrDefault(x => x.ID == -1) :
                            _magaciniKomercijalno.Result.FirstOrDefault(x => x.ID == (int)p.MagacinID);
                        DataRow dr = dt.NewRow();
                        dr["PPID"] = p.ID;
                        dr["Naziv"] = p.Naziv;
                        dr["Mobilni"] = MobileNumber.IsValid(p.Mobilni) ? MobileNumber.Collate(p.Mobilni) : "";
                        dr["MestoID"] = p.Grad;
                        ...
                        dr["PripadaGrupama"] = p.Grupe == null ? "" : string.Join(", ", _partnerGrupe.Result.Where(x => p.Grupe.Any(y => y == x.ID)).Select(x => x.Naziv));
        ...
        {
            int partnerID = Convert.ToInt32(dataGridView1.Rows[dataGridView1.SelectedCells[0].RowIndex].Cells["PPID"].Value);

            using (fm_Partner_Index pi = new fm_Partner_Index(partnerID))
            {
                pi.PartnerIzmenjen += () =>
                {
                    _tdOfficePartneri = TDOffice.Partner.ListAsync("NAZIV NOT LIKE '%undefined%' AND NAZIV NOT LIKE '%BLOKIRAN%'");
        ...
        {
            using(InputBox ib = new InputBox("Tekst SMS poruke", "Unesite tekst SMS poruke (max 140 karaktera). Ukoliko zelite odustati pritisnite 'odustani'."))
            {
                ib.ShowDialog();
                ...
                try
                {
                    ...
                    {
                        SMSGateway.PosaljiSms(izlistani.Rows[i]["Mobilni"].ToString(), ib.returnData);
                        nPoslato++;
                    }
                ...
                {
                    MessageBox.Show("Doslo je do greske prilikom komunikacije sa SMSGatewayom. Poslatih poruka: " + nPoslato);
                    MessageBox.Show(ex.ToString());
                }
        ...
        private void filtriraj_btn_Click(object sender, EventArgs e)
        {
            ...
            List<string> cekiraneGrupe = grupe_clb.CheckedItems.OfType<TDOffice.PartnerGrupa>().Select(x => x.ID.ToString()).ToList();

            for (int i = newDt.Rows.Count - 1; i >= 0; i--)
                if (cekiraneGrupe.Any(x => newDt.Rows[i]["Grupe"].ToString().Split(',').Contains(x)))
                    continue;
                else
                    newDt.Rows.RemoveAt(i);


            List<int> cekiraniMagacini = magacin_clb.CheckedItems.OfType<Komercijalno.Magacin>().Select(x => x.ID).ToList();
