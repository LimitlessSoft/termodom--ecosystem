...
namespace TDOffice_v2
{
    public partial class fm_Partner_Index : Form
    {
        ...
        public fm_Partner_Index(int partnerID)
        {
            ...
            magacin_cmb.SelectedValue = _partner.MagacinID == null ? -1 : _magaciniKomercijalno.Result.Any(x => x.ID == _partner.MagacinID) ? (int)_partner.MagacinID : -2;
        ...
        private void sacuvaj_btn_Click(object sender, EventArgs e)
        {
            if(_partner == null)
            {
                ...
                TDOffice.Partner p = TDOffice.Partner.List($"MOBILNI = '{MobileNumber.Collate(mobilni_txt.Text)}'").FirstOrDefault();

                if(p != null)
                {
                    if(p.Naziv.ToLower().Contains("undefined"))
                    {
                        p.WebID = string.IsNullOrWhiteSpace(webID_txt.Text) ? null : (int?)Convert.ToInt32(webID_txt.Text);
                        p.KomercijalnoPPID = string.IsNullOrWhiteSpace(ppidKomercijalno_txt.Text) ? null : (int?)Convert.ToInt32(ppidKomercijalno_txt.Text);
                        p.MagacinID = Convert.ToInt32(magacin_cmb.SelectedValue) > 0 ? (int?)Convert.ToInt32(magacin_cmb.SelectedValue) : null;
                    ...
                    {
                        MessageBox.Show($"Partner sa ovim mobilnim telefonom vec postoji!\nNjegov ID je {p.ID}, a naziv {p.Naziv}!");
                        return;
                    }
                }
                ...
                TDOffice.Partner.Insert(
                    ...
                    komentar_rtx.Text,
                    grupe,
                    string.IsNullOrWhiteSpace(webID_txt.Text) ? null : (int?)Convert.ToInt32(webID_txt.Text),
                    string.IsNullOrWhiteSpace(ppidKomercijalno_txt.Text) ? null : (int?)Convert.ToInt32(ppidKomercijalno_txt.Text),
                    Convert.ToInt32(magacin_cmb.SelectedValue) > 0 ? (int?)Convert.ToInt32(magacin_cmb.SelectedValue) : null
                    );
            ...
            {
                _partner.WebID = string.IsNullOrWhiteSpace(webID_txt.Text) ? null : (int?)Convert.ToInt32(webID_txt.Text);
                _partner.KomercijalnoPPID = string.IsNullOrWhiteSpace(ppidKomercijalno_txt.Text) ? null : (int?)Convert.ToInt32(ppidKomercijalno_txt.Text);
                _partner.Naziv = naziv_txt.Text;
                _partner.Mobilni = MobileNumber.Collate(mobilni_txt.Text);
                _partner.MagacinID = Convert.ToInt32(magacin_cmb.SelectedValue) > 0 ? (int?)Convert.ToInt32(magacin_cmb.SelectedValue) : null;
        ...
        private void uporediInformacijeToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                Komercijalno.Magacin webMag = _magaciniKomercijalno.Result.FirstOrDefault(x => x.ID == kor.MagacinID);
                Komercijalno.Magacin tdMag = _partner.MagacinID == null ? _magaciniKomercijalno.Result.FirstOrDefault(x => x.ID == -1) : _magaciniKomercijalno.Result.FirstOrDefault(x => x.ID == kor.MagacinID);
                Task.Run(() =>
                {
                    MessageBox.Show($"Korisnik ima neslaganje u parametru MagacinID na webu i tdofficeu.\nTDOffice_v2: {tdMag.Naziv}\nWEB: {webMag.Naziv}");
                });
            }
