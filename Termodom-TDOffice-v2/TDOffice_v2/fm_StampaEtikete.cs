...
namespace TDOffice_v2
{
    ...
    { 
        ...
        private void SetDGV()
        {
            ...
            dataGridView1.Columns["ProdajnaCenaDokumenta"].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
            ...
            dataGridView1.Columns["NabavnaCenaSaPorezom"].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
            ...
            dataGridView1.Columns["FakturnaCena"].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
            ...
            dataGridView1.Columns["ProdajnaCenaBezPoreza"].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
        ...
        private void btn_Prikazi_Click(object sender, EventArgs e)
        {
            ...
            if (!int.TryParse(tb_BrojDokumenta.Text, out brDok)) { MessageBox.Show("Neispravan broj dokumenta!"); return; }

            Dokument _dokument = Dokument.Get(DateTime.Now.Year, vrDok, brDok);

            if (_dokument == null) { MessageBox.Show("Dokument " + brDok + " nije pronadjen"); return; }
        ...
        {
            if (MessageBox.Show("Da li sigurno zelite obrisati sve stavke iz liste?", "Potvrdi", MessageBoxButtons.YesNo) != DialogResult.Yes)
                return;
        ...
        private void btn_Stampaj_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < _dt.Rows.Count; i++)
            {
                ...
                gfx.DrawLine(XPens.Black, new Point(currentX, currentY), new Point(marginLeft + ((i % 3) + 1) * sirinaEtikete, currentY));
                gfx.DrawLine(XPens.Black, new Point(currentX, currentY + visinaEtikete), new Point(marginLeft + ((i % 3) + 1) * sirinaEtikete, currentY + visinaEtikete));
                gfx.DrawLine(XPens.Black, new Point(currentX, currentY), new Point(marginLeft + (i % 3) * sirinaEtikete, currentY + visinaEtikete));
                gfx.DrawLine(XPens.Black, new Point(marginLeft + ((i % 3) + 1) * sirinaEtikete, currentY), new Point(marginLeft + ((i % 3) + 1) * sirinaEtikete, currentY + visinaEtikete));

                tempY += padding + (int)p.Size + 1;
                gfx.DrawString("Kataloski broj", p, XBrushes.Black, new Point(currentX + padding, tempY));

                tempY += (int)p1.Size + 1;
                gfx.DrawString(_dt.Rows[i]["KatBr"].ToString().Substring(0, Math.Min(_dt.Rows[i]["KatBr"].ToString().Length, 11)), p1, XBrushes.Black, new Point(currentX + padding, tempY));
                ...
                gfx.DrawString(_dt.Rows[i]["JM"].ToString(), p1, XBrushes.Black, new Point(currentX + padding, currentY + visinaEtikete - 2*padding));


                tempY = currentY + visinaEtikete - padding - 30;
                gfx.DrawString("SA POREZOM", p2, XBrushes.Black, new Point(currentX + sirinaEtikete/2 + padding, tempY));

                tempY += (int)p2.Size + 10;
                string strProdajnaCena = string.Format("{0:#,##0.00 DIN}", Convert.ToDouble(_dt.Rows[i]["ProdajnaCena"]));
                Size s = TextRenderer.MeasureText(strProdajnaCena, new Font(new FontFamily(p3.FontFamily.Name.ToString()), 1.0f, p3.Bold ? FontStyle.Bold : FontStyle.Regular));

                gfx.DrawString(strProdajnaCena, p3, XBrushes.Black, new Point(currentX + sirinaEtikete - padding - s.Width * 7, tempY));

                gfx.DrawString(DateTime.Now.ToString("dd MM yyyy"), p4, XBrushes.Black, new Point(currentX + sirinaEtikete - 6 * padding, currentY + visinaEtikete - 5));
