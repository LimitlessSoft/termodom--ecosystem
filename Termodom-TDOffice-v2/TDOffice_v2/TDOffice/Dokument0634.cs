...
namespace TDOffice_v2.TDOffice
{
    public class Dokument0634
    {
        ...
        {
            using (FbCommand cmd = new FbCommand(@"UPDATE DOKUMENT_0634 SET MAGACINID = @MID, STATUS = @STATUS, KOMERCIJALNO_FAKTURA = @KOMERCFAKTURA WHERE ID = @ID", con))
            {
        ...
        {
            using (FbCommand cmd = new FbCommand("SELECT ID, DATUM, USERID, MAGACINID, STATUS, KOMERCIJALNO_FAKTURA FROM DOKUMENT_0634 WHERE ID = @ID", con))
            {
                ...
                using (FbDataReader dr = cmd.ExecuteReader())
                {
                    if (dr.Read())
                    {
                        return new Dokument0634()
                        {
                            ...
                            KomercijalnoFaktura = dr["KOMERCIJALNO_FAKTURA"] is DBNull ? null : (int?)Convert.ToInt32(dr["KOMERCIJALNO_FAKTURA"])
                        };
                    }
                }
            }
        ...
        public static int Insert(FbConnection con, int userID, int magacinID, int status, int? komercijalnoFaktura)
        {
            using(FbCommand cmd = new FbCommand(@"INSERT INTO DOKUMENT_0634
        ...
        public static List<Dokument0634> List(FbConnection con, string queryString)
        {
            ...
            using(FbCommand cmd = new FbCommand("SELECT ID, DATUM, USERID, MAGACINID, STATUS, KOMERCIJALNO_FAKTURA FROM DOKUMENT_0634 WHERE 1 = 2" + queryString, con))
            {
                using(FbDataReader dr = cmd.ExecuteReader())
                {
                    while(dr.Read())
                    {
                        list.Add(new Dokument0634()
                        {
                            ID = Convert.ToInt32(dr["ID"]),
                            Datum = Convert.ToDateTime(dr["DATUM"]),
                            UserID = Convert.ToInt32(dr["USERID"]),
