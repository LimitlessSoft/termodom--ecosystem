...
namespace TDOffice_v2.TDOffice
{
    public partial class User
    {
        ...
        {
            using(FbCommand cmd = new FbCommand(@"UPDATE USERS SET MAGACINID = @MID,
                ...
                WHERE ID = @ID", con))
            {
                ...
                cmd.Parameters.AddWithValue("@TAG", Encoding.UTF8.GetBytes(JsonConvert.SerializeObject(Tag)));
        ...
        {
            using (FbCommand cmd = new FbCommand("SELECT ID, USERNAME, PW, MAGACINID, KOMERCIJALNO_USER_ID, GRAD, TAG, OPOMENI_ZA_NEIZVRSENI_ZADATAK, BONUS_ZAKLJUCAVANJA_COUNT, BONUS_ZAKLJUCAVANJA_LIMIT FROM USERS WHERE ID = @ID", con))
            {
        ...
        {
            using (FbCommand cmd = new FbCommand(@"INSERT INTO USERS (ID, USERNAME, PW, MAGACINID, GRAD) VALUES
(((SELECT COALESCE(MAX(ID), 0) FROM USERS) + 1), @UN, @PW, 0, @GRAD)", con))
            {
        ...
        public static Dictionary<int, User> Dict(FbConnection con, string whereQuery = null)
        {
            ...
            using (FbCommand cmd = new FbCommand("SELECT ID, USERNAME, PW, MAGACINID, KOMERCIJALNO_USER_ID, GRAD, TAG, OPOMENI_ZA_NEIZVRSENI_ZADATAK, BONUS_ZAKLJUCAVANJA_COUNT, BONUS_ZAKLJUCAVANJA_LIMIT FROM USERS WHERE 1 = 1 " + whereQuery, con))
            {
                FbDataReader dr = cmd.ExecuteReader();
                ...
                while (dr.Read())
                {
                    ...
                    u.KomercijalnoUserID = dr["KOMERCIJALNO_USER_ID"] is DBNull ? null : (int?)Convert.ToInt32(dr["KOMERCIJALNO_USER_ID"]);
                    ...
                    u.OpomeniZaNeizvrseneZadatke = Convert.ToInt32(dr["OPOMENI_ZA_NEIZVRSENI_ZADATAK"]) == 1;
                    try
                    {
                        u.Tag = dr["TAG"] is DBNull ? new Info() : JsonConvert.DeserializeObject<Info>(Encoding.UTF8.GetString((byte[])dr["TAG"]));
                    }
                    catch
                    {
                        u.Tag = new Info();
