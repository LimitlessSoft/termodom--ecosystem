...
namespace TDOffice_v2.TDOffice
{
    public partial class TaskboardTaskKomentar
    {
        ...
        {
            using (FbCommand cmd = new FbCommand("UPDATE TASKBOARD_TASK_KOMENTAR SET KORISNIKID = @K, DATUM = @D, TASK_ID = @TID,  TEXT = @T WHERE ID = @ID", con))
            {
        ...
        {
            using (FbCommand cmd = new FbCommand("SELECT ID, KORISNIKID, DATUM, TASK_ID,  TEXT FROM TASKBOARD_TASK_KOMENTAR WHERE ID = @ID", con))
            {
                cmd.Parameters.AddWithValue("@ID", id);
        ...
        public static List<TaskboardTaskKomentar> List(FbConnection con)
        {
            ...
            using (FbCommand cmd = new FbCommand("SELECT ID, KORISNIKID, DATUM, TASK_ID, TEXT FROM TASKBOARD_TASK_KOMENTAR ", con))
            {
                using (FbDataReader dr = cmd.ExecuteReader())
                    while (dr.Read())
                        list.Add(new TaskboardTaskKomentar()
                        {
                            ID = Convert.ToInt32(dr["ID"]),
                            KorisnikID = Convert.ToInt32(dr["KORISNIKID"]),
                            Datum = Convert.ToDateTime(dr["DATUM"]),
        ...
        {
            List<TaskboardTaskKomentar> list = new List<TaskboardTaskKomentar>();
            using (FbCommand cmd = new FbCommand("SELECT ID, KORISNIKID, DATUM, TASK_ID, TEXT FROM TASKBOARD_TASK_KOMENTAR WHERE TASK_ID = @TID", con))
            {
                cmd.Parameters.AddWithValue("@TID", TaskID);
                using (FbDataReader dr = cmd.ExecuteReader())
                    while (dr.Read())
        ...
        public static int Insert(FbConnection con, int KorisnikID, int TaskID, DateTime datum, string tekst)
        {
            using (FbCommand cmd = new FbCommand("INSERT INTO TASKBOARD_TASK_KOMENTAR (ID, KORISNIKID, DATUM, TASK_ID, TEXT) VALUES (((SELECT COALESCE(MAX(ID), 0) FROM TASKBOARD_TASK_KOMENTAR) + 1), @KID, @D, @TID, @T)", con))
            {
