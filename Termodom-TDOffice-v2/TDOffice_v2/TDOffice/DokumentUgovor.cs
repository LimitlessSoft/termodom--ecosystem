...
namespace TDOffice_v2.TDOffice
{
    public class DokumentUgovor
    {
        ...
        {
            using(FbCommand cmd = new FbCommand(@"UPDATE DOKUMENT_UGOVOR
                SET
                TIP = @TIP,
                NAZIV = @NAZ,
                DATUM = @DATUM,
...
FROM DOKUMENT_UGOVOR WHERE ID = @ID", con))
            {
                cmd.Parameters.AddWithValue("@ID", id);
        ...
        public static List<DokumentUgovor> List(FbConnection con, string whereQuery = null)
        {
            ...
            using (FbCommand cmd = new FbCommand(@"SELECT
ID, TIP, DATUM, KORISNIK_ID, MAGACIN_ID, KUPAC, NAZIV, PRODAVAC
FROM DOKUMENT_UGOVOR WHERE 1 = 1 " + whereQuery, con))
            {
                using (FbDataReader dr = cmd.ExecuteReader())
                    while (dr.Read())
                        list.Add(new DokumentUgovor()
                        {
                            ID = Convert.ToInt32(dr["ID"]),
                            Tip = (DokumentUgovorTip)Convert.ToInt32(dr["TIP"]),
                            Datum = Convert.ToDateTime(dr["DATUM"]),
        ...
        public static int Insert(DokumentUgovorTip tip, string naziv, int korisnikID, int magacinID, int kupac, int prodavac)
        {
            using(FbConnection con = new FbConnection(TDOffice.connectionString))
            {
                con.Open();
        ...
        public static int Insert(FbConnection con, DokumentUgovorTip tip, string naziv, int korisnikID, int magacinID, int kupac, int prodavac)
        {
            using(FbCommand cmd = new FbCommand(@"INSERT INTO DOKUMENT_UGOVOR
(ID, TIP, DATUM, KORISNIK_ID, MAGACIN_ID, KUPAC, NAZIV, PRODAVAC)
VALUES
(((SELECT COALESCE(MAX(ID), 0)) + 1), @TIP, @DATUM, @KID, @MID, @KUP, @NAZ, @PROD)", con))
            {
