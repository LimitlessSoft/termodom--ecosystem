...
namespace TDOffice_v2.TDOffice
{
    public class MagacinClan
    {
        ...
        {
            using (FbCommand cmd = new FbCommand(@"SELECT ID, MAGACINID, USERID, TIP FROM MAGACIN_CLAN WHERE ID = @ID", con))
            {
        ...
        {
            using (FbCommand cmd = new FbCommand("UPDATE MAGACIN_CLAN SET USERID = @U, TIP = @T, MAGACINID = @MID WHERE ID = @ID", con))
            {
        ...
        public static void Insert(FbConnection con, int magacinID, int korisnikID, Enums.MagacinClanTip tip)
        {
            if(tip == Enums.MagacinClanTip.Vlasnik)
            {
                List<MagacinClan> mcs = ListByMagacinID(con, magacinID);
                MagacinClan mc = mcs.Where(x => x.Tip == Enums.MagacinClanTip.Vlasnik).FirstOrDefault();
            ...
            using (FbCommand cmd = new FbCommand("INSERT INTO MAGACIN_CLAN (ID, MAGACINID, USERID, TIP) VALUES (((SELECT COALESCE(MAX(ID), 0) FROM MAGACIN_CLAN) + 1), @MID, @U, @TIP)", con))
            {
                cmd.Parameters.AddWithValue("@MID", magacinID);
                cmd.Parameters.AddWithValue("@U", korisnikID);
        ...
        public static List<MagacinClan> List(FbConnection con)
        {
            ...
            using (FbCommand cmd = new FbCommand("SELECT ID, MAGACINID, USERID, TIP FROM MAGACIN_CLAN", con))
            {
                using (FbDataReader dr = cmd.ExecuteReader())
                    while (dr.Read())
                        list.Add(new MagacinClan()
                        {
                            ID = Convert.ToInt32(dr["ID"]),
                            MagacinID = Convert.ToInt32(dr["MAGACINID"]),
                            KorisnikID = Convert.ToInt32(dr["USERID"]),
        ...
        {
            List<MagacinClan> list = new List<MagacinClan>();
            using (FbCommand cmd = new FbCommand("SELECT ID, MAGACINID, USERID, TIP FROM MAGACIN_CLAN WHERE MAGACINID = @MID", con))
            {
                cmd.Parameters.AddWithValue("@MID", magacinID);
                using (FbDataReader dr = cmd.ExecuteReader())
                    while (dr.Read())
        ...
        {
            List<MagacinClan> list = new List<MagacinClan>();
            using (FbCommand cmd = new FbCommand("SELECT ID, MAGACINID, USERID, TIP FROM MAGACIN_CLAN WHERE USERID = @UID", con))
            {
