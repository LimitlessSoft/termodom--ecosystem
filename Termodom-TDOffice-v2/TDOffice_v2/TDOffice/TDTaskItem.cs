...
namespace TDOffice_v2.TDOffice
{
    public class TDTaskItem
    {
        ...
        {
            using(FbCommand cmd = new FbCommand("UPDATE TDTASK_ITEM SET DONE = @DONE, INTERVAL = @INT WHERE ID = @ID", con))
            {
        ...
        {
            using(FbCommand cmd = new FbCommand("SELECT ID, TDTASKID, USERID, DONE, INTERVAL FROM TDTASK_ITEM WHERE ID = @ID", con))
            {
                cmd.Parameters.AddWithValue("@ID", id);
                ...
                using (FbDataReader dr = cmd.ExecuteReader())
                    if (dr.Read())
                        return new TDTaskItem()
                        {
                            ...
                            Done = dr["DONE"] is DBNull ? null : (DateTime?)Convert.ToDateTime(dr["DONE"]),
        ...
        public static List<TDTaskItem> List(FbConnection con)
        {
            ...
            using (FbCommand cmd = new FbCommand("SELECT ID, TDTASKID, USERID, DONE, INTERVAL FROM TDTASK_ITEM", con))
            {
                using (FbDataReader dr = cmd.ExecuteReader())
                    while (dr.Read())
                        list.Add(new TDTaskItem()
                        {
                            ID = Convert.ToInt32(dr["ID"]),
                            TDTaskID = Convert.ToInt32(dr["TDTASKID"]),
                            UserID = Convert.ToInt32(dr["USERID"]),
        ...
        {
            List<TDTaskItem> list = new List<TDTaskItem>();
            using (FbCommand cmd = new FbCommand("SELECT ID, TDTASKID, USERID, DONE, INTERVAL FROM TDTASK_ITEM WHERE USERID = @UID", con))
            {
        ...
        {
            using(FbCommand cmd = new FbCommand("INSERT INTO TDTASK_ITEM (ID, TDTASKID, USERID, DONE, INTERVAL) VALUES (((SELECT COALESCE(MAX(ID), 0) FROM TDTASK_ITEM) + 1), @TID, @UID, @DONE, 60)", con))
            {
