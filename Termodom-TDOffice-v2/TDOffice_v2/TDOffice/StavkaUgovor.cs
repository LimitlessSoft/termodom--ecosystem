...
namespace TDOffice_v2.TDOffice
{
    public class StavkaUgovor
    {
            ...
            get
            {
                if (_izvodi == null)
                    if (string.IsNullOrWhiteSpace(_izvodiJsonStringReadBuffer))
                        ...
                        _izvodi = JsonConvert.DeserializeObject<List<int>>(_izvodiJsonStringReadBuffer);
        ...
        {
            using(FbCommand cmd = new FbCommand(@"UPDATE STAVKA_UGOVOR
                SET
                BRDOK = @BRDOK,
                ROBAID = @ROBAID,
                DATUM_VALUTE = @DV,
        ...
        {
            using(FbCommand cmd = new FbCommand("SELECT ID, BRDOK, ROBAID, DATUM_VALUTE, IZNOS, VALUTA, IZVODI FROM STAVKA_UGOVOR WHERE ID = @ID", con))
            {
                cmd.Parameters.AddWithValue("@ID", id);
        ...
        public static List<StavkaUgovor> List(FbConnection con, string whereQuery = null)
        {
            ...
            using (FbCommand cmd = new FbCommand("SELECT ID, BRDOK, ROBAID, DATUM_VALUTE, IZNOS, VALUTA, IZVODI FROM STAVKA_UGOVOR WHERE 1 = 1 " + whereQuery, con))
            {
                using (FbDataReader dr = cmd.ExecuteReader())
                    while (dr.Read())
                        list.Add(new StavkaUgovor()
                        {
                            ID = Convert.ToInt32(dr["ID"]),
                            BrDok = Convert.ToInt32(dr["BRDOK"]),
                            RobaID = Convert.ToInt32(dr["ROBAID"]),
        ...
        public static int Insert(int brDok, int robaID, DateTime datumValute, double iznos, Valuta valuta)
        {
            using(FbConnection con = new FbConnection(TDOffice.connectionString))
            {
                con.Open();
        ...
        public static int Insert(FbConnection con, int brDok, int robaID, DateTime datumValute, double iznos, Valuta valuta)
        {
            using(FbCommand cmd = new FbCommand(@"INSERT INTO STAVKA_UGOVOR
(ID, BRDOK, ROBAID, DATUM_VALUTE, IZNOS, VALUTA, IZVODI)
VALUES
(((SELECT COALESCE(MAX(ID), 0)) + 1), @BRDOK, @RID, @DV, @IZNOS, @VALUTA, @IZVODI)", con))
            {
