...
namespace TDOffice_v2.TDOffice
{
    public class Zaposleni
    {
        ...
        {
            using (FbCommand cmd = new FbCommand("UPDATE ZAPOSLENI SET IME = @I, PREZIME = @P WHERE ID = @ID", con))
            {
        ...
        {
            using (FbCommand cmd = new FbCommand("INSERT INTO ZAPOSLENI (ID, IME, PREZIME) VALUES (((SELECT COALESCE(MAX(ID), 0) from ZAPOSLENI) + 1), @I, @P) RETURNING ID", con))
            {
        ...
        {
            using (FbCommand cmd = new FbCommand("SELECT ID, IME, PREZIME  FROM ZAPOSLENI WHERE ID = @ID", con))
            {
                cmd.Parameters.AddWithValue("@ID", id);
        ...
        public static List<Zaposleni> List(FbConnection con, string whereQuery = null)
        {
            ...
            using (FbCommand cmd = new FbCommand("SELECT ID, IME, PREZIME FROM ZAPOSLENI WHERE 1 = 1" + whereQuery, con))
            {
                using (FbDataReader dr = cmd.ExecuteReader())
                    while (dr.Read())
