...
namespace TDOffice_v2.Komercijalno
{
    public class RobaUMagacinu
    {
        ...
        public static List<RobaUMagacinu> List(FbConnection con)
        {
            ...
            using (FbCommand cmd = new FbCommand("SELECT MAGACINID, ROBAID, PRODAJNACENA, STANJE, OPTZAL, KRITZAL, NABAVNACENA FROM ROBAUMAGACINU", con))
                using (FbDataReader dr = cmd.ExecuteReader())
                    while (dr.Read())
        ...
        public static List<RobaUMagacinu> ListByMagacinID(FbConnection con, int magacinID)
        {
            ...
            using (FbCommand cmd = new FbCommand("SELECT MAGACINID, ROBAID, PRODAJNACENA, STANJE, OPTZAL, KRITZAL, NABAVNACENA FROM ROBAUMAGACINU WHERE MAGACINID = @MID", con))
            {
                cmd.Parameters.AddWithValue("@MID", magacinID);
                using (FbDataReader dr = cmd.ExecuteReader())
                    while (dr.Read())
