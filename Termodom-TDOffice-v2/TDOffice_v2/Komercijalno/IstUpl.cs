...
namespace TDOffice_v2.Komercijalno
{
    public class IstUpl
    {
        ...
        {
            using(FbCommand cmd = new FbCommand("UPDATE ISTUPL SET NUID = @NUID WHERE ISTUPLID = @ID", con))
            {
        ...
        public static void Insert(int vrDok, int brDok, double iznos, string opis, int ppid, int zapid, int io, int nuid, int a, int kasaid, string mtid,
            string valuta, double kurs, int promenaID, int pkID, int? linkedID, int? placanjeID, int? godina)
        {
            using(FbConnection con = new FbConnection(Komercijalno.CONNECTION_STRING[DateTime.Now.Year]))
            {
                con.Open();
                Insert(vrDok, brDok, iznos, opis, ppid, zapid, io, nuid, a, kasaid, mtid, valuta, kurs, promenaID, pkID, linkedID, placanjeID, godina);
            }
        }
        public static void Insert(FbConnection con, int vrDok, int brDok, double iznos, string opis, int ppid, int zapid, int io, int nuid, int a, int kasaid, string mtid,
            string valuta, double kurs, int promenaID, int pkID, int? linkedID, int? placanjeID, int? godina)
        {
            using(FbCommand cmd = new FbCommand(@"INSERT INTO ISTUPL
        ...
        public static List<IstUpl> List(FbConnection con, string whereQuery = null)
        {
            ...
            using(FbCommand cmd = new FbCommand("SELECT ISTUPLID, VRDOK, BRDOK, DATUM, IZNOS, PPID, IO, NUID, PROMENAID FROM ISTUPL" + whereQuery, con))
            {
                using (FbDataReader dr = cmd.ExecuteReader())
                {
                    while(dr.Read())
                        list.Add(new IstUpl()
                        {
                            ...
                            PromenaID = dr["PROMENAID"] is DBNull ? null : (int?)Convert.ToInt32(dr["PROMENAID"])
                        });
                }
            }
            return list;
