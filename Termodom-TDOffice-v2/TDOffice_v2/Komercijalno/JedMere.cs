        ...
        public static List<JedMere> List(FbConnection con)
        {
            ...
            using (FbCommand cmd = new FbCommand("SELECT * FROM JEDMERE", con))
            {
                using (FbDataReader dr = cmd.ExecuteReader())
                    while (dr.Read())
                        list.Add(new JedMere()
                        {
                            JM = dr["JM"].ToString(),
                            Naziv = dr["NAZIV"] is DBNull ? null : dr["NAZIV"].ToString(),
                            FKod = dr["F_KOD"] is DBNull ? null : (int?)Convert.ToInt32(dr["F_KOD"]),
                            Odnos = dr["ODNOS"] is DBNull ? null : (double?)Convert.ToDouble(dr["ODNOS"]),
                            OdnosJM = dr["ODNOS_JM"] is DBNull ? null : dr["ODNOS_JM"].ToString(),
                            EkspID = dr["EKSPID"] is DBNull ? null : (int?)Convert.ToInt32(dr["EKSPID"]),
