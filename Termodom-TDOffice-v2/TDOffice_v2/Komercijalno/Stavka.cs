...
namespace TDOffice_v2.Komercijalno
{
    public class Stavka
    {
        ...
        /// <returns></returns>
        public static int Insert(int godina, Dokument dokument, Roba roba, RobaUMagacinu robaUMagacinu, double? kolicina, double rabat, double? prodajnaCenaBezPDV = null)
        {
            using(FbConnection con = new FbConnection(Komercijalno.CONNECTION_STRING[godina]))
            {
                con.Open();
            ...
            lock (_insertLock)
            {
                ...
                @MTID, 'P', 0, 0, 0, @POREZ) RETURNING STAVKAID", con))
                {
                    ...
                    cmd.Parameters.AddWithValue("@CENA_SA_PDV", prodajnaCenaBezPDV == null ?
                        Procedure.ProdajnaCenaNaDan(dokument.MagacinID, roba.ID, dokument.Datum) :
                        prodajnaCenaBezPDV * (1 + (tarife.Where(x => x.TarifaID == roba.TarifaID).FirstOrDefault().Stopa / 100)));
                    cmd.Parameters.AddWithValue("@CENA_BEZ_PDV", prodajnaCenaBezPDV == null ?
                        (double)cmd.Parameters["@CENA_SA_PDV"].Value * (1 - (tarife.Where(x => x.TarifaID == roba.TarifaID).FirstOrDefault().Stopa / (100 + tarife.Where(x => x.TarifaID == roba.TarifaID).FirstOrDefault().Stopa))) :
                        Procedure.ProdajnaCenaNaDan(dokument.MagacinID, roba.ID, dokument.Datum));
                    cmd.Parameters.AddWithValue("@KOL", kolicina);
                    cmd.Parameters.AddWithValue("@TARIFAID", roba.TarifaID);
                    cmd.Parameters.AddWithValue("@POREZ", Tarife.BufferedList().Where(x => x.TarifaID == roba.TarifaID).FirstOrDefault().Stopa);
                    cmd.Parameters.AddWithValue("@RABAT", rabat);
                    cmd.Parameters.AddWithValue("@MTID", Magacin.Get(DateTime.Now.Year, dokument.MagacinID).MTID);
        ...
        {
            using (FbCommand cmd = new FbCommand("DELETE FROM STAVKA WHERE VRDOK = @VRDOK AND BRDOK = @BRDOK", con))
            {
        ...
        {
            using (FbCommand cmd = new FbCommand("SELECT COALESCE(COUNT(ROBAID), 0) FROM STAVKA", con))
                using (FbDataReader dr = cmd.ExecuteReader())
                    if (dr.Read())
        ...
        {
            using (FbCommand cmd = new FbCommand("SELECT COALESCE(MAX(STAVKAID), 0) FROM STAVKA", con))
            using (FbDataReader dr = cmd.ExecuteReader())
                if (dr.Read())
                    return Convert.ToInt32(dr[0]);
