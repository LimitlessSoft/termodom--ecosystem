...
namespace TDOffice_v2.Komercijalno
{
    public class Promena
    {
        ...
        public static List<Promena> List(FbConnection con, string whereQuery = null)
        {
            ...
            using (FbCommand cmd = new FbCommand("SELECT PPID, POTRAZUJE, DUGUJE, KONTO, VRSTANAL, VRDOK, DATNAL FROM PROMENE" + whereQuery, con))
            {
                using (FbDataReader dr = cmd.ExecuteReader())
                    while (dr.Read())
                        pr.Add(new Promena()
                        {
                            ...
                            VrDok = dr["VRDOK"] is DBNull ? null : (int?)Convert.ToInt32(dr["VRDOK"]),
        ...
        {
            List<Promena> pr = new List<Promena>();
            using (FbCommand cmd = new FbCommand("SELECT PPID, POTRAZUJE, DUGUJE, KONTO, VRSTANAL, VRDOK FROM PROMENE WHERE PPID = @PPID", con))
            {
        ...
        /// <returns></returns>
        public async static Task<PromenaCollection> CollectionAsync(int godina, string konto, int[] vrstaNal)
        {
            ...
            {
                var result = JsonConvert.DeserializeObject<Dictionary<int, Promena>>(await response.Content.ReadAsStringAsync());
