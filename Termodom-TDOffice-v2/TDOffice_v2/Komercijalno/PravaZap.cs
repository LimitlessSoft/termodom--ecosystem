...
namespace TDOffice_v2.Komercijalno
{
    class PravaZap
    {
        ...
        {
            using (FbCommand cmd = new FbCommand(@"UPDATE PRAVAZAP SET MODULID = @MID, V = @V,
                 I = @I, M = @M, D = @D, P = @P, EKSPID = @EKSPID
                 WHERE ZAPID = @ZID AND OBJEKATID = @OID", con))
            {
        ...
        {
            using (FbCommand cmd = new FbCommand("SELECT MODULID, OBJEKATID, ZAPID, V, I, M, D, P, EKSPID FROM PRAVAZAP  WHERE ZAPID = @ZID AND MODULID = @MID AND OBJEKATID = @OID", con))
            {
                cmd.Parameters.AddWithValue("@ZID", zapID);
                cmd.Parameters.AddWithValue("@MID", modulID);
                cmd.Parameters.AddWithValue("@OID", objekatID);
        ...
        public static List<PravaZap> List(FbConnection con)
        {
            ...
            using (FbCommand cmd = new FbCommand("SELECT MODULID, OBJEKATID, ZAPID, V, I, M, D, P, EKSPID FROM PRAVAZAP ", con))
            {
                using (FbDataReader dr = cmd.ExecuteReader())
                    while (dr.Read())
                        list.Add(new PravaZap()
                        {
                            ModulID = Convert.ToInt16(dr["MODULID"]),
                            ZapID = Convert.ToInt16(dr["ZAPID"]),
                            ObjekatID = Convert.ToInt16(dr["OBJEKATID"]),
