...
namespace TDOffice_v2
{
    public partial class fm_Izvestaj_Promet_Magacina : Form
    {
        ...
        private void prikaziIzvestaj_btn_Click(object sender, EventArgs e)
        {
            ...
            Task.Run(() =>
            {
                ConcurrentDictionary<int, Tuple<DateTime, DateTime>> periodi = new ConcurrentDictionary<int, Tuple<DateTime, DateTime>>();
                foreach (DataGridViewRow row in godine_dgv.Rows)
                    if (Convert.ToBoolean(row.Cells["KoristiSe"].Value))
                        periodi[Convert.ToInt32(row.Cells["Godina"].Value)] = new Tuple<DateTime, DateTime>(Convert.ToDateTime(row.Cells["OdDatuma"].Value), Convert.ToDateTime(row.Cells["DoDatuma"].Value));
...
SELECT t1.MAGACINID, SUM(t1.vGOD" + godina + @" - t2.vGOD" + godina + @") AS vGOD" + godina + @"
FROM
    (
    SELECT s.MAGACINID, COALESCE(SUM(s.KOLICINA * s.PRODAJNACENA * (100 - s.rabat)/100), 0) AS vGOD" + godina + @"
    FROM STAVKA s
    LEFT OUTER JOIN DOKUMENT d ON d.VRDOK = s.VRDOK AND d.BRDOK = s.BRDOK
    WHERE d.FLAG = 1 AND d.MAGACINID IN (" + string.Join(", ", magacini) + @") AND (d.VRDOK = 13 OR d.VRDOK = 15) AND DATUM >= @ODDATUMA AND DATUM <= @DODATUMA
    ...
    FROM STAVKA s
    LEFT OUTER JOIN DOKUMENT d ON d.VRDOK = s.VRDOK AND d.BRDOK = s.BRDOK
    WHERE d.MAGACINID IN (" + string.Join(", ", magacini) + @") AND (d.VRDOK = 22) AND DATUM >= @ODDATUMA AND DATUM <= @DODATUMA
    GROUP BY s.MAGACINID ORDER BY S.MAGACINID desc
    ) as t2 on t1.MAGACINID = t2.MAGACINID
    GROUP BY t1.MAGACINID", con))
                        {
                            ...
                            using (FbDataAdapter da = new FbDataAdapter(cmd))
                            {
                                ...
                                da.Fill(tempDT);
                                tempDT.PrimaryKey = new DataColumn[] { tempDT.Columns["MAGACINID"] };
                ...
                this.Invoke((MethodInvoker)delegate
                {
                    ...
                    foreach (int godina in periodi.Keys.OrderBy(x => x))
                    {
                        ...
                        dataGridView1.Columns["vGOD" + godina].DefaultCellStyle.Format = "#,##0.00 RSD";
                        ...
                        dataGridView1.Columns["vGOD" + godina].HeaderText = $"GOD {godina} [{periodi[godina].Item1.ToString("dd.MM.yyyy")} - {periodi[godina].Item2.ToString("dd.MM.yyyy")}]";
                    }
                    prikaziIzvestaj_btn.Enabled = true;
                    btn_PosaljiIzvestaj.Enabled = true;
                });
        ...
        private void btn_PosaljiIzvestaj_Click(object sender, EventArgs e)
        {
           ...
           DataTable dataTable = (DataTable)dataGridView1.DataSource;
            ...
            Dictionary<int, Tuple<DateTime, DateTime>> periodi = new Dictionary<int, Tuple<DateTime, DateTime>>();
            foreach (DataGridViewRow row in godine_dgv.Rows)
                if (Convert.ToBoolean(row.Cells["KoristiSe"].Value))
                    periodi[Convert.ToInt32(row.Cells["Godina"].Value)] = new Tuple<DateTime, DateTime>(Convert.ToDateTime(row.Cells["OdDatuma"].Value), Convert.ToDateTime(row.Cells["DoDatuma"].Value));

            using (fm_Izvestaj_Promet_Magacina_PosaljiKaoIzvestaj pmi = new fm_Izvestaj_Promet_Magacina_PosaljiKaoIzvestaj(dataTable, periodi))
                pmi.ShowDialog();
        }
