...
namespace TDOffice_v2
{
    ...
    {
        private Task<List<Komercijalno.Dokument>> _dokument { get; set; } = Komercijalno.Dokument.ListAsync(null, new int[] { 15 });
        private Task<List<Komercijalno.IstUpl>> _istUplate { get; set; } = Komercijalno.IstUpl.ListAsync("VRDOK = 15");
        ...
        private void btn_ProveriNelogicnostiNacinaPlacanja_Click(object sender, EventArgs e)
        {
            ...
            Task.Run(() =>
            {
                ...
                List<Komercijalno.Partner> par = Komercijalno.Partner.ListAsync(DateTime.Now.Year).Result;
                List<Komercijalno.Magacin> mag = Komercijalno.Magacin.ListAsync(DateTime.Now.Year).Result;

                ConcurrentBag<DataDTO> bag = new ConcurrentBag<DataDTO>();

                Parallel.ForEach(_dokument.Result, d =>
                {
                    double p = 0;
                    cStep++;
                    ...
                    List<Komercijalno.IstUpl> istUpls = _istUplate.Result.Where(x => x.BrDok == d.BrDok).ToList();
                    ...
                    dto.Partner = d.PPID is null ? "" : par.FirstOrDefault(x => x.PPID == d.PPID).Naziv;
                ...
                this.Invoke((MethodInvoker) delegate
                {
                    ...
                    dataGridView1.Columns["Iznos"].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
                    dataGridView1.Columns["Iznos"].HeaderText = "Zbir uplata za dokument";
                    dataGridView1.Columns["Iznos"].Width = 100;
                    dataGridView1.Columns["IznosDokumenta"].DefaultCellStyle.Format = "#,##0.00 RSD";
                    dataGridView1.Columns["IznosDokumenta"].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
                    dataGridView1.Columns["IznosDokumenta"].HeaderText = "Vrednost dokumenta";
                    dataGridView1.Columns["IznosDokumenta"].Width = 100;
                    dataGridView1.Columns["NacinPlacanjaDokumenta"].HeaderText = "Nacin placanja Dokument";
                    ...
                    dataGridView1.Sort(dataGridView1.Columns["BrDok"], ListSortDirection.Descending);
                    this.Enabled = true;
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                for (int y = dt.Rows.Count - 1; y >= 0; y--)
                {
                    ...
                    int[] nups = string.IsNullOrWhiteSpace(dt.Rows[y]["NUID"].ToString()) ? null : dt.Rows[y]["NUID"].ToString().Replace(" ", String.Empty).Split(',').Select(x => Convert.ToInt32(x)).ToArray();
                    ...
                    {
                        if (nuDok == (int)Komercijalno.NacinUplate.Virman &&
                            Convert.ToDouble(dt.Rows[y]["Iznos"]) < Convert.ToDouble(dt.Rows[y]["IznosDokumenta"]) &&
                            nups != null && nups.Where(x => x == 1).Count() == nups.Length)
                            continue;
                    }

                    int iznosDokumenta = (int)(Convert.ToDouble(dt.Rows[y]["IznosDokumenta"]) * 100);
                    int zbirIstUpl = (int)(Convert.ToDouble(dt.Rows[y]["Iznos"]) * 100);

                    if (clb_Filter.GetItemChecked(1))
                    {
                        if (nuDok == 1 && !clb_Filter.GetItemChecked(2) && zbirIstUpl < iznosDokumenta)
                        {
