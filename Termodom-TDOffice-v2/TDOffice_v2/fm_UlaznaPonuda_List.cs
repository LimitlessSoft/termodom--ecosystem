...
namespace TDOffice_v2
{
    public partial class fm_UlaznaPonuda_List : Form
    {
        ...
        {
            List<TDOffice.DokumentUlaznaPonuda> list = TDOffice.DokumentUlaznaPonuda.List($"DATUM > '{odDatuma_dtp.Value.Date.ToString("dd.MM.yyyy")}' AND DATUM < '{doDatuma_dtp.Value.Date.AddDays(1).ToString("dd.MM.yyyy")}'");
        ...
        {
            using (fm_UlaznaPonuda_Index p = new fm_UlaznaPonuda_Index(TDOffice.DokumentUlaznaPonuda.Insert(Program.TrenutniKorisnik.ID, DateTime.Now, DateTime.Now.AddMonths(3))))
                if (!p.IsDisposed)
                    p.ShowDialog();
