...
namespace TDOffice_v2
{
    public partial class fm_Kontakt_List : Form
    {
        ...
        {
            List<TDOffice.Partner> par = TDOffice.Partner.List().Where(x => x.SMSBlok == true).ToList();
            if (par.Count <=0 )
            {
                MessageBox.Show("Nema blokiranih kontakata");
                return;
            }
            using (DataGridViewSelectBox sb = new DataGridViewSelectBox(_dt.Copy().Select("SMSBlok = 1").AsEnumerable().CopyToDataTable()))
            {
                sb.ShowDialog();
            }
        }
