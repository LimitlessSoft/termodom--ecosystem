...
docker run -p 33770:80 -e API_HOST=$env:API_HOST -e JWT_ISSUER=$env:WEB_VELEPRODAJA_JWT_ISSUER -e JWT_AUDIENCE=$env:WEB_VELEPRODAJA_JWT_AUDIENCE -e JWT_KEY=$env:WEB_VELEPRODAJA_JWT_KEY --name termodom--tdoffice-web-veleprodaja-api -m 1G --restart=always -d limitlesssoft/termodom--web-veleprodaja-api:$env:BUILD_NUMBER